...
public final class ReadBatch {
    ...
    public ReadBatch(
        ...
        if (compatibilityObjectOffset >= 0) {
            try {
                Math.addExact(compatibilityObjectOffset, compatibilityObjectLength);
    ...
    public List<SchemaRef> schemaRefs() {
        return schemaRefs;
    }

    public Optional<ProjectionRef> projectionRef() {
        return projectionRef;
    }

    public ReadSourceRef source() {
        return source;
    }
    ...
    private ObjectSliceReadTarget objectTarget() {
        ...
        throw new IllegalStateException(
                "Object-only ReadBatch accessor used for " + source.target().type());
    }

    private static ReadSourceRef legacySource(
            OffsetRange range, ObjectId objectId, long objectOffset, long objectLength, EntryIndexRef entryIndexRef) {
        ...
        ObjectKey objectKey = entryIndexRef.objectKey().orElseGet(() -> new ObjectKey("legacy/" + objectId.value()));
    ...
    @Override
    public int hashCode() {
        return 31 * Objects.hash(range, payloadFormat, schemaRefs, projectionRef, source) + Arrays.hashCode(payload);
    }

    @Override
    public String toString() {
        return "ReadBatch[range=" + range + ", payloadFormat=" + payloadFormat + ", payloadBytes=" + payload.length
                + ", source=" + source + "]";
    }
}
