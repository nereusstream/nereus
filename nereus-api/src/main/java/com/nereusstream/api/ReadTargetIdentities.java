...
public final class ReadTargetIdentities {
    ...
    public static byte[] canonicalPayload(ReadTarget target) {
        ...
        throw new IllegalArgumentException(
                "unsupported read target " + target.getClass().getName());
    }

    private static final class BinaryWriter {
        private final ByteArrayOutputStream bytes = new ByteArrayOutputStream();

        void magic(String value) {
            bytes.writeBytes(value.getBytes(StandardCharsets.US_ASCII));
        }
        ...
        void intValue(int value) {
            bytes.writeBytes(ByteBuffer.allocate(Integer.BYTES).putInt(value).array());
        }
