...
public final class DefaultManagedLedgerGenerationProtocolActivationCoordinator
    ...
    public CompletableFuture<Void> activatePublication() {
        return activations.get(cluster).thenCompose(existing -> {
            if (!firstActivationEnabled
                    && (existing.isEmpty()
                            || existing.orElseThrow().value().lifecycle()
                                    != GenerationProtocolActivationLifecycle.ACTIVE)) {
                return CompletableFuture.failedFuture(notReady("first generation publication activation is disabled"));
            }
            return readinessProvider.requireGenerationCapabilityReadiness().thenCompose(readiness -> authority(
                            existing, readiness)
                    .thenCompose(current -> activate(readiness, current, 0))
                    .thenCompose(installed -> finalRevalidate(readiness, installed)));
        });
    }

    private CompletableFuture<VersionedGenerationProtocolActivation> authority(
            Optional<VersionedGenerationProtocolActivation> existing, GenerationCapabilityReadiness readiness) {
        if (existing.isPresent()) {
            return CompletableFuture.completedFuture(existing.orElseThrow());
        }
        return activations.getOrCreate(cluster).thenApply(created -> {
            requireAuthority(readiness, created.value());
            return created;
        });
    }

    private CompletableFuture<VersionedGenerationProtocolActivation> activate(
            GenerationCapabilityReadiness readiness, VersionedGenerationProtocolActivation current, int attempt) {
        if (attempt >= MAX_CAS_ATTEMPTS) {
            return CompletableFuture.failedFuture(
                    notReady("generation publication activation CAS retry budget exhausted"));
        }
        GenerationProtocolActivationRecord value = current.value();
        requireAuthority(readiness, value);
        if (value.lifecycle() == GenerationProtocolActivationLifecycle.ACTIVE) {
            return CompletableFuture.completedFuture(current);
        }
        long activatedAt =
                Math.max(Math.max(value.preparedAtMillis(), value.updatedAtMillis()), Math.max(1, clock.millis()));
        GenerationProtocolActivationRecord replacement = new GenerationProtocolActivationRecord(
                ...
                0);
        return activations
                .compareAndSet(cluster, replacement, current.metadataVersion())
                .handle((updated, failure) -> {
                    if (failure == null) {
                        return CompletableFuture.completedFuture(updated);
                    }
                    Throwable cause = unwrap(failure);
                    return activations.get(cluster).thenCompose(optional -> {
                        VersionedGenerationProtocolActivation reloaded = optional.orElseThrow(
                                () -> notReady("generation activation disappeared after publication CAS"));
                        requireAuthority(readiness, reloaded.value());
                        if (reloaded.value().lifecycle() == GenerationProtocolActivationLifecycle.ACTIVE) {
                            return CompletableFuture.completedFuture(reloaded);
                        }
                        if (cause instanceof F4MetadataConditionFailedException) {
                            return activate(readiness, reloaded, attempt + 1);
                        }
    ...
    private CompletableFuture<Void> finalRevalidate(
            GenerationCapabilityReadiness expected, VersionedGenerationProtocolActivation installed) {
        Optional<GenerationCapabilityReadiness> cached = readinessProvider.currentGenerationCapabilityReadiness();
        if (cached.isEmpty() || !cached.orElseThrow().equals(expected)) {
            return CompletableFuture.failedFuture(
                    notReady("generation readiness was invalidated after publication activation"));
        }
        try {
        ...
        return activations.get(cluster).thenAccept(optional -> {
            VersionedGenerationProtocolActivation current = optional.orElseThrow(
                    () -> notReady("generation activation disappeared during final publication read"));
            requireActiveAuthority(expected, current.value());
            GenerationCapabilityReadiness finalReadiness = readinessProvider
                    .currentGenerationCapabilityReadiness()
                    .orElseThrow(() -> notReady("generation readiness was invalidated during final publication read"));
            if (!finalReadiness.equals(expected)) {
                throw notReady("generation readiness changed during final publication read");
            }
        });
    }
    ...
    private void requireActiveAuthority(
        ...
        if (activation.lifecycle() != GenerationProtocolActivationLifecycle.ACTIVE
            ...
            throw notReady("generation publication activation is not ACTIVE");
        }
    }
    ...
    private void requireAuthority(
        ...
        if (activation.brokerCapabilityReadinessEpoch() != readiness.brokerReadinessEpoch()) {
            throw notReady("generation registration proof belongs to another broker readiness epoch");
        }
        ...
        if (!registration.complete() || registration.brokerReadinessEpoch() != readiness.brokerReadinessEpoch()) {
            throw notReady("generation registration backfill is incomplete for the current readiness epoch");
        }
    }
