...
public record ManagedLedgerGenerationProjectionRefV1(
        String managedLedgerName, ManagedLedgerProjectionIdentity identity) {
    ...
    private static final int MAX_CANONICAL_BYTES = MAGIC.length
            + Integer.BYTES
            ...
            + 64
            + CRC_BYTES;
    private static final int MAX_BASE64URL_CHARACTERS = (MAX_CANONICAL_BYTES * 4 + 2) / 3;
    ...
    public ManagedLedgerGenerationProjectionRefV1 {
        ...
        if (!ManagedLedgerProjectionNames.streamId(managedLedgerName, identity.incarnation())
                .value()
                .equals(identity.streamId())) {
            throw new IllegalArgumentException("projection reference name/incarnation does not match its stream ID");
        }
    }
    ...
    public ProjectionRef toProjectionRef() {
        return new ProjectionRef(
                ...
                VALUE_PREFIX + Base64.getUrlEncoder().withoutPadding().encodeToString(canonicalBytes()));
    }
    ...
    public byte[] canonicalBytes() {
        ...
        ByteArrayOutputStream output = new ByteArrayOutputStream(MAGIC.length
                + Integer.BYTES
                + name.length
                + Long.BYTES * 3
                + Integer.BYTES
    ...
    public static ManagedLedgerGenerationProjectionRefV1 from(ProjectionRef ref) {
        ...
        if (exact.type() != ProjectionType.VIRTUAL_LEDGER || !exact.value().startsWith(VALUE_PREFIX)) {
            throw new IllegalArgumentException("projection reference is not a managed-ledger NPR1 value");
        }
        String encoded = exact.value().substring(VALUE_PREFIX.length());
        if (encoded.isEmpty() || encoded.length() > MAX_BASE64URL_CHARACTERS || encoded.indexOf('=') >= 0) {
            throw new IllegalArgumentException("NPR1 projection reference must use unpadded base64url");
        }
        final byte[] bytes;
        try {
            bytes = Base64.getUrlDecoder().decode(encoded);
        } catch (IllegalArgumentException failure) {
            throw new IllegalArgumentException("NPR1 projection reference has invalid base64url", failure);
        }
        if (!Base64.getUrlEncoder().withoutPadding().encodeToString(bytes).equals(encoded)) {
            throw new IllegalArgumentException("NPR1 projection reference is not canonically encoded");
        }
        if (bytes.length < MAGIC.length + Integer.BYTES * 2 + Long.BYTES * 3 + CRC_BYTES) {
        ...
        if (!Arrays.equals(magic, MAGIC)) {
            throw new IllegalArgumentException("NPR1 projection reference has an unknown magic");
        }
        String managedLedgerName = readString(input, payloadLength, "managedLedgerName");
        ...
        if (input.position() != payloadLength) {
            throw new IllegalArgumentException("NPR1 projection reference contains trailing payload bytes");
        }
        int expectedCrc = input.getInt();
        if (input.hasRemaining() || crc32c(bytes, 0, payloadLength) != expectedCrc) {
            throw new IllegalArgumentException("NPR1 projection reference CRC32C does not match");
        }
        ManagedLedgerGenerationProjectionRefV1 decoded = new ManagedLedgerGenerationProjectionRefV1(
                managedLedgerName,
                new ManagedLedgerProjectionIdentity(bindingGeneration, incarnation, streamId, virtualLedgerId));
        if (!decoded.toProjectionRef().equals(exact)) {
            throw new IllegalArgumentException("NPR1 projection reference is not canonical");
        }
        return decoded;
    }

    private static String readString(ByteBuffer input, int payloadLimit, String field) {
        if (input.position() > payloadLimit - Integer.BYTES) {
            throw new IllegalArgumentException("NPR1 projection reference is truncated before " + field);
        }
        long unsignedLength = Integer.toUnsignedLong(input.getInt());
        if (unsignedLength > Integer.MAX_VALUE || unsignedLength > payloadLimit - input.position()) {
            throw new IllegalArgumentException("NPR1 projection reference has an invalid " + field + " length");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("NPR1 projection reference " + field + " is not strict UTF-8", failure);
        }
    }

    private static long readLong(ByteBuffer input, int payloadLimit, String field) {
