...
public record NereusRetentionConfig(
    ...
    public NereusRetentionConfig {
        if (statsScanPageSize <= 0 || statsScanPageSize > MAX_STATS_SCAN_PAGE_SIZE) {
            throw new IllegalArgumentException("statsScanPageSize must be in [1, 512]");
        }
        if (maxConcurrentPlans <= 0 || maxQueuedPlans <= 0) {
            throw new IllegalArgumentException("retention concurrency and queue limits must be positive");
        }
    ...
    public static NereusRetentionConfig defaults() {
        return new NereusRetentionConfig(128, 8, 1_024, Duration.ofSeconds(30), Duration.ofSeconds(30));
    }
    ...
    private static Duration requirePositiveMillis(Duration value, String field) {
        ...
        try {
            ...
            throw new IllegalArgumentException(field + " must be millisecond-representable", failure);
        }
        if (value.isNegative() || value.isZero() || millis <= 0) {
            throw new IllegalArgumentException(field + " must be a positive whole-millisecond duration");
        }
        return value;
    }
}
