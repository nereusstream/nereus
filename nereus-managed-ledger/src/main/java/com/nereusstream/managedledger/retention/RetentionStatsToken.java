 ...
 */
public record RetentionStatsToken(String key, long metadataVersion, Checksum durableValueSha256)
        implements Comparable<RetentionStatsToken> {
    private static final int MAX_KEY_BYTES = 4_096;

    public RetentionStatsToken {
        Objects.requireNonNull(key, "key");
        if (key.isBlank() || key.getBytes(StandardCharsets.UTF_8).length > MAX_KEY_BYTES) {
            throw new IllegalArgumentException("retention stats key must be non-blank and at most 4096 UTF-8 bytes");
        }
