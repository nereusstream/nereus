...
public final class NereusManagedLedgerRetentionService {
    ...
    public NereusManagedLedgerRetentionService(
        ...
        this.liveProjection = Objects.requireNonNull(liveProjection, "liveProjection");
        ...
        this.completedTrimObserver = Objects.requireNonNull(completedTrimObserver, "completedTrimObserver");
        if (!liveProjection.streamId().equals(streamId)
                || !owner.ledger().projection().streamId().equals(streamId.value())) {
            throw new IllegalArgumentException("retention service stream, projection, and cursor owner must agree");
        }
    }
     ...
     */
    public CompletableFuture<Optional<RetentionCandidate>> trim(String reason) {
        ...
        return prepare(0).thenCompose(prepared -> {
            if (prepared.candidate().isEmpty()) {
                return ownershipGuard
                        .requireOwned("logical retention no-op completion")
                        .thenApply(ignored -> Optional.empty());
            }
            RetentionCandidate candidate = prepared.candidate().orElseThrow();
            return cursorRetention
                    .requestTrim(owner, candidate.candidateTrimOffset(), exactReason)
                    .thenCompose(view -> ownershipGuard
                            .requireOwned("logical retention callback completion")
    ...
    public CompletableFuture<Void> ensurePolicyAdmissionReady() {
        return ownershipGuard
                .requireOwned("logical retention policy admission")
                .thenCompose(
                        ignored -> activationGuard.requireReady(GenerationOperation.LOGICAL_TRIM, liveProjection, true))
                .thenCompose(activationGuard::revalidate)
                .thenCompose(ignored -> ownershipGuard.requireOwned("logical retention policy admission final check"));
    }
    ...
    private CompletableFuture<PreparedTrim> prepare(int attempt) {
        ...
        return ownershipGuard
                .requireOwned("logical retention admission")
                .thenCompose(
                        ignored -> activationGuard.requireReady(GenerationOperation.LOGICAL_TRIM, liveProjection, true))
                .thenCompose(proof -> snapshotPolicy().thenCompose(policy -> planner.plan(streamId, policy)
                        .thenCompose(candidate -> prepareCandidate(proof, policy, candidate))))
                .handle((prepared, failure) -> {
                    ...
                    if (cause != null) {
                        return CompletableFuture.<PreparedTrim>failedFuture(cause);
                    }
    ...
    private CompletableFuture<PreparedTrim> prepareCandidate(
            GenerationActivationProof proof, RetentionPolicySnapshot policy, Optional<RetentionCandidate> candidate) {
        if (candidate.isEmpty()) {
            return CompletableFuture.completedFuture(new PreparedTrim(Optional.empty()));
        }
        RetentionCandidate exact = candidate.orElseThrow();
        return activationGuard
                .revalidate(proof)
                .thenCompose(ignored -> planner.revalidate(exact, policy))
                .thenApply(ignored -> new PreparedTrim(Optional.of(exact)));
    }
    ...
    private static String requireReason(String reason) {
        ...
        if (reason.isBlank() || reason.indexOf('\0') >= 0) {
            throw new IllegalArgumentException("logical retention reason must be non-blank and cannot contain NUL");
        }
        return reason;
    }
    ...
    private record PreparedTrim(Optional<RetentionCandidate> candidate) {
