...
public final class CursorSnapshotKeys {
    ...
    public static ObjectKeyPrefix clusterPrefix(String cluster) {
        ...
        return new ObjectKeyPrefix(KeyComponentCodec.encodeComponent(canonicalCluster) + "/cursor-snapshots/v1/");
    }
    ...
    public static ObjectKey objectKey(String cluster, CursorIdentity identity, String snapshotId) {
        Objects.requireNonNull(identity, "identity");
        String exactSnapshotId = CursorIds.requireRandomId(snapshotId, "snapshotId");
        return new ObjectKey(streamPrefix(cluster, identity.ledger())
    ...
    public static boolean belongsToStream(String cluster, CursorLedgerIdentity ledger, ObjectKey key) {
     ...
     */
    public static ParsedSnapshotKey parse(String cluster, CursorLedgerIdentity ledger, ObjectKey key) {
        Objects.requireNonNull(key, "key");
        ...
        return new ParsedSnapshotKey(parsed.cursorNameHash(), parsed.cursorGeneration(), parsed.snapshotId());
    }

    /**
     * Strict cluster-wide inverse used by ownerless F4 object inventory.
     */
    public static ParsedOwnerlessSnapshotKey parseOwnerless(String cluster, ObjectKey key) {
        Objects.requireNonNull(key, "key");
        ...
        String snapshotId = components[3].substring(0, components[3].length() - ".ncs".length());
        ...
        return new ParsedOwnerlessSnapshotKey(streamId, components[1], cursorGeneration, snapshotId);
    }
    ...
    public record ParsedSnapshotKey(String cursorNameHash, long cursorGeneration, String snapshotId) {
    ...
    public record ParsedOwnerlessSnapshotKey(
            StreamId streamId, String cursorNameHash, long cursorGeneration, String snapshotId) {
