...
public record CursorSnapshotWriteAuthority(
        VersionedCursorState currentRoot, String ownerSessionId, long targetMutationSequence) {
    public CursorSnapshotWriteAuthority {
        ...
        if (currentRoot.value().lifecycle() != CursorRecordLifecycle.ACTIVE
            ...
            throw new IllegalArgumentException("snapshot write authority requires the exact ACTIVE owner session");
        }
        if (targetMutationSequence <= currentRoot.value().mutationSequence()) {
            throw new IllegalArgumentException("snapshot target mutation sequence must advance the current root");
        }
    }

    public void requireMatches(CursorSnapshotWriteRequest request) {
        Objects.requireNonNull(request, "request");
        if (!currentRoot.value().projection().equals(request.identity().ledger().projection())
                || !currentRoot.value().cursorName().equals(request.identity().cursorName())
                || !currentRoot
                        .value()
                        .cursorNameHash()
                        .equals(request.identity().cursorNameHash())
                || currentRoot.value().cursorGeneration() != request.identity().cursorGeneration()
                || targetMutationSequence != request.sourceMutationSequence()) {
            throw new IllegalArgumentException("snapshot request does not match its captured cursor authority");
        }
    }
}
