...
class CursorStateMachinePropertyTest {
    ...
    @Test
    void randomizedIndividualAndCumulativeAcksMatchIndependentEntryModel() throws Exception {
        ...
        for (int seed = 0; seed < 40; seed++) {
            ...
            for (int step = 0; step < 250; step++) {
                if (model.markDelete() < END_OFFSET && random.nextInt(4) == 0) {
                    long offset = model.markDelete() + random.nextInt(END_OFFSET - Math.toIntExact(model.markDelete()));
                    CursorAckRequest request = request(random, offset, true, step);
                    state = machine.cumulativeAck(state, request, 0, END_OFFSET, 101L + step)
                            .state();
                    ...
                    for (int index = 0; index < count; index++) {
                        requests.add(request(random, random.nextInt(END_OFFSET), false, step));
                    }
                    state = machine.individualAck(state, requests, 0, END_OFFSET, 101L + step)
                            .state();
    ...
    private static CursorAckRequest request(Random random, long offset, boolean cumulative, int step) {
        Map<String, Long> properties = cumulative ? Map.of("revision", (long) step) : Map.of();
        if (random.nextInt(3) == 0) {
            return new CursorAckRequest(offset, Optional.empty(), properties);
        }
        long remaining = random.nextInt(256);
        return new CursorAckRequest(offset, Optional.of(new BatchAckState(8, new long[] {remaining})), properties);
    }

    private static void assertMatches(AckModel model, CursorState state, int seed, int step) {
        CursorAckState actual = state.acknowledgements();
        String description = "seed=" + seed + ", step=" + step;
        assertThat(actual.markDeleteOffset()).as(description + " mark-delete").isEqualTo(model.markDelete());
        assertThat(actual.wholeAckRanges()).as(description + " whole ranges").isEqualTo(model.ranges());
        assertThat(actual.partialBatchAcks()).as(description + " partials").isEqualTo(model.partials());
    ...
    private static final class AckModel {
        ...
        private void applyEntry(CursorAckRequest request) {
            ...
            int requested = request.batchAck().orElseThrow().isWholeEntryAcknowledged()
                    ? 0
                    : Math.toIntExact(request.batchAck().orElseThrow().remainingWords()[0]);
        ...
        private TreeMap<Long, BatchAckState> partials() {
            ...
            for (int offset = markDelete; offset < whole.length; offset++) {
                if (!whole[offset] && remaining[offset] != 0xff) {
                    result.put((long) offset, new BatchAckState(8, new long[] {remaining[offset]}));
                }
            }
            return result;
        }
