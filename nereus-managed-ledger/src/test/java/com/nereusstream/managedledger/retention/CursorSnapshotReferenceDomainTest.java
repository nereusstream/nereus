...
class CursorSnapshotReferenceDomainTest {
    ...
    void completePagedRootScanVetoesLiveReferenceAndRevalidatesEveryAuthority() {
        try (var store = new FakeCursorMetadataStore()) {
            ...
            var retention = store.createRetention(CLUSTER, activeRetention(projection, OWNER_ONE, 1))
                    .join();
            store.createCursor(CLUSTER, cursor(projection, "subscription-a", Optional.of(snapshot())))
                    .join();
            store.createCursor(CLUSTER, cursor(projection, "subscription-b", Optional.empty()))
                    .join();
            ...
            assertThat(live.references()).singleElement().satisfies(reference -> {
            ...
            store.compareAndSetRetention(
                            CLUSTER, activeRetention(projection, OWNER_TWO, 2), retention.metadataVersion())
                    .join();
    ...
    void pendingRetentionAndSnapshotBoundsFailClosed() {
        try (var store = new FakeCursorMetadataStore()) {
            ManagedLedgerProjectionIdentity projection = projection();
            var active = store.createRetention(CLUSTER, activeRetention(projection, OWNER_ONE, 1))
                    .join();
            store.createCursor(CLUSTER, cursor(projection, "subscription-a", Optional.empty()))
                    .join();
            store.compareAndSetRetention(CLUSTER, trimPendingRetention(projection), active.metadataVersion())
                    .join();
            ...
            var bounded = new CursorSnapshotReferenceDomain(CLUSTER, store, new GcReferenceDomainConfig(1, 1, 100))
                    .snapshot(query("cursor-snapshots/old.ncs"))
                    .join();
    ...
    void ownerlessGlobalScopePagesTheExactCursorNamespaceAndDetectsDrift() {
        try (var store = new FakeCursorMetadataStore()) {
            ...
            store.createCursor(CLUSTER, cursor(projection(), "subscription-global-drift", Optional.empty()))
                    .join();
    ...
    private static CursorRetentionRecord activeRetention(
            ManagedLedgerProjectionIdentity projection, String owner, long sequence) {
    ...
    private static CursorRetentionRecord trimPendingRetention(ManagedLedgerProjectionIdentity projection) {
        return new CursorRetentionRecord(
                0,
                projection,
                OWNER_ONE,
    ...
    private static CursorStateRecord cursor(
            ManagedLedgerProjectionIdentity projection, String name, Optional<CursorSnapshotReferenceRecord> snapshot) {
    ...
    private static CursorSnapshotReferenceRecord snapshot() {
        return new CursorSnapshotReferenceRecord(
                LIVE_KEY, SNAPSHOT_ID, 1, 1, 0, 8, ChecksumType.CRC32C.name(), "00000000", 123, 1, 1);
    }
