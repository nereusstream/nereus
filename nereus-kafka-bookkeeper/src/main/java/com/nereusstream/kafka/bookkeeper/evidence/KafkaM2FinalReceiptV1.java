...
/** Closed canonical K10 receipt and secure attachment verifier for Kafka-owned M2 scenario promotion. */
public final class KafkaM2FinalReceiptV1 {
    public static final String SCHEMA = "NEREUS_V2_M2_KAFKA_FINAL_RECEIPT_V1";
    public static final String K0_INPUTS_PATH = "docs/v2/evidence/v2-m2/kafka/k0-inputs/kafka-inputs.json";
    public static final String K9_EVIDENCE_PATH = "docs/v2/evidence/v2-m2/kafka/k9/k9-evidence.json";
    ...
    public record SourceTuple(
            ...
            String k9EvidenceReceiptSha256,
            String kafkaForkCommit,
            String kafkaInputsReceiptSha256,
            String nereusCommit,
    ...
    public static byte[] canonicalBytes(Receipt receipt) {
        ...
        field(output, "bookKeeperSourceCommit", source.bookKeeperSourceCommit(), false);
        field(output, "k9EvidenceReceiptSha256", source.k9EvidenceReceiptSha256(), true);
        field(output, "kafkaForkCommit", source.kafkaForkCommit(), true);
        field(output, "kafkaInputsReceiptSha256", source.kafkaInputsReceiptSha256(), true);
    ...
    public static Receipt parseCanonical(byte[] bytes) {
        ...
        cursor.expect(",\"sourceTuple\":{\"bookKeeperSourceCommit\":");
        String bookKeeperCommit = cursor.string();
        cursor.expect(",\"k9EvidenceReceiptSha256\":");
        String k9Sha = cursor.string();
        cursor.expect(",\"kafkaForkCommit\":");
        String kafkaCommit = cursor.string();
        cursor.expect(",\"kafkaInputsReceiptSha256\":");
        String inputsSha = cursor.string();
        ...
        Receipt receipt = new Receipt(
                ...
                scenarios,
                schema,
                new SourceTuple(bookKeeperCommit, k9Sha, kafkaCommit, inputsSha, nereusCommit, locksSha));
        validate(receipt);
    ...
    public static void validate(Receipt receipt) {
        ...
        if (!SCHEMA.equals(receipt.schema())
                || receipt.kind() != ReceiptKind.KAFKA_M2_FINAL
                || receipt.result() != ReceiptResult.PASS_KAFKA_M2_FINAL
                || !receipt.promotionEligible()) {
            throw reject(RejectionCode.SCHEMA_KIND_RESULT_INVALID, "invalid Kafka Final identity");
        }
        validateSource(receipt.sourceTuple());
        if (receipt.scenarios().size() != MAX_SCENARIOS) {
            throw reject(RejectionCode.SCENARIO_SET_INVALID, "Kafka Final must contain ten scenarios");
        }
        ...
        AttachmentRef k0 = requireTypedAttachment(attachmentByPath, K0_INPUTS_PATH, AttachmentKind.K0_INPUTS_RECEIPT);
        AttachmentRef k9 =
                requireTypedAttachment(attachmentByPath, K9_EVIDENCE_PATH, AttachmentKind.K9_EVIDENCE_RECEIPT);
        if (!k0.sha256().equals(receipt.sourceTuple().kafkaInputsReceiptSha256())
                || !k9.sha256().equals(receipt.sourceTuple().k9EvidenceReceiptSha256())) {
            throw reject(RejectionCode.PREREQUISITE_BINDING_INVALID, "K0 or K9 digest differs from source tuple");
        }
        for (AttachmentRef attachment : receipt.attachments()) {
            if (!attachment.path().equals(K0_INPUTS_PATH)
                    && !attachment.path().equals(K9_EVIDENCE_PATH)
    ...
    private static void validateSource(SourceTuple source) {
        ...
        requireSha(source.k9EvidenceReceiptSha256());
        requireCommit(source.kafkaForkCommit());
        requireSha(source.kafkaInputsReceiptSha256());
