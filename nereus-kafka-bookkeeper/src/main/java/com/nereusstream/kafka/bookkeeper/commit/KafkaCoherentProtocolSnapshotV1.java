...
/** One resolved immutable K1 root and the exact K5 component objects named by that root. */
public record KafkaCoherentProtocolSnapshotV1(
        KafkaPartitionProtocolStateV1 root,
        KafkaActiveTailStateV1 activeTail,
    ...
    public KafkaCoherentProtocolSnapshotV1 {
        ...
        if (activeTail.endOffsetExclusive() != root.frontiers().readableEndOffset()) {
            throw new IllegalArgumentException("active-tail coverage differs from the coherent readable frontier");
        }
    }

    public OptionalLong firstUnstableOffset() {
        return transactionState.firstUnstableOffset(root.frontiers().highWatermark());
    }
}
