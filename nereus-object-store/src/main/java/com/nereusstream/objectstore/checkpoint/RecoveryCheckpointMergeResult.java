...
public record RecoveryCheckpointMergeResult(
        RecoveryCheckpointWriteRequest request, RecoveryCheckpointWriteResult object, int sourceCount)
        implements AutoCloseable {
    public RecoveryCheckpointMergeResult {
        Objects.requireNonNull(request, "request");
        Objects.requireNonNull(object, "object");
        if (sourceCount < 2
                || !object.objectKey().equals(RecoveryCheckpointFormatV1.objectKey(request, object.contentSha256()))) {
