...
public record RecoveryCheckpointEntry(
    ...
    public RecoveryCheckpointEntry {
        ...
        previousCommitId = RecoveryCheckpointValidation.requirePossiblyEmptyText(previousCommitId, "previousCommitId");
        canonicalCommitRecord = RecoveryCheckpointValidation.immutableRecordBytes(
                canonicalCommitRecord, canonicalCommitRecordSha256, "canonicalCommitRecord");
        canonicalCommitRecordSha256 =
                RecoveryCheckpointValidation.requireSha256(canonicalCommitRecordSha256, "canonicalCommitRecordSha256");
        coveringPublicationIndexes =
                List.copyOf(Objects.requireNonNull(coveringPublicationIndexes, "coveringPublicationIndexes"));
        if (coveringPublicationIndexes.isEmpty()
                || coveringPublicationIndexes.size() > RecoveryCheckpointFormatV1.MAX_PUBLICATION_REFS_PER_ENTRY) {
