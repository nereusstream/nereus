 ...
 */
public record KafkaCheckpointSection(int sectionType, int sectionVersion, int sectionFlags, byte[] payload) {
    public KafkaCheckpointSection {
        if (sectionType <= 0
                || sectionType > 0xffff
                || sectionVersion <= 0
                || sectionVersion > 0xffff
    ...
    public static KafkaCheckpointSection required(KafkaCheckpointSectionType type, byte[] payload) {
        return new KafkaCheckpointSection(type.wireId(), 1, KafkaCheckpointFormatV1.SECTION_REQUIRED_FLAG, payload);
    }
    ...
    public boolean equals(Object other) {
        return this == other
                || other instanceof KafkaCheckpointSection that
    ...
    @Override
    public int hashCode() {
        return 31 * java.util.Objects.hash(sectionType, sectionVersion, sectionFlags) + Arrays.hashCode(payload);
    }
}
