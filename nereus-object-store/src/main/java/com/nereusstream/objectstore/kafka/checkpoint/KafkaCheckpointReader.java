...
public final class KafkaCheckpointReader {
    ...
    public CompletableFuture<KafkaCheckpointObject> openAndVerify(
        ...
        try {
            ...
            if (expectedLength <= 0
                    || expectedLength > KafkaCheckpointFormatV1.MAX_OBJECT_BYTES
                    || timeout.isZero()
                    || timeout.isNegative()) {
            ...
            return CompletableFuture.failedFuture(
                    new NereusException(ErrorCode.INVALID_ARGUMENT, false, "invalid NKC1 open request", failure));
        }
        CompletableFuture<com.nereusstream.objectstore.HeadObjectResult> head =
                objectStore.headObject(key, new HeadObjectOptions(timeout));
                    ...
                    return objectStore
                            ...
                            .thenApply(range -> {
                                ...
                                return new KafkaCheckpointObject(
                                        ...
                                        key,
                                        expectedLength,
                                        decoded.storageCrc32c(),
                                        decoded.objectSha256(),
                                        decoded.contentSha256(),
                                        actual.etag());
                            });
                })
    ...
    private static void requireChecksum(Checksum value, ChecksumType type, String name) {
        ...
        if (value.type() != type) {
            throw new IllegalArgumentException(name + " has the wrong checksum type");
        }
