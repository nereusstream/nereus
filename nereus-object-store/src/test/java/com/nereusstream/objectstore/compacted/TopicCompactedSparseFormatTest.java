...
class TopicCompactedSparseFormatTest {
    ...
    void sparseValuesAndTombstonesPreserveDenseSourceCoverageInNtc1Only() throws Exception {
        CompactedObjectWriteRequest request = CompactedParquetTestSupport.topicRequest(8, 3, 128, 2, "ZSTD");
        ...
        try (StagingFileManager staging = CompactedParquetTestSupport.staging(temporaryDirectory, 32L << 20);
            ...
            ParquetCompactedObjectReader storageReader = new ParquetCompactedObjectReader(store, Runnable::run);
            ...
            CompactedObjectReadResult full =
                    reader.read(readRequest(request, verification, 20, 10)).join();
            assertThat(full.rows()).extracting(CompactedObjectRow::streamOffset).containsExactly(20L, 23L, 27L);
            ...
            assertThat(full.metadata().topicCompaction())
                    .contains(request.topicCompaction().orElseThrow());

            CompactedObjectReadResult suffix =
                    reader.read(readRequest(request, verification, 21, 1)).join();
            assertThat(suffix.rows())
