    ...
    void writesOpensAndRangeLooksUpOneCanonicalNrc1Object() {
        try (LocalFileObjectStore objectStore = new LocalFileObjectStore(temporaryDirectory.resolve("objects"));
            ...
            assertThat(codec.findPublication(
                                    opened, 2, new PublicationId("b".repeat(26)), RecoveryCheckpointTestSupport.TIMEOUT)
                            .join())
                    .contains(RecoveryCheckpointTestSupport.publications().get(1));
            assertThat(codec.findPublication(
                                    opened, 3, new PublicationId("c".repeat(26)), RecoveryCheckpointTestSupport.TIMEOUT)
                            .join())
                    .isEmpty();
            assertThat(codec.findCommit(opened, 6, "commit-6", RecoveryCheckpointTestSupport.TIMEOUT)
                            .join())
                    .contains(RecoveryCheckpointTestSupport.entries().get(1));
            assertThat(codec.findCommit(opened, 6, "wrong-commit", RecoveryCheckpointTestSupport.TIMEOUT)
                            .join())
