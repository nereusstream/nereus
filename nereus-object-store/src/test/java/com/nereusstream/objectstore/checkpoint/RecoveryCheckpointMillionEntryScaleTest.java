...
class RecoveryCheckpointMillionEntryScaleTest {
    private static final int ENTRY_COUNT = RecoveryCheckpointFormatV1.MAX_ENTRY_COUNT;
    ...
    @Test
    void streamsOneMillionEntriesAndReadsSparseDirectoryBoundaries() throws Exception {
        RecoveryCheckpointWriteRequest request = request();
        RecoveryCheckpointPublication publication =
                RecoveryCheckpointTestSupport.publication(1, "p".repeat(26), 0, ENTRY_COUNT);
        RecoveryCheckpointTestSupport.TrackingPublisher<RecoveryCheckpointPublication> publications =
                new RecoveryCheckpointTestSupport.TrackingPublisher<>(List.of(publication));
        ...
        try (LocalFileObjectStore objectStore = new LocalFileObjectStore(temporaryDirectory.resolve("objects"));
                StagingFileManager staging = RecoveryCheckpointTestSupport.staging(temporaryDirectory, 384L << 20)) {
            ...
            try (RecoveryCheckpointWriteResult written =
                    codec.write(request, publications, entries).join()) {
                ...
                assertThat(written.objectLength())
                        .isPositive()
                        .isLessThanOrEqualTo(RecoveryCheckpointFormatV1.MAX_OBJECT_BYTES);
                assertThat(written.directory().publicationDirectoryLength())
                        .isEqualTo(Integer.BYTES + Integer.BYTES + Long.BYTES);
                int commitAnchors = (ENTRY_COUNT + RecoveryCheckpointFormatV1.COMMIT_DIRECTORY_STRIDE - 1)
                ...
                assertThat(written.directory().commitDirectoryLength())
                        .isEqualTo(Integer.BYTES * 2L + commitAnchors * Long.BYTES * 3L);
                assertThat(written.directory().publicationDirectoryLength()
                                + written.directory().commitDirectoryLength())
                        .isLessThanOrEqualTo(RecoveryCheckpointFormatV1.MAX_DIRECTORY_BYTES);
                assertThat(staging.reservedBytes()).isPositive().isLessThanOrEqualTo(staging.maxStagingBytes());
                ...
                RecoveryCheckpointObject opened = codec.openAndVerify(
                                written.objectKey(), written.objectLength(), written.contentSha256(), SCALE_TIMEOUT)
                        .join();

                assertThat(opened.header()).isEqualTo(request);
                assertThat(opened.directory()).isEqualTo(written.directory());
                assertEntry(
                        codec.findCommit(opened, 1, commitId(1), SCALE_TIMEOUT)
                                .join()
                                .orElseThrow(),
                        1);
                assertEntry(
                        codec.findCommitCoveringOffset(opened, ENTRY_COUNT / 2L, SCALE_TIMEOUT)
                                .join()
                                .orElseThrow(),
                        ENTRY_COUNT / 2L + 1);
                assertEntry(
                        codec.findCommit(opened, ENTRY_COUNT, commitId(ENTRY_COUNT), SCALE_TIMEOUT)
                                .join()
                                .orElseThrow(),
                        ENTRY_COUNT);
                assertThat(codec.findCommitCoveringOffset(opened, ENTRY_COUNT, SCALE_TIMEOUT)
                                .join())
                        .isEmpty();
            }
    ...
    private static void assertEntry(RecoveryCheckpointEntry entry, long version) {
        assertThat(entry.commitVersion()).isEqualTo(version);
        assertThat(entry.range()).isEqualTo(new OffsetRange(version - 1, version));
        ...
        assertThat(RecoveryCheckpointTestSupport.text(entry.canonicalCommitRecord()))
    ...
    private static RecoveryCheckpointEntry entry(long version) {
        return RecoveryCheckpointTestSupport.entry(
                version, version - 1, version, version, commitId(version), previousCommitId(version), List.of(0));
    }
    ...
    private static final class GeneratedEntryPublisher implements Flow.Publisher<RecoveryCheckpointEntry> {
        ...
        @Override
        public void subscribe(Flow.Subscriber<? super RecoveryCheckpointEntry> subscriber) {
            if (!subscribed.compareAndSet(false, true)) {
                subscriber.onSubscribe(new EmptySubscription());
                subscriber.onError(new AssertionError("generated checkpoint entries subscribed more than once"));
                return;
            }
            subscriber.onSubscribe(new Flow.Subscription() {
                ...
                public void request(long count) {
                    ...
                    if (count != 1) {
                        terminal = true;
                        subscriber.onError(new AssertionError("checkpoint codec demand must be exactly one entry"));
                        return;
                    }
