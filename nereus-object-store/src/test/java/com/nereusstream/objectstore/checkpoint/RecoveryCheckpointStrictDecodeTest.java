...
class RecoveryCheckpointStrictDecodeTest {
    ...
    void lookupRejectsBadEntryCrcEvenWhenObjectAndFooterDigestsWereRecomputed() throws Exception {
        try (Fixture fixture = fixture("entry-crc")) {
            ...
            long firstEntryOffset =
                    firstCommitEntryOffset(corrupted, fixture.written().directory());
            ...
            RecoveryCheckpointObject opened = fixture.codec()
                    .openAndVerify(
                            uploaded.key(),
                            corrupted.length,
            ...
            assertThatThrownBy(() -> fixture.codec()
                            .findCommit(opened, 5, "commit-5", RecoveryCheckpointTestSupport.TIMEOUT)
                            .join())
    ...
    void openRejectsNonCanonicalDirectoryEvenWhenAllObjectDigestsMatch() throws Exception {
        try (Fixture fixture = fixture("directory")) {
            byte[] corrupted = fixture.bytes().clone();
            int firstIndex =
                    Math.toIntExact(fixture.written().directory().publicationDirectoryOffset() + Integer.BYTES);
            ...
            assertThatThrownBy(() -> fixture.codec()
                            ...
                            .join())
                    .hasRootCauseInstanceOf(RecoveryCheckpointFormatException.class)
                    .hasRootCauseMessage("NRC1 publication directory is not canonical and strictly ordered");
        }
    }

    private Fixture fixture(String suffix) throws Exception {
        LocalFileObjectStore objectStore = new LocalFileObjectStore(temporaryDirectory.resolve("objects-" + suffix));
        ...
        RecoveryCheckpointWriteResult written = codec.write(
                ...
                .join();
        return new Fixture(
                objectStore, staging, codec, written, RecoveryCheckpointTestSupport.collect(written.stagingFile()));
    }

    private static long firstCommitEntryOffset(byte[] bytes, RecoveryCheckpointDirectory directory) {
        int valueOffset = Math.toIntExact(directory.commitDirectoryOffset()) + Integer.BYTES * 2 + Long.BYTES * 2;
    ...
    private static UploadedCorruption uploadSelfConsistent(Fixture fixture, byte[] bytes) {
        ...
        Checksum body = RecoveryCheckpointTestSupport.sha256(java.util.Arrays.copyOfRange(bytes, 0, footerOffset));
        ...
        byte[] protectedFooter = java.util.Arrays.copyOfRange(bytes, footerOffset, bytes.length - Integer.BYTES);
        footer.putInt(
                bytes.length - Integer.BYTES, Crc32cChecksums.intValue(Crc32cChecksums.checksum(protectedFooter)));
        Checksum content = RecoveryCheckpointTestSupport.sha256(bytes);
        ObjectKey key =
                RecoveryCheckpointFormatV1.objectKey(RecoveryCheckpointTestSupport.request("a".repeat(26)), content);
        fixture.objectStore()
                .putObject(
                        key,
