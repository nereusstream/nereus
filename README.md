...
Phase 3 当前已进入 F3-M1 metadata/snapshot foundation 实现：metadata records/codecs、single-key
CursorMetadataStore/Oxia adapter contract、F2 activation marker preservation、ack domain、snapshot codec/store 和
focused unit/contract/golden tests 已存在。M1 Gradle gate 与真实 Oxia/ObjectStore integration 尚未完成，
F3-M2-M6 尚未开始；因此仍不能描述为已支持 durable Nereus subscriptions。
