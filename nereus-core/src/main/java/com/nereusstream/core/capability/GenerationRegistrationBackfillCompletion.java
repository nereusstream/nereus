...
public record GenerationRegistrationBackfillCompletion(
        String runId, GenerationCapabilityReadiness readiness, Checksum coverageSha256, long failureCount) {
    public GenerationRegistrationBackfillCompletion {
        runId = requireBase32Id(runId);
        Objects.requireNonNull(readiness, "readiness");
        coverageSha256 = GcReferenceQuery.requireSha256(coverageSha256, "coverageSha256");
        if (failureCount < 0) {
    ...
    private static String requireBase32Id(String value) {
        ...
        if (value.length() < 26 || value.length() > 128 || !value.matches("[a-z2-7]+")) {
            throw new IllegalArgumentException("runId must be lowercase base32 and encode at least 128 bits");
        }
        return value;
    }
}
