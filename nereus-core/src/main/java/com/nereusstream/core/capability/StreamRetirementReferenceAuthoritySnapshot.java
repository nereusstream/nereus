    ...
    public StreamRetirementReferenceAuthoritySnapshot {
        ...
        for (GcAuthorityToken authority : canonical) {
            if (!keys.add(authority.authorityKey())) {
                throw new IllegalArgumentException("stream-retirement authorities contain duplicate keys");
            }
        }
        authorities = List.copyOf(canonical);
        if (authorityCount < authorities.size()) {
            throw new IllegalArgumentException("authorityCount cannot be smaller than retained authorities");
        }
        if (complete && authorityCount != authorities.size()) {
            throw new IllegalArgumentException("a complete stream-retirement snapshot must retain every authority");
        }
        if (referenceFree != (complete && liveReferenceCount == 0)) {
            throw new IllegalArgumentException("referenceFree requires a complete zero-reference snapshot");
        }
    }

    public static StreamRetirementReferenceAuthoritySnapshot complete(
            LiveProjectionSubject subject, long liveReferenceCount, List<GcAuthorityToken> authorities) {
        List<GcAuthorityToken> exact = List.copyOf(authorities);
        return new StreamRetirementReferenceAuthoritySnapshot(
                subject, true, liveReferenceCount == 0, exact.size(), liveReferenceCount, exact);
    }
}
