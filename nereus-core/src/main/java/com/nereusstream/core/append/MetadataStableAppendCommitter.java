...
final class MetadataStableAppendCommitter implements StableAppendCommitter {
    ...
    @Override
    public CompletableFuture<PreparedStableAppend> prepare(CommitAppendRequest request) {
        return metadata.prepareStableAppend(cluster, request);
    }

    @Override
    public CompletableFuture<StableAppendResult> commit(ProtectedStableAppend append) {
        return metadata.commitPreparedStableAppend(cluster, append.prepared(), append.proof());
    }
}
