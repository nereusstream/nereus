 ...
 */
public record ProtectedGenerationZero(MaterializedGenerationZero materialized, PhysicalReferenceProof proof) {
    public ProtectedGenerationZero {
        ...
        if (proof.purpose() != PhysicalReferencePurpose.VISIBLE_GENERATION
                || proof.targetType()
                        != materialized.committedAppend().readTarget().type()
                || !proof.targetIdentitySha256()
                        .equals(ReadTargetIdentities.sha256(
                                materialized.committedAppend().readTarget()))
                || !proof.referenceId()
                        .equals(GenerationZeroProtectionIdentities.visibleGenerationReferenceId(materialized))) {
