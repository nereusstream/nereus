...
public final class GenerationZeroProtectionIdentities {
    ...
    static String reachableAppendReferenceId(PreparedStableAppend prepared) {
        if (prepared.request().readTarget() instanceof ObjectSliceReadTarget target) {
            return reachableAppendReferenceId(
                    prepared.request().streamId(), prepared.commitId(), ObjectKeyHash.from(target.objectKey()));
        }
        return reachableAppendReferenceId(
                prepared.request().streamId(), prepared.commitId(), prepared.primaryTargetIdentitySha256());
    }

    static String visibleGenerationReferenceId(MaterializedGenerationZero materialized) {
        return visibleGenerationReferenceId(
                materialized.committedAppend().streamId(), materialized.indexKey(), materialized.indexRecordSha256());
    }

    public static String reachableAppendReferenceId(
            com.nereusstream.api.StreamId streamId, String commitId, ObjectKeyHash objectKeyHash) {
        ...
        return "ra1-" + DeterministicIds.stableHashComponent(streamId.value() + commitId + objectKeyHash.value());
    }
    ...
    public static String reachableAppendReferenceId(
        ...
        return "ra1-"
                + DeterministicIds.stableHashComponent(streamId.value() + commitId + targetIdentitySha256.value());
    }

    public static String visibleGenerationReferenceId(
            com.nereusstream.api.StreamId streamId, String indexKey, com.nereusstream.api.Checksum indexRecordSha256) {
        java.util.Objects.requireNonNull(streamId, "streamId");
        ...
        if (indexRecordSha256.type() != com.nereusstream.api.ChecksumType.SHA256) {
            throw new IllegalArgumentException("indexRecordSha256 must use SHA256");
        }
        return "vg0-" + DeterministicIds.stableHashComponent(streamId.value() + indexKey + indexRecordSha256.value());
    }
