...
public interface PrimaryPhysicalReferenceAdapter<T extends ReadTarget> {
    ...
    CompletableFuture<ProtectedStableAppend> protectBeforeHead(PreparedStableAppend append, T target, Duration timeout);

    CompletableFuture<ProtectedGenerationZero> protectVisibleIndex(
            MaterializedGenerationZero append, T target, Duration timeout);
}
