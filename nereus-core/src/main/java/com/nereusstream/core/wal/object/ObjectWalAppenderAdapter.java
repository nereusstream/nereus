...
public final class ObjectWalAppenderAdapter implements PrimaryWalAppender<ObjectPreparedPrimaryAppend> {
    ...
    public ObjectWalAppenderAdapter(
            String cluster, String writerId, String writerRunIdHash, int maxObjectBytes, WalObjectWriter writer) {
        this(
                cluster,
                writerId,
                writerRunIdHash,
                maxObjectBytes,
                writer,
                ObjectWalUploadAuthorizer.noOp(),
                ObjectWalManifestPublisher.noOp());
    }
    ...
    public ObjectWalAppenderAdapter(
        ...
        this.writerRunIdHash = Objects.requireNonNull(writerRunIdHash);
    ...
    @Override
    public Class<ObjectPreparedPrimaryAppend> preparedClass() {
        return ObjectPreparedPrimaryAppend.class;
    }

    @Override
    public ObjectPreparedPrimaryAppend prepare(PrimaryAppendRequest request) {
        PreparedWalObject prepared = writer.prepare(new WalWriteRequest(
                cluster,
                writerId,
                writerRunIdHash,
                request.session().epoch(),
                List.of(new WalStreamSliceInput(request.streamId(), request.batch())),
        ...
        return new ObjectPreparedPrimaryAppend(
                request.streamId(),
                slice.recordCount(),
                slice.entryCount(),
                slice.logicalBytes(),
                prepared.objectLength(),
                prepared,
                request);
    }

    @Override
    public CompletableFuture<DurablePrimaryAppend> persist(ObjectPreparedPrimaryAppend prepared, Duration timeout) {
        return persist(
                prepared,
                timeout,
                (key, attempt) -> uploadAuthorizer.authorize(
                        prepared, physicalObject(prepared.preparedObject().result()), key, attempt, timeout));
    }

    @Override
    public CompletableFuture<Void> publishDurableMetadata(
    ...
    public CompletableFuture<DurablePrimaryAppend> persist(
            ObjectPreparedPrimaryAppend prepared, Duration timeout, PutObjectAttemptGuard attemptGuard) {
        ...
        return writer.upload(prepared.preparedObject(), attemptGuard).thenApply(result -> {
            ...
            return new DurablePrimaryAppend(
                    prepared.streamId(),
                    target,
                    ObjectPrimaryPhysicalIdentity.from(target),
                    slice.sliceChecksum(),
                    slice.payloadFormat(),
                    slice.recordCount(),
                    slice.entryCount(),
                    ...
                    new ObjectWalCommitEvidence(result));
        });
    }
