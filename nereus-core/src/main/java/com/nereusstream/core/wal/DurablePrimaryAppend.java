...
public record DurablePrimaryAppend(
    ...
    public DurablePrimaryAppend {
        Objects.requireNonNull(streamId);
        Objects.requireNonNull(readTarget);
        Objects.requireNonNull(physicalIdentity);
        Objects.requireNonNull(payloadChecksum);
        Objects.requireNonNull(payloadFormat);
        schemaRefs = MetadataCanonicalizer.canonicalSchemaRefs(schemaRefs);
        Objects.requireNonNull(providerToken);
        if (physicalIdentity.targetType() != readTarget.type()
                || recordCount <= 0
                || entryCount <= 0
                || logicalBytes < 0
    ...
    public ProviderAppendToken providerCommitEvidence() {
