...
public final class Phase15StorageProfileResolver implements StorageProfileResolver {
    ...
    public StorageExecutionPlan requireExecutable(
            StorageProfile raw, DurabilityLevel durability, boolean appender, boolean reader) {
        ...
        return new StorageExecutionPlan(
                profile, ReadTargetType.OBJECT_SLICE, ObjectPublicationMode.SYNCHRONOUS, durability);
    }
    ...
    public ReadTargetType requireReadable(StorageProfile raw, Predicate<ReadTargetType> readerInstalled) {
        ...
        if (profile != StorageProfile.OBJECT_WAL_SYNC_OBJECT || !installed.test(ReadTargetType.OBJECT_SLICE)) {
            throw new NereusException(
                    ErrorCode.UNSUPPORTED_STORAGE_PROFILE,
                    false,
