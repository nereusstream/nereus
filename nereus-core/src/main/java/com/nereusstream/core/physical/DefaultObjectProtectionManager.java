...
public final class DefaultObjectProtectionManager implements ObjectProtectionManager {
    ...
    public DefaultObjectProtectionManager(
        ...
        this.pendingProtectionDurationMillis =
                requirePositiveMillis(pendingProtectionDuration, "pendingProtectionDuration");
    ...
    public CompletableFuture<ObjectProtection> acquireOrTransfer(
        ...
        return serialized(request.identity(), () -> acquireOrTransferSerialized(request, ownerRevalidator));
    }

    @Override
    public CompletableFuture<Optional<ObjectProtection>> findExisting(
            PhysicalObjectIdentity object, ObjectProtectionIdentity identity) {
        ...
        if (!exactIdentity.object().equals(exactObject.objectKeyHash())) {
            return failed(new IllegalArgumentException("protection identity does not match the physical object"));
        }
        if (closed.get()) {
            return failed(closedFailure());
        }
        return serialized(exactIdentity, () -> findProtection(exactIdentity, Optional.empty())
    ...
    public CompletableFuture<ObjectProtection> transfer(
        ...
        return serialized(protection.identity(), () -> transferSerialized(protection, newOwner, newOwnerRevalidator));
    }
    ...
    public CompletableFuture<Void> release(ObjectProtection protection, RemovalAuthorizer removalAuthorizer) {
        ...
        return serialized(protection.identity(), () -> releaseSerialized(protection, removalAuthorizer));
    }
    ...
    private CompletableFuture<ObjectProtection> acquireSerialized(
        ...
        return ensureActiveRoot(request.object(), now).thenCompose(root -> findProtection(
                        request.identity(), Optional.empty())
                .thenCompose(
                        existing -> createOrReuse(request, root, existing, now).thenCompose(acquisition -> {
                            ObjectProtection protection = fromVersioned(request.object(), acquisition.value());
                            CompletableFuture<ObjectProtection> checked = postCheck(protection, root, ownerRevalidator);
    ...
    private CompletableFuture<ObjectProtection> acquireOrTransferSerialized(
        ...
        return ensureActiveRoot(request.object(), now)
                .thenCompose(root -> findProtection(request.identity(), Optional.empty())
                        .thenCompose(existing -> existing.isPresent()
                                ? reconcileExisting(request, root, existing.orElseThrow(), ownerRevalidator)
                                : createForReconciliation(request, root, ownerRevalidator, now)));
    }
    ...
    private CompletableFuture<ObjectProtection> createForReconciliation(
        ...
        return store.createProtection(cluster, record)
                .handle((created, failure) -> {
                    ...
                    return findProtection(request.identity(), Optional.empty()).thenCompose(recovered -> {
                        ...
                        return reconcileExisting(request, root, recovered.orElseThrow(), ownerRevalidator)
    ...
    private CompletableFuture<ObjectProtection> reconcileExisting(
        ...
        if (existing.owner().equals(request.owner())
                && existing.rootLifecycleEpoch() == root.value().lifecycleEpoch()) {
        ...
        return invokeOwnerRevalidator(ownerRevalidator, request.owner()).thenCompose(ignored -> {
            ...
            return compareAndSetOrRecover(existing, current, desired)
                    .thenCompose(
                            updated -> postCheck(fromVersioned(request.object(), updated), root, ownerRevalidator));
        });
    }

    private CompletableFuture<ObjectProtection> revalidateSerialized(
            ObjectProtection expected, OwnerRevalidator ownerRevalidator) {
        if (closed.get()) {
            return failed(closedFailure());
        }
        return findProtection(expected.identity(), Optional.empty()).thenCompose(optional -> {
            VersionedObjectProtection current =
                    optional.orElseThrow(() -> condition("object protection is absent during revalidation"));
            requireSameHandle(expected, current);
            return getExactActiveRoot(expected.object())
                    .thenCompose(root -> postCheck(fromVersioned(expected.object(), current), root, ownerRevalidator));
        });
    }

    private CompletableFuture<ObjectProtection> transferSerialized(
            ObjectProtection expected, ObjectProtectionOwner newOwner, OwnerRevalidator newOwnerRevalidator) {
        if (closed.get()) {
            return failed(closedFailure());
        }
        return invokeOwnerRevalidator(newOwnerRevalidator, newOwner)
                .thenCompose(ignored -> findProtection(expected.identity(), Optional.empty())
                        .thenCompose(optional -> {
                            ...
                            if (!matchesHandle(expected, current)
                                ...
                                return failed(condition("object protection changed before owner transfer"));
                            }
    ...
    private CompletableFuture<Void> releaseSerialized(ObjectProtection expected, RemovalAuthorizer removalAuthorizer) {
        return findProtection(expected.identity(), Optional.empty()).thenCompose(optional -> {
            if (optional.isEmpty()) {
            ...
            return invokeRemovalAuthorizer(removalAuthorizer, exact)
                    .thenCompose(ignored -> findProtection(expected.identity(), Optional.empty())
                            .thenCompose(afterAuthorization -> {
                                ...
                                if (!sameVersioned(current, stillCurrent)) {
                                    return failed(condition("object protection changed while removal was authorized"));
                                }
    ...
    private CompletableFuture<VersionedObjectProtection> compareAndSetOrRecover(
            ObjectProtection expected, VersionedObjectProtection current, ObjectProtectionRecord desired) {
    ...
    private CompletableFuture<ObjectProtection> postCheck(
            ObjectProtection protection, VersionedPhysicalObjectRoot expectedRoot, OwnerRevalidator ownerRevalidator) {
        return getExactActiveRoot(protection.object()).thenCompose(currentRoot -> {
            if (!PhysicalObjectRecords.sameActiveRoot(expectedRoot, currentRoot, protection.object())
                    || protection.rootLifecycleEpoch() != currentRoot.value().lifecycleEpoch()) {
                return failed(condition("physical root changed during object protection handshake"));
            }
            try {
            ...
            return invokeOwnerRevalidator(ownerRevalidator, protection.owner()).thenApply(ignored -> protection);
        });
    }
    ...
    private CompletableFuture<Optional<VersionedObjectProtection>> findProtection(
        ...
        return store.scanProtections(cluster, identity.object(), continuation, SCAN_PAGE_SIZE)
                ...
                .handle((ignored, failure) -> {
                    ...
                    return findProtection(identity, Optional.empty()).thenCompose(current -> {
                        ...
                        if (!sameVersioned(expected, exact)) {
                            return failed(invariant("protection delete response loss found a different value"));
                        }
                        return store.deleteProtection(cluster, identity, exact.metadataVersion());
                    });
                })
                .thenCompose(value -> value);
    }
    ...
    private void requireReusable(
            ObjectProtectionRequest request, VersionedPhysicalObjectRoot root, VersionedObjectProtection current) {
    ...
    private void validateRequestedExpiry(
            com.nereusstream.metadata.oxia.records.ObjectProtectionType type, long expiresAtMillis, long now) {
        ...
        if (expiresAtMillis <= skewSafeNow || expiresAtMillis > maximumExpiry) {
            throw new IllegalArgumentException("pending protection expiry is outside its skew-safe configured window");
        }
    }
    ...
    private void validateDurableExpiry(
        ...
        if (ObjectProtectionRequest.isPending(type)
                && expiresAtMillis > Math.addExact(createdAtMillis, pendingProtectionDurationMillis)) {
    ...
    private static ObjectProtectionRecord transferRecord(
            ObjectProtectionRecord current, ObjectProtectionOwner newOwner, long rootLifecycleEpoch) {
    ...
    private static ObjectProtectionRecord requestedRecord(
            ObjectProtectionRequest request, VersionedPhysicalObjectRoot root, long now) {
        return new ObjectProtectionRecord(
                1,
    ...
    private static ObjectProtection fromVersioned(PhysicalObjectIdentity object, VersionedObjectProtection value) {
    ...
    private static ObjectProtectionIdentity identity(ObjectProtectionRecord value) {
        return new ObjectProtectionIdentity(
                ...
                com.nereusstream.metadata.oxia.records.ObjectProtectionType.fromWireId(value.protectionTypeId()),
                value.referenceId());
    }
    ...
    private static ObjectProtectionOwner owner(ObjectProtectionRecord value) {
        return new ObjectProtectionOwner(
                ...
                new com.nereusstream.api.Checksum(
                        com.nereusstream.api.ChecksumType.SHA256, value.ownerIdentitySha256()));
    }

    private static void requireRecordIdentity(ObjectProtectionIdentity expected, ObjectProtectionRecord actual) {
        if (!recordMatchesIdentity(expected, actual)) {
            throw invariant("protection record does not match its exact identity");
        }
    }

    private static boolean recordMatchesIdentity(ObjectProtectionIdentity identity, ObjectProtectionRecord value) {
    ...
    private static void requireSameHandle(ObjectProtection expected, VersionedObjectProtection actual) {
    ...
    private static boolean matchesHandle(ObjectProtection expected, VersionedObjectProtection actual) {
    ...
    private static boolean matchesTransferredOutcome(
            ObjectProtection previous, ObjectProtectionOwner newOwner, ObjectProtectionRecord current) {
    ...
    private static boolean sameRecordIgnoringMetadataVersion(
            ObjectProtectionRecord left, ObjectProtectionRecord right) {
        return left.withMetadataVersion(0).equals(right.withMetadataVersion(0));
    }

    private static boolean sameVersioned(VersionedObjectProtection left, VersionedObjectProtection right) {
    ...
    private static CompletableFuture<Void> invokeOwnerRevalidator(
            OwnerRevalidator revalidator, ObjectProtectionOwner owner) {
        try {
            return Objects.requireNonNull(revalidator.revalidate(owner), "owner revalidator result");
    ...
    private static CompletableFuture<Void> invokeRemovalAuthorizer(
            RemovalAuthorizer authorizer, ObjectProtection protection) {
        try {
            return Objects.requireNonNull(authorizer.authorizeRemoval(protection), "removal authorizer result");
        } catch (Throwable failure) {
            return failed(failure);
        }
    }
    ...
    private <T> CompletableFuture<T> serialized(
            ObjectProtectionIdentity identity, Supplier<CompletableFuture<T>> operation) {
        ...
        synchronized (state) {
            CompletableFuture<T> result = state.tail
                    ...
                    .thenCompose(ignored -> {
                        try {
                            return Objects.requireNonNull(operation.get(), "serialized operation result");
                        } catch (Throwable failure) {
                            return failed(failure);
                        }
                    });
    ...
    private static NereusException closedFailure() {
        return new NereusException(ErrorCode.STORAGE_CLOSED, false, "object protection manager is closed");
    }
    ...
    private record Acquisition(VersionedObjectProtection value, boolean createdByThisCall) {}
