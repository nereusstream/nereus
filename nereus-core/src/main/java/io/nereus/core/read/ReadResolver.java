...
import io.nereus.metadata.oxia.DerivedIndexRepairCursor;
...
public final class ReadResolver implements AutoCloseable {
    ...
    public ReadResolver(
        ...
        this.cache = new OffsetIndexCache(config.enableOffsetIndexCache(), config.offsetIndexCacheTtl(), clock);
    ...
    private CompletableFuture<Snapshot> loadSnapshot(
            StreamId streamId,
            ReadOperationDeadline deadline) {
        CompletableFuture<StreamMetadataRecord> metadata = deadline.bound(
                () -> metadataStore.getStream(config.cluster(), streamId),
                "load stream metadata for resolve");
        CompletableFuture<TrimRecord> trim = deadline.bound(
                () -> metadataStore.getTrim(config.cluster(), streamId),
                "load trim offset for resolve");
        CompletableFuture<CommittedEndOffsetRecord> committed = deadline.bound(
                () -> metadataStore.getCommittedEndOffset(config.cluster(), streamId),
                "load committed end for resolve");
        return CompletableFuture.allOf(metadata, trim, committed).thenApplyAsync(ignored -> {
            StreamMetadataRecord stream = metadata.join();
            TrimRecord lowWatermark = trim.join();
            CommittedEndOffsetRecord end = committed.join();
            if (!stream.streamId().equals(streamId.value())
                    || !lowWatermark.streamId().equals(streamId.value())
                    || !end.streamId().equals(streamId.value())) {
                throw invariant("resolve metadata records belong to different streams");
            }
            long metadataVersion = Math.max(
                    stream.metadataVersion(),
                    Math.max(lowWatermark.metadataVersion(), end.metadataVersion()));
            return new Snapshot(stream, lowWatermark, end, metadataVersion);
        }, callbackExecutor);
    }
    ...
    private void ensureWatch(StreamId streamId) {
        ...
        try {
            WatchRegistration registration = metadataStore.watchStream(
                    config.cluster(), streamId, new CacheInvalidatingWatcher());
            WatchRegistration existing = watches.putIfAbsent(streamId, registration);
            if (existing != null) {
                registration.close();
            }
