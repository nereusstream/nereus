...
import io.nereus.metadata.oxia.codec.MetadataCodecException;
...
public final class OxiaJavaClientMetadataStore implements OxiaMetadataStore {
    ...
    OxiaJavaClientMetadataStore(
        ...
        this.operationExecutor = Executors.newFixedThreadPool(4, daemonFactory("nereus-oxia-operation"));
        this.clientExecutor = Executors.newFixedThreadPool(8, daemonFactory("nereus-oxia-client"));
        this.client = new PartitionedOxiaClient(new OxiaJavaClientBackend(oxiaClient, clientExecutor));
    }
    ...
    public CompletableFuture<Optional<ObjectReferenceRecord>> getObjectReferences(
            String cluster,
            ObjectId objectId) {
        return complete(() -> getRecord(
                new OxiaKeyspace(cluster).objectReferencesKey(objectId),
                new OxiaKeyspace(cluster).objectPartitionKey(objectId),
    ...
    public CompletableFuture<List<OffsetIndexRecord>> scanOffsetIndex(
        ...
        return complete(() -> {
            ...
            return join(client.list(
                            keyspace.offsetIndexScanFromExclusive(streamId, startOffset),
                            keyspace.offsetIndexScanToExclusive(streamId),
                            partitionKey))
                    .stream()
                    .map(key -> getRecord(key, partitionKey, OffsetIndexRecord.class)
                            .orElseThrow(() -> invariant("listed offset index disappeared")))
    ...
    private ObjectReferenceRecord repairObjectReferencesSync(OxiaKeyspace keyspace, ObjectId objectId) {
        ...
        visible = visible.stream()
                ...
                .toList();
        for (int attempt = 0; attempt < MAX_CAS_RETRIES; attempt++) {
            Optional<ObjectReferenceRecord> existing = getRecord(
