...
public interface KafkaPartitionMetadataStore extends AutoCloseable {
    ...
    CompletableFuture<VersionedKafkaPartitionBinding> putCreatingIfAbsent(KafkaPartitionBindingRecord value);
    ...
    default CompletableFuture<VersionedKafkaPartitionBinding> claimOperation(
            VersionedKafkaPartitionBinding expected, KafkaPartitionPendingOperationRecord operation, long nowMillis) {
        return compareAndSet(
                expected, KafkaPartitionMetadataTransitions.claimOperation(expected.value(), operation, nowMillis));
    }

    default CompletableFuture<VersionedKafkaPartitionBinding> clearOperation(
            VersionedKafkaPartitionBinding expected, String attemptId, long updatedAtMillis) {
        return compareAndSet(
                expected,
    ...
    CompletableFuture<KafkaPartitionScanPage> scanRegistry(int shard, Optional<String> continuation, int limit);
