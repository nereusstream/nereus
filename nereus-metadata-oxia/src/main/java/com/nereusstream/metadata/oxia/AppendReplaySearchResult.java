...
public record AppendReplaySearchResult(
        ...
        int scannedRecords) {
    public AppendReplaySearchResult {
        Objects.requireNonNull(status);
        committedAppend = Objects.requireNonNull(committedAppend);
        continuation = Objects.requireNonNull(continuation);
        if (scannedRecords < 0) {
            throw new IllegalArgumentException("negative scanned records");
        }
