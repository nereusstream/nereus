...
public final class KafkaPartitionMetadataTransitions {
    ...
    public static KafkaPartitionBindingRecord creating(
        ...
        return new KafkaPartitionBindingRecord(
                ...
                storageProfile,
                KafkaPartitionLifecycle.CREATING.wireId(),
                1,
    ...
    public static KafkaPartitionBindingRecord activate(
        ...
        KafkaPartitionBindingRecord update = copy(
                ...
                streamName,
                streamId,
                KafkaPartitionLifecycle.ACTIVE,
                appliedMetadataOffset,
    ...
    public static KafkaPartitionBindingRecord observe(
        ...
        if (leaderEpoch >= 0 && current.observedLeaderEpoch() >= 0 && leaderEpoch < current.observedLeaderEpoch()) {
        ...
        KafkaPartitionBindingRecord update = new KafkaPartitionBindingRecord(
                ...
                current.lifecycleId(),
                Math.addExact(current.bindingEpoch(), 1),
    ...
    public static KafkaPartitionBindingRecord claimOperation(
            KafkaPartitionBindingRecord current, KafkaPartitionPendingOperationRecord requested, long nowMillis) {
        Objects.requireNonNull(requested, "requested");
        if (requested.isEmpty()) {
            throw new IllegalArgumentException("claimed operation cannot be NONE");
        }
        ...
        if (sameAttempt
                && existing.ownerId().equals(requested.ownerId())
    ...
    public static KafkaPartitionBindingRecord clearOperation(
            KafkaPartitionBindingRecord current, String attemptId, long nowMillis) {
    ...
    public static KafkaPartitionBindingRecord beginDelete(
            KafkaPartitionBindingRecord current, KafkaPartitionPendingOperationRecord deleteOperation, long nowMillis) {
        ...
        KafkaPartitionBindingRecord update = copy(
                ...
                current.streamName(),
                current.streamId(),
                KafkaPartitionLifecycle.DELETING,
    ...
    public static KafkaPartitionBindingRecord markDeleted(
        ...
        KafkaPartitionBindingRecord update = copy(
                ...
                current.streamName(),
                current.streamId(),
                KafkaPartitionLifecycle.DELETED,
                current.lastAppliedMetadataOffset(),
                -1,
                current.observedLeaderEpoch(),
                -1,
    ...
    public static KafkaPartitionBindingRecord markCorrupt(
            KafkaPartitionBindingRecord current, String errorCode, long nowMillis) {
        KafkaPartitionPendingOperationRecord existing = current.pendingOperation();
        KafkaPartitionPendingOperationRecord repair = existing.isEmpty()
                ...
                : new KafkaPartitionPendingOperationRecord(
                        ...
                        existing.ownerId(),
                        existing.ownerEpoch(),
                        Math.max(existing.leaseExpiresAtMillis(), Math.addExact(nowMillis, 1)),
        ...
        KafkaPartitionBindingRecord update = copy(
                ...
                current.streamName(),
                current.streamId(),
                KafkaPartitionLifecycle.CORRUPT,
                current.lastAppliedMetadataOffset(),
                current.observedLeaderId(),
                current.observedLeaderEpoch(),
                current.observedBrokerEpoch(),
    ...
    public static KafkaPartitionBindingRecord prependCheckpoint(
        ...
        if (authoritativeTrimOffset < 0
                || authoritativeEndOffset < authoritativeTrimOffset
        ...
        KafkaPartitionBindingRecord update = copy(
                ...
                current.observedLeaderEpoch(),
                current.observedBrokerEpoch(),
                authoritativeTrimOffset,
                authoritativeEndOffset,
                current.compactionCoverage(),
                references,
                current.pendingOperation(),
                nowMillis);
    ...
    public static KafkaPartitionBindingRecord activateCompactionCoverage(
        ...
        if (authoritativeStableEndOffset < current.observedStableEndOffset()
            ...
            throw new IllegalArgumentException("compaction activation is outside the authoritative partition window");
        }
        switch (mode) {
            case INITIAL -> {
                if (before.coverageVersion() != 0 || startOffset != current.observedLogStartOffset()) {
            ...
            case REPLACE -> {
                if (before.coverageVersion() == 0
                        || startOffset < before.startOffset()
                        || endOffset < before.endOffset()
                        || (startOffset > before.startOffset() && startOffset > current.observedLogStartOffset())
                        || (startOffset > before.endOffset() && startOffset != current.observedLogStartOffset())) {
                    throw new KafkaMetadataConditionFailedException(
                            "compaction replacement cannot leave an untrimmed mandatory gap");
                }
            }
        }
        long activationEpoch = before.coverageVersion() == 0 ? 1 : Math.addExact(before.activationEpoch(), 1);
        KafkaCompactionCoverageRecord coverage = new KafkaCompactionCoverageRecord(
                1, startOffset, endOffset, activationEpoch, generationSetSha256, policySha256, nowMillis);
        KafkaPartitionBindingRecord update = copy(
                current,
                current.streamName(),
                current.streamId(),
    ...
    public static void validate(KafkaPartitionBindingRecord current, KafkaPartitionBindingRecord update) {
        ...
        if (update.bindingEpoch() != current.bindingEpoch() + 1
                || update.metadataVersion() != 0
        ...
        if (after.coverageVersion() == 0
                ...
                || (before.coverageVersion() == 0
                        && (after.startOffset() != update.observedLogStartOffset() || after.activationEpoch() != 1))
                || (before.coverageVersion() != 0
                        && (after.endOffset() < before.endOffset()
                                || after.activationEpoch() != Math.addExact(before.activationEpoch(), 1)
    ...
    public static String deterministicStreamName(KafkaPartitionId id, long incarnation) {
        ...
        return "kafka/" + id.kafkaClusterId() + "/" + id.topicId() + "/" + id.partitionId() + "/incarnation-"
                + incarnation;
    }
    ...
    private static void requireLifecycle(KafkaPartitionBindingRecord current, KafkaPartitionLifecycle expected) {
    ...
    private static void requireOperationLifecycle(
        ...
        if (!allowed) {
            throw new IllegalArgumentException("operation is incompatible with lifecycle");
        }
    }

    private static boolean allowedLifecycle(KafkaPartitionLifecycle before, KafkaPartitionLifecycle after) {
        if (before == after) {
            return before != KafkaPartitionLifecycle.DELETED;
        }
