 ...
 */
public final class OxiaJavaGenerationProtocolActivationStore implements GenerationProtocolActivationStore {
    ...
    public static OxiaJavaGenerationProtocolActivationStore usingSharedRuntime(
        ...
        return new OxiaJavaGenerationProtocolActivationStore(
                runtime.client(), clock, activatingBrokerRunId, requiredReferenceDomains);
    }
    ...
    OxiaJavaGenerationProtocolActivationStore(
        ...
        this.activatingBrokerRunId = requireBase32Id(activatingBrokerRunId, "activatingBrokerRunId");
        this.requiredReferenceDomains = canonicalDomains(requiredReferenceDomains);
    }
    ...
    public CompletableFuture<VersionedGenerationProtocolActivation> getOrCreate(String cluster) {
        ...
        return get(cluster).thenCompose(existing -> {
            ...
            GenerationProtocolActivationRecord bootstrap = new GenerationProtocolActivationRecord(
                    1,
                    ...
                    now,
                    0);
            CompletableFuture<VersionedGenerationProtocolActivation> create = support.create(
                            keys.generationProtocolActivationKey(),
                            keys.generationProtocolActivationPartitionKey(),
                            bootstrap,
                            GenerationProtocolActivationRecord.class)
            ...
            return create.handle((created, failure) -> {
                        ...
                        return get(cluster)
                                .thenApply(value -> value.orElseThrow(() -> F4MetadataStoreSupport.invariant(
    ...
    public CompletableFuture<VersionedGenerationProtocolActivation> compareAndSet(
        ...
        F4Keyspace keys = new F4Keyspace(cluster);
        GenerationProtocolActivationRecord exact = Objects.requireNonNull(replacement, "replacement");
        if (exact.metadataVersion() != 0) {
            throw new IllegalArgumentException("replacement metadataVersion must be zero");
        }
        return get(cluster).thenCompose(optional -> {
            VersionedGenerationProtocolActivation current = optional.orElseThrow(
                    () -> new F4MetadataConditionFailedException("generation activation is absent"));
            if (current.metadataVersion() != expectedVersion) {
                return CompletableFuture.failedFuture(
                        new F4MetadataConditionFailedException("generation activation version mismatch"));
            }
            GenerationProtocolActivationTransitions.requireValidReplacement(current.value(), exact);
            CompletableFuture<VersionedGenerationProtocolActivation> cas = support.compareAndSet(
                            keys.generationProtocolActivationKey(),
                            keys.generationProtocolActivationPartitionKey(),
                            exact,
                            GenerationProtocolActivationRecord.class,
            ...
            return recoverWrite(
                    cas,
                    () -> get(cluster)
                            .thenApply(value -> value.orElseThrow(() -> new F4MetadataConditionFailedException(
                                    "generation activation disappeared after uncertain CAS"))),
                    exact,
                    expectedVersion);
        });
    }
    ...
    private CompletableFuture<VersionedGenerationProtocolActivation> recoverWrite(
        ...
        return write.handle((result, failure) -> {
                    ...
                    return reload.get().thenCompose(current -> {
                        boolean sameDesired =
                                current.value().withMetadataVersion(0).equals(desired);
                        if (sameDesired && (expectedVersion < 0 || current.metadataVersion() > expectedVersion)) {
    ...
    private static VersionedGenerationProtocolActivation activation(
            F4Keyspace keys, F4MetadataStoreSupport.Decoded<GenerationProtocolActivationRecord> item) {
        if (!item.key().equals(keys.generationProtocolActivationKey())) {
            throw F4MetadataStoreSupport.invariant("generation activation key/value identity mismatch");
        }
        return new VersionedGenerationProtocolActivation(
                item.key(), item.value(), item.version(), item.durableSha256());
    }
    ...
    private static List<ReferenceDomainVersionRecord> canonicalDomains(List<ReferenceDomainVersionRecord> supplied) {
        List<ReferenceDomainVersionRecord> domains =
                List.copyOf(Objects.requireNonNull(supplied, "requiredReferenceDomains"));
    ...
    private static String requireBase32Id(String value, String name) {
        ...
        if (value.length() < 26 || value.length() > 128 || !value.matches("[a-z2-7]+")) {
            throw new IllegalArgumentException(name + " must be lowercase base32 and encode at least 128 bits");
        }
        return value;
    }
}
