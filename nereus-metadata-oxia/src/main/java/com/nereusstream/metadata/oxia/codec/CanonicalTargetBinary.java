...
final class CanonicalTargetBinary {
    ...
    static final class Writer {
        ...
        void magic(String magic) {
            bytes(magic.getBytes(StandardCharsets.US_ASCII));
        }

        void string(String value) {
            lengthBytes(StrictUtf8.encode(value));
        }
        ...
        private void lengthBytes(byte[] value) {
    ...
    static final class Reader {
        ...
        String string(String name) {
            return StrictUtf8.decode(byteArray(), name);
        }

        byte[] byteArray() {
            int length = intValue();
            if (length < 0 || length > buffer.remaining()) {
                throw new MetadataCodecException("invalid target field length");
            }
        ...
        void finish() {
            if (buffer.hasRemaining()) {
                throw new MetadataCodecException("canonical target has trailing bytes");
            }
        }

        private void take(byte[] value) {
        ...
        private void require(int length) {
            if (length < 0 || buffer.remaining() < length) {
                throw new MetadataCodecException("truncated canonical target");
            }
        }
    }
}
