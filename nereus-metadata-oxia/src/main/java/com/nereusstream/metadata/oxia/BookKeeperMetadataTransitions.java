...
public final class BookKeeperMetadataTransitions {
    ...
    public static void writer(BookKeeperWriterStateRecord before, BookKeeperWriterStateRecord after) {
        ...
        require(
                after.writerStateEpoch() == before.writerStateEpoch() + 1,
        ...
        if (before.activeLedgerId() == after.activeLedgerId() && before.activeLedgerId() != 0) {
            require(
                    after.nextEntryId() >= before.nextEntryId()
                            && after.activePhysicalBytes() >= before.activePhysicalBytes()
                            && after.activeAppendRangeCount() >= before.activeAppendRangeCount(),
                    "active writer counters moved backward");
        }
    }
    ...
    public static void allocationSlot(BookKeeperAllocationSlotRecord before, BookKeeperAllocationSlotRecord after) {
        require(
                before.slot() == after.slot()
                        ...
                        && before.configurationBindingSha256().equals(after.configurationBindingSha256())
                        && before.createdAtMillis() == after.createdAtMillis(),
                "allocation slot identity drift");
        require(
                allocationSlotEdge(before.lifecycle(), after.lifecycle()),
    ...
    public static void root(BookKeeperLedgerRootRecord before, BookKeeperLedgerRootRecord after) {
        require(
                before.ledgerIdentitySha256().equals(after.ledgerIdentitySha256())
                        ...
                        && before.createdAtMillis() == after.createdAtMillis(),
                "ledger root identity drift");
        require(
                after.lifecycleEpoch() == before.lifecycleEpoch() + 1,
                "ledger lifecycleEpoch must increment exactly once");
        ...
        if (before.lifecycle() == after.lifecycle()) {
            ...
            require(
                    lateCreateEscalation || firstDeleteAbsence,
    ...
    public static void reservation(BookKeeperAppendReservationRecord before, BookKeeperAppendReservationRecord after) {
        require(
                before.reservationId().equals(after.reservationId())
                        ...
                        && before.createdAtMillis() == after.createdAtMillis(),
                "reservation identity drift");
        require(reservationEdge(before.lifecycle(), after.lifecycle()), "illegal reservation lifecycle replacement");
    ...
    public static void protection(BookKeeperLedgerProtectionRecord before, BookKeeperLedgerProtectionRecord after) {
        require(
                before.ledgerIdentitySha256().equals(after.ledgerIdentitySha256())
                        ...
                        && before.createdAtMillis() == after.createdAtMillis()
                        && before.expiresAtMillis() == after.expiresAtMillis(),
        ...
        require(
                (before.lifecycle() == ProtectionLifecycle.RESERVED && after.lifecycle() == ProtectionLifecycle.ACTIVE)
        ...
        if (before.lifecycle() == ProtectionLifecycle.RESERVED) {
            require(
                    before.ownerKey().isEmpty()
                            && before.ownerMetadataVersion() == 0
                            && before.ownerIdentitySha256().isEmpty(),
                    "RESERVED protection cannot already name an owner");
            require(
                    !after.referenceId().isBlank()
                            && !after.ownerKey().isBlank()
                            && after.ownerMetadataVersion() >= 0
                            && !after.ownerIdentitySha256().isBlank(),
    ...
    public static void readerLease(BookKeeperLedgerReaderLeaseRecord before, BookKeeperLedgerReaderLeaseRecord after) {
        require(
                before.ledgerIdentitySha256().equals(after.ledgerIdentitySha256())
                        ...
                        && before.ledgerId() == after.ledgerId()
                        && before.rootLifecycleEpoch() == after.rootLifecycleEpoch()
                        && before.acquiredAtMillis() == after.acquiredAtMillis(),
                "reader lease identity drift");
        require(
                after.leaseEpoch() == before.leaseEpoch() + 1 && after.expiresAtMillis() > before.expiresAtMillis(),
    ...
    private static boolean writerEdge(BookKeeperWriterLifecycle before, BookKeeperWriterLifecycle after) {
        if (before == after) {
            return before != BookKeeperWriterLifecycle.CLOSED;
        }
        return switch (before) {
            case IDLE -> after == BookKeeperWriterLifecycle.ALLOCATING || after == BookKeeperWriterLifecycle.CLOSED;
            case ALLOCATING ->
                after == BookKeeperWriterLifecycle.ACTIVE
    ...
    private static boolean allocationEdge(LedgerAllocationLifecycle before, LedgerAllocationLifecycle after) {
        return switch (before) {
            case PREPARED ->
                after == LedgerAllocationLifecycle.ROOT_RESERVED || after == LedgerAllocationLifecycle.ABORTED;
            case ROOT_RESERVED ->
                after == LedgerAllocationLifecycle.CREATE_UNCERTAIN
            ...
            case CREATE_UNCERTAIN ->
                after == LedgerAllocationLifecycle.PHYSICAL_CREATED
                        || after == LedgerAllocationLifecycle.FOREIGN_COLLISION;
            case PHYSICAL_CREATED ->
                after == LedgerAllocationLifecycle.ACTIVATED || after == LedgerAllocationLifecycle.FOREIGN_COLLISION;
    ...
    private static boolean rootEdge(BookKeeperLedgerLifecycle before, BookKeeperLedgerLifecycle after) {
        ...
        return switch (before) {
            case ALLOCATING ->
                after == BookKeeperLedgerLifecycle.ACTIVE
            ...
            case MARKED -> after == BookKeeperLedgerLifecycle.DELETING || after == BookKeeperLedgerLifecycle.SEALED;
            case DELETING -> after == BookKeeperLedgerLifecycle.DELETING || after == BookKeeperLedgerLifecycle.DELETED;
            case DELETED, ABORTED, QUARANTINED -> false;
        };
    }

    private static boolean reservationEdge(AppendReservationLifecycle before, AppendReservationLifecycle after) {
        return switch (before) {
            case RESERVED ->
                after == AppendReservationLifecycle.WRITING || after == AppendReservationLifecycle.ABANDONED;
            case WRITING ->
                after == AppendReservationLifecycle.DURABLE || after == AppendReservationLifecycle.ABANDONED;
            case DURABLE ->
                after == AppendReservationLifecycle.COMMIT_PREPARED
