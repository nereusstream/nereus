...
public final class OxiaJavaBookKeeperMetadataStore
    ...
    OxiaJavaBookKeeperMetadataStore(
            PartitionedOxiaClient client, Clock clock, BookKeeperMetadataStoreConfig configuration) {
    ...
    public CompletableFuture<Optional<BookKeeperVersionedValue<BookKeeperWriterStateRecord>>> getWriter(
        ...
        return getExact(
                key, keys.streamPartitionKey(exactStream), BookKeeperWriterStateRecord.class, (storedKey, value) -> {
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperWriterStateRecord>> createWriter(
        ...
        return createExact(
                ...
                exact,
                BookKeeperWriterStateRecord.class,
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperWriterStateRecord>> compareAndSetWriter(
        ...
        requireWriterBound(exact);
        StreamId stream = new StreamId(exact.streamId());
        return compareAndSetExact(
                keys.writerStateKey(stream),
                keys.streamPartitionKey(stream),
                exact,
                BookKeeperWriterStateRecord.class,
                expectedVersion,
    ...
    public CompletableFuture<Optional<BookKeeperVersionedValue<LedgerAllocationIntentRecord>>> getAllocation(
        ...
        return getExact(
                key, keys.streamPartitionKey(exactStream), LedgerAllocationIntentRecord.class, (storedKey, value) -> {
    ...
    public CompletableFuture<BookKeeperVersionedValue<LedgerAllocationIntentRecord>> compareAndSetAllocation(
        ...
        StreamId stream = new StreamId(exact.streamId());
        return compareAndSetExact(
                keys.allocationKey(stream, exact.allocationId()),
                keys.streamPartitionKey(stream),
                exact,
                LedgerAllocationIntentRecord.class,
                expectedVersion,
                () -> getAllocation(cluster, stream, exact.allocationId()),
                BookKeeperMetadataTransitions::allocation);
    }
    ...
    public CompletableFuture<Void> deleteAllocation(
        ...
        StreamId exactStream = Objects.requireNonNull(stream, "stream");
        return deleteExact(
                keys.allocationKey(exactStream, allocationId),
                keys.streamPartitionKey(exactStream),
                expectedVersion,
    ...
    public CompletableFuture<BookKeeperScanPage<BookKeeperVersionedValue<LedgerAllocationIntentRecord>>>
        ...
        return scan(
                cluster,
                BookKeeperScanKind.ALLOCATION,
                "stream\0" + exactStream.value(),
                keys.allocationPrefix(exactStream),
                1,
                keys.streamPartitionKey(exactStream),
                continuation,
                limit,
    ...
    public CompletableFuture<Optional<BookKeeperVersionedValue<BookKeeperAllocationSlotRecord>>> getAllocationSlot(
        ...
        return getExact(
                key,
                keys.allocationSlotPartitionKey(keys.allocationSlotShard(slot)),
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperAllocationSlotRecord>> compareAndSetAllocationSlot(
        ...
        return compareAndSetExact(
                ...
                expectedVersion,
                () -> getAllocationSlot(cluster, exact.slot()),
                BookKeeperMetadataTransitions::allocationSlot);
    }
    ...
    public CompletableFuture<BookKeeperScanPage<BookKeeperVersionedValue<BookKeeperAllocationSlotRecord>>>
        ...
        return scan(
                cluster,
                BookKeeperScanKind.ALLOCATION_SLOT,
                "allocation-slot-shard\0" + slotShard,
                keys.allocationSlotShardPrefix(slotShard),
                1,
                keys.allocationSlotPartitionKey(slotShard),
                continuation,
                limit,
                BookKeeperAllocationSlotRecord.class,
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperAppendReservationRecord>> compareAndSetReservation(
        ...
        StreamId stream = new StreamId(exact.streamId());
        return compareAndSetExact(
                keys.appendReservationKey(stream, exact.reservationId()),
                keys.streamPartitionKey(stream),
                exact,
                BookKeeperAppendReservationRecord.class,
                expectedVersion,
                () -> getReservation(cluster, stream, exact.reservationId()),
                BookKeeperMetadataTransitions::reservation);
    }
    ...
    public CompletableFuture<Void> deleteReservation(
        ...
        StreamId exactStream = Objects.requireNonNull(stream, "stream");
        return deleteExact(
                keys.appendReservationKey(exactStream, reservationId),
                keys.streamPartitionKey(exactStream),
                expectedVersion,
    ...
    public CompletableFuture<BookKeeperScanPage<BookKeeperVersionedValue<BookKeeperAppendReservationRecord>>>
        ...
        return scan(
                cluster,
                BookKeeperScanKind.APPEND_RESERVATION,
                "stream\0" + exactStream.value(),
                keys.appendReservationPrefix(exactStream),
                1,
                keys.streamPartitionKey(exactStream),
                continuation,
                limit,
    ...
    public CompletableFuture<Optional<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>>> getRoot(
        ...
        return getExact(
                key,
                keys.ledgerPartitionKey(providerScopeSha256, ledgerId),
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> createRoot(
        ...
        return createExact(
                key,
                keys.ledgerPartitionKey(exact.providerScopeSha256(), exact.ledgerId()),
                exact,
                BookKeeperLedgerRootRecord.class,
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> compareAndSetRoot(
        ...
        return compareAndSetExact(
                ...
                expectedVersion,
                () -> getRoot(cluster, exact.providerScopeSha256(), exact.ledgerId()),
                BookKeeperMetadataTransitions::root);
    }
    ...
    public CompletableFuture<BookKeeperScanPage<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>>> scanRoots(
        ...
        return scan(
                cluster,
                BookKeeperScanKind.LEDGER_ROOT,
                "ledger-root-shard\0" + shard,
                keys.ledgerRootShardPrefix(shard),
                1,
                keys.ledgerPartitionKeyByShard(shard),
                continuation,
                limit,
                BookKeeperLedgerRootRecord.class,
                (key, value) -> validateRoot(keys, key, value));
    }
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerProtectionRecord>> createProtection(
        ...
        BookKeeperKeyspace keys = keys(cluster);
        String key = keys.protectionKey(
                providerScopeSha256, exact.ledgerId(), exact.ledgerRangeSlot(), exact.protectionSlot());
        validateProtection(keys, key, providerScopeSha256, exact);
        return createExact(
                key,
                keys.ledgerPartitionKey(providerScopeSha256, exact.ledgerId()),
                exact,
                BookKeeperLedgerProtectionRecord.class,
                () -> getProtection(
                        cluster,
                        providerScopeSha256,
                        exact.ledgerId(),
                        exact.ledgerRangeSlot(),
                        exact.protectionSlot()));
    }
    ...
    public CompletableFuture<Void> deleteProtection(
        ...
        BookKeeperKeyspace keys = keys(cluster);
        return deleteExact(
                keys.protectionKey(providerScopeSha256, ledgerId, rangeSlot, protectionSlot),
                keys.ledgerPartitionKey(providerScopeSha256, ledgerId),
                expectedVersion,
    ...
    public CompletableFuture<BookKeeperScanPage<BookKeeperVersionedValue<BookKeeperLedgerProtectionRecord>>>
        ...
        return scan(
                cluster,
                BookKeeperScanKind.PROTECTION,
                "ledger-protection\0" + identity,
                keys.protectionPrefix(providerScopeSha256, ledgerId),
                2,
                keys.ledgerPartitionKey(providerScopeSha256, ledgerId),
                continuation,
                limit,
    ...
    public CompletableFuture<Void> deleteReaderLease(
        ...
        BookKeeperKeyspace keys = keys(cluster);
        return deleteExact(
                keys.readerLeaseKey(providerScopeSha256, ledgerId, readerSlot),
                keys.ledgerPartitionKey(providerScopeSha256, ledgerId),
                expectedVersion,
    ...
    public CompletableFuture<BookKeeperScanPage<BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord>>>
        ...
        return scan(
                cluster,
                BookKeeperScanKind.READER_LEASE,
                "ledger-reader\0" + identity,
                keys.readerLeasePrefix(providerScopeSha256, ledgerId),
                1,
                keys.ledgerPartitionKey(providerScopeSha256, ledgerId),
                continuation,
                limit,
    ...
    private <T> CompletableFuture<Optional<BookKeeperVersionedValue<T>>> getExact(
        ...
        return support.get(key, partition, type)
                .thenApply(optional -> optional.map(item -> {
    ...
    private <T> CompletableFuture<BookKeeperVersionedValue<T>> createExact(
        ...
        CompletableFuture<BookKeeperVersionedValue<T>> create =
                support.create(key, partition, value, type).thenApply(OxiaJavaBookKeeperMetadataStore::versioned);
    ...
    private <T> CompletableFuture<BookKeeperScanPage<BookKeeperVersionedValue<T>>> scan(
        ...
        BookKeeperScanToken token = validateToken(continuation, cluster, kind, scopeSha256, prefix, limit);
        ...
        return support.client().rangeScan(start, end, limit + 1, partition).thenApply(rows -> {
            ...
            List<BookKeeperVersionedValue<T>> values = rows.stream()
                    .limit(limit)
                    .map(row -> {
            ...
            Optional<BookKeeperScanToken> next = hasMore
                    ? Optional.of(new BookKeeperScanToken(
                            cluster,
                            kind,
                            scopeSha256,
                            prefix,
                            values.get(values.size() - 1).key(),
                            limit))
                    : Optional.empty();
    ...
    private static void validateRoot(BookKeeperKeyspace keys, String key, BookKeeperLedgerRootRecord value) {
        keys.requireAllocationSlot(value.allocationSlot());
        ...
        keys.parseRootKey(
                requireKey(key, keys.ledgerRootKey(value.providerScopeSha256(), value.ledgerId())),
                value.providerScopeSha256(),
                value.ledgerId());
    }

    private static void validateProtection(
            BookKeeperKeyspace keys, String key, String providerScopeSha256, BookKeeperLedgerProtectionRecord value) {
        keys.requireLedgerRangeSlot(value.ledgerRangeSlot());
    ...
    private static void validateReaderLease(
            BookKeeperKeyspace keys, String key, String providerScopeSha256, BookKeeperLedgerReaderLeaseRecord value) {
        ...
        keys.parseReaderLeaseKey(
                requireKey(key, keys.readerLeaseKey(providerScopeSha256, value.ledgerId(), value.readerSlot())),
                providerScopeSha256,
                value.ledgerId());
    }
    ...
    private static <T> BookKeeperVersionedValue<T> versioned(F4MetadataStoreSupport.Decoded<T> value) {
        return new BookKeeperVersionedValue<>(value.key(), value.value(), value.version(), value.durableSha256());
    }
}
