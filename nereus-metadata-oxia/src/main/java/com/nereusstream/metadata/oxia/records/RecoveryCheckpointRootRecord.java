...
public record RecoveryCheckpointRootRecord(
    ...
    public RecoveryCheckpointRootRecord {
        ...
        checkpointSetSha256 = F4RecordValidation.requireOptionalSha256(checkpointSetSha256, "checkpointSetSha256");
        sourceHeadCommitId = F4RecordValidation.requireOptionalText(sourceHeadCommitId, "sourceHeadCommitId", 512);
        ...
        if (checkpoints.isEmpty()) {
            if (checkpointSequence != 0
                    ...
                    || coveredEndOffset != 0
                    || firstCommitVersion != 0
                    || lastCommitVersion != 0
                    || cumulativeSizeAtStart != 0
                    || cumulativeSizeAtEnd != 0
                    || !firstCommitId.isEmpty()
                    || !lastCommitId.isEmpty()
                    || !checkpointSetSha256.isEmpty()
                    || !sourceHeadCommitId.isEmpty()
                    || sourceHeadCommitVersion != 0
                    || publishedAtMillis != 0) {
            ...
            for (RecoveryCheckpointReferenceRecord checkpoint : checkpoints) {
                if (previous != null
                        && (checkpoint.coveredStartOffset() != previous.coveredEndOffset()
            ...
            if (first.coveredStartOffset() != coveredStartOffset
                    || last.coveredEndOffset() != coveredEndOffset
                    ...
                    || !first.firstCommitId().equals(firstCommitId)
                    || !last.lastCommitId().equals(lastCommitId)) {
                throw new IllegalArgumentException("recovery root summary does not match checkpoint references");
            }
            if (!checkpointSetSha256.equals(RecoveryCheckpointRootDigests.checkpointSetSha256(checkpoints)
                    .value())) {
