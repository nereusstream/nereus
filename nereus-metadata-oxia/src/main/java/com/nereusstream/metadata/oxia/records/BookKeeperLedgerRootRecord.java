...
public record BookKeeperLedgerRootRecord(
    ...
    public BookKeeperLedgerRootRecord {
        ...
        if (sealedLastEntryId < -1) {
            throw new IllegalArgumentException("sealedLastEntryId must be at least -1");
        }
        ...
        if (lifecycle == BookKeeperLedgerLifecycle.DELETED
                && !(deleteStartedAtMillis > 0
                        && firstAbsentAtMillis >= deleteStartedAtMillis
