...
public record StreamStateTransitionRequest(
        StreamId streamId, StreamState expectedState, StreamState targetState, long expectedMetadataVersion) {
    public StreamStateTransitionRequest {
        ...
        boolean legal = expectedState == StreamState.ACTIVE && targetState == StreamState.SEALED
                || (expectedState == StreamState.ACTIVE || expectedState == StreamState.SEALED)
                        && targetState == StreamState.DELETING
                || expectedState == StreamState.DELETING && targetState == StreamState.DELETED;
        if (!legal) {
            throw new IllegalArgumentException("illegal stream state transition");
        }
    }
}
