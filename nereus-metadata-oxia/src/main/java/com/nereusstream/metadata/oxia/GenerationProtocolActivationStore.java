...
public interface GenerationProtocolActivationStore extends AutoCloseable {
    static GenerationProtocolActivationStore usingSharedRuntime(
        ...
        return OxiaJavaGenerationProtocolActivationStore.usingSharedRuntime(
                clientConfig, runtime, clock, activatingBrokerRunId, requiredReferenceDomains);
    }

    CompletableFuture<Optional<VersionedGenerationProtocolActivation>> get(String cluster);

    CompletableFuture<VersionedGenerationProtocolActivation> getOrCreate(String cluster);
