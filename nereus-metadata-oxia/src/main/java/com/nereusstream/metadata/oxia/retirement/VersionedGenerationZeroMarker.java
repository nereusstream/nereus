...
public record VersionedGenerationZeroMarker(
    ...
    public VersionedGenerationZeroMarker {
        ...
        readTargetIdentitySha256 = Objects.requireNonNull(readTargetIdentitySha256, "readTargetIdentitySha256");
        if (key.isBlank() || offsetStart < 0 || offsetEnd <= offsetStart || commitVersion <= 0 || metadataVersion < 0) {
            throw new IllegalArgumentException("generation-zero marker facts are invalid");
        }
        readTargetIdentitySha256.ifPresent(
                value -> RetirementMetadataSupport.requireSha256(value, "readTargetIdentitySha256"));
        if ((identity instanceof GenericCommittedAppendIdentity) != readTargetIdentitySha256.isPresent()) {
            throw new IllegalArgumentException("only a generic committed marker carries read-target identity");
        }
