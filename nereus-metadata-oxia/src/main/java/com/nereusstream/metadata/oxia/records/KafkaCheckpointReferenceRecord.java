...
public record KafkaCheckpointReferenceRecord(
    ...
    public KafkaCheckpointReferenceRecord {
        if (referenceVersion != 1) {
            throw new IllegalArgumentException("referenceVersion must be 1");
        }
        ...
        if (objectLength <= 0
                || checkpointOffset < 0
                || logStartOffsetAtCheckpoint < 0
                || logStartOffsetAtCheckpoint > checkpointOffset
    ...
    @Override
    public byte[] objectSha256() {
        return objectSha256.clone();
    }

    @Override
    public byte[] sourceHeadSha256() {
        return sourceHeadSha256.clone();
    }

    @Override
    public boolean equals(Object other) {
        return this == other
                || other instanceof KafkaCheckpointReferenceRecord that
                        && referenceVersion == that.referenceVersion
                        && objectLength == that.objectLength
                        && checkpointOffset == that.checkpointOffset
                        && logStartOffsetAtCheckpoint == that.logStartOffsetAtCheckpoint
                        && sourceCommitVersion == that.sourceCommitVersion
