...
public final class BookKeeperStableAppendProtectionValidator {
    ...
    public BookKeeperStableAppendProtectionValidator(
            BookKeeperLedgerMetadataStore metadata, BookKeeperMetadataStoreConfig configuration) {
        this.metadata = Objects.requireNonNull(metadata, "metadata");
        this.configuration = Objects.requireNonNull(configuration, "configuration");
    }

    public CompletableFuture<Void> validate(
            String cluster, PreparedStableAppend prepared, BookKeeperPhysicalReferenceProof proof) {
        ...
        String expectedReferenceId = "ra1-"
        ...
        String expectedLedgerIdentity = keys.ledgerIdentitySha256(proof.providerScopeSha256(), target.ledgerId());
        ...
        CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> root = metadata.getRoot(
                        cluster, proof.providerScopeSha256(), target.ledgerId())
                .thenApply(optional ->
                        optional.orElseThrow(() -> invariant("BookKeeper ledger root is absent before head commit")));
        CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerProtectionRecord>> protection =
                metadata.getProtection(
                        ...
                        .thenApply(optional -> optional.orElseThrow(
                                () -> invariant("BookKeeper REACHABLE_APPEND protection is absent")));
        return root.thenCombine(protection, (exactRoot, exactProtection) -> {
            ...
            validateProtection(prepared, proof, target, expectedReferenceId, expectedLedgerIdentity, exactProtection);
            return null;
        });
    }
    ...
    private static void validateProtection(
        ...
        boolean rootEpochMatches = value.rootLifecycleEpoch() == proof.rootLifecycleEpoch()
                || (prepared.replayWasReachable() && value.rootLifecycleEpoch() < proof.rootLifecycleEpoch());
