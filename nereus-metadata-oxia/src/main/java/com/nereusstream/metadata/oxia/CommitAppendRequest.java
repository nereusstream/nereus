...
public record CommitAppendRequest(
    ...
    public CommitAppendRequest {
        ...
        try {
            Math.addExact(expectedStartOffset, recordCount);
        } catch (ArithmeticException e) {
            throw new IllegalArgumentException("logical range overflows", e);
        }
    }
    ...
    public ReadTargetRecord readTargetRecord() {
        return ReadTargetCodecRegistry.phase15().encode(readTarget);
    }
    ...
    private static String projectionIdentity(Optional<ProjectionRef> projectionRef) {
        ...
        projectionRef.ifPresentOrElse(
                projection -> {
                    add(value, "present");
                    add(value, projection.type().name());
                    add(value, projection.value());
                },
                () -> add(value, "absent"));
    ...
    public String commitId() {
        ...
        add(value, streamId.value());
        add(value, Long.toString(expectedStartOffset));
        add(value, writerId);
        add(value, writerRunIdHash);
        add(value, Long.toString(epoch));
        add(value, fencingTokenHash());
        add(value, Integer.toString(recordCount));
        add(value, Integer.toString(entryCount));
        add(value, Long.toString(logicalBytes));
        add(value, payloadFormat.name());
        add(value, Integer.toString(schemaRefs.size()));
        schemaRefs.forEach(schema -> {
            add(value, schema.namespace());
            add(value, schema.id());
            add(value, Long.toString(schema.version()));
        });
        add(value, Long.toString(minEventTimeMillis));
        add(value, Long.toString(maxEventTimeMillis));
        add(value, projectionIdentity());
        add(value, target.targetType());
        add(value, Integer.toString(target.targetVersion()));
        add(value, target.identityChecksumValue());
