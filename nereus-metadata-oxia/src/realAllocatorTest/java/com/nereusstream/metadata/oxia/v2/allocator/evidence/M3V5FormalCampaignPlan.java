...
/** ADR-0137 projection of the unchanged logical action inventory onto the source-bound V5 execution profile. */
final class M3V5FormalCampaignPlan {
    static final int MAXIMUM_TOTAL_EXECUTED_ACTIONS = 720;
    static final long CAMPAIGN_WALL_CLOCK_CAP_SECONDS = 48_000;

    private M3V5FormalCampaignPlan() {}
    ...
    static List<PlannedActionV3> zeroDecisionActions() {
        ...
        if (!actions.stream().map(PlannedActionV3::identity).toList()
                .equals(AllocatorCampaignPlanProfileV5.zeroDecisionActionIdentities())) {
            throw new IllegalStateException("allocator V5 physical plan differs from its domain profile");
        }
        return actions;
    }
    ...
    static Sha256Digest zeroDecisionPlanDigest() {
        zeroDecisionActions();
        return AllocatorCampaignPlanProfileV5.zeroDecisionPlanDigest();
    }
