...
/** V4 authority wrapper around the unchanged, validator-driven V3 logical orchestration algebra. */
final class M3V4AdaptiveCampaignExecutor {
    ...
    M3V4AdaptiveCampaignExecutor(
        ...
        if (!source.workloadDigest().equals(M3V4FormalCampaignPlan.zeroDecisionPlanDigest())) {
            throw new IllegalArgumentException("allocator V4 source binding differs from its zero-decision plan");
        }
    ...
    Result resume(CanonicalBytes predecessorBytes) throws IOException {
        AllocatorCampaignCheckpointV4 predecessor =
                AllocatorCampaignCheckpointV4.decode(Objects.requireNonNull(predecessorBytes, "predecessorBytes"));
        if (!predecessor.source().equals(source)) {
            throw new IllegalArgumentException("allocator V4 resume source/profile tuple differs");
        }
        ...
        static CheckpointSink createNewDirectory(Path directory) {
            ...
            return (sequence, checkpointBytes) -> {
                if (!Files.isDirectory(exactDirectory, LinkOption.NOFOLLOW_LINKS)) {
                    throw new IllegalArgumentException("allocator V4 checkpoint directory is absent or a link");
                }
                String digest = AllocatorCampaignCheckpointV4.digest(checkpointBytes).toHex();
                Path output = exactDirectory.resolve(String.format(
                        Locale.ROOT, "checkpoint-%020d-%s.nacp4", sequence, digest));
                Files.write(
                        output,
    ...
    record Result(CanonicalBytes checkpointBytes, Status status, TerminalReason reason, String detail) {
        Result {
            ...
            if (AllocatorCampaignCheckpointV4.decode(checkpointBytes).status() != status) {
                throw new IllegalArgumentException("allocator V4 campaign result status differs from checkpoint");
            }
        }
    ...
    private static final class SinkBridge implements M3V3AdaptiveCampaignExecutor.CheckpointSink {
        ...
        public void persist(long sequence, CanonicalBytes logicalBytes) throws IOException {
            AllocatorCampaignCheckpointV4 checkpoint = previous == null
                    ? AllocatorCampaignCheckpointV4.initial(logicalBytes)
                    : AllocatorCampaignCheckpointV4.resume(previous, logicalBytes);
            CanonicalBytes encoded = AllocatorCampaignCheckpointV4.encode(checkpoint);
            if (checkpoint.checkpointSequence() != sequence) {
                throw new IllegalArgumentException("allocator V4 checkpoint sequence differs from logical executor");
            }
        ...
        private Result result(CampaignResult logical) {
            if (previous == null) {
                throw new IllegalStateException("allocator V4 logical executor produced no checkpoint");
            }
            AllocatorCampaignCheckpointV4 checkpoint = AllocatorCampaignCheckpointV4.decode(previous);
            if (!checkpoint.logicalCheckpointBytes().equals(logical.checkpointBytes())) {
                throw new IllegalStateException("allocator V4 final logical checkpoint differs from persisted NACP4");
            }
