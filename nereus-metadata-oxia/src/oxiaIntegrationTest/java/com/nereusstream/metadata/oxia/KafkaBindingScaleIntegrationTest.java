...
class KafkaBindingScaleIntegrationTest {
    ...
    private static final int BINDINGS_PER_SHARD = BINDING_COUNT / KafkaPartitionKeyspace.REGISTRY_SHARDS;
    ...
    void scenarioKfScl001() {
        ...
        try (SharedOxiaClientRuntime runtime = SharedOxiaClientRuntime.connect(configuration, Clock.systemUTC());
                OxiaJavaKafkaPartitionMetadataStore store = OxiaJavaKafkaPartitionMetadataStore.usingSharedRuntime(
            ...
            for (int start = 0; start < identities.size(); start += WRITE_WINDOW) {
                ...
                for (int index = start; index < end; index++) {
                    writes[index - start] = createActiveBinding(store, keys, identities.get(index), index);
                }
                ...
                do {
                    ...
                    for (VersionedKafkaPartitionRegistry hint : page.values()) {
                        ...
                        assertThat(HexFormat.of()
                                        .parseHex(root.durableValueSha256().value()))
    ...
    private static CompletableFuture<VersionedKafkaPartitionBinding> createActiveBinding(
            OxiaJavaKafkaPartitionMetadataStore store, KafkaPartitionKeyspace keys, KafkaPartitionId id, int index) {
        ...
        return store.putCreatingIfAbsent(creating)
                .thenCompose(created -> store.compareAndSet(
                        ...
                        KafkaPartitionMetadataTransitions.activate(
                                ...
                                metadataOffset,
                                1_001)))
                .thenCompose(active -> store.putRegistryHint(new KafkaPartitionRegistryRecord(
                                1,
                                id.kafkaClusterId(),
                                id.topicId(),
                                id.partitionId(),
                                ...
                                HexFormat.of()
                                        .parseHex(active.durableValueSha256().value()),
    ...
    private static List<KafkaPartitionId> identitiesWithExactShardCardinality(KafkaPartitionKeyspace keys) {
        ...
        for (long candidate = 1; identities.size() < BINDING_COUNT; candidate++) {
            KafkaPartitionId id = new KafkaPartitionId("kraft-scale", topicId(candidate), 0);
    ...
    private static OxiaClientConfiguration configuration() {
        return new OxiaClientConfiguration(
                OXIA.getServiceAddress(), "default", Duration.ofSeconds(30), Duration.ofSeconds(60), 512, 1_024);
    }
}
