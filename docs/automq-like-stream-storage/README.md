...
> 状态：Implemented / F4-M1–M5 final-gated；F4-M6 BD–BH focused-green，aggregate compatibility pending；
...
- F4-M4 checkpoint AP：the configured-scope probe now exercises guarded PUT、exact HEAD、complete LIST、ETag-bound
  exact DELETE、response-loss absence convergence and idempotent cleanup, producing one deterministic non-secret V1
  capability digest. The AP probe alone neither persists it nor activates deletion.
- F4-M4 checkpoint AQ：the product-owned bounded coordinator now composes stable readiness、ACTIVE publication、exact
  domains、same-epoch registration、physical/cursor coverage and the configured-scope canary, then atomically installs
  the digest with both V1 deletion bits. At AQ, provider/Pulsar startup composition and restart scope-digest gating
  remained pending.
- F4-M4 checkpoint AR：the provider/runtime/factory and locked Pulsar sequence now own AQ activation；one shared
  expected scope digest fences destructive operations and mutating restart recovery. Safe defaults still invoke no
  canary/delete activation/lifecycle, while explicit `enabled && !dryRun` waits for exact durable authority.
- F4-M4 checkpoint AS：activation revalidation and physical GC now share the exact registered-stream global scope、
  configured bounds and projection-domain instance. A real four-shard Oxia + pinned LocalStack process sequence proves
  publication-only deferral、atomic activation、wrong-prefix restart rejection、empty-LIST MARKED recovery and real
  DELETE response-loss convergence to durable DELETED；the remaining destructive/scale matrix is not final-gated.
- F4-M4 checkpoint AT：after a confirmed real target DELETE, the interrupted runtime is stopped before its
  DELETED-root CAS. A fresh runtime reloads durable DELETING/sealed-journal authority and HEAD absence to converge
  DELETED without inherited LIST/callback/candidate state；the remaining kill-point/scale matrix is not final-gated.
- F4-M4 checkpoint AU：a real DELETED-root Oxia CAS applies before its response is replaced by a timeout. Exact root
  reload converges without repeating the target DELETE, and a fresh runtime also performs no DELETE；the remaining
  metadata/source/protection/multi-broker/scale matrix is not final-gated.
- F4-M4 checkpoint AV：two independently assembled workers race one MARKED root；one raw DELETING CAS wins, the loser
  reloads the same durable intent and both exact idempotent delete paths converge one DELETED root. Two-broker and
...
- F4-M4 checkpoint AX：a writer process persists 1,001 ACTIVE roots in one physical shard plus one in each remaining
  ...
  all 1,256 identities exactly once in 16 hot-shard pages plus one page per other shard.
- F4-M4 checkpoint AY：10,000 DELETED roots cross a durable first-absence pass and a separate orphan-window pass via
  the production scanner/coordinator；matching Phase 1 references、manifest and root retire in that order under
  32-entry pages and one-at-a-time visitation. Materialization/S3 cancelled deadline tasks are removed immediately.
  The remaining cursor-10k、late-PUT and two-broker gates stayed pending at AY.
- F4-M4 checkpoint AZ：a stack-bounded pass visits 10,000 synchronous cursor-snapshot candidates sequentially, and a
...
- F4-M4 checkpoint BB：Object-WAL provider retries now carry a durable session/root guard from append coordination to
  every transmission. A DELETED root rejects attempt two before bytes；an expired old session sends no first attempt
  and a newer epoch uses a fresh key. Tombstone tests inject exact bytes after references and manifest deletion, while
...
- F4-M4 checkpoint BC：deletion-active broker-readiness changes now run physical/cursor coverage without publishing
  ...
  locked broker forwards exact traversal concurrency/remaining deadline through a product-owned monotonic budget。
  BC by itself remained a checkpoint boundary.
- F4-M4 final gate：a retry-disabled real two-broker fixture activates the exact physical-GC scope, materializes
  ordinary/batched Object-WAL entries, deletes every captured generation-zero source key, preserves COMMITTED reads
  and exact MessageIds through unload、owner failover、restart/reverse takeover, and keeps a stock BookKeeper topic
  writable/readable. F4-M4 is complete/final-gated；safe defaults still schedule no physical deletion.
...
F4-M6 BD–BH cover the current merge/candidate/checkpoint/reference/task limits and exact 16,448-stream all-shard
registry restart；the remaining two-worker/two-broker、scenario-mapping and aggregate gates are still the Phase 4-wide completion boundary. This is not permission to
