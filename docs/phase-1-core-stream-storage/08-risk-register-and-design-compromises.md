...
Status: confirmed design blocker for the original `commitStreamSlice` protocol.
...
- M2 不能再按原多 key atomic commit 假设实现 fake store；
- fake store 只能模拟真实 public adapter 可表达的能力，不能让 fake semantics 比真实 adapter 更强；
- M4 `AppendCoordinator` 不能在 commit linearization 设计重写并回写文档前进入真实 append 实现。

当前动作：停止 `commitStreamSlice` 当前多 key atomic commit 设计，重新设计线性化点。首选替代
方向是一个 authoritative stream-head record CAS，把 offset index 和 committed-slice marker 改成
可修复派生索引；具体协议必须回写到 `02`、`04`、`06`、`07` 后再继续。
