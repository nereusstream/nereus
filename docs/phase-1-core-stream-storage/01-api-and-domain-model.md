...
3. `append` 的成功结果代表 WAL durable 且 Oxia offset index 已提交。
4. `read` 只能读取 Oxia offset index 已发布的 range。
...
- Phase 1 uses deterministic generation from the exact `StreamName.value()` so stream creation can keep
  by-name and stream-scoped initial records in one Oxia key group；
...
- stale token must fail before offset index commit。
...
offset index records and any trim/committed-end records consulted by the resolver. It is not a stream
...
`trim(streamId, beforeOffset)` updates a trim low-watermark:
...
| `TIMEOUT` | yes | configured operation timeout elapsed; append final state may be unknown if the commit RPC was already sent |
