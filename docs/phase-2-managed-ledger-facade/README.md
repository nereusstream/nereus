...
均通过普通与 Docker gate。F2-M3 正在实现：配置捕获、runtime ownership、snapshot overlay、callback
ordering 与 creation/write-fence 协议基础已落地；下一子阶段是 factory open/inspect 与 ledger append/read。
...
| F2-M3 ManagedLedger facade | In progress | Runtime/config/snapshot/callback foundation complete；factory/ledger surfaces pending |
