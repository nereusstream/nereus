...
- `f9MultiControllerFailoverProcessIntegrationTest` adds ACTIVE steady-state controller P/C evidence with three real
  ...
  `phase9M6KafkaProcessCheck`；
- `f9ActivationCutFailoverProcessIntegrationTest` adds provider-applied PREPARED-create and first-ACTIVE-CAS C evidence
  with three dedicated controller release JVMs plus one broker。A test-only agent leaves the real Oxia create/CAS
  untouched、waits for it to apply，then withholds only the completion observed by the current controller。The harness
  verifies PREPARED or ACTIVE directly from Oxia、kills that exact controller before its reconciliation-success log，
  requires a different higher-epoch controller to preserve the prepared tuple or byte-identical ACTIVE record and
  non-regressing readiness for broker `[4]`，then proves offset-0 Produce/Fetch/ListOffsets `0/1` and real Object
  persistence。Fresh `--rerun-tasks` execution passes 66/66 actionable tasks in 1m06s and is included in
  `phase9M6KafkaProcessCheck`。Coordinator/internal-topic、checkpoint/virtual-segment、second-proof/pre-CAS/unapplied
  controller process and broader chaos cuts remain open；
...
- the organization fork exists and the published branch
  ...
  ACTIVE controller kill/reconciliation gate and the provider-applied PREPARED/ACTIVE publication-cut gate also pass，
  while second-proof/pre-CAS/unapplied controller、checkpoint/virtual-segment/coordinator migration and broader kill-cut
...
the common BookKeeper provider-applied C cut、ACTIVE-state multi-controller kill/reconciliation and provider-applied
PREPARED/ACTIVE publication cuts are also covered；second-proof/pre-CAS/unapplied controller cuts、priority budgets、
...
Final extends the current ACTIVE steady-state and provider-applied publication-cut proofs with second-proof/pre-CAS/unapplied
activation cuts、rolling restart、capability mismatch、all selected profiles and clean shutdown/kill cuts。
