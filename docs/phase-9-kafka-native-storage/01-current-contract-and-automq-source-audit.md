...
`phase9KafkaForkDevelopmentSourceLockCheck` 锁定 branch/head/base ancestry/five-commit count/version、Apache 与组织 remote
identity、cached organization trunk ancestry、二十一文件 exact change set/blob、成对 inject marker、adapter/async bridge/
exception-mapper/ListOffsets lifecycle/topic-delta lifecycle/metadata-publisher method signature 和 package-wide no-reflection
...
`phase9M3KafkaForkStockCheck` 不传参数从头验证 stock core compile/checkstyle、四个 `Partition` seam tests、
一个 `ReplicaManager` leader-publication test 和七个完整 `BrokerMetadataPublisherTest`；
...
seam tests、一个 `ReplicaManager` test 和七个完整 `BrokerMetadataPublisherTest`，以及 core/storage checkstyle、
SpotBugs 与 Spotless。lifecycle 只消费 manager 返回的已恢复 writable storage，不重建 storage/recovery；安装失败会按
