...
M0 的初始 blobs 保留在 Git 历史。下表在 F9-M1 ranged-read slice 后刷新；未修改的
materialization rows 仍是待实现 gap，新增 API/core/primary-reader rows 是当前已实现事实。
...
| `nereus-core/.../ReadCoordinator.java` | `98f7e94a46ed88ccf26a24db0543f52bc225bf83` | public request owns exact/containing boundaries、global first overflow、source/coverage/result validation |
...
已消费 ranged semantics，但 NCP2/NTC2 higher generations 尚未实现。默认第三方 provider 对 non-empty
precondition/non-legacy read fail closed，避免静默降级。
