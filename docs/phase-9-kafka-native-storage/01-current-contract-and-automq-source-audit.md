...
M0 的初始 blobs 保留在 Git 历史。下表在 F9-M1 conditional-append slice 后刷新；未修改的
reader/object/materialization rows 仍是待实现 gap，新增 API/core rows 是当前已实现事实。
...
| `nereus-core/.../DefaultStreamStorage.java` | `d6a769e4d5879e3289a0b10c1a2f06cba6fb6ca5` | production overrides the conditional append overload and delegates to the shared coordinator |
...
| `nereus-core/.../ViewReadResult.java` | `ff7d55f2ac374fcad7996886acb1d2cab0617d1d` | coverage exists but is not public adapter API |
...
| `nereus-object-store/.../DefaultWalObjectReader.java` | `35a069be83485307f9eeec474054fc4b91664f6d` | clip skips entry whose start is before requested offset |
...
### 5.1 Public domain and conditional append slices are range-ready；read pipeline is not
...
这仍不等于 ranged Kafka storage 已支持：production append 已消费 expected-start，但 primary readers 和 higher
generations 尚未消费 ranged read/V2 physical format 语义。默认第三方 provider 对 non-empty precondition/non-legacy
read fail closed，避免静默降级。
...
### 6.2 Reader boundary is not range-aware

`DefaultWalObjectReader.clip` 当前只选择 `entry.absoluteOffset >= requestedStart`。start 落在一个多 offset entry
内部时，该 entry 会被跳过。它也在首 entry 超过 maxRecords/maxBytes 时返回空，而 Kafka fetch 需要
“不可拆首批可以单独超 limit”。
