...
> Kafka fork development head：`nereusstream/kafka:nereus/future9-native-kafka-storage@1e3783458b62c9dede252446fde6729ac8075ae3`（52 commits / 126 files from Apache `427b409cf440f745ad6195673d3342f6bd3974d4`；第 49 个 `bd9963c980` 增加 stock retention oracle，第 50 个 `c4a0a2d1fa` 增加 stock `LogCleaner` compaction oracle，第 51 个 `bf8a2946e5` 让隔离 Nereus development coordinates 以 changing/zero-cache 解析，第 52 个 `1e3783458b` 让 metadata 仍存在但 leader 暂时不可用的 transaction marker 保留在 retry queue）；working clone `/Users/liusinan/apps/ideaproject/nereusstream/kafka`；SSH push is configured and the remote head is verified
...
non-Object NTC2 profile、checkpoint cut 与完整 M4
