...
> 状态：In progress；F9-M1/M2 implementation complete；F9-M3 Nereus raw RecordBatch + serialized partition IO + bounded append/async Fetch + binding-first leader manager + storage-profile policy + exact bounded ListOffsets scan + activation-backed Object-WAL provider/checkpoint/read-pin/paged-replay runtime + local Kafka-fork stock-RecordBatch recovery-state/metadata-lifecycle/deferred-provider composition slices in progress；F9-M6 config schema/typed snapshot/pure startup validation + adapter process lifecycle/resource-ownership + activation metadata/coordinator + broker publisher/verifier/runtime startup fence + generic BrokerServer lifecycle partial slices implemented；M2 direct real-service gates pass；fresh inherited final gate blocked by local Pulsar source-lock drift；恢复状态已能在 exact Partition 上 provisional 发布并在失败时撤销，但尚无 CLI/KafkaRaftServer log selection 与真实 KRaft Produce/Fetch 数据面
> Future：F9 Native Kafka Shared Storage
> 目标日期基线：2026-07-23
> AutoMQ 参考锁：`1c648d84819d5c3fef2af585f02149c397584870`（`3.9.0-SNAPSHOT`）
> Kafka fork development lock：local `nereus/future9-native-kafka-storage@672429d94fd82679d7091585ef27c38a7a39f743` from Apache `427b409cf440f745ad6195673d3342f6bd3974d4`（remote push pending）
...
integration、checkpoint failure 的 durable quarantine/audit observer、CLI/KafkaRaftServer production factory selection、`UnifiedLog`/factory、
...
继续 `UNSUPPORTED_FORMAT` fail closed，交给 M4。Controller first-activation scheduling、CLI/KafkaRaftServer selection、
BookKeeper/async provider 和 `UnifiedLog`/log factory 数据面仍未组装。
若以后
