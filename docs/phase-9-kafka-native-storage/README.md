...
完整五档 fresh-process profile matrix、真实多 Controller failover仍未实现。`NereusUnifiedLog` 已有首条同步
...
default；当前 fork head 仍对 BookKeeper async/sync fail closed，下一 fork checkpoint 将把已实现的 product
composition 映射为完整五档 capability/default profile。
...
NCP2 publication 和正常 Kafka read；fork 五档 mapper 与 BookKeeper async/sync fresh-process gates 仍待合入。
