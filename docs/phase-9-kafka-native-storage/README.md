...
`BookKeeperPrimaryWalRuntime.createRetentionService` 才创建 scanner/service，并按 materialization-before-retention
启动、retention-before-materialization 关闭。该里程碑尚未提供 Kafka stream-coverage deletion activation，
因此不能据此宣称已经允许物理删除 ledger。
