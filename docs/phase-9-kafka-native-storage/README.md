...
> 状态：In progress；F9-M1/M2 implementation complete；F9-M3 Nereus raw RecordBatch + serialized partition IO + bounded append/async Fetch + binding-first leader manager + storage-profile policy + exact bounded ListOffsets scan + strict Object-WAL provider runtime + local Kafka-fork record-inspector/async-result/metadata-lifecycle slices in progress；F9-M6 config schema/typed snapshot/pure startup validation + adapter process lifecycle/resource-ownership + activation metadata/coordinator + broker publisher/verifier/runtime startup fence + generic BrokerServer lifecycle partial slices implemented；M2 direct real-service gates pass；fresh inherited final gate blocked by local Pulsar source-lock drift；已有 activation-backed Nereus runtime，但尚无其 Kafka BrokerServer/KRaft composition
> Future：F9 Native Kafka Shared Storage
> 目标日期基线：2026-07-23
> AutoMQ 参考锁：`1c648d84819d5c3fef2af585f02149c397584870`（`3.9.0-SNAPSHOT`）
> Kafka fork development lock：local `nereus/future9-native-kafka-storage@94ecf8c105ad2d765aa9fd4a4929ff86c20882a1` from Apache `427b409cf440f745ad6195673d3342f6bd3974d4`（remote push pending）
...
integration、fork runtime creator/context binding、`UnifiedLog`/factory、
...
stock Kafka 4.3 `MemoryRecords` 实现；bridge/lifecycle tests、14 个 config/mapping-specific tests、完整 stock
...
Kafka config/context 映射到新的 product assembly；BookKeeper/async provider、activation/capability 与 log factory 仍未组装。
...
`OBJECT_WAL_SYNC_OBJECT`，没有 reflection/provider-class fallback；Kafka context 到 provider instance、borrowed scheduler、
KRaft snapshot 与 recovery launcher 的 production creator 仍待接线。
若以后
