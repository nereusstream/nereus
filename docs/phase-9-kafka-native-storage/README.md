...
焦点测试已通过；这些 commit 现均包含在远端 F9 branch。真实 checkpoint object round trip、fresh process restart/takeover 和
...
Produce、consumer Fetch、earliest/latest ListOffsets、S3 object assertion 和 SIGTERM shutdown；随后同一 config/
KRaft directories 启动 fresh JVM，要求 higher broker epoch readiness CAS refresh、旧 offset 0 byte-exact recovery、
第二次 append offset 1、earliest=0/latest=2 和第二次正常 shutdown。M6 process/feature task-name detection 强制根
构建使用并重发 `0.1.0-f9-dev`，避免 release tarball 复用旧 development artifact。BookKeeper/async provider、
真实 controller failover、live takeover、kill cuts 和 chaos/profile 扩展仍未组装。
若以后
