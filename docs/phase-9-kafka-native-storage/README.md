...
> 状态：In progress；F9-M1/M2 implementation complete；F9-M3 Nereus raw RecordBatch + serialized partition IO + bounded append/async Fetch + binding-first leader manager + storage-profile policy + exact bounded ListOffsets scan + strict Object-WAL provider runtime + local Kafka-fork record-inspector/async-result/metadata-lifecycle/deferred-provider composition slices in progress；F9-M6 config schema/typed snapshot/pure startup validation + adapter process lifecycle/resource-ownership + activation metadata/coordinator + broker publisher/verifier/runtime startup fence + generic BrokerServer lifecycle partial slices implemented；M2 direct real-service gates pass；fresh inherited final gate blocked by local Pulsar source-lock drift；已有 activation-backed Kafka context/provider composition，但尚无 concrete recovery/CLI/log selection 与真实 KRaft 数据面
> Future：F9 Native Kafka Shared Storage
> 目标日期基线：2026-07-23
> AutoMQ 参考锁：`1c648d84819d5c3fef2af585f02149c397584870`（`3.9.0-SNAPSHOT`）
> Kafka fork development lock：local `nereus/future9-native-kafka-storage@752953d0ef9c38ae50340147bb4f3172ee9ddc5a` from Apache `427b409cf440f745ad6195673d3342f6bd3974d4`（remote push pending）
...
逆序回收。production `createActivated` 进一步在同一 shared Oxia 上组装 activation store/runtime：先发布 broker capability，
...
integration、concrete fork recovery launcher、CLI/default factory selection、`UnifiedLog`/factory、
...
wiring 已落地并通过 stock KRaft restart；provider-backed runtime composition 已落地，controller scheduling、log wiring 和真实 native-storage KRaft
gate 尚未闭合。
...
交给 product binding-aware wrapper，manager proxy 等待 readiness 且逐操作复查 admission。Recovery bridge 只在 exact
ReplicaManager 出现后 one-time bind。Concrete recovery launcher、controller first-activation scheduling、CLI selection、
BookKeeper/async provider 和 log factory 仍未组装。
若以后
