...
ListOffsets lookup；任一步失败都按 epoch/instance 撤销。fork 已注册完整 91-key
...
及其首个真实 process profile 已实现；Object async creator 与首个真实 process profile 也已实现；BookKeeper async/sync creator、checkpoint time-index candidate、完整五档
real-service profile matrix、真实多 Controller failover仍未实现。`NereusUnifiedLog` 已有首条同步
...
real-provider fresh-process retention/restart/takeover 与完整 stock differential oracle 仍待实现，因此
...
durable quarantine 已在 product runtime 组装。Fork `80445853a3` 将配置面扩展为 91 keys，并为
...
顺序关闭。Mapper 将 Object sync/async 显式映射到同一已安装 Object provider 集合，并把所选 profile 保留为
default；对 BookKeeper async/sync 两个尚无 production composition 的 profile 继续在资源创建前 fail closed。
...
2026-07-28 的 NCP2 物化核心 checkpoint 新增独立 `nereus-kafka-committed-v2` policy，并让
worker、planner、registry scanner、Generation committer 与 required-generation coordinator 在显式
`DIRECT_STREAM` authority 下执行 ranged Kafka batch 的写入、校验和发布；现有 NCP1/NTC1 构造路径继续强制
projection authority。聚焦 materialization/metadata/Kafka activation 回归已通过；production service
lifecycle、append lag admission 与 profile resolver 接线尚未合入。BookKeeper-backed async/sync materialization、
