...
ListOffsets lookup；任一步失败都按 epoch/instance 撤销。fork 已注册完整 97-key
...
durable quarantine 已在 product runtime 组装。Fork `b443750be4` 将配置面扩展为 97 keys，并为
...
`BookKeeperLedgerGcConfiguration` 并参与 compatibility digest；默认 `enabled=false/dryRun=true` 不创建 scanner。
...
versioned logical format `KAFKA_RECORD_BATCH_V1` 比较。fresh-process restart/read fallback 与 response-loss/
takeover 仍待 release-process gate，因此 KF-RET-009 仍不升级为 final-gated。
