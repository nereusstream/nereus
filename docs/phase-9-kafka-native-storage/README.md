...
> 状态：In progress；F9-M1 ranged foundation implementation complete；F9-M2 authority/head-V2 slice complete，binding/checkpoint slices in progress；fresh inherited final gate blocked by local Pulsar source-lock drift；无 native Kafka runtime
...
authority-bound head CAS 及 V1/V2 dual codec，Kafka binding/checkpoint/runtime 仍在实现。若以后
