...
versioned logical format `KAFKA_RECORD_BATCH_V1` 比较。`f9BookKeeperWalAsyncObjectProcessIntegrationTest`
...
经 NCP2 fallback 恢复并继续 append/fetch/ListOffsets。delete-response-loss 与 multi-broker takeover
仍待 release-process gate，因此 KF-RET-009 仍不升级为 final-gated。
