...
`createReadiness`、`createActivation(PREPARED)` and `compareAndSetActivation(...ACTIVE)`；before-provider requires `blocked`
without provider application，while after-provider requires `applied` after the real future succeeds。Oxia must expose the
expected empty/readiness-only/PREPARED/ACTIVE state，and that leader must not yet have emitted reconciliation success when
...
before method invocation or after the first successful returned-stage completion。The after-snapshot cut therefore includes
the fork KRaft broker/topic/local-log fact and all 64 binding-registry shard reads；the after-capability cut includes broker
...
Produce/Fetch/ListOffsets `0/1` plus Object persistence。Fresh execution passes 66/66 actionable tasks in 1m49s and the task
...
reaches an exact single-voter KRaft epoch，Toxiproxy then resets downstream real-Oxia connections while one broker starts。
Both release JVMs remain alive and direct Oxia inspection requires readiness/activation absent during the four-second cut；
after proxy recovery the same controller epoch must emit its reconciliation marker，publish ACTIVE/readiness `[2]` and pass
native Produce/Fetch/ListOffsets `0/1` plus Object persistence。The production store maps unknown Oxia runtime failures to
retriable `METADATA_UNAVAILABLE` while preserving typed condition/invariant outcomes；the deterministic contract proves raw
read and non-applied write failures no longer escape。Fresh execution passes 73/73 actionable tasks in 1m10s and is included
...
remain `PLANNED` because the complete provider/profile/response-loss/chaos and stock differential tiers are still missing。
...
`lastCommitId/end/logicalBytes` instead of mutable durable-head digest；binding observed offsets may lag but never lead the
...
converts the live segment facts to `KafkaVirtualSegmentState` and requires product candidate/selected count to equal stock
...
Oxia/LocalStack release broker，then invokes native targets `3/4/6/HIGH_WATERMARK(-1)`。After each request it independently
...
It additionally runs the dedicated-controller activation publication/proof-cut gates：the exact leader is blocked before or
after snapshot proof、capability aggregation、PREPARED create and ACTIVE CAS，then killed；a higher-epoch controller preserves
...
`89b66ab03b`；published locked head `76f62f3b83`）：the product loads the exact binding、caps planning at activated coverage end、resolves the binding digest to
...
Transaction B holds LSO 4 at data offset 4，migrates `[2] -> [1]` and aborts marker 5；the gate waits for asynchronous marker
...
replay after fixed-anchor retirement and typed unprotected retirement。`Ncp2Ntc2GoldenAndCorruptionTest` freezes non-empty
