...
> worker/protection-recovery checkpoints 已实现，service 和完整 M3 gate 尚未完成；
...
| Stream compacted object (`NCP1`) | per-stream lossless higher-generation `COMMITTED` target | generation index `PREPARED -> COMMITTED` CAS | F4-M3 writer/reader/full verifier/core adapter + planner/recovery + exact-source worker/protection-recovery checkpoints implemented/tested；service/gates pending |
