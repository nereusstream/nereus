...
flowchart TB
    ...
    subgraph Adapters["Compatibility projections"]
        ...
        KAF9["Native Kafka adapter (F9 Designed / absent)"]
    end
...
| `nereus-kafka-adapter`（planned） | F9 native Kafka binding/runtime/checkpoint/codec | Designed / absent；must not be inferred from `nereus-kop-adapter` |
...
| F9 | Native KRaft Kafka fork backed directly by Nereus | In progress；F9-M1 public API slice implemented，core/read/formats and Kafka runtime absent |
