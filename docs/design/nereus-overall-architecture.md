...
read position、per-writable-open owner-session claim/fencing 和 recoverable retention/trim barrier。F3-M1-M6
production/test implementation 尚未开始。The broker-supplied ownership checker gates claim/publication，but durable
...
- Future 3 is code-level designed/M0R-gated but not implemented；Future 4 materialization/compaction and later tracks
  remain designed。
...
| F3 | Cursor/subscription durable state | Designed；M0/M0R gated，M1-M6 not started |
