# Nereus 总体架构设计：可选 Ursa-like / AutoMQ-like 的共享存储引擎

> 这是内部总体架构设计文档，不是已经编号的 Apache Pulsar PIP。
> Nereus 是我们面向 Pulsar/Kafka 的商业化共享存储产品方向：Pulsar 协议层 +
> KoP Kafka 兼容 + 可选 Oxia/KRaft 元数据路径 + 共享存储数据面 +
> lakehouse-native stream-table duality。存储层需要同时支持 Ursa-like 同步可见提交
> 和 AutoMQ-like 异步对象化两种模式。
...
S3。真正的目标是一个可配置的共享存储流引擎：在同一套 `streamId + offset` truth
上支持 Ursa-like 强元数据提交和 AutoMQ-like 异步对象化。
...
这个设计是一步到位的目标架构。所谓 latency optimized、balanced 和 cost optimized
只是同一架构下的 topic/stream 级 profile，不是分阶段产品路线：

| Storage profile | Primary WAL | Object materialization | Ack boundary | 主要场景 |
| --- | --- | --- | --- | --- |
| `BOOKKEEPER_WAL_ONLY` | BookKeeper | disabled | BK quorum durable + stable stream position | 低延迟、迁移敏感、暂不启用远端对象 |
| `BOOKKEEPER_WAL_SYNC_OBJECT` | BookKeeper | Ursa-like sync | BK durable + Oxia visible commit | 性能/成本均衡，读路径和对象索引同步发布 |
| `BOOKKEEPER_WAL_ASYNC_OBJECT` | BookKeeper | AutoMQ-like async | BK durable fast ack；后台对象化 | 性能/成本均衡，优先 producer latency |
| `OBJECT_WAL_SYNC_OBJECT` | Object WAL | Ursa-like sync | Object WAL durable + Oxia visible commit | 低成本，写入即对象 WAL 可读 |
| `OBJECT_WAL_ASYNC_OBJECT` | Object WAL | AutoMQ-like async | Object WAL durable fast ack；后台生成读优化对象 | 低成本，写放大和对象整理后台化 |

`StorageProfile.OBJECT_WAL` 是早期 Phase 1 名称，语义上等价于
`OBJECT_WAL_SYNC_OBJECT`，后续实现应逐步迁移到更精确的 profile 名称。

对象存储关闭必须是 topic/stream 级能力，不能只做 broker 全局开关。Kafka Tiered
Storage / Remote Log Storage 和 Pulsar offload 的配置需要通过 Nereus adapter 做兼容映射：
当 Nereus profile 已经接管对象化时，上层 remote storage 配置只能作为策略输入或被显式拒绝，
不能再启动第二套远端日志。
...
Nereus 底层 storage truth 保持一致，但上层元数据和协议集成有三条路线：

| Integration | Metadata plane | Protocol/runtime | Storage truth | 需要改造 |
| --- | --- | --- | --- | --- |
| Pulsar native + Oxia | Oxia | 社区版 Pulsar native | Nereus `StreamStorage` | Nereus 侧实现 ManagedLedger-compatible facade；Pulsar 可先不改 |
| Pulsar Broker + KoP + Oxia | Oxia | KoP + Pulsar Broker | Nereus `StreamStorage` | KoP/Pulsar 的 offset、MessageId、Position 映射要重新设计 |
| Kafka native + KRaft | KRaft / Kafka controller metadata | Kafka 原生 | Nereus stream/log adapter | 需要 Kafka log API、leader epoch、remote log metadata 的原生适配 |

当前仓库优先落 Oxia + Pulsar-native 路线；KoP 和 Kafka native 不能引入第二套 durable
log，只能作为同一 `streamId + offset` truth 的协议投影。
...
Nereus 对 AutoMQ-like 的采纳边界是：

- AutoMQ-like 是一种 async object materialization profile，不是第二套 offset truth；
- producer ack 可以不等待读优化对象、compacted object 或 remote segment publish；
- 但对 Pulsar/Kafka 客户端返回成功时，必须已经有稳定的 stream offset / MessageId 投影；
- 如果未来要支持完全不等待 Oxia head/index 的纯 WAL-only ack，需要单独设计 WAL-resident
  offset reservation、crash recovery 和 duplicate suppression，不能混入当前 Phase 1 语义。
...
### 7.3 ManagedLedger-compatible Facade
...
| WAL | 用途 | 可搭配 profile | producer ack 条件 |
| --- | --- | --- | --- |
| `BookKeeperWalStorage` | 低延迟、迁移友好、Pulsar-native profile | BK-only / BK sync object / BK async object | 取决于 profile：BK quorum durable 后可 fast ack，或继续等待 Oxia visible commit |
| `ObjectWalStorage` | 低成本、stateless broker、对象存储优先 | Object sync / Object async | Object WAL durable 后可 fast ack，或继续等待 Oxia visible commit |
| `LocalDiskWalStorage` | 可选边缘/单 AZ 优化 | 未来 local-first profile | local durable 后必须按配置完成复制或 checkpoint，不能单副本对外承诺共享存储语义 |

BookKeeper 在新架构里退化为一种 WAL profile，不再拥有完整 ManagedLedger 元数据和
cursor 真相。

Profile 决定 producer ack 的等待边界：

- Ursa-like sync object profile：`WAL_DURABLE_AND_INDEX_COMMITTED`，ack 等待 primary WAL durable、
  stream-head/Oxia visible commit，以及当前 profile 要求的读索引物化。
- AutoMQ-like async object profile：`WAL_DURABLE`，ack 不等待读优化对象或 remote segment 发布；
  但 append result 仍必须携带稳定 offset / MessageId 投影，后台 materializer 再把 WAL range
  发布为对象索引、compacted object 或 lakehouse-ready object。
- BK-only profile：禁用 Nereus 对象化；读路径和 retention 只依赖 BookKeeper WAL 与 Oxia
  metadata。上层 Kafka/Pulsar remote storage 配置必须被 adapter 显式映射或拒绝。

### 7.6 Object Data Plane
...
### 9.2 Nereus Commit-time Offset Assignment
...
### 9.3 Pulsar MessageId Projection
...
### 15.2 Stream-Delivered-to-Table (SDT)
...
### 15.3 Distributed Compaction Service
...
nereus.storage.defaultProfile=OBJECT_WAL_SYNC_OBJECT
nereus.storage.allowTopicOverride=true
nereus.wal.primary=object
nereus.object.materializationMode=sync
...
`managedLedgerStorageClassName`，并增加 Nereus profile policy：

```properties
nereus.topic.storageProfile=BOOKKEEPER_WAL_ONLY
nereus.topic.storageProfile=BOOKKEEPER_WAL_SYNC_OBJECT
nereus.topic.storageProfile=BOOKKEEPER_WAL_ASYNC_OBJECT
nereus.topic.storageProfile=OBJECT_WAL_SYNC_OBJECT
nereus.topic.storageProfile=OBJECT_WAL_ASYNC_OBJECT
```

兼容规则：

- `managedLedgerStorageClassName != nereus` 时，Nereus profile 不生效；
- `BOOKKEEPER_WAL_ONLY` 与 Kafka Tiered Storage / Pulsar offload 可以共存，但 Nereus 不接管
  remote object truth；
- 任何启用 Nereus object materialization 的 profile 都必须拦截或映射上层 remote storage
  配置，避免同一 topic 有两套远端 segment；
- topic profile 变更只能影响新 append 和后台 materialization policy，不能改变已提交 offset
  range 的 truth。
...
`nereus-core` 内部应进一步按 profile 拆包，避免 Ursa-like 和 AutoMQ-like 互相复制：

```text
io.nereus.core.common      shared session, offset, WAL, projection, and read selection contracts
io.nereus.core.ursalike    sync append coordinator and visible-index publication
io.nereus.core.automqlike  fast-ack append coordinator and async object materialization
```

AutoMQ-like 的详细实现边界见 `docs/automq-like-stream-storage/README.md`。
...
### 26.2 Nereus 的 Pulsar-native 增强
