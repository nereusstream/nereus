...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5 + Future 2 complete/final-gated；F3-M0/M0R design-gated
...
metadata/snapshot 基础生产代码、focused/golden tests、真实 Oxia 和 LocalStack final gate 已完成，M2-M6
尚未开始。The broker-supplied ownership checker gates claim/publication，but durable
...
- Future 3 remains `Designed/In progress` overall：its M1 storage primitive is implemented/final-gated，but durable
  cursor execution requires M2-M6；Future 4 materialization/compaction and later tracks remain designed。
...
| F3 | Cursor/subscription durable state | Designed/In progress；M0/M0R gated，M1 foundation final-gated，M2 next |
