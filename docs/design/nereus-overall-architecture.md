...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M2 complete/final-gated；F4-M3–M6 未实现
...
ordinary/Docker-backed final gates；M3-M6 的 compacted format/worker、recovery/GC、async/Pulsar execution
path 仍是 Designed target。
...
- Future 4 end-to-end materialization/compaction remains unavailable until F4-M3–M6 are implemented；later tracks
  remain north-star designs；Future 3 and F4-M1–M2 are implemented/final-gated。
...
| F4 | Materialization/compaction/generation/GC | In progress；F4-M1–M2 final-gated，F4-M3–M6 not implemented |
