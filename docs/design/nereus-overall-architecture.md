...
| `nereus-bookkeeper` | BookKeeper primary-WAL adapter/lifecycle/retention | BK-M1 foundation implemented/final-gated；BK-M2 exact allocator/writer/recovery/reader、bounded uncertain-allocation reconciliation、keyspace/records/codecs、production/fake metadata stores、whole-ledger retention、cold all-shard Oxia coverage、multi-entry/partial-write cuts and create-loss/late-create/restart/delete real-service checkpoints implemented，remaining crash-recovery/fencing/read/retention matrix and aggregate pending；uses public BookKeeper client API and exposes no ManagedLedger types to L0 |
