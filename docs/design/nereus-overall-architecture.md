...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M6 complete/final-gated；F1-BK BK-M0–BK-M2 complete/final-gated，BK-M3 focused implementation through real publication/read/physical-delete、fresh-runtime response loss、lag admission and unreadable-output fail-closed evidence，BK-M4–M6 not implemented
...
| `nereus-bookkeeper` | BookKeeper primary-WAL adapter/lifecycle/retention | BK-M1 foundation and BK-M2 exact allocator/writer/recovery/reader、O(1) attempt/range recovery、bounded uncertain-allocation reconciliation、keyspace/records/codecs、production/fake metadata stores、whole-ledger retention、cold all-shard Oxia coverage、multi-entry/partial-write cuts and create-loss/late-create/restart/delete real-service evidence complete/final-gated；later independent-process chaos remains BK-M6；uses public BookKeeper client API and exposes no ManagedLedger types to L0 |
