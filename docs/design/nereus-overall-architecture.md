...
| F9 | Native KRaft Kafka fork backed directly by Nereus | In progress；F9-M1/M2 and substantial M3–M6 product/fork slices implemented；explicit launcher and real combined-node KRaft/Oxia/S3 gate pass initial IO、fresh-JVM user/group/committed-transaction recovery and forced open-transaction abort recovery；multi-broker/checkpoint/virtual-segment、multi-controller、provider-profile、retention/compaction and aggregate gates remain open |
