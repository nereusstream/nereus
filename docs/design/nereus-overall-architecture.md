...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5 + Future 2 complete/final-gated；F3-M0/M0R
> design-gated，F3-M1-M5 complete/gated，F3-M6 pending
> 最近设计/实现同步：2026-07-14
...
BookKeeper recovery gates pass `phase3M5Check` and `phase3M5FinalCheck`；only M6 final compatibility/aggregate gates
remain pending。
...
- durable cursor final compatibility/aggregate acceptance、KoP、routing、compaction、lakehouse、advanced Pulsar semantics；
- Future 3 remains `Designed/In progress` overall：its M1-M5 storage/state-machine/facade/broker/real-recovery slices
  are implemented/gated，but completion still requires the M6 final compatibility gate；Future 4
  materialization/compaction and later tracks remain designed。
...
| `nereus-managed-ledger` | ManagedLedger facade | F2-M1-M4 plus F3-M1-M5 implemented/tested；projection、ledger/factory、append/read/lifecycle、durable cursor/retention and 10k scale boundary complete |
| `nereus-pulsar-adapter` | broker integration/config/policy | product runtime/S3 provider implemented；fork binding/admission/capability/policy guards and M5 real two-broker acceptance complete |
...
| F3 | Cursor/subscription durable state | Designed/In progress；M0/M0R gated，M1-M5 complete/gated，M6 next |
...
| Cursor-snapshot physical GC/read-reference grace | Future 4 implementation |
