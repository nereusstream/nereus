...
> 状态：North-star design；Future 1 / Phase 1 complete，Future 2 in progress（design/API review only）
> 最近实现同步：2026-07-11
...
Future 1 / Phase 1 M0-M8 已完成。当前实现轨道按 design index 进入 Future 2；F2-M0/M0R 仅完成
设计/API review，生产 facade 尚未实现。Future 1 后续只接受
...
| `nereus-core` | coordinators and state machines | Phase 1 M4-M8 implemented and final-gated |
...
| Track | Scope | Status |
| --- | --- | --- |
| F1 | L0 API、Object WAL、Oxia commit、resolve/read/trim | Implemented / Phase 1 |
| F2 | ManagedLedger facade and virtual positions | In progress（M0/M0R design complete） |
