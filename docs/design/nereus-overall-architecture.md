...
| `nereus-kafka-adapter` | F9 native Kafka binding/runtime/checkpoint/codec | In progress；M1/M2 plus substantial M3–M6 adapter/runtime/recovery/retention/compaction/activation slices implemented，with real single-node recovery、multi-broker Object/BookKeeper takeover、ACTIVE-state three-voter controller failover and provider-applied PREPARED/ACTIVE publication-cut evidence；checkpoint/virtual-segment、remaining pre-publication proof/pre-CAS process cuts、coordinator and aggregate gates remain open；must not be inferred from `nereus-kop-adapter` |
...
| F9 | Native KRaft Kafka fork backed directly by Nereus | In progress；F9-M1/M2 and substantial M3–M6 product/fork slices implemented；explicit launcher and real process gates pass initial IO、fresh-JVM user/group/transaction recovery、Object/BookKeeper takeover、ACTIVE-state three-voter controller failover and provider-applied PREPARED/ACTIVE response-loss cuts；checkpoint/virtual-segment、remaining pre-publication proof/pre-CAS process activation、coordinator、retention/compaction and aggregate gates remain open |
