...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 complete/final-gated；Future 4 M0 code-level design complete
...
`OBJECT_WAL_ASYNC_OBJECT` 与 Pulsar rollout 边界。这是 Designed target contract，Phase 4
生产代码仍未开始；下一里程碑是 F4-M1。
...
- Future 4 materialization/compaction remains Designed with its M0 code-level gate complete；later tracks remain
  north-star designs；Future 3 is implemented/final-gated。
...
| F4 | Materialization/compaction/generation/GC | Designed / F4-M0 code-level gate complete；M1 planned |
