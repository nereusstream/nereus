...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M2 complete/final-gated；F4-M3 format + planner/recovery + exact-source worker + protection/checkpoint reconciliation + bounded service lifecycle checkpoints implemented，Pulsar evidence/gates and F4 milestones 4–6 pending
...
- F4-M1–M2 final-gated API/metadata/codecs、conditional metadata delete、slash-aware fixed-depth Oxia scans、
  ...
  M3 compacted format、exact-source worker、checkpoint reconciliation 与 bounded service lifecycle 已接入该基础，但 Pulsar evidence/gates 与 M4
  retirement/GC 尚未完成；
...
service lifecycle checkpoints 已实现，但 Pulsar evidence/gates 未完成，M4-M6 的 recovery/GC、async/Pulsar
...
- Future 4 end-to-end materialization/compaction remains unavailable until the remaining F4-M3 Pulsar evidence/gates and
  F4-M4–M6 are implemented；later tracks remain north-star designs；Future 3 and F4-M1–M2 are
  implemented/final-gated，and only the F4-M3 format/planning/recovery/worker/checkpoint/service checkpoints are executable。
...
| F4 | Materialization/compaction/generation/GC | In progress；F4-M1–M2 final-gated；F4-M3 Parquet read/write + planner/recovery + exact-source worker + protection/checkpoint reconciliation + bounded service lifecycle checkpoints implemented，Pulsar evidence/gates and F4 milestones 4–6 pending |
