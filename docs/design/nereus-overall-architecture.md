...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 complete/final-gated；Future 4 F4-M1 implementation in progress
...
- F4-M1 partial API/metadata/codecs、conditional metadata delete、replayable/guarded object IO、physical reference
  proof values、durable reader-pin and same-key protection handshakes；这些 primitives 尚未接入 generation/GC；
...
`OBJECT_WAL_ASYNC_OBJECT` 与 Pulsar rollout 边界。F4-M1 已开始并完成 API/metadata/object IO 与 core
reader/protection primitives 的阶段性实现；M1 aggregate/final gate 尚未完成，M2-M6 的 generation、worker、
recovery/GC、async/Pulsar execution path 仍是 Designed target。
...
- Future 4 end-to-end materialization/compaction remains unavailable while F4-M1 primitives are in progress；later
  tracks remain north-star designs；Future 3 is implemented/final-gated。
...
| F4 | Materialization/compaction/generation/GC | In progress；F4-M1 primitives partial，aggregate/final gate pending |
