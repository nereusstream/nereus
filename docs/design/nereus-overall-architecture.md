...
| F9 | Native KRaft Kafka fork backed directly by Nereus | In progress；F9-M1/M2 and substantial M3–M6 product/fork slices implemented；explicit launcher and real combined-node KRaft/Oxia/S3 gate pass initial IO plus fresh-JVM recovery of user、group and committed transaction state；live takeover、ongoing/aborted transaction failover、multi-controller、retention/compaction and kill/aggregate gates remain open |
