...
- F4-M1–M2 final-gated API/metadata/codecs、conditional metadata delete、slash-aware fixed-depth Oxia scans、
  ...
  M3 compacted format 与 exact-source worker checkpoint 已接入该基础，但 service/gates 与 M4
  retirement/GC 尚未完成；
...
- `WAL_DURABLE` fast boundary 和 async materialization profile（F4-M0 已设计；M3 planner/recovery 与
  exact-source worker checkpoint 已实现，但 service/profile 尚未启用）；
- KoP、routing、compaction、lakehouse、advanced Pulsar semantics；
- Future 4 end-to-end materialization/compaction remains unavailable until the remaining F4-M3 recovery/service/gates and
  F4-M4–M6 are implemented；later tracks remain north-star designs；Future 3 and F4-M1–M2 are
  implemented/final-gated，and only the F4-M3 format/planning/recovery/worker checkpoints are executable。
