...
Oxia/LocalStack final gates。M4 through checkpoint AJ 已实现 NRC1 protocol、protected generation-zero append、
...
inventory and the single central GC fence's post-drain revalidation callback without enabling deletion。M5 checkpoint X 又实现 exact durable registration create/refresh/final
