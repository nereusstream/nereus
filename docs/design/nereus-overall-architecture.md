...
- F4-M1–M2 final-gated API/metadata/codecs、conditional metadata delete、slash-aware fixed-depth Oxia scans、
  ...
  M3 compacted format、exact-source worker、checkpoint reconciliation、bounded service lifecycle、Pulsar Entry/NCP1 byte round trip、topic-compaction neutral SPI/registry 与 terminal workflow-metadata retirement 已接入该基础，但 topic execution engine/worker/gates 与 M4
  retirement/GC 尚未完成；
...
workflow-metadata retirement checkpoints 已实现，但 topic execution engine/worker/gates 未完成，M4-M6 的
recovery/GC、async/Pulsar
...
- Future 4 end-to-end materialization/compaction remains unavailable until the remaining F4-M3 topic execution engine/worker/gates and
  F4-M4–M6 are implemented；later tracks remain north-star designs；Future 3 and F4-M1–M2 are
  implemented/final-gated，and only the F4-M3 format/planning/recovery/worker/checkpoint/service/Pulsar-round-trip/
  neutral-topic-SPI/terminal-metadata-retirement checkpoints are executable。
...
| `nereus-materialization` | planner/task/worker/publication/checkpoint/recovery/GC orchestration | module present；M1/M2 publication final-gated and M3 format/planner/task recovery/exact-source worker/protection/checkpoint reconciliation/bounded service lifecycle plus Pulsar exact-byte round trip、topic-compaction neutral SPI/registry and terminal workflow-metadata retirement checkpoints implemented；topic execution engine/worker/gates and M4 GC pending；depends on core, never the reverse |
...
| F4 | Materialization/compaction/generation/GC | In progress；F4-M1–M2 final-gated；F4-M3 Parquet read/write + planner/recovery + exact-source worker + protection/checkpoint reconciliation + bounded service lifecycle + Pulsar exact-byte round trip + topic-compaction neutral SPI/registry + terminal workflow-metadata retirement checkpoints implemented，topic execution engine/worker/gates and F4 milestones 4–6 pending |
