...
- F4-M1–M2 final-gated API/metadata/codecs、conditional metadata delete、slash-aware fixed-depth Oxia scans、
  ...
  M3 compacted format、exact-source worker、checkpoint reconciliation、bounded service lifecycle、Pulsar Entry/NCP1 byte round trip、topic-compaction SPI/COMMITTED-source bootstrap/tagged-key/sorted-spill engine-worker-publication 与 terminal workflow-metadata retirement 已接入该基础，但 M3 gates 与 M4
  retirement/GC 尚未完成；
...
workflow-metadata retirement checkpoints，以及 topic COMMITTED-source bootstrap、tagged-v1 unkeyed 表示、
sorted-spill two-pass engine/worker/isolated publication 已实现，但 M3 aggregate/final gates 未完成，M4-M6 的
...
- Future 4 end-to-end materialization/compaction remains unavailable until the remaining F4-M3 gates and
  ...
  topic-engine-worker/terminal-metadata-retirement checkpoints are executable。
...
| `nereus-materialization` | planner/task/worker/publication/checkpoint/recovery/GC orchestration | module present；M1/M2 publication final-gated and M3 format/planner/task recovery/exact-source worker/protection/checkpoint reconciliation/bounded service lifecycle plus Pulsar exact-byte round trip、topic SPI/registry、terminal workflow retirement and COMMITTED-source/tagged-v1/sorted-spill topic engine/worker/publication checkpoints implemented；M3 aggregate/final gates and M4 GC pending；depends on core, never the reverse |
