...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M6 complete/final-gated；F1-BK BK-M0/BK-M1 complete/final-gated，BK-M2 through real Oxia/BookKeeper restart/delete checkpoint，BK-M3 source/protection/profile/lag checkpoint in progress，BK-M4–M6 not implemented
...
| `BOOKKEEPER_WAL_ASYNC_OBJECT` | BookKeeper | WAL durable + stable head commit after lag admission | background via shared F4 task/path | BK-M3 source/protection/profile/lag checkpoint implemented；retirement/final gate pending / production profile reserved |
