...
> design-gated，F3-M1/M2 complete/final-gated
...
concurrency model 与真实 Oxia + LocalStack S3 跨 runtime final gate 也已完成。M3-M6 尚未开始。The
broker-supplied ownership checker gates claim/publication，but durable
root session/version remains the cursor CAS fence。
...
- durable cursor/ack execution、KoP、routing、compaction、lakehouse、advanced Pulsar semantics；
- Future 3 remains `Designed/In progress` overall：its M1 storage primitive and M2 protocol-neutral durable cursor
  state machines are implemented/final-gated，but a deployable cursor execution path still requires M3-M6；Future 4
...
| F3 | Cursor/subscription durable state | Designed/In progress；M0/M0R gated，M1 foundation and M2 state machines final-gated，M3 next |
