...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1 complete/final-gated；F4-M2–M6 未实现
...
- F4-M1 final-gated API/metadata/codecs、conditional metadata delete、slash-aware fixed-depth Oxia scans、
  replayable/guarded object IO、physical reference proof values、durable reader-pin and same-key protection
  handshakes；这些 primitives 尚未接入 generation/GC；
...
`OBJECT_WAL_ASYNC_OBJECT` 与 Pulsar rollout 边界。F4-M1 已完成 API/metadata/object IO 与 core
reader/protection primitives，并于 2026-07-15 通过 ordinary/Docker-backed final gate；M2-M6 的 generation、
worker、recovery/GC、async/Pulsar execution path 仍是 Designed target。
...
- `WAL_DURABLE` fast boundary 和 async materialization workers（F4-M0 已设计，未实现）；
- KoP、routing、compaction、lakehouse、advanced Pulsar semantics；
- Future 4 end-to-end materialization/compaction remains unavailable until F4-M2–M6 are implemented；later tracks
  remain north-star designs；Future 3 and F4-M1 are implemented/final-gated。
...
| `nereus-materialization` | planner/task/worker/publication/checkpoint/recovery/GC orchestration | F4-M0 designed；module not yet present in `settings.gradle.kts`；depends on core, never the reverse |
...
| F4 | Materialization/compaction/generation/GC | In progress；F4-M1 final-gated，later F4 milestones not implemented |
