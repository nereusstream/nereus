...
| `nereus-bookkeeper` | BookKeeper primary-WAL adapter/lifecycle/retention | BK-M1 foundation implemented/final-gated；BK-M2 exact allocator/writer/recovery/reader、keyspace/records/codecs、production/fake metadata stores、whole-ledger retention and real-service checkpoint implemented，remaining matrix/aggregate pending；uses public BookKeeper client API and exposes no ManagedLedger types to L0 |
