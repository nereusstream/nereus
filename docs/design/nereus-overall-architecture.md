...
> 状态：North-star design；Future 1 / Phase 1 complete，Phase 1.5 P15-M0 design complete，F2-M0/M0R reviewed
> 最近实现同步：2026-07-11
...
Future 1 / Phase 1 M0-M8 已完成，F2-M0/M0R 也已完成设计/API review。该 review 锁定了当前 L0
尚不具备的共享前置，因此当前交付插入 Phase 1.5：P15-M0 code-level design complete，P15-M1 target API
是下一实现里程碑；F2-M1 在 P15-M5 后恢复。Phase 1 production contract 在 Phase 1.5 相应代码/gate
落地前保持不变。
...
- Phase 1.5 generic read-target/adapter、metadata evolution、append recovery、seal/delete（code-level designed，
...
authoritative. Policy text alone never migrates an active binding.
...
| --- | --- | --- |
| `nereus-api` | stable protocol-neutral L0 surface | Phase 1 implemented；Phase 1.5 target API designed |
| `nereus-core` | coordinators and state machines | Phase 1 M4-M8 implemented；Phase 1.5 adapter/recovery/lifecycle designed |
...
Phase 1.5 已在代码级冻结 tagged `ReadTarget`、generic `AppendResult/ResolvedRange`、primary-WAL registry
和 generic durable target records；P15-M1-M5 尚待实现。BookKeeper profile 在其真实 writer/reader 注册前
...
Phase 1.5 P15-M2/M3 will make stable commit and generation-zero materialization separate idempotent operations while
retaining strict public success。P15-M4 will also retain an exact process-local attempt after any post-head non-known result；recovery resumes
...
| Track | Scope | Status |
| --- | --- | --- |
| F1 | L0 API、Object WAL、Oxia commit、resolve/read/trim | Phase 1 implemented；Phase 1.5 P15-M0 designed |
| F2 | ManagedLedger facade and virtual positions | In progress（M0/M0R design complete；implementation after P15-M5） |
...
| --- | --- |
| Phase 1.5 generic target/metadata rollout and old-binary cutoff | P15-M5 production rollout |
