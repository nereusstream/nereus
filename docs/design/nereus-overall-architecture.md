...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M6 complete/final-gated；F1-BK BK-M0/BK-M1 complete/final-gated，BK-M2 metadata/store/scanner checkpoint implemented，BK-M3–M6 not implemented
...
| --- | --- | --- | --- | --- |
| `BOOKKEEPER_WAL_ONLY` | BookKeeper | WAL durable + stable head commit | disabled | BK-M2 metadata store/scanner implemented；physical runtime incomplete / profile pre-IO rejected |
...
| `nereus-bookkeeper` | BookKeeper primary-WAL adapter/lifecycle/retention | BK-M1 module/API/config/checksum/resource foundation implemented/final-gated；BK-M2 keyspace/records/codecs and focused metadata stores/scanners implemented，physical writer/reader/lifecycle/retention in progress；uses public BookKeeper client API and exposes no ManagedLedger types to L0 |
