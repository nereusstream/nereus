...
Oxia/LocalStack final gates。M4 through checkpoint S 已实现 NRC1 protocol、protected generation-zero append、
...
grace-fenced higher-generation pre-drain/reproof，并增加 durable generation-activation metadata authority foundation。
Backfill/global domains、runtime composition、cursor/root/audit completion 及 M5-M6 仍是 target；production deletion
继续关闭。
...
| `nereus-core` | coordinators and state machines | primary-WAL adapters、protected prepare/head/materialize、exact recovery、seal/delete and F4 physical lease/protection/reference SPI implemented；M4 recovery/root/GC consumers are in progress through checkpoint S |
