...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M2 complete/final-gated；F4-M3 format + planner/recovery checkpoints implemented，exact-source worker/gates and M4–M6 pending
...
- F4-M1–M2 final-gated API/metadata/codecs、conditional metadata delete、slash-aware fixed-depth Oxia scans、
  ...
  这些路径尚未接入 M3 compacted format/worker 或 M4 retirement/GC；
...
- `WAL_DURABLE` fast boundary 和 async materialization workers（F4-M0 已设计；M3 planner/recovery primitives
  已实现，但 worker/profile 尚未实现）；
- KoP、routing、compaction、lakehouse、advanced Pulsar semantics；
- Future 4 end-to-end materialization/compaction remains unavailable until the remaining F4-M3 worker/gates and
  F4-M4–M6 are implemented；later tracks remain north-star designs；Future 3 and F4-M1–M2 are
  implemented/final-gated，and only the F4-M3 format/planning/recovery checkpoints are executable。
...
| `nereus-materialization` | planner/task/worker/publication/checkpoint/recovery/GC orchestration | module present；M1/M2 publication final-gated and M3 format/planner/task recovery checkpoints implemented；exact-source worker/service and M4 GC pending；depends on core, never the reverse |
...
| F4 | Materialization/compaction/generation/GC | In progress；F4-M1–M2 final-gated；F4-M3 Parquet read/write + planner/recovery checkpoints implemented，exact-source worker/gates and M4–M6 pending |
