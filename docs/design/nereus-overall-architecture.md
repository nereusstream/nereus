...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M6 complete/final-gated；F1-BK BK-M0/BK-M1 complete/final-gated, BK-M2–M6 not implemented
...
| --- | --- | --- | --- | --- |
| `BOOKKEEPER_WAL_ONLY` | BookKeeper | WAL durable + stable head commit | disabled | BK-M1 foundation complete / BK-M2 profile next |
...
| `nereus-bookkeeper` | BookKeeper primary-WAL adapter/lifecycle/retention | BK-M1 module/API/config/checksum/resource foundation implemented/final-gated；BK-M2 writer/reader/lifecycle/retention next；uses public BookKeeper client API and exposes no ManagedLedger types to L0 |
