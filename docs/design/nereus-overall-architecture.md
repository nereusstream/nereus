...
> 状态：North-star design；Future 1 / Phase 1 + Phase 1.5、Future 2、Future 3 与 Future 4 F4-M1–M6 complete/final-gated；F1-BK BK-M0/BK-M1 complete/final-gated，BK-M2 through real Oxia/BookKeeper restart/delete checkpoint，BK-M3 focused implementation through real publication/read/physical-delete、fresh-runtime response loss、lag admission and unreadable-output fail-closed evidence，BK-M4–M6 not implemented
...
ordinary/Docker-backed final gates；F4-M4 于 2026-07-19 通过 real-service/scale/failure/two-broker final gate。
M3 的 Parquet writer/strict-reader/full verifier、NTC1 facade、core adapter、
...
owned monotonic deadline。The retry-disabled real two-broker gate now deletes generation-zero source bytes while
preserving compacted reads、exact MessageIds、unload/failover/restart/reverse takeover and stock BookKeeper；M4 is
final-gated. M5
...
| --- | --- | --- | --- | --- |
| `BOOKKEEPER_WAL_ONLY` | BookKeeper | WAL durable + stable head commit | disabled | BK-M2 module/facade runtime and real Oxia/BookKeeper restart/delete checkpoint implemented；aggregate pending，production broker pre-IO rejected until BK-M5 |
| `BOOKKEEPER_WAL_SYNC_OBJECT` | BookKeeper | stable head + gen0 BK index + `REQUIRED_OBJECT_GENERATION` COMMITTED/read proof | synchronous via shared F4 task/path | BK-M1 foundation complete / profile reserved |
| `BOOKKEEPER_WAL_ASYNC_OBJECT` | BookKeeper | WAL durable + stable head commit after lag admission | background via shared F4 task/path | BK-M3 focused source/protection/profile/lag、retirement、real publication/read/physical-delete/response-loss and unreadable-output fail-closed evidence implemented；predecessor/aggregate final gate pending / production profile reserved |
...
| `nereus-bookkeeper` | BookKeeper primary-WAL adapter/lifecycle/retention | BK-M1 foundation implemented/final-gated；BK-M2 exact allocator/writer/recovery/reader、O(1) attempt/range recovery、bounded uncertain-allocation reconciliation、keyspace/records/codecs、production/fake metadata stores、whole-ledger retention、cold all-shard Oxia coverage、multi-entry/partial-write cuts and create-loss/late-create/restart/delete real-service checkpoints implemented，remaining real crash/fencing/read/retention matrix and aggregate pending；uses public BookKeeper client API and exposes no ManagedLedger types to L0 |
