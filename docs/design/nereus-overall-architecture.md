...
> design-gated，F3-M1-M4 complete/gated，F3-M5-M6 pending
...
remains the cursor CAS fence。M5-M6 real recovery/final gates remain pending。
...
- complete durable cursor real-service/final acceptance、KoP、routing、compaction、lakehouse、advanced Pulsar semantics；
- Future 3 remains `Designed/In progress` overall：its M1-M4 storage/state-machine/facade/broker slices are
  implemented/gated，but completion still requires M5-M6 real recovery and final compatibility gates；Future 4
...
| F3 | Cursor/subscription durable state | Designed/In progress；M0/M0R gated，M1-M4 complete/gated，M5 next |
