...
| F9 | Native KRaft Kafka fork backed directly by Nereus | In progress；F9-M1/M2 and substantial M3–M6 product/fork slices implemented；explicit launcher and real process gates pass initial IO、fresh-JVM user/group/transaction recovery、Object/BookKeeper takeover and ACTIVE-state three-voter controller failover；checkpoint/virtual-segment、PREPARED/response-loss activation、coordinator、retention/compaction and aggregate gates remain open |
