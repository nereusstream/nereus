...
共存。M5 ordinary gate 105/105、包含全部前驱的 final gate 231/231 均通过；仅 BK-M6 aggregate
scenario/scale/chaos/compatibility 仍未实现。
...
下一项底层交付是 F1-BK，其代码级 target 在 `../phase-bk-bookkeeper-primary-wal/README.md`；BK-M0–BK-M5
...
rollout。独立 process/two-client chaos、scale 与全矩阵兼容性仍属于 BK-M6；在线 profile migration 不在
当前交付范围。
Legacy L0 合同以
