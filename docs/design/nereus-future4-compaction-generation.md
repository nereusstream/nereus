...
> bounded service lifecycle、Pulsar Entry/NCP1 exact-byte round trip and topic-compaction neutral SPI/registry
> checkpoints implemented，topic execution engine/worker、terminal metadata retirement/gates and F4 milestones
> 4–6 pending
...
lifecycle。随后完成 Pulsar opaque Entry 经 NCP1 的 byte-for-byte 与 middle-batch MessageId 往返。下一步是
topic-compaction execution engine/worker、terminal workflow-metadata retirement 和 M3 gates，不重新打开协议选型。
