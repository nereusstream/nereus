...
> restart/BookKeeper gate passed；F4-M1–M5 are final-gated；F4-M6 checkpoints BD–BM cover 32-ref merge、
...
> provider-neutral Hadoop/Oxia logging composition、bounded Docker aggregate scheduling and
...
Pulsar retention rollout but does not claim M6 scale/failure/compatibility certification. M6 checkpoints BD–BM are
...
local and nested-Pulsar release tasks through one shared permit. The complete aggregate
