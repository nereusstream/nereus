...
> restart/BookKeeper gate passed；F4-M1–M5 are final-gated；F4-M6 checkpoints BD–BI cover 32-ref merge、
...
> two-broker/two-worker compressed-read convergence，while aggregate M6 remains pending
...
Pulsar retention rollout but does not claim M6 scale/failure/compatibility certification. M6 checkpoints BD–BI are
focused foundation evidence only；BI closes the two-worker/two-broker composition row, while 52-scenario mapping and
aggregate gates remain open. F4-M6 的确切文件、测试、
