...
F4-M4 的第二个 checkpoint 随后已实现 protected generation-zero append：exact intent prepare 不改 head，
commit-owned permanent `REACHABLE_APPEND` 先于 head CAS，exact generation-zero index-owned permanent
`VISIBLE_GENERATION` 先于 strict success，普通 append 与 recovery 共用同一序列；production shared-Oxia
runtime 也已接入独立 physical store/protection manager。下一步从 recovery-root publication 开始，随后是
anchor-aware replay/repair、retirement 与 physical/cursor GC，不重新打开协议选型。

F4-M0 只是 design gate；F4-M1–M3 final gates 和 M4 checkpoints A–B 也不声称 physical GC、
