...
F4-M4 随后的 checkpoints B–R 已继续实现 protected generation-zero append、recovery-root publication、
...
eligibility、grace-fenced higher-generation pre-drain/reproof。Global domains、production runtime composition、
cursor/root/audit completion 和 final M4 gate 仍待完成；production deletion 保持关闭。

F4-M0 只是 design gate；F4-M1–M3 final gates 和 M4 through checkpoint R 也不声称 production physical GC、
