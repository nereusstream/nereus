...
> restart/BookKeeper gate passed；F4-M1–M5 are final-gated；F4-M6 checkpoints BD–BK cover 32-ref merge、
...
> two-broker/two-worker compressed-read convergence、protected-intent retirement、partitioned admin compatibility and
...
Pulsar retention rollout but does not claim M6 scale/failure/compatibility certification. M6 checkpoints BD–BK are
focused/evidence-green；BI closes the two-worker/two-broker composition row, BJ closes abandoned protected-intent
retirement, and BK closes partitioned admin compatibility plus 52/52 executable traceability. The complete aggregate
