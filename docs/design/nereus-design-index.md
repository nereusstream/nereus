...
| `nereus-managed-ledger` | `Implemented`（F2-M1-M4 + F3-M1-M6；F4 T/W + M5 checkpoints X/Z/AA） | F2 ledger facade/cursor boundary plus F3 state machines、F4 projection/cursor reference domains、strict NPR1 authority、checkpoint-X exact durable registration coordinator/open-return barrier、checkpoint-Z immutable unloaded projection candidate and checkpoint-AA product-owned durable registration proof coordinator are implemented/tested |
| `nereus-pulsar-adapter` | `Implemented`（F2 complete + F3 complete；F4 M5 checkpoint X/AA wiring + fork checkpoints Y/Z/AA） | typed runtime/S3 provider plus fork binding/admission/capability/policy/admin compatibility、checkpoint-X shared generation-store/registration ownership、checkpoint-Y generation capability/readiness barrier、checkpoint-Z canonical bounded cold-topic traversal/report and checkpoint-AA exact readiness/proof handoff are implemented/tested；activation guard remains pending |
...
`master@6914bce939550a2d4929c7920b8cb9ed7cea5857`。M6 covers exact ordinary/middle-batch MessageIds across history/
...
4. 使用本地 Pulsar `master@6914bce939550a2d4929c7920b8cb9ed7cea5857` 重新验证当前 implementation/source lock；M0 历史 API/blob audit 仍固定在 `7efae25af39a15407c1397d9e1f4ac4658d09daa`，M4 历史证据固定在 `12edc9381c147ceec8bedd530acb5be7db339707`，M5 历史证据固定在 `a2bad4cfa260cc4575ae759f8a345ce969c8ec3a`；
...
5. 重新审计时使用本地 Pulsar
   `master@6914bce939550a2d4929c7920b8cb9ed7cea5857`，不把未发布的 Maven snapshot 当作权威源；
