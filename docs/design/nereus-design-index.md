...
> 下一项底层交付 F1-BK 已形成 `docs/phase-bk-bookkeeper-primary-wal/` 代码级 target；BK-M1–M6 尚未实现，
...
| BookKeeper primary WAL | `Designed / Reserved`（F1-BK code-level target） | `../phase-bk-bookkeeper-primary-wal/` freezes provider-neutral seam、ledger lifecycle/fencing/retention、BK_ONLY/async/sync and rollout；writer/reader/runtime remain absent and profiles are rejected before IO |
