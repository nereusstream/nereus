...
5. **F9 native Kafka code-level target + in-progress evidence**：`docs/phase-9-kafka-native-storage/` 的代码级合同；
   ...
   first-activation Oxia transport reset/retry 已通过；未闭合的 checkpoint/virtual-segment、initial empty-cluster
   snapshot/proof and capability-aggregation process cuts、coordinator 和 aggregate rows
