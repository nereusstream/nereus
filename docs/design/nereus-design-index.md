...
> metadata/keyspace/codec checkpoint；BK-M3–M6 尚未实现，
...
| BookKeeper primary WAL | `BK-M2 implementation in progress`（BK-M0/BK-M1 complete） | independent module、generic read/append/accounting/commit/protection/gen0、BookKeeper 4.18 client/config/namespace/NBKR1/resource foundation plus BK keyspace/seven V1 records/codecs implemented；stores/writer/reader/lifecycle/retention runtime remains incomplete and profiles are rejected before IO |
...
| `../phase-bk-bookkeeper-primary-wal/README.md` | F1-BK writer/reader/ledger lifecycle/retention/profile rollout code-level target | BK-M1 complete/final-gated；BK-M2 metadata/keyspace/codec implementation in progress；BK-M3–M6 not implemented |
