...
5. **F9 native Kafka code-level target + in-progress evidence**：`docs/phase-9-kafka-native-storage/` 的代码级合同；
   ...
   graceful cold-restart、Object/BookKeeper multi-broker takeover 与 ACTIVE-state three-voter controller-failover gates
   已通过；未闭合的 checkpoint/virtual-segment、PREPARED/response-loss activation、coordinator 和 aggregate rows
