...
5. **F9 native Kafka code-level target + in-progress evidence**：`docs/phase-9-kafka-native-storage/` 的代码级合同；
   ...
   graceful cold-restart gate 已通过；未闭合的 live takeover、ongoing/aborted transaction、multi-controller 和
   aggregate rows 仍不得描述为 production capability；
