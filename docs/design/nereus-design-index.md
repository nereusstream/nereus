...
| `nereus-materialization` | `Implemented`（F4-M1–M3 final-gated；F4-M4 through checkpoint T） | M1–M3 planner/worker/publication/topic engine plus M4 NRC1/recovery、root/journal fences、typed source retirement、completed-trim/COMMITTED/TOPIC_COMPACTED eligibility、activation-gated global scope/future sentinel and ownerless generation/append/materialization domains；production GC composition remains disabled |
...
| `../phase-4-compaction-generation/README.md` | F4 API/metadata/object/state-machine/rollout/implementation target contract | in progress / F4-M1–M3 final-gated；M4 through checkpoint T，remainder of M4–M6 remains |
...
| `nereus-future4-compaction-generation.md` | F4 compaction/materialization/generation | `In progress / F4-M1–M3 final-gated；M4 through checkpoint T`；精确合同见 `../phase-4-compaction-generation/` |
