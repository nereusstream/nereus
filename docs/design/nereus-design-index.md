...
> recovery。BK-M5 已 complete/final-gated：production runtime、first-create activation/capability、显式 namespace/
...
5. **F9 native Kafka code-level target + in-progress evidence**：`docs/phase-9-kafka-native-storage/` 的代码级合同；
   ...
   before-provider/after-provider readiness-create/PREPARED-create/ACTIVE-CAS store-publication gates 已通过；未闭合的
   checkpoint/virtual-segment、initial empty-cluster snapshot/proof and capability aggregation、actual transport-error、
   coordinator 和 aggregate rows
...
2. 以 `../phase-9-kafka-native-storage/README.md` 为入口，按 `01` 到 `08` 阅读代码级合同，再读 `09` 的
   dated F9-M0 review evidence；
