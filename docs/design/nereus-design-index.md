...
> 已于 2026-07-19 complete/final-gated；BK-M2 `BOOKKEEPER_WAL_ONLY` implementation 已开始并完成
> metadata/store/scanner checkpoint；BK-M3–M6 尚未实现，
> BookKeeper primary profiles 仍为 pre-IO rejected。
...
| BookKeeper primary WAL | `BK-M2 implementation in progress`（BK-M0/BK-M1 complete） | independent module、generic read/append/accounting/commit/protection/gen0、BookKeeper 4.18 client/config/namespace/NBKR1/resource foundation plus BK keyspace/seven V1 records/codecs、production/fake focused stores、response-loss CAS/delete recovery and bounded 256/16-shard scans implemented；physical writer/reader/lifecycle/retention runtime remains incomplete and profiles are rejected before IO |
| Async object materialization | `Implemented / final-gated`（F4-M5 profile + F4-M6 aggregate） | Object-WAL `WAL_DURABLE` boundary、generation-zero repair、pre-I/O proof/lag admission、coupled production read-repair/materialization runtime and exact Pulsar logical-retention rollout pass the retry-disabled real two-broker and Phase 4 aggregate gates；BookKeeper primary profiles remain reserved |
...
| `../phase-bk-bookkeeper-primary-wal/README.md` | F1-BK writer/reader/ledger lifecycle/retention/profile rollout code-level target | BK-M1 complete/final-gated；BK-M2 metadata/store/scanner checkpoint implemented；BK-M3–M6 not implemented |
...
5. 重新审计时使用本地 Pulsar
   `master@eaf7b9a704890a9265c21f30d9f351e02d00c600`，不把未发布的 Maven snapshot 当作权威源；checkpoint BQ
