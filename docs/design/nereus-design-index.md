...
| `nereus-metadata-oxia` | `Implemented`（P15/F2/F3 + F4-M1–M5 final-gated；M6 BD–BG） | existing metadata plus F4 keys/records/codecs/CAS/scans/all-shard fixtures、generation activation/registration、dual-reader task schema V2、1,000-reference pagination evidence and explicit versioned/semantic `StreamMetadataSnapshot` authority comparisons；hydrated trim observation fields are never correctness authority |
...
| `nereus-materialization` | `Implemented`（F4-M1–M5 final-gated；M6 BD–BG） | M1–M3 planner/worker/publication/topic engine、M4 NRC1/recovery/retirement/physical+cursor GC/scale/failure matrix、M5 coupled async read-repair/materialization/lag runtime and M6 32-ref/4,096-candidate/million-entry/128-source task boundaries are implemented；the current broker safe defaults keep scheduling/deletion disabled |
...
| Future 4 materialization/compaction | `In progress / F4-M1–M5 final-gated；M6 BD–BG focused-green` | M4 NRC1/recovery、retirement/GC fences、activation/global domains、scale/failure/late-PUT cuts and retry-disabled real source-deletion acceptance are implemented/tested；M5 final-gates async/retention compatibility；M6 now covers 32-ref merge、4,096/4,097 candidates、million-entry NRC1、1,000+1,000 references and the 128-source/1,048,576-record task with schema V2/capability V2；remaining registry/two-worker/two-broker/scenario mapping and aggregate gates are open |
...
| `../phase-4-compaction-generation/README.md` | F4 API/metadata/object/state-machine/rollout/implementation target contract | in progress / F4-M1–M5 final-gated；M6 BD–BG focused-green |
...
| `nereus-future4-compaction-generation.md` | F4 compaction/materialization/generation | `In progress / F4-M1–M5 final-gated；M6 BD–BG focused-green`；精确合同见 `../phase-4-compaction-generation/` |
