...
5. **F9 native Kafka code-level target + in-progress evidence**：`docs/phase-9-kafka-native-storage/` 的代码级合同；
   ...
   before-provider/after-provider PREPARED/ACTIVE store-publication gates 已通过；未闭合的 checkpoint/virtual-segment、
   initial-proof/readiness、actual transport-error、coordinator 和 aggregate rows
