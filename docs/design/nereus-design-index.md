...
| `nereus-metadata-oxia` | `Implemented`（P15/F2/F3 + F4-M1–M5 final-gated） | existing metadata plus F4 keys/records/codecs/CAS/scans/all-shard fixtures、generation activation/registration and explicit versioned/semantic `StreamMetadataSnapshot` authority comparisons；hydrated trim observation fields are never correctness authority |
...
| `nereus-materialization` | `Implemented`（F4-M1–M5 final-gated） | M1–M3 planner/worker/publication/topic engine、M4 NRC1/recovery/retirement/physical+cursor GC/scale/failure matrix and M5 coupled async read-repair/materialization/lag runtime are implemented；the current broker safe defaults keep scheduling/deletion disabled |
...
| Future 4 materialization/compaction | `In progress / F4-M1–M5 final-gated；M6 pending` | M4 NRC1/recovery、retirement/GC fences、activation/global domains、scale/failure/late-PUT cuts and retry-disabled real source-deletion acceptance are implemented/tested；M5 adds exact durable registration/readiness/activation、protected async Object-WAL acknowledgement、pre-I/O lag admission、coupled production runtime、versioned exact-evidence retention planning、durable backlog eviction、ownership-safe F3 logical trim and exact Pulsar policy/admin admission，then final-gates ordinary/compressed-batch MessageIds、owner failover/rejoin、unloaded trim、post-trim IO and BookKeeper coexistence；only M6/aggregate completion remains |
...
| `../phase-4-compaction-generation/README.md` | F4 API/metadata/object/state-machine/rollout/implementation target contract | in progress / F4-M1–M5 final-gated；M6 remains |
...
| `nereus-future4-compaction-generation.md` | F4 compaction/materialization/generation | `In progress / F4-M1–M5 final-gated；M6 pending`；精确合同见 `../phase-4-compaction-generation/` |
...
4. F3-M6 历史验收使用本地 Pulsar `master@ff6e4fdfc03ffd8535ab2ece58d247dd1c64e8b4`；当前 Phase 4 implementation/source lock 已推进到 `master@0e9829a7453497910ab468669e644e88b4bc2f93`。M0 历史 API/blob audit 仍固定在 `7efae25af39a15407c1397d9e1f4ac4658d09daa`，M4 历史证据固定在 `12edc9381c147ceec8bedd530acb5be7db339707`，M5 历史证据固定在 `a2bad4cfa260cc4575ae759f8a345ce969c8ec3a`；
...
5. 重新审计时使用本地 Pulsar
   `master@0e9829a7453497910ab468669e644e88b4bc2f93`，不把未发布的 Maven snapshot 当作权威源；
