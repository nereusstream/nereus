...
| `nereus-materialization` | `Implemented`（F4-M1–M3 final-gated；F4-M4 through checkpoint Q） | M1–M3 planner/worker/publication/topic engine plus M4 NRC1/recovery、root/journal fences、typed source retirement and COMMITTED-view whole-range higher pre-drain；production GC composition remains disabled |
...
| Future 4 materialization/compaction | `In progress / F4-M1–M3 final-gated；M4 through checkpoint Q` | M4 NRC1/recovery、root-authenticated journal/destructive skeleton、typed generation-zero source retirement and COMMITTED-view higher pre-drain are implemented/tested；remaining M4 production/global/trim/view paths and M5–M6 remain |
...
| `../phase-4-compaction-generation/README.md` | F4 API/metadata/object/state-machine/rollout/implementation target contract | in progress / F4-M1–M3 final-gated；M4 through checkpoint Q，remainder of M4–M6 remains |
...
| `nereus-future4-compaction-generation.md` | F4 compaction/materialization/generation | `In progress / F4-M1–M3 final-gated；M4 through checkpoint Q`；精确合同见 `../phase-4-compaction-generation/` |
