...
> 已于 2026-07-19 complete/final-gated；BK-M2 `BOOKKEEPER_WAL_ONLY` 是下一里程碑，BK-M2–M6 尚未实现，
...
| BookKeeper primary WAL | `BK-M1 complete/final-gated`（BK-M0/BK-M1） | independent module、generic read/append/accounting、generic commit/protection/gen0、BookKeeper 4.18 client/config/namespace/NBKR1/resource foundation implemented；BK-M2 writer/reader/lifecycle/retention runtime remains incomplete and profiles are rejected before IO |
...
| `../phase-bk-bookkeeper-primary-wal/README.md` | F1-BK writer/reader/ledger lifecycle/retention/profile rollout code-level target | BK-M1 complete/final-gated；BK-M2 `BOOKKEEPER_WAL_ONLY` next；BK-M2–M6 not implemented |
