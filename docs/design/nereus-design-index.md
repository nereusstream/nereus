...
> real Oxia + BookKeeper restart/delete checkpoint，remaining matrix/aggregate pending；BK-M3–M6 尚未实现，
...
| BookKeeper primary WAL | `BK-M2 implementation in progress`（BK-M0/BK-M1 complete） | independent module、generic read/append/accounting/commit/protection/gen0、BookKeeper 4.18 exact allocator/writer/recovery/reader、production/fake Oxia stores、whole-ledger retention and module/facade BK_ONLY execution implemented；real Oxia + BookKeeper rollover/cold-restart/lost-delete-response/fresh-process dual-absence checkpoint passes，remaining matrix/aggregate pending；production broker admission remains BK-M5 fail-closed |
...
| `../phase-bk-bookkeeper-primary-wal/README.md` | F1-BK writer/reader/ledger lifecycle/retention/profile rollout code-level target | BK-M1 complete/final-gated；BK-M2 through real Oxia/BookKeeper restart/delete checkpoint；BK-M3–M6 not implemented |
