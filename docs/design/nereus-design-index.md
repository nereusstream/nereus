...
5. **F9 native Kafka code-level target + in-progress evidence**：`docs/phase-9-kafka-native-storage/` 的代码级合同；
   ...
   provider-applied PREPARED/ACTIVE publication-cut gates 已通过；未闭合的 checkpoint/virtual-segment、
   pre-publication proof/pre-CAS process activation、coordinator 和 aggregate rows
...
| `nereus-kafka-adapter` | `In progress`（F9-M1/M2 + substantial M3–M6 slices） | ranged API、binding/checkpoint/recovery、raw batch codec、partition runtime、activation、retention/compaction composition and real single-node restart、multi-broker Object/BookKeeper takeover、ACTIVE-state three-voter controller failover plus provider-applied PREPARED/ACTIVE response-loss cuts are implemented/tested；checkpoint/virtual-segment、pre-publication proof/pre-CAS process activation、coordinator and aggregate gates remain open |
