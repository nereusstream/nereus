...
5. **F9 native Kafka code-level target + in-progress evidence**：`docs/phase-9-kafka-native-storage/` 的代码级合同；
   ...
   graceful cold-restart 与 forced open-transaction abort-recovery gate 已通过；未闭合的 multi-broker/
   checkpoint/virtual-segment、provider-profile、multi-controller 和 aggregate rows 仍不得描述为 production capability；
