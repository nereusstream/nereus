...
> implemented，F4 milestone 3 topic execution engine/worker/gates remainder and
...
| `nereus-materialization` | `Implemented`（F4-M1–M2 final-gated；F4-M3 in progress） | strict domain/publication and format verification plus deterministic policy/planner、exact-revalidated task store、claim/publication recovery、all-64-shard registered-stream scanner、stream-scoped exact-source reader、claim-to-output-ready worker、task-protection crash-cut reconciliation、monotonic checkpoint reconciliation、bounded dispatcher/service lifecycle、topic-compaction neutral SPI/registry and terminal workflow-metadata retirement；Pulsar Entry/NCP1 exact-byte evidence passes，while topic execution engine/worker and M3 gates remain |
