...
| `nereus-materialization` | `Implemented`（F4-M1–M2 final-gated；F4-M3 in progress） | strict domain/publication and format verification plus deterministic policy/planner、exact-revalidated task store、claim/publication recovery、all-64-shard registered-stream scanner、stream-scoped exact-source reader and claim-to-output-ready worker；protection crash-cut reconciliation/service/gates remain |
...
| Future 4 materialization/compaction | `In progress / F4-M1–M2 final-gated；M3 checkpoints` | M1/M2 gates passed；M3 format/read、deterministic policy/planner/task/recovery/registry-scan and exact-source claim-to-output-ready worker checkpoints are implemented/tested, while protection crash-cut reconciliation、Pulsar opaque round trip、service/M3 gates and M4–M6 remain |
