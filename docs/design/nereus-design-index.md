...
> 状态：当前设计索引
> 最近一次架构重整：2026-07-10
> 当前实现阶段：Future 1 / Phase 1，M0-M3 与 pre-M4 hardening gate 已完成；M4 Core Append 待实现
...
| `nereus-metadata-oxia` | `Implemented`（fake/codec contract） | keyspace、records、codec、shared manifest validator、append outcome、bounded repair cursor、fake store；M7 real adapter pending |
| `nereus-object-store` | `Implemented`（M3） | object-store API、WAL v1 writer/reader、entry index、local test fixture、checksums/tests |
| `nereus-core` | `In progress` | module/dependencies exist；M4 append、M5 read、M6 trim/recovery 尚未实现；M7 closes real Oxia integration |
...
Therefore the M4 entry gate is open。The Docker-backed spike conclusion and M7 real-adapter requirement
remain unchanged。
...
9. Final Phase 1 exit includes M7 production Oxia adapter contract and integration gates；M6 fake/local
   scenario alone is not complete Oxia-backed delivery。
