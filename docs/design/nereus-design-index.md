...
> 当前交付阶段：Future 2、Future 3 与 Future 4 F4-M1 complete/final-gated；Future 4 M2–M6 未实现
...
3. **当前 Phase 4 代码级合同**：`docs/phase-4-compaction-generation/` 的 F4-M0 target、已 final-gated 的
   F4-M1 implementation checkpoint 与 M2–M6 target；它优先于 F4 north-star 摘要，但已实现部分仍以
   代码/测试为最高权威；
...
| 模块/能力 | 状态 | 当前事实 |
| --- | --- | --- |
| `nereus-api` | `Implemented`（P15-M1/M4/M6 + F4-M1） | generic target/result、exact cumulative append snapshot、append recovery/lifecycle API plus F4 view/generation/publication/object-hash values |
| `nereus-metadata-oxia` | `Implemented`（P15/F2/F3 + F4-M1 final-gated） | existing metadata plus F4 keys、records、43-vector codec golden、generation/physical stores、closed CAS guards、conditional delete、Oxia slash-aware fixed-depth scans、all-shard fixtures and shared physical-root transition validation；real-service final gate passed |
| `nereus-object-store` | `Implemented`（M3 + F4-M1 final-gated） | WAL v1 IO plus replayable private staging、guarded retry PUT、SDK-response/exact-byte completion、bounded logical-prefix LIST、HEAD-before-DELETE、405/501 conditional-delete compatibility fallback and safe local fixture |
| `nereus-core` | `Implemented`（P15 + F4-M1 final-gated） | stable L0 core plus F4 physical/reference proof values、activation proof contract、durable reader-pin and protection handshakes；not yet integrated into higher-generation reads/GC |
...
| Future 4 materialization/compaction | `In progress / F4-M1 final-gated` | API/metadata/codecs/object IO、metadata store contracts/CAS guards、M1 Gradle audits and core reader/protection primitives passed ordinary and Docker-backed real-service gates；M2-M6 execution paths remain |
...
| `../phase-4-compaction-generation/README.md` | F4 API/metadata/object/state-machine/rollout/implementation target contract | in progress / F4-M1 final-gated |
...
| `nereus-future4-compaction-generation.md` | F4 compaction/materialization/generation | `In progress / F4-M1 final-gated`；精确合同见 `../phase-4-compaction-generation/` |
