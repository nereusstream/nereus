...
> M5 checkpoint X closes the durable registration create/open frontier；the remainder of milestones 4–6 is pending
...
| `nereus-pulsar-adapter` | `Implemented`（F2 complete + F3 complete；F4 M5 checkpoint X wiring） | typed runtime/S3 provider plus fork binding/admission/capability/policy/admin compatibility and checkpoint-X shared generation-store/registration-coordinator production ownership are implemented/tested；generation broker capability/backfill remains pending |
...
| Future 4 materialization/compaction | `In progress / F4-M1–M3 final-gated；M4 through W；M5 through X` | M4 NRC1/recovery、retirement/GC fences、activation/global domains、cursor protection and physical/cursor live-reference backfill are implemented/tested；M5 checkpoint X adds exact durable registration create/refresh/final revalidation、open/recreate return barrier and production ownership；broker capability/cold-topic backfill、activation guard、remaining M4 GC runtime and M5–M6 remain |
...
| `../phase-4-compaction-generation/README.md` | F4 API/metadata/object/state-machine/rollout/implementation target contract | in progress / F4-M1–M3 final-gated；M4 through W；M5 through X，remainder of M4–M6 remains |
...
| `nereus-future4-compaction-generation.md` | F4 compaction/materialization/generation | `In progress / F4-M1–M3 final-gated；M4 through W；M5 through X`；精确合同见 `../phase-4-compaction-generation/` |
