...
| `BOOKKEEPER_WAL_ONLY` | BookKeeper | disabled | `WAL_DURABLE` | BK-M2 module/facade runtime and real Oxia/BookKeeper restart/delete evidence complete/final-gated；BK-M5 production rollout final-gated behind exact activation/capability admission |
| `BOOKKEEPER_WAL_SYNC_OBJECT` | BookKeeper | higher Object generation required before producer completion | profile durability + `REQUIRED_OBJECT_GENERATION` completion | BK-M4 module-local profile and BK-M5 production mixed-profile rollout complete/final-gated |
...
| F9 native Kafka projection | `Kafka client -> KRaft Kafka fork -> nereus-kafka-adapter -> Nereus`；F9-M1/M2 and substantial M3–M6 slices implemented，including explicit CLI/log selection、activation-backed Object-WAL runtime、real single-node recovery、multi-broker Object/BookKeeper takeover、ACTIVE-state three-voter controller failover、the complete before-provider/after-provider readiness-create/PREPARED-create/ACTIVE-CAS store-publication matrix and actual first-activation Oxia transport reset/retry；checkpoint/virtual-segment、initial empty-cluster snapshot/proof and capability-aggregation process cuts、coordinator and aggregate gates remain open |
...
| Group offset | 不等于 Pulsar cursor。F5 依其 KoP contract；F9 以 native Kafka compacted internal topic 为 coordinator truth，不创建第二套 F9 Oxia coordinator log |
| Leader | F5 是 routing projection；F9 的 KRaft leader epoch 通过 append authority fence 写入，但 KRaft 仍是 protocol leadership truth |
| Mandatory compaction coverage | F9 binding 中不可逆的 coverage/view fact；一旦某范围激活 NTC2，读取只能在同一 topic-compacted view 内 fallback，不能回退到会复活已 compacted records 的 COMMITTED view |
...
| L2a KoP Kafka Projection | F5 KoP produce/fetch/group/txn projection through Pulsar |
| L2b Native Kafka Projection | F9 fork log、binding、checkpoint、producer/txn/internal-topic recovery and KRaft authority projection |
