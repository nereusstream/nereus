...
| `AppendAttemptId` | Phase 1.5 设计的 process-local opaque handle；只用于恢复 exact retained physical attempt，不是 durable producer dedup key |
...
| Read target | BookKeeper range、object WAL slice 或 compacted object range；Phase 1.5 tagged abstraction 已设计，生产代码仍是 Phase 1 object-shaped boundary |
...
- **Phase 1.5 / P15-M0-M5**：F1 与 F2 production 之间的 L0 foundation delivery；P15-M0 是已完成设计，
  P15-M1-M5 尚未实现，不是新的 Future 编号。
