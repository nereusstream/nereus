...
| `BOOKKEEPER_WAL_ONLY` | BookKeeper | disabled | `WAL_DURABLE` | BK-M1 foundation complete；BK-M2 profile not implemented |
...
- **F1-BK / BookKeeper Primary WAL Delivery**：Future 1 的后续扩展，不是 Future 5；BK-M0–M6 的代码级
  ...
  通过，BK-M1 foundation 已 complete/final-gated，BK-M2–M6 仍未实现。
