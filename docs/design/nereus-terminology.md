...
| F9 native Kafka projection | `Kafka client -> KRaft Kafka fork -> nereus-kafka-adapter -> Nereus`；F9-M1/M2 and substantial M3–M6 slices implemented，including explicit CLI/log selection、activation-backed Object-WAL runtime、real single-node recovery、multi-broker Object/BookKeeper takeover、ACTIVE-state three-voter controller failover and provider-applied PREPARED/ACTIVE publication cuts；checkpoint/virtual-segment、remaining pre-publication proof/pre-CAS process activation、coordinator and aggregate gates remain open |
