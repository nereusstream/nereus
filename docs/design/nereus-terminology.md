...
| F9 native Kafka projection | `Kafka client -> KRaft Kafka fork -> nereus-kafka-adapter -> Nereus`；F9-M1/M2 and substantial M3–M6 slices implemented，including explicit CLI/log selection、activation-backed Object-WAL runtime and real single-node KRaft/Oxia/S3 user/group/committed-transaction cold restart；live takeover、ongoing/aborted transaction、multi-controller and aggregate gates remain open |
