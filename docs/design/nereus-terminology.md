...
| F9 native Kafka projection | `Kafka client -> KRaft Kafka fork -> nereus-kafka-adapter -> Nereus`；F9-M1/M2 and partial M3/M6 implemented，including deferred activation-backed Object-WAL runtime composition；concrete recovery、CLI/log selection and native KRaft E2E are not yet available |
