...
| F4 Materialization/Compaction | Phase 4 F4-M0-M6 | In progress / F4-M1–M3 final-gated；M4 through W implements recovery/retirement/GC fences、activation/global domains、cursor protection and physical/cursor live-reference backfill；M5 through Y adds exact durable registration create/open frontier plus deterministic generation broker readiness；production deletion remains disabled | Complete broker cold-topic registration backfill/proof、activation guard、cursor snapshot candidate/deletion scanning、object inventory、registration retirement、remaining runtime composition and final M4–M6 gates |
