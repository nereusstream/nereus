...
| F4 Materialization/Compaction | Phase 4 F4-M0-M6 | In progress / F4-M1–M3 final-gated；M4 through W implements recovery/retirement/GC fences、activation/global domains、cursor protection and physical/cursor live-reference backfill；M5 through AH adds durable registration/readiness/activation、protected async Object-WAL acknowledgement、pre-I/O lag admission、the coupled production read-repair/materialization runtime、stable exact-evidence retention planning/F3 logical-trim delegation、shared bounded retention execution and production ledger/config installation；production deletion remains disabled | Complete exact Pulsar retention-policy/admin activation、cursor snapshot candidate/deletion scanning、object inventory、registration retirement、physical GC composition and final M4–M6 gates |
