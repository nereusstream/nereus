...
| F4 Materialization/Compaction | Phase 4 F4-M0-M6 | In progress / F4-M1–M3 final-gated；M4 through W implements recovery/retirement/GC fences、activation/global domains、cursor protection and physical/cursor live-reference backfill；M5 through AC adds exact durable registration create/open frontier、deterministic generation broker readiness、canonical bounded cold-topic traversal/report、product-owned durable registration proof CAS、exact activation proof/revalidation with a default-off first-marker switch and proof-gated publication-only cluster ACTIVE orchestration；production deletion remains disabled | Complete mutation call-site admission、cursor snapshot candidate/deletion scanning、object inventory、registration retirement、remaining runtime composition and final M4–M6 gates |
