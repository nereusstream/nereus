...
| F4 Materialization/Compaction | Phase 4 F4-M0-M6 | In progress / F4-M1–M2 final-gated；M3 format + planner/recovery + exact-source worker + protection/checkpoint reconciliation + bounded service lifecycle checkpoints landed | Complete F4-M3 Pulsar opaque round trip and gates |
...
reconciliation and bounded service lifecycle checkpoints are also implemented, but Pulsar evidence/gates、GC and the
async path do not exist yet
