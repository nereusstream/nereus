...
| --- | --- | --- | --- |
| F1 Core Stream Storage | Phase 1 M0-M8 + Phase 1.5 P15-M0-M6 + F1-BK BK-M0-M6 | Phase 1/1.5 implemented/final-gated；F1-BK BK-M0/BK-M1 complete/final-gated；BK-M2 through real Oxia/BookKeeper restart/delete checkpoint；BK-M3 focused implementation through real publication/read/physical-delete、fresh-runtime response loss、lag admission and unreadable-output fail-closed evidence；BK-M4–M6 not implemented | close remaining BK-M2 predecessor aggregate, then register the dependent BK-M3 final gate |
...
`OBJECT_WAL_ASYNC_OBJECT` without changing that legacy boundary；BK_ONLY is executable only in the independently gated
module/facade composition while production broker admission remains reserved until BK-M5。The exact F1-BK target is
