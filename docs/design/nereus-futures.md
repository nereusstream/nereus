...
| F4 Materialization/Compaction | Phase 4 F4-M0-M6 | In progress / F4-M1–M3 final-gated；M4 through W implements recovery/retirement/GC fences、activation/global domains、cursor protection and physical/cursor live-reference backfill；M5 through AG adds durable registration/readiness/activation、protected async Object-WAL acknowledgement、pre-I/O lag admission、the coupled production read-repair/materialization runtime with exact Pulsar profile/config mapping，以及 stable exact-evidence retention planning/F3 logical-trim delegation；production deletion remains disabled | Complete Pulsar retention/admin mapping and production service installation、shared retention lane、cursor snapshot candidate/deletion scanning、object inventory、registration retirement、physical GC composition and final M4–M6 gates |
