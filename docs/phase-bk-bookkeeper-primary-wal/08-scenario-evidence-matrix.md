...
| BK-61 | M3 | B/O/S/C | worker restart re-reads exact BK range and reuses task/output intent | First graceful fresh-runtime O/B/S reconstruction: `BookKeeperAsyncObjectOxiaBkS3IntegrationTest.stableHeadFallsBackToBookKeeperThenFreshRuntimePublishesAndReadsExactObject`; task/output response-loss cuts remain open |
