...
BK-M5 Pulsar rollout            complete/final-gated on 2026-07-22
BK-M6 aggregate final gate      complete/final-gated on 2026-07-22
...
`master@2f9c1eb93be96e2036fbdc8c5e39545f21fa6200`；it retains the BK-M1 historical evidence above and adds the focused
...
blind provider delete。This is BK-55 D evidence；BK-M6 subsequently closes the process-loss suffix with the same
durable response-loss cut reloaded through a fresh runtime that owns no local workflow state。
...
This closes the real O/B portions of BK-28 and BK-29；BK-M6 closes the process-loss suffix with durable cut injection
followed by a fresh runtime that owns no process-local handles or futures。
...
BK-50/BK-51；BK-M6 subsequently closes the writer/process-loss variants through the bounded inventory and
fresh-runtime chaos gates。
...
levels；BK-M6 subsequently closes the broader process-loss matrix at the protocol boundary with durable cuts and
fresh runtimes。
...
lag/failure chain。The abrupt-process/chaos boundary was assigned to BK-M6 rather than hidden in M3 and is now closed
by its durable-cut/fresh-runtime and real broker/worker contention evidence。
...
The M4-specific completion cuts are closed here；the exhaustive process-loss/chaos matrix remained explicitly owned
by BK-M6 and is now closed by that milestone rather than misreported as M4 evidence。
...
Implementation checkpoint E.1 (2026-07-20) closes proof-owned physical-deletion activation：

- `NBKROOT1` traverses all 256 root shards from empty continuations and validates canonical roots、exact reconstructed
  provider metadata、fixed/dynamic protection owners and reader leases before hashing durable key/version/value
  identities；
- `NBKSTREAM1` traverses all 64 materialization registration shards and requires every registered BK profile to agree
  with canonical registration、L0 stream/profile/state/frontier、F2 binding/current topic and `NPR1` projection
  identity；
- `NBKSCOPE1` reserves a permanent QUARANTINED audit root before provider IO，then proves exact advanced-id
  create/write/non-recovery-read/recovery-fence/delete/two-absence behavior。Applied create/delete response loss is
  recovered only from exact `NBKL1` metadata/absence；foreign or unknown outcomes never authorize delete；
- `BookKeeperDeletionActivationCoordinator` repeatedly revalidates one strongest-profile broker readiness，requires
  all proofs to bind that epoch/SHA，runs the provider canary before root coverage so its permanent audit root is part
  of the scanned inventory，revalidates the namespace and installs all three producer-owned digests plus the deletion
  bit in one activation CAS。CAS response loss reloads the exact tuple，and an already-active retry returns without
  rerunning canary IO；
...
Checkpoint E.4 closes the remaining M5 surface：namespace-level durable-profile ownership filtering removes
incapable candidates and fails closed on conflicting exact signatures；an old broker redirects to a capable owner but
cannot publish writable ownership；readiness-set changes regenerate and CAS-rebind every deletion proof without
changing publication identity；and the mixed-profile two-broker fixture preserves immutable BK_SYNC、BK_ASYNC and
Object-WAL topic profiles、history and MessageIds across cold load and both ownership takeovers。
...
Checkstyle plus the retry-disabled exact E.3 real fixture。Checkpoint E.4 registers `bookKeeperPrimaryWalM5Check`
over ownership-filter、capability-rollover and mixed-profile suites，and registers `bookKeeperPrimaryWalM5FinalCheck`
over that ordinary gate plus the complete BK-M4 final predecessor chain。
...
The checkpoint-E.4 `bookKeeperPrimaryWalM5Check --rerun-tasks` passes 105/105 outer tasks in 6m47s；its final locked
Pulsar leg passes 138/138 fresh tasks with retry count zero。`bookKeeperPrimaryWalM5FinalCheck --rerun-tasks` then
passes 231/231 fresh tasks in 27m42s at
`master@2f9c1eb93be96e2036fbdc8c5e39545f21fa6200`，including Phase 1.5、Phase 2/3/4 final and BK-M1–M4 final
predecessors。BK-M5 is complete/final-gated；BK-M6 subsequently closes the F1-BK aggregate below。
...
### 9.4 Implemented checkpoints and final evidence

BK-M6 adds executable source sets rather than another runtime subsystem：

- `BookKeeperPrimaryWalScaleTest` proves hot/all-shard pagination，the exact 256 x 64 range/protection inventory，cold
  restart stability and stack-bounded 10,000-root visitation；
- `BookKeeperMixedSourceTaskScaleTest` round-trips a 128-source、1,048,576-record task with both tagged BK and Object
  targets；the existing generation resolver proves 4,096 candidates and 4,097 fail-closed；
- the allocator exercises all 32 permanent hazard slots and rejects the next create before provider IO；
- the real BK fixtures use two bookies so a correctly excluded failed channel can select a replacement ensemble
  instead of converting a transport fault into a single-node `BKNotEnoughBookiesException` artifact；
- Oxia two-segment child pagination now binds continuation validation to the stable base prefix，so page two cannot
  reinterpret the page-one last key as a new scan root；
- all Docker-owning real-service tasks and every nested local-Pulsar build share one Gradle build service，preventing
  an Oxia/BK fixture from overlapping a fork build that owns the same infrastructure；
- the retry-disabled Pulsar contention fixture selects topics from distinct namespace bundles before asserting one
  owner per broker，so bundle placement is a fixture fact rather than a sequential-name probability。

The executable manifest in `09-m6-executable-evidence-matrix.md` maps BK-87 through BK-96 to exact methods and owning
gates. “Process loss” means durable cut injection followed by a fresh runtime with no process-local handles；real
broker/process concurrency is separately exercised by the two-broker ownership and two-worker contention fixtures。

Fresh results on 2026-07-22 at
`master@2f9c1eb93be96e2036fbdc8c5e39545f21fa6200`：

- `bookKeeperPrimaryWalM6ScaleCheck --rerun-tasks`：35/35 outer tasks；
- `bookKeeperPrimaryWalM6ChaosCheck --rerun-tasks`：76/76 outer tasks；
- `bookKeeperPrimaryWalM6Check --rerun-tasks`：123/123 outer tasks in 10m22s；
- `bookKeeperPrimaryWalFinalCheck --rerun-tasks`：236/236 outer tasks in 30m57s，including Phase 1.5、Phase 2/3/4
