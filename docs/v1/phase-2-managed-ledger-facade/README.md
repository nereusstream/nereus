...
M3 facade 与 M4 broker integration 已完成普通 gate，M5 real recovery/retention/scale 已完成最终 gate；仅
M6 final compatibility/runtime 仍待实现。
...
```bash
NEREUS_F2_PROBE_DIR=/path/to/nereus/docs/v1/phase-2-managed-ledger-facade/spikes \
  ./gradlew \
  -I /path/to/nereus/docs/v1/phase-2-managed-ledger-facade/spikes/pulsar-api-probe.init.gradle \
