...
authority: HistoricalDeliveryLog
sourceTuple: v0.1@a14d925da5763f36208f8ddca7bef31f3eb90b0b
---

# Nereus V1 historical delivery log

This file preserves the former root README delivery history. Paths shown inside older entries describe the layout at
the time of those entries; use the [V1 archive index](README.md) for the current repository layout.
...
nereus/
  ...
  docs/v1/phase-1-core-stream-storage/     implemented Phase 1 contracts and milestones
  docs/v1/phase-1.5-core-storage-foundation/ implemented L0 evolution and gates
  docs/v1/phase-2-managed-ledger-facade/   F2 code-level contracts, API spike and milestones
  docs/v1/phase-3-cursor-subscription/      implemented/final-gated F3 code-level contract
  docs/v1/phase-4-compaction-generation/    F4 code-level contract and implementation gates
  docs/v1/phase-9-kafka-native-storage/     F9 native Kafka code-level target and in-progress M1-M6 evidence
  docs/v1/phase-bk-bookkeeper-primary-wal/  final-gated F1-BK BK-M0–M6 contract and executable evidence
  docs/v1/automq-like-stream-storage/       async materialization profile design
```
...
[`docs/v1/phase-9-kafka-native-storage/`](phase-9-kafka-native-storage/README.md). The SSH-published fork head is
...
[`docs/v1/phase-bk-bookkeeper-primary-wal/`](phase-bk-bookkeeper-primary-wal/README.md)：
...
执行证据。分区主题创建留下的零字节 ManagedLedger catalog placeholder 不再被误判为 BookKeeper 耐久状态；
跨 broker 分区管理校验只解释本地 loaded projection，远端或 unloaded 分区统一回到 durable storage binding。
...
Docker-backed final gates；F4-M4 已于 2026-07-19 完成并通过真实服务、规模/故障与 retry-disabled
two-broker final gate，F4-M5 又于同日通过完整 ordinary 与 retry-disabled async-retention two-broker final gate。
M1 已落地
...
新的
`docs/v1/phase-4-compaction-generation/` 冻结了 API/module ownership、`NCP1/NTC1/NRC1` formats、
...
F4-M1–M5 已 final-gated；M3 真实 Parquet writer/strict reader、whole-file verifier、NTC1 storage facade、
...
LocalStack 上通过；其余 MARKED/metadata/protection/root-CAS cuts、双 broker ownership/failover 和规模矩阵仍待完成。
...
points、双 broker ownership/failover 和规模矩阵仍待完成。
Checkpoint AV 进一步把两个独立 runtime 强制同步竞争同一 MARKED root。两个 worker 都到达真实
`MARKED -> DELETING` Oxia CAS，恰好一个原始 CAS 成功；失败方只在 exact reload 到同一 replacement 后继续。
随后两条恢复路径同时执行同一 immutable identity 的幂等 DELETE，最终观察到相同 durable DELETED root 和
缺失对象。该聚焦真实服务场景以 47/47 executed tasks 通过；`phase4M4TwoWorkerConvergenceCheck` 也已于
2026-07-18 在 Java 21、Docker 28.5.2 和 locked Pulsar
...
其两组串行 locked-Pulsar 验证为 141/141 和 138/138 executed。
Checkpoint AY 继续关闭 10,000 个 DELETED root 的 dual-window retirement 规模线：第一轮完整 256-shard 扫描
为每个 root 持久化 first-absence proof，第二轮在独立 orphan window 后逐个执行 Phase 1 references → manifest →
root-last conditional retirement，第三轮扫描为空。生产 materialization 与 S3 deadline scheduler 同时改为立即
移除 cancelled timer，避免高基数成功请求把远期 timeout 滞留在队列。聚焦 scheduler/scale suite 已于
2026-07-18 以 26/26 executed tasks 通过，三个受影响模块以 50/50 executed tasks 通过；
...
locked-Pulsar 验证为 141/141 和 138/138 executed）。
Checkpoint BB 继续关闭 late-PUT/tombstone 线：Object-WAL 的 guarded provider attempt 现在从
`AppendCoordinator` 一直传到 object store，每次 transmission 都执行 durable append-session 与
absent-or-exact-ACTIVE root 双重证明；确定性测试证明首个 transmission 后 root 进入 DELETED 时第二次发送为
零，旧 session 在 root tombstone 退休后也不会发送首包，只有新 epoch/fresh key 可继续。tombstone 测试又在
references 与 manifest 删除后注入 exact bytes，最终 proof 会删除 bytes、清空 observation 并保留 root。
真实四分片 Oxia + LocalStack 方法进一步在 root 已退休后外部重建同 key，证明 inventory 把它注册为新的
ACTIVE ownerless root、施加完整 grace，随后再 MARK/delete。聚焦 core/metadata/materialization 与真实服务
测试已于 2026-07-19 通过；完整 `phase4M4LatePutTombstoneCheck --rerun-tasks` 同日在 Java 21、Docker
28.5.2 与 locked Pulsar `c59da789e88df2b57829de3277c60194b44fceb6` 上以 3m56s 通过（159/159 tasks
executed），并作为该检查点的聚合 gate。At checkpoint BB，实际 Pulsar two-broker ownership/unload/failover
仍是 M4 final 前的剩余 destructive 证据；最终门禁已在 checkpoint BC 后关闭。
...
executed）。最终 `NereusPhysicalGcMultiBrokerIntegrationTest` 已在 shared real Oxia、pinned LocalStack 与 stock
BookKeeper 上以 retry disabled 通过：production materialization/GC 删除全部捕获的 generation-zero WAL source
keys 并保留 compacted keys；普通与 compressed-batch payload/properties/MessageIdAdv 经 source deletion、unload、
owner failover、broker restart/reverse takeover 保持精确一致，BookKeeper control topic 继续可写可读。门禁还将
Pulsar tenant-local namespace canonicalization、topic-vs-namespace failure accounting、独立 cursor scan page bound，
以及忽略 append-session/synthetic-trim 观测噪声但保留 commit/trim/policy truth 的 stable backfill authority
冻结为代码和测试。`phase4M4FinalCheck` 是 F4-M4 completion boundary；safe defaults 仍不调度 physical delete。
...
F4-M5 final gate 现已在 `master@0e9829a7453497910ab468669e644e88b4bc2f93` 上以 retry disabled 通过：真实
`OBJECT_WAL_ASYNC_OBJECT` topic 完成 cold registration、普通/压缩批消息精确 MessageId、broker owner stop/rejoin、
...
Start with `docs/v2/README.md` for document authority and current status. Use
...
`docs/v1/phase-1-core-stream-storage/README.md` for the implemented L0 contract and
`docs/v1/phase-1.5-core-storage-foundation/README.md` for the active L0 evolution contract. Use
`docs/v1/phase-2-managed-ledger-facade/README.md` for the implemented F2 contract，and
`docs/v1/phase-3-cursor-subscription/README.md` for the implemented F3 contract. Use
`docs/v1/phase-4-compaction-generation/README.md` for the current F4 code-level target contract and M0 outcome.
