...
> production/fake store parity 与 manifest-last seal/load service 也已实现，但尚未注入 collector。该 fence 明确停在 durable
...
该 durable storage primitive 仍未注入 `PhysicalObjectGarbageCollector`：当前 root MARK 路径尚未强制
PREPARE-before-MARK，DELETING recovery coordinator 也尚未消费 journal。因此本 checkpoint 不授权 source、
protection、metadata 或 object delete，physical deletion 继续关闭。
...
| F4-M4 | recovery checkpoint、source/index retirement and physical/cursor-snapshot GC | in progress；through checkpoint L durable-store slice, NRC1/recovery replay/index repair、exact retirement metadata、GC plans/root fence/scanner、affected-stream generation/append/materialization/projection/cursor domains、composed generation-marker authority and manifest-last retirement-journal storage are implemented/tested；collector PREPARE wiring、future sentinel/ownerless-global proof、runtime composition、retirement/delete and cursor GC completion pending |
