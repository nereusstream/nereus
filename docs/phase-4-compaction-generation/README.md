...
> 状态：Implementation in progress；F4-M1 已完成并通过 ordinary/Docker-backed final gates，F4-M2 读取与发布领域基础正在实现，F4-M3–M6 未实现
>
> 设计基线日期：2026-07-14
>
...
missing/checksum corruption 会先释放 lease、best-effort quarantine exact physical root 和选中的 higher index，
再通过 per-operation exclusion 执行 fresh same-view fallback；临时 `OBJECT_READ_FAILED` 不写 quarantine metadata。
...
组合接口，以及供整次异步发布共享的 monotonic deadline。它们冻结发布输入和 durable mapping，但尚未执行
`PREPARED -> COMMITTED` CAS。该检查点仍未实现 generation publication committer/reconciler、同对象全引用域
quarantine repair、瞬态读取重试阈值或
M2 ordinary/final gates，因此不能标记 F4-M2 完成。Phase 4 整体仍为 `Implementation in progress`；physical
delete、materialization 和 async profile 均未开放。
...
| F4-M2 | generation publication、committed resolver、target-reader dispatch and fallback | in progress；resolver/allocator/exact dispatch/pinned coordinator/selected-candidate quarantine and publication domain/canonical mapping/verifier foundation implemented，committer/reconciler/full quarantine repair/gates pending |
