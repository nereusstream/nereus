...
该检查点仍未实现 generation publication/reconciler、同对象全引用域 quarantine repair、瞬态读取重试阈值或
...
| F4-M2 | generation publication、committed resolver、target-reader dispatch and fallback | in progress；resolver/allocator/exact dispatch/pinned coordinator/selected-candidate quarantine implemented，publication/full quarantine repair/gates pending |
