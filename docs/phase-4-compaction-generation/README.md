...
> 状态：Implemented / final-gated；F4-M1–M6 与 aggregate Phase 4 gate 已通过。F4-M3 已交付
...
> isolated publication；F4-M4 已完成并通过 final gate，NRC1 object-protocol、protected generation-zero append、anchor-aware
...
> Checkpoint AY then drives 10,000 DELETED roots through the production tombstone coordinator's durable first-absence
> checkpoint and a later retirement pass. It proves 32-entry root pagination、one-at-a-time visitation、conditional
> Phase 1 references-before-manifest cleanup、root-last deletion and zero retained cancelled deadline tasks. The shared
> materialization runtime and S3 request schedulers now remove cancelled timers immediately instead of retaining them
> until their original due time.
> Checkpoint AZ closes the separate 10,000-cursor-root scale line. The scanner now builds its sequential candidate
> future chain iteratively, and a 10,000-candidate fixture proves synchronous completions cannot grow the Java stack or
> overlap visitors. A second exact fixture pages 10,000 durable cursor roots and 10,000 NCS objects, classifies 9,997
> live/current references plus old、expired CAS-lost pending and deleted-cursor candidates, then drives all three
> candidates through MARK、restart reconstruction、final full-inventory revalidation、protection retirement and exact
> object DELETE while the current object/root/protection remain live.
> Checkpoint BA closes the journaled source/protection retirement process-cut line. Deterministic coordinator tests
> stop after an exact metadata removal and after an exact protection removal while the root remains DELETING and the
> object remains present, then assemble a fresh coordinator that accepts only journaled absence and completes the
> remaining stages. A separate applied-protection-delete/lost-response case reauthenticates the same root/journal and
...
> Checkpoint BC adds the product-owned deletion-active readiness rollover. A new stable capable broker epoch no longer
> attempts to advance only `streamRegistrationBackfill` while the monotonic delete bits remain true. The runtime scans
> and protects physical/cursor roots without publishing partial proofs, repeats the configured-scope delete canary and
> installs the new epoch、all three complete proofs and capability digest in one activation-record CAS. Readiness drift
> blocks the call, old-epoch GC remains fenced, and successful rollover starts the mutating lifecycle on a broker whose
> startup was previously deferred. The locked Pulsar traversal now forwards its exact concurrency and remaining
> whole-run deadline through the storage bridge；the product proof coordinator consumes that same monotonic budget
...
> passed on 2026-07-19；F4-M6 final acceptance follows through checkpoint BQ below.
> Checkpoints BD–BE start M6 with bounded 32-reference NRC1 merge、the exact 4,096/4,097 generation-candidate edge and
> a streaming one-million-entry NRC1 fixture. Checkpoint BF adds complete restart-from-empty pagination for exactly
...
> the storage-isolation audit and the real Phase 1 Oxia restart gate pass. At BL the complete aggregate still needed a
> fresh rerun；BQ below records that qualifying rerun.
> Checkpoint BM makes that rerun resource-deterministic under the repository's required
> `org.gradle.parallel=true`. All ten local Testcontainers tasks and six nested multi-broker Pulsar builds declare one
...
> Checkpoint BO extends that audit from mutual exclusion to evidence freshness. All seventeen nested Pulsar wrappers
> now pass `--rerun-tasks`, and the static inventory gate rejects any checkout-owning wrapper that can reuse stale
> inner outputs. The first fully serialized aggregate completed all 194 outer tasks in 20m03s, but its completion
...
> 127 and 129 fresh inner tasks and passed all 103 outer tasks in 2m28s. At BO a fresh complete aggregate remained
> required；BQ below records it.
...
> Pulsar tasks on `master@eaf7b9a704890a9265c21f30d9f351e02d00c600`；BQ restarts and completes the aggregate on
> this source lock.
> Checkpoint BQ is the final acceptance：from clean, pushed Nereus
> `main@fa533a934c33f5bcc4fda328c4df64cb96c6b485` and Pulsar
> `master@eaf7b9a704890a9265c21f30d9f351e02d00c600`,
> `./gradlew phase4FinalCheck --rerun-tasks --console=plain` passes in 21m47s with 203/203 outer tasks executed. It
> includes the M1–M6 final gates、all prior phase final gates、52/52 executable evidence、real Oxia/LocalStack/
> two-broker scenarios and the Docker/checkout/fresh-inner audits. This is the aggregate completion claim.
>
> 设计基线日期：2026-07-14
>
> Nereus 输入基线：`nereusstream/nereus@e330969cd5c2c11cd38d0bd7f687185171ae91e2`
>
> Phase 4 验收代码锁：`nereusstream/nereus@fa533a934c33f5bcc4fda328c4df64cb96c6b485`
>
> Pulsar 输入基线：本地 `/Users/liusinan/apps/ideaproject/nereusstream/pulsar`
> `master@eaf7b9a704890a9265c21f30d9f351e02d00c600`
>
> 当前 post-BQ maintenance/source lock：`master@cd2a6e309ab8a6ef6983cacfc112ce513832b838`；上面的 BQ
...
watch/process-local hint 丢失和无 task 的 committed head 重建。F4-M3 当时仍保持 production rollout
...
继续保留；后续 F1-BK BK-M3 已把 BK source/protection/profile/lag、retirement-metadata/sealed-trigger 接入这套
共享 F4 truth，并通过首个 real Oxia/BK/Object publication/read/retirement-proof checkpoint。
...
an implementation detail in this directory, this directory owns the current Phase 4 code-level contract. Production
code + executable tests are the highest authority；both document sets are synchronized with checkpoint BQ.
...
46 vectors with the retirement-journal schema，checkpoint S extends it to 49 vectors with activation lifecycle/
capability states，and checkpoint BG extends it to 50 with the task-V2 planned-state envelope. The
...
证据，也不授权任何 physical delete；因此在该 checkpoint 上 F4-M4 仍为 `in progress`。
...
所有 delete enablement 在该 checkpoint 上继续关闭，F4-M4 当时仍为 `in progress`。
...
index repair、source/index retirement 或任何 GC/delete enablement，F4-M4 在该 checkpoint 上仍为 `in progress`。
...
Runtime composition 保留共同的 authority/reference 总量上界，但 cursor-backed domain 的 page size 固定为
`min(physicalGc.metadataScanPageSize, cursorStorage.cursorScanPageSize)`。因此 broker 默认 `1000` 的通用 GC
page 不会越过 cursor store 默认 `256` 的硬上界；future sentinel 等非 cursor domain 仍使用完整 GC page。
...
read-pin -> protection -> physical-store 顺序关闭。F2 process identity 现在直接由 128-bit fresh entropy 经
SHA-256/lowercase-base32 生成，因而可原样作为 F4 reader-lease identity；writer 仍固定为
`pulsar-f2/{processRunId}`，append-attempt namespace 仍由该 process identity 派生。
...
`closeTimeout`, without shutting down the borrowed scheduler or callback executor. The materialization service keeps
the active scanner source as an explicit lifecycle fact and cancels it before its exposed pass future at the deadline,
so close completion cannot race ahead of source cancellation.
...
### 6.25p F4-M4 DELETED-root retirement scale checkpoint

Checkpoint AY adds `retiresTenThousandDeletedRootsThroughTwoBoundedWindowsWithoutAccumulation`. The fixture creates
exactly 10,000 valid DELETED physical roots, each with matching Phase 1 object-reference and manifest audit records,
then uses the production `PhysicalObjectRootScanner`、`GcReferenceDomainRegistry` and
`DefaultPhysicalRootTombstoneRetirementCoordinator`. The first complete 256-shard pass observes strict
`tombstoneAuditGrace` and persists a canonical first-absence timestamp/proof on every root without deleting an audit
record. A later clock value satisfies the separate orphan window；the second pass reloads/revalidates each exact root,
observes fresh HEAD absence, conditionally deletes references before manifest and deletes the DELETED root last. A
third complete scan returns zero roots.

The test fixes `metadataScanPageSize=32`, proves continuation is exercised, no page exceeds that bound and no two
root visitors、audit operations、HEAD requests or final root deletes overlap. It retains no per-root result list. The
deadline scheduler queue is empty after both 10,000-root passes and S3's independent request scheduler has the same
remove-on-cancel/shutdown policy. This closes a production memory hazard：JDK's default scheduled executor retains a
cancelled far-future task until its due time, so successful high-rate metadata/HEAD operations could otherwise leave
an ever-growing timeout queue between scans. The focused scheduler/tombstone suite passed on 2026-07-18 with 26/26
executed tasks and the three affected module checks passed with 50/50 executed tasks.
`phase4M4TombstoneScaleCheck` composes checkpoint AX, full materialization/object-store/adapter checks,
source/module/document audits and the dedicated AY contract surface. The aggregate passed on 2026-07-18 under Java
21、Docker 28.5.2 and locked Pulsar `master@c59da789e88df2b57829de3277c60194b44fceb6`；the root build reported
156 actionable tasks（86 executed、70 up-to-date）, while its two serialized locked-Pulsar builds executed 141/141
and 138/138 tasks.

Checkpoint AY closes the required `10,000 DELETED roots` dual-window/audit/root-CAS scale line. It does not close the
10,000 cursor-root fixture, source/protection deletion cuts, the complete late PUT/tombstone race matrix or actual
two-broker ownership/failover gate.

### 6.25q F4-M4 cursor-snapshot GC scale checkpoint

Checkpoint AZ adds two named deterministic scale methods. The first,
`visitsTenThousandCandidatesSequentiallyWithoutStackGrowthOrDeadlineRetention`, creates exactly 10,000 old NCS
objects with exact ACTIVE physical roots and no cursor reference. `CursorSnapshotGcScanner` lists them in 256-entry
pages and emits all 10,000 candidates to a synchronous visitor. Candidate sequencing is now an iteratively assembled
future chain rather than a recursive `index + 1` call, so the maximum visitor concurrency remains one, all candidates
complete without stack growth and the remove-on-cancel scheduler queue is empty after the pass.

The second method, `classifiesAndDeletesLiveOldCasLostAndDeletedCursorSnapshotsAtTenThousandRoots`, creates exactly
10,000 durable F3 cursor roots and exactly 10,000 listed snapshot objects. The inventory contains 9,997 exact
ACTIVE/current references and three unreferenced objects built from the real protocol states：an ACTIVE cursor whose
permanent protection identifies the older metadata version after snapshot replacement；a cursor-root CAS loser with
an expired `CURSOR_SNAPSHOT_PENDING` protection；and a DELETED cursor whose deletion CAS cleared the reference while
the old permanent protection remains. Both cursor and object scans take 40 bounded pages. The initial executor pass
marks exactly those three objects and waits for drain grace. After a clock advance, three restart-style
`recoverMarked` calls each reconstruct the root-authenticated plan, repeat the complete 10,000-root/object cut, enter
DELETING, remove the exact protection and issue one exact-identity DELETE. The final object inventory contains all
9,997 current objects；their representative root and permanent protection remain ACTIVE, and no deadline task is
retained.

The focused Java 21 run passed on 2026-07-18 with 37/37 executed tasks in 27 seconds.
`phase4M4CursorGcScaleCheck` composes checkpoint AY, affected module checks, source/module/document audits and the
dedicated AZ contract surface. The `--rerun-tasks` aggregate passed on 2026-07-18 under Java 21、Docker 28.5.2 and
locked Pulsar `master@c59da789e88df2b57829de3277c60194b44fceb6` in 3m49s；the root build executed 157/157 tasks and
the two serialized locked-Pulsar builds executed 141/141 and 138/138 tasks. AZ closes required scenario 20 and the
`10,000 durable cursor roots for one stream` scale line. It is still not `phase4M4FinalCheck`：source/protection
deletion cuts、the complete late PUT/tombstone race matrix and actual two-broker ownership/unload/failover evidence
remain mandatory.

### 6.25r F4-M4 source/protection retirement process-cut checkpoint

Checkpoint BA adds three exact central-coordinator methods. The metadata-cut method conditionally removes one
journaled source record, injects process loss at the next root read and proves the DELETING root, protection and object
are unchanged；a fresh coordinator sees the exact metadata key already absent, removes only the journaled protection
and then deletes the object. The protection-cut method applies the exact protection delete and loses the process before
the following root/journal reauthentication；restart treats only that journaled key as already absent and performs one
object DELETE. The third method loses the response after the protection delete has applied；the same attempt reloads
the unchanged DELETING root and sealed journal, scans the exact protection scope to absence and only then reaches HEAD/
DELETE. Every fixture asserts that object HEAD/DELETE is untouched at the injected pre-object cut.

`freshProcessRecoversJournaledMetadataAndProtectionPostDeleteCuts` carries the same states through real services. The
setup process writes two LocalStack objects, their ACTIVE roots/protections and manifest-last retirement journals to
four-shard Oxia, then transitions both roots to DELETING. One journal includes an exact already-retired
`generation-zero-index` removal while its protection remains；the other has its journaled protection conditionally
removed while the object remains. Root scans are then failed to model process stop without allowing the setup runtime
to consume either cut. An independently assembled runtime recovers both solely from Oxia root/journal authority. Its
remaining protection delete succeeds in Oxia but returns an injected timeout；exact absence recovery permits both
identity-bound LocalStack DELETEs and both roots become DELETED, with no protection left.

The deterministic materialization suite and the focused real-service method pass under Java 21；the latter uses Docker
28.5.2 and completed 38 actionable tasks（2 executed、36 up-to-date）in 15 seconds on 2026-07-19.
`phase4M4SourceProtectionCutCheck` composes checkpoint AZ, affected module checks, the complete F4-M4 real-service
source set, source/module/document audits and the dedicated BA static contract. BA closes the source/protection
...
### 6.25t F4-M4 deletion-active readiness rollover checkpoint

Checkpoint BC closes the protocol dead end exposed by broker restart or membership change after physical deletion is
already active. `ManagedLedgerGenerationRegistrationBackfillProofCoordinator.complete` now accepts the caller's
bounded stream concurrency and remaining deadline. For a deletion-inactive record it retains the existing registration-
only refresh；for a deletion-active record it delegates to the new
`ManagedLedgerGenerationReadinessRolloverCoordinator` and never clears either monotonic delete bit.

`DefaultPhysicalRootBackfillCoordinator.runRollover` freezes the exact old activation wrapper, requires a different
opaque readiness epoch token, walks the same complete registration/data/cursor authority and installs required roots and
protections, but deliberately performs no activation CAS. It reloads the exact old wrapper after the scan, so a
concurrent authority change makes the report ineligible. `DefaultPhase4PhysicalDeletionActivationCoordinator` then
validates the zero-failure report、rechecks the full readiness identity、reruns the configured-scope delete canary and
constructs one replacement containing the new registration proof、physical-root proof、cursor-snapshot proof、scope
digest and readiness epoch. One version CAS publishes that complete tuple while publication and both deletion bits
remain true；condition conflict/lost response accepts only a complete concurrent authority for the same new epoch and
registration coverage. Final cached-readiness and durable-record reads repeat the check.

`Phase4PhysicalGcRuntime` implements the rollover boundary and starts its previously deferred mutating lifecycle only
after the new exact authority is durable. `DefaultNereusRuntimeProvider` injects that runtime into the registration-
proof coordinator, so the product path has one correctness owner. Deterministic tests cover bounded delegation、a
non-publishing empty-registry rollover and the single-CAS replacement of every proof. The ordinary
`phase4M4ReadinessRolloverCheck` composes the prior BB gate、affected module checks、a locked-Pulsar formatting/style/
focused gate and a static contract audit. `DefaultNereusGenerationRegistrationBackfill` forwards the request's exact
concurrency and the remaining traversal deadline through the three-argument storage/factory boundary；both the broker
request and typed configuration reject concurrency above 1,024. The product proof coordinator starts one monotonic
deadline before readiness admission, bounds activation/CAS/reload/final-read stages and supplies only its remaining
budget to the deletion-active rollover. The checkpoint still does not claim the M4 final gate：the real two-broker
owner unload/failover/restart fixture remains mandatory. The new locked-Pulsar sub-gate passed on 2026-07-19 under
Java 21：the outer build completed 65 tasks and the nested broker build executed all 141 tasks.

### 6.25u F4-M4 real two-broker final acceptance

`NereusPhysicalGcMultiBrokerIntegrationTest`
`deletesMaterializedWalSourcesAndPreservesMessageIdsAcrossOwnershipCuts` closes the final broker boundary against the
locked local Pulsar fork. The fixture starts two real brokers with shared four-shard Oxia、pinned LocalStack S3 and
stock BookKeeper. It activates physical deletion through the public registration-backfill/activation path, writes
ordinary and compressed batched Nereus messages plus a BookKeeper control topic, runs the production materialization
and metadata-first GC lifecycles, and proves every captured generation-zero Object-WAL source key disappears while
the COMMITTED compacted keys remain. Reads after source deletion and after explicit topic unload preserve payload、
properties and the exact `MessageIdAdv` ledger/entry/partition/batch coordinates.

The final production graph also closes the two orchestration seams exercised by that fixture.
`Phase4ObjectWalRuntime` constructs the real `RecoveryCheckpointCoordinator` behind the new
`RecoveryCheckpointPublisher` boundary and injects it into `RegisteredMaterializationStreamScanner`, which runs NRC1
publication after source repair/task convergence and before checkpoint reconciliation/terminal retirement. For every
non-cursor ACTIVE or MARKED physical root, `Phase4PhysicalRootLifecycleRouter` tries
`ReferencedObjectGcExecutor` before the ownerless path. A current `VISIBLE_GENERATION` protection makes the referenced
executor the sole handler: it rebuilds higher-generation eligibility/source removals、revalidates all protection
owners, delegates MARK/DELETE_INTENT to the central collector and resumes source retirement from the sealed journal.
Only an empty referenced discovery may fall through to ownerless GC. `Phase4PhysicalGcRuntime.referencedGcResults()`
exposes one immutable, pass-scoped latest result per visited referenced object；the map is cleared when a new root pass
starts so it is diagnostic state, never deletion authority.

A prior complete drift can unmark a root into a newer ACTIVE lifecycle epoch while retaining the exact logical owner
protections. The referenced executor detects those older protection epochs, revalidates the exact retirement owner,
uses `ObjectProtectionManager.acquireOrTransfer` to CAS-rebind only the same identity/expiry/logical owner to the
current ACTIVE epoch, and returns handled `NOT_ELIGIBLE` so a later pass replans from fresh facts. Different owner、
expiry drift、owner rollback or a protection epoch ahead of the root fails closed. The recovery builder also preserves
pre-F2 compatibility by treating a successfully decoded empty legacy commit projection as absent while requiring the
registration and every present commit projection to agree；current/older NRC1 publication candidates are skipped,
not promoted into a higher-generation replacement.

The test then stops the current owner, waits for the survivor's generation capability, performs deletion-active
readiness rollover, reads and appends on the survivor, restarts the old broker, stops the survivor and performs a
reverse rollover. The restarted broker reads the unchanged Nereus history, while the BookKeeper topic accepts and
reads another message. Every activation/backfill retry reports its bounded failures, and the authoritative gate sets
`testRetryCount=0` so a flaky rerun cannot manufacture acceptance.

The final run exposed two correctness-boundary bugs before it passed. Pulsar namespace resources return tenant-local
names, so the cold-topic scanner now canonicalizes them with the current tenant；namespace-list failures remain run
failures but are not counted as topic outcomes. The physical-root backfill now uses its separately bounded cursor
page size instead of passing the broader metadata page size into F3 cursor scans.
...
fresh synthetic trim timestamp does not. Focused tests freeze both sides of this distinction.

The locked Pulsar test passed on 2026-07-19 under Java 21 in 2m19s with retry disabled. `phase4M4FinalCheck` composes
the complete checkpoint-BC product chain、the stable-authority/static audit and
`phase4M4PhysicalGcMultiBrokerPulsarCheck`. This is the F4-M4 completion boundary；it does not claim F4-M5/M6 or the
aggregate Phase 4 completion definition.
...
contract surface、spotless、checkstyle，以及 generation/cursor/binding focused suites。Checkpoint Y 最初的
generation lookup V1 two-broker fixture epoch 为 `4351585672493013605`，full digest 为
`bc63f01d0aa01a65c7205625a2714f0246d8ba7e7b88b8a653137abbc719cc0d`；checkpoint BG 因 task schema V2 将
lookup capability 提升为 `2` 后，当前冻结值为 epoch `36151462167742895`、full digest
`80806f90349e89afb16f65d2e90f06339f48babe836f9954ad41fefc2869ab75`。该 checkpoint 不枚举 cold
topics、不创建 registration backfill proof、不设置 topic generation marker，也不启用 activation/publication/
deletion。Gate 的 V1 首次验收已于 2026-07-16 通过；checkpoint BG 又于 2026-07-19 在当前 source lock 上以
`--rerun-tasks` 完整通过（root 166/166 executed，8m58s），并继承重跑了 retry-disabled 的真实 M4 多 broker/
Oxia/LocalStack final predecessor（138/138 executed，3m02s）。
...
Locked Pulsar fork 新增 canonical one-namespace-at-a-time traversal。它把
`listNamespacesAsync(tenant)` 返回的 bare local name 以 `NamespaceName.get(tenant, localName)` 恢复为 exact
identity，再按 tenant/namespace/topic 排序；system topic 只进入 coverage digest、不进入 persistent-topic
counter；non-Nereus、missing 或 deleting/deleted binding
...
Report 强制 `persistentTopicsScanned == registered + skipped + topicFailures`，其中可由 topic counter 推导的
`topicFailures <= failureCount`；`failureCount` 还覆盖 tenant/namespace/topic-list 等未产生 topic outcome 的
traversal failure。Report hash 每个 canonical resource/outcome 和全部 failure，只保留前 100 个 redacted
failure。固定 fixture coverage SHA 为
...
Registered-stream scanner 是普通 sync topic 的首个 generation-publication admission point，因此使用
`activateLiveProjectionIfAbsent=true`：cluster publication ACTIVE 后可执行 registration-before-marker
收敛，但在 proof revalidation 前不创建 task/index。Commit、recovery 和 GC 等后续路径继续传 `false`，只消费
已经建立的 marker authority。
...
the aggregate M6/Phase 4 tasks. At checkpoint BH execution remained open；checkpoint BQ below closes it.
...
tasks were declared at checkpoint BK；checkpoint BQ below records their complete passing `--rerun-tasks` aggregate.
...
at checkpoint BL the Phase 4 aggregate still had to restart before completion could be claimed. Checkpoint BQ records
that complete restart.

### 6.41 F4-M6 deterministic Docker release-gate checkpoint

Checkpoint BM fixes the second failure exposed by `phase4FinalCheck --rerun-tasks`. The repository intentionally uses
`org.gradle.parallel=true`, but the release DAG previously allowed independent Oxia、LocalStack and nested Pulsar
Testcontainers tasks to start together. Under that load one healthy Oxia container timed out serving its `/metrics`
readiness probe；the exact `oxiaIntegrationTest` passed immediately when rerun alone, proving a gate resource race
rather than a storage assertion failure.

`DockerIntegrationGateService` is a root Gradle shared build service with `maxParallelUsages=1`. The ten local
Docker-backed tasks—Phase 1 Oxia、metadata capability/integration/F4 tests、ObjectStore S3、F3 cursor S3/Oxia and F4
M2/M3/M4 real-service suites—and all six nested multi-broker Pulsar final tasks call `usesService` on that same permit.
This adds no task dependencies：an individually requested gate still runs alone, while any aggregate can compile and
run ordinary work in parallel but starts at most one Docker owner. `checkPhase4FinalDockerIsolation` freezes the full
task inventory and is required by both `phase4M6FinalCheck` and `phase4FinalCheck`. A forced parallel invocation of
the F4 Oxia and S3 tasks passes with both tasks serialized by the service. The complete aggregate was still required
at checkpoint BM and is supplied by BQ.
...
2m11s. The complete aggregate was still required at checkpoint BN and is supplied by BQ.

### 6.43 F4-M6 fresh nested-Pulsar execution checkpoint

Checkpoint BO closes the evidence-quality gap found after checkout serialization. The first fully serialized
`phase4FinalCheck --rerun-tasks` completed 194/194 outer tasks in 20m03s, but `phase2PulsarCheck` and
`phase3M4PulsarCheck` had not forwarded `--rerun-tasks` to their nested Gradle commands. Their selected inner tests
were therefore eligible to report `UP-TO-DATE`; a green outer graph was not sufficient fresh evidence.

Every one of the seventeen source-locked Pulsar `Exec` tasks now forwards `--rerun-tasks`.
`checkPhase4FinalPulsarCheckoutIsolation` parses the same checkout-owning inventory and fails if any member is either
outside the exclusive service or lacks fresh inner execution. A forced-parallel request for the two corrected
historical wrappers completed 127/127 and 129/129 inner tasks, then passed all 103 outer tasks in 2m28s. This validates
the correction without promoting the earlier aggregate run；the BO-qualified full aggregate is checkpoint BQ.
...
tasks passes 138/138 fresh tasks in 1m52s. Checkpoint BQ runs the new complete aggregate on this source lock.

### 6.45 F4-M6 final aggregate checkpoint

Checkpoint BQ is the Phase 4 exit result. From clean, pushed Nereus
`main@fa533a934c33f5bcc4fda328c4df64cb96c6b485` and Pulsar
`master@eaf7b9a704890a9265c21f30d9f351e02d00c600`, the exact command
`./gradlew phase4FinalCheck --rerun-tasks --console=plain` passes in 21m47s with 203/203 outer tasks executed. The
aggregate completes `phase4M6Check`、`phase4M6FinalCheck`、`phase4Check` and `phase4FinalCheck`; composes every
M1–M6 and prior Phase 1/1.5/2/3 final gate；executes the real Oxia、LocalStack、two-broker and BookKeeper-control
fixtures；and passes the 52/52 scenario、planned artifact、dependency-isolation、Docker ownership、Pulsar checkout and
fresh-inner-execution audits. F4-M6 and Phase 4 are `Implemented / final-gated`.
...
not inferred。The later F1-BK delivery owns those adapters and is currently at the BK-M3
source/protection/profile/lag、retirement-metadata/sealed-trigger and first real Oxia/BK/Object
publication/read/retirement-proof checkpoint，outside this F4 completion claim。
