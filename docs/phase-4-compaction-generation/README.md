...
> authority foundation 不会自行启用 publication/delete bits。Future-sentinel、ownerless global domains、backfill/
> broker activation guard、production runtime composition、
...
Checkpoint R 是 source-eligibility 的 ordinary completion checkpoint，不是 M4 final gate。Future-sentinel、
ownerless global absence proof、production runtime composition、cursor/root/audit retirement、real-service
...
| F4-M4 | recovery checkpoint、source/index retirement and physical/cursor-snapshot GC | in progress；through checkpoint S, NRC1/recovery replay/index repair、exact retirement metadata、GC plans/root fence/scanner、affected-stream generation/append/materialization/projection/cursor domains、root-authenticated journal/destructive recovery、typed source handlers、all completed-trim/COMMITTED/TOPIC_COMPACTED source-eligibility paths、grace-fenced higher pre-drain/reproof and the durable generation-activation metadata authority foundation are implemented/tested；future sentinel/ownerless-global proof、backfill/broker guard、runtime composition、cursor/root/audit completion and final gate pending |
