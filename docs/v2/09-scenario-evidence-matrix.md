...
| V2-META-002 | M1 | Topic Protocol Binding and its initial Storage Epoch have one atomic visible aggregate; partial/uncertain create never admits I/O | duplicate/lost-response/concurrent create, half-record, activation failover, incomplete-open rejection, and intent cleanup | PLANNED |
...
| V2-POSITION-002 | M1/M3 | Pulsar Object WAL uses reserved-domain virtual ledger IDs and explicit chain order; Object identity never becomes a position | identity collision, numeric-sort rejection, current-ledger fencing, zero append metadata I/O, rollover/restart, and Object-extent mapping tests | PLANNED |
...
| V2-OBJ-001 | M3 | Object ACK waits for verified group durability and the canonical request-body Object Extent Digest | real-provider completion, digest/length/version match, decoded frame validation, and restart | PLANNED |
| V2-OBJ-002 | M3 | unrelated bindings avoid shard-wide typed-frontier HOL | stalled/corrupt binding concurrency test | PLANNED |
| V2-OBJ-003 | M3 | lost PUT response verifies the Object Extent Digest by trusted provider proof or bounded full GET; frame payload checksums remain a separate domain | real-provider success/lost-response, stored-body versus decoded-payload corruption, missing/mismatched object, conditional-create race, and budget tests | PLANNED |
