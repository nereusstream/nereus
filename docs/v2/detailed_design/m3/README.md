...
261/0/0/0 respectively at this implementation boundary. These are receipt prerequisites only: neither R nor K has an
exact-source child receipt yet, and the protocol fixture does not freeze complete synthetic Root/Pointer wire.
...
| M3-R1 | WalRun Root/Pointer/checkpoint/Seal and Provider/KMS session implementation | common control/session/recovery source implemented; the closed eight-row recovery manifest plus its self-test are fail-closed child inputs, and `:nereus-storage-object:check` passes 148/0/0/0; an exact-source R child and post-selection freshness remain open |
| M3-K1 | Object `NWKCP1` plus `KafkaProtocolCheckpointHeadV1` | production-codec fixture binds a 324-byte Object and 434-byte OPEN/TERMINAL Heads; content validator and local 261/0/0/0 module check cover exact wire/key/caps, state transition, backend mapping, response-loss recovery and bounded recovery; an exact-source K child and native receipt remain open |
