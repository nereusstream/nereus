...
The same four-test prerequisite was rerun after the current ordinary-source closure at exact clean published source
`48df1793a6c24d38233a065107dd50c5375d3257`. It again passes `4/0/0/0` against dedicated Pulsar
`7ff908330809f2e9bc5c69ead87bb85c566bc0a9` and Oxia image ID `7eef9af2...f4da`, sealing a 212-byte diagnostic-only
NADV2 with SHA-256 `b425055273e5a3a0171170d4a1ce14f84b6a1c1b1fead915ad51db6a90888115`; the exact JUnit
SHA-256 is `382de9ab725164ad209f27e9e494e9f0259f09ad811b4b2ff8fd8e533a357a32`. It binds source-lock
`e128c24d...84d`, executor `b0bdc2f4...5b5f`, and workload-plan `1291acd8...72f`, and is preserved under
`/Users/liusinan/Documents/Codex/2026-08-27/nereus-v2-m3-allocator/diagnostic-v2-48df1793-r1`. This newer diagnostic
still creates no NACP2, NAEV2, formal campaign evidence, selection, child receipt, or scenario PASS; a selected-mode
source change requires another freshness rerun before promotion.

The prerequisite was freshened again at exact clean published source
`520838a05dffe3f421f1001c5023662426b2f631` after the direct-task/dedicated-worktree and `--rerun-tasks`
enforcement landed. The four real-Oxia diagnostics pass `4/0/0/0`, sealing a 212-byte diagnostic-only NADV2 with
SHA-256 `4bd6d4feefb7a6e3df05ee0a54b7ab996ad2faf6a158a734d9ac47056d26c0cc`; the exact JUnit SHA-256 is
`942171e8647e18dcd9719586dc7ecaf9ca3680a1b77d701145e7f2e46a929eef`. The immutable output is preserved under
`/Users/liusinan/Documents/Codex/2026-08-27/nereus-v2-m3-allocator/diagnostic-v2-520838a0-r1`. It remains a
...
At clean published source `c27c2f3fd375a3cbdc090fcd5e35d42c58afb382`, the governed C2 candidate test passes
1/0/0/0 and publishes the exact-source [C2 child](../../evidence/v2-m3/children/09-C2_SEGMENTED_PREFIX/receipt.json)
`b55114e4...485e`. The child is valid evidence-only inventory but deliberately non-promotable: it supplies no
independent benefit evidence, cannot substitute for C1, and remains outside the production allowlist and scenario
promotion.
...
At clean published source `1d181dc65e86f1cc9da6d0a6bd840b8fffee21bf`, the governed C execution passes
7/0/0/0 and publishes the exact-source, non-promotable
...
scenario promotion and a fresh post-selection Final-source rerun remain open.
...
Root-bounded tail inventory, and Provider/KMS lifecycle to governed test identities; a ninth test verifies the manifest
itself. The production-codec NWKCP1 emitter freezes a 324-byte Object and 434-byte OPEN/TERMINAL Head fixture with
exact Root-bound keys, SHA-256, and wire hex. The child validator independently checks both manifests and rejects even
fully rehashed row, key, byte, or state substitutions. Full storage-object and Kafka module checks pass 148/0/0/0 and
261/0/0/0 respectively at this implementation boundary. The exact `35e6784d...` R execution passes 9/0/0/0 and
publishes non-promotable [control/recovery child](../../evidence/v2-m3/children/05-R_CONTROL_RECOVERY/receipt.json)
...
At the next clean published source `4c5466391b46d8a9f936b58b93a748a43ce3c1fb`, the complete Pulsar offload
module passes 140/0/0/0 and the exact P execution seals its two closed suites at 46/0/0/0, six exact Nereus source
artifacts, and the dedicated Pulsar M3 commit `7ff908330809f2e9bc5c69ead87bb85c566bc0a9`. The independent governed
JUnit and native wrappers publish the non-promotable
[Pulsar Object-WAL child](../../evidence/v2-m3/children/08-P_PULSAR_OBJECT_WAL/receipt.json) `94084eac...071c`
with derived totals 92/0/0/0. It covers fixed-slice successor/no-gap behavior, ticket and manifest authority,
shared/Binding isolation, response-loss reconciliation, production adapter boundaries, and bounded recovery. It does
not select an allocator mode, activate Pulsar broker/controller paths, promote a scenario, or remove the required
post-selection Final-source rerun.
...
Direct `realAllocator*`, `*RealAllocator*`, `validateRealAllocatorV2*`, and `sealRealAllocatorV2*` tasks inherit the
same dedicated-worktree default. The pre-campaign entrypoint additionally forces `--rerun-tasks`, so cached Gradle
outcomes cannot masquerade as a current-source prerequisite. Four governance tests freeze the byte-stable plan-only
projection, fresh dedicated pre-campaign execution, the direct-task default, and the fail-closed disabled legacy
formal entrypoint.

At exact clean published source `520838a05dffe3f421f1001c5023662426b2f631`, the no-argument authoritative
`v2M3SourceCheck` passes end to end in 6 minutes 7 seconds. Its ordinary module phase executes 937 tests across ten
modules with zero failure/error/skip and verifies all style gates; it publishes the eleven source-qualified module,
POM, Gradle-metadata, sources, and BOM closure and compiles an independent consumer. The aggregate also passes 91
non-empty governance contract tests, historical M2 Final immutability and M3 inputs, the 17-test/114-row NWG1 wire
gate, the 3-test 84-record/240-path mutation gate, and the 7-test 50-trace/21-outcome state gate. This is current-source
...
[ADR 0107](../../../decisions/0107-v2-m3-allocator-bounded-adaptive-formal-entry-wiring-amendment.md) now closes
the Phase-A formal-entry wiring gap without authorizing execution. The source-independent frozen plan expands the
unchanged 288 logical interval cells into at most 288 exact interval actions, 360 single-cut fault actions, and 32
RANGE-row scale actions, for a maximum of 680 physical actions under a separate 48,000-second process cap. Its
zero-decision plan SHA-256 is `4fbeb2d43bd5865cb6139277a5021ed1b0762223f4983fc8fa50f8edc975ff08`.
`--plan-only` reports that inventory plus the exact source tuple without inspecting external worktrees or services,
creating population, or writing evidence. One explicit default-off task,
`:nereus-metadata-oxia:realAllocatorV2BoundedAdaptiveFormalCampaign`, and the matching
`--bounded-adaptive-formal NEW_EMPTY_OUTPUT_DIRECTORY` script mode now bind one planned action to one real adapter
invocation. The interval adapter uses four independent bounded production-workflow coordinators and never wraps the
old exhaustive loop; fault and scale adapters execute exactly one cut or one RANGE row prerequisite. Formal startup
fails closed on the separately authorized commit, clean/main/origin equality, all source/dependency/runtime hashes,
the frozen plan, clean dedicated external worktrees, and a new empty output. Registration is not authorization: this
Phase-A slice runs no formal campaign and creates no NACP2, NAEV2, NARS2, selection child, source-lock update,
...
result.
The [Stage B.2 current-source recertification record](stage-b2-native-executor-current-source-recertification.md)
defines the create-new diagnostic replay and no-formal stop boundary for the exact pushed source that contains it.
...
The ADR-0125 V4 diagnostic baseline was a create-new, diagnostic-only 21-test/eight-suite inventory. It preserves all 18 V3
...
[ADR 0126](../../../decisions/0126-v2-m3-allocator-v4-range-latency-attribution-amendment.md) preserves the resulting
exact `c44a56c2...-r1` V4 `NONE_QUALIFIED` campaign and its 135-file external archive. Native sustained 1,000 RPS in
all eight rows; RANGE-64/256/1024 instead shared a first eliminating 10k/10ms row, and RANGE-1024 derived-800 retained
1,335 pre-admission drops despite zero admitted failure/timeout and complete lifecycle drain. The
[RANGE latency attribution record](stage-b-v4-range-latency-attribution.md) adds an exact formal-sequence 10ms
...
[ADR 0131](../../../decisions/0131-v2-m3-allocator-v4-applied-mutation-instrumentation-forwarding-amendment.md)
preserves the exact `3bc11088...` wrapper-fallback diagnostic and the associated
[instrumentation-forwarding record](stage-b-v4-applied-mutation-instrumentation-forwarding.md). Both shared wrapper
...
mutation 3/0/0/0 over 84 records/240 paths/50 roots, NWG1 wire 17/0/0/0 over 114 rows, and Object-WAL state traces
7/0/0/0 over 50 traces/21 outcomes. The subsequent Final aggregate correctly rejects the absent
