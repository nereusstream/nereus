...
- Registry backend codec/capacity, the then-unaccepted `maxWriterCount=8` candidate, writer-interlock execution, or real
  Oxia conformance; M1.1c-R0 later accepts 14/51,016 as an R1 input without adding that production authority here;
...
- Registry work may replace the foundation Registry value only in the later R1 slice; M1.1c-R0 has now supplied the
  accepted 14-writer/51,016-byte capacity input, but no bounded production codec or Store implementation.
...
- at M1.1a-A completion, M1.1b and Registry capacity remained explicitly OPEN; both later closed only their stated
  local/input boundaries, and M1-2 later closed only the receipt-cap input while production G1/promotion evidence
  remains OPEN;
