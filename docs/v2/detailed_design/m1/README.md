...
- P1 focused implementation is complete through Nereus `e9bcd039`; its immutable focused receipt remains bound to
  ...
  `072aa1c440f85b808f60e7ea59de8a73c4e2a202`: NPS1, closed transitions, selector/aggregate coordination,
...
flowchart TD
    ...
    K1 --> V1P["Cut edges, then mechanical V1 prune"]
    P1 --> V1P
    R1 --> V1P
    V1P --> N2["N2 exact source tuple and gates"]
    N2 --> N3["N3 evidence-only promotion"]
    N3 --> M2R["M2 readiness"]
```
...
| `M1.1a-O2` | V2 aggregate/selector/Registry adapter scaffolding and store-wide continuity capability | `M1.1a-D`, `M1.1a-O1` | local fake tests; exact-source conformance belongs to later P1/R1/N2 descendants | locally verified at Nereus `050f908a`: 69 focused and 299 whole-module tests; [accepted design](m1.1a-oxia-capability-scaffold.md) and [local-only receipt](../../evidence/v2-m0/m1.1a-o2/README.md); later descendants are current-source complete without relabelling this receipt |
...
| `P1` | selector CAS, authoritative ownership A/read/B, atomic ACTIVE fence and invalidation | immutable N1 artifact plus `M1.1a-O2` | Pulsar/Oxia focused source gate | focused source-qualified implementation and non-promotable receipt complete at Nereus `e9bcd039`; pure-V2 Pulsar `072aa1c4` is bound by the final Exact Source result |
...
| `v2M1FinalCheck` | N2/N3 | fresh referenced gate/receipt schema and aggregate result | work not explicitly referenced by the receipts |
