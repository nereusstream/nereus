...
| M3 | one-cell NWG1 Object WAL groups; binding-context epoch authority and commit-set co-location; run-key/per-Object AEAD; self-describing prefix hints; up to three lazy lane-local sequences under one Root/pointer; one vector checkpoint chain with aggregate hard bounds; provider-absent crash cuts; bounded recovery/strong-LIST; provider proof/session lifecycle; per-binding frontier; fixed-slice Pulsar virtual-ledger path with mass-takeover evidence and RANGE grant-takeover conformance | Planned | `v2M3Check` |
