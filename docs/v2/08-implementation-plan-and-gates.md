...
| M4 | manifest, protocol-position/timestamp indexes, Storage Epoch resolver, logical Binding read snapshot, bounded sharded generation-tagged hazard slots, ABA-safe lease word and terminal source drain, active-tail/manifest `PREFERRED_WITH_FALLBACK -> PREFERRED_ONLY` generations, Binding Read Admission Epoch order, reusable source-independent quiescence proofs/window, and bounded two-stage retirement | Planned | `v2M4Check` |
