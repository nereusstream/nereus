...
| M4 | manifest, protocol-position/timestamp indexes, Storage Epoch resolver, logical Binding read snapshot, bounded sharded generation-tagged hazard slots, complete async source-access lifetime, active-tail/manifest `PREFERRED_WITH_FALLBACK -> PREFERRED_ONLY` generations, durable owner-read quiescence evidence, and bounded two-stage retirement | Planned | `v2M4Check` |
| M5 | materialization, compaction, retention, exact source-protection release, retained-source admission/alerting, per-cell cache/task isolation, physical GC | Planned | `v2M5Check` |
