...
| M4 | allocation-free Binding-scoped logical `BindingReadViewSnapshot`; deterministic typed protocol/profile source plan and one-shot pre-observability fallback; bounded generation-tagged hazard slots with stable conservative scanning and ABA-safe terminal drain; fused selector/terminal/proof-window/fold/capability control; exact per-source interval verification and protection-generation release CAS; four-child evidence hierarchy; no physical deletion | **IMPLEMENTATION COMPLETE / FORMAL EVIDENCE PENDING. Read kernel, typed Kafka/Pulsar planning, durable selector/quiescence control, current-source Object-WAL integration, closed receipt validators, and four exclusive evidence tasks are implemented. No scenario is promoted until one clean tested source publishes all children and immutable Final.** | `v2M4DesignCheck`; `v2M4ReadKernelCheck`; `v2M4ControlPlaneCheck`; `v2M4CurrentSourceIntegrationCheck`; `v2M4Check` |
...
evidence tooling do not amend those frozen inputs. `v2M4DesignCheck` remains explicitly non-promotable; `v2M4Check`
becomes authoritative only after its four exact-source children and canonical Final are published and synchronized.
