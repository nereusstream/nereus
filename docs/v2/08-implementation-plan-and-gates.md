...
Oxia container restarted between two JVMs. The runner archives six suites/57 cases and phases, captures its checked
...
The coordinator now persists validation rejection as one bounded same-key recovery veto and blocks intent until a
qualified refresh clears it; veto response loss remains failed/unknown and stale failures cannot overwrite successors.
The restart phase verifies this persisted veto. Ordinary wire-4 authority bytes remain unchanged; the wire-5 extension
is used only for READ_FENCED veto values. Typed INTENT recovery uses wire 6, retains one latest refresh context pair,
binds capability and complete fresh eligibility into the dispatch token, and requires native identity rereads between
owner/fact validations. Both veto and typed intent survive the metadata runner's separate-JVM restart. The BK runner
...
adapter. `scripts/run-v2-m5-bookkeeper-oxia-control-check.sh` now runs `v2M5BookKeeperOxiaControlCheck` with real Oxia
and BK together, plus separate JVM phases across an Oxia restart. It verifies typed immutable compaction records,
...
`scripts/run-v2-m5-gc-quota-oxia-check.sh` adds durable canonical byte reservations, permanent resource accounting,
fixed-size pending-operation recovery, fail-closed fixed-depth native initialization scans and explicit quota expansion.
It checks native grant/refund response-loss and ABA races, 130 settled records against native byte totals, and an existing
...
90 archived cases/phases plus 71 independent legacy cases/phases, with both source maps unchanged.
...
replacement. The pure target-scoped authority key/record/codec/state-machine foundation now passes
`v2M5TargetDeleteAuthorityFoundationCheck`; persisted same-key CAS, closed-writer runtime wiring, real Oxia execution,
external dispatch/recovery, and evidence remain unimplemented.
...
`v2M5TargetDeleteAuthorityFoundationCheck` fixes the pure target-scoped authority boundary: the `M5DA` v1 codec,
permanent Cell/target-derived key, exact four-state phase shape, complete ten-class writer enrollment, every-successor
revision/predecessor binding, ticket/fence race, CAS-2 dispatch-token construction, fenced takeover and permanent done.
It has no metadata store, Provider/BookKeeper call, real Oxia execution or source-bound receipt; pure candidate values
are not live dispatch or physical-delete authority.

`v2M5TargetDeleteAuthorityCoordinatorCheck` adds one same-key CAS coordinator and a durable writer-ticket guard. Each
mutation reconciles the exact candidate/predecessor/conflict/unknown result by rereading the permanent authority key;
proof-changing work receives its dispatch value only after the ticketed successor is visible, and unknown outcomes
retain the ticket. The nine tests use an in-memory metadata port and call no multi-key transaction. They do not prove
all concrete writers, external deletion, real Oxia, or any source-bound authority.
