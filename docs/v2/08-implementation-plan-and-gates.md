...
| M4 | manifest, protocol-position/timestamp indexes, Storage Epoch resolver, logical Binding read snapshot, allocation-free read-batch pins, active-tail/manifest source generations, bounded two-stage locator/protection retirement, and durable no-fallback cut | Planned | `v2M4Check` |
