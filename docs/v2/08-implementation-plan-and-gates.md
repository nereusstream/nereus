...
| M4 | allocation-free Binding-scoped logical `BindingReadViewSnapshot`; deterministic typed protocol/profile source plan and one-shot pre-observability fallback; bounded generation-tagged hazard slots with ABA-safe terminal drain; proof-window/control-plane integration; M4-owned evidence without M5 physical deletion | **Design in progress. Grills 32-33 read-view authority and source/fallback matrices are accepted and synchronized; Grills 34-35, hard freeze, implementation, and evidence remain open.** | `v2M4Check` |
...
[M4-B typed source plan](detailed_design/m4/m4-b-source-plan-and-fallback.md) start the design boundary without
