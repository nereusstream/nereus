...
| M1 | typed protocol-native incarnations and deterministic IDs; closed aggregate logical schema v1; Kafka level-2 generated TopicImage-owned record; Pulsar permanent generation selector/retired tombstone; deployment registry plus immutable fixed-slice identity/lifecycle/geometry/exhaustion; capability-split metadata SPI; remove superseded V1 API | Planned | `v2M1Check` |
| M2 | Owner Epoch lane, typed frontier contract, BookKeeper foundation, Pulsar deterministic NPD1-data/NPO1-root pair plus ManagedLedger-owned dual-source handle/read pins/final-delete revalidation lifecycle, Kafka ledger-layout scale spike | Planned | `v2M2Check` |
| M3 | one-cell NWG1 Object WAL groups; binding-context epoch authority and commit-set co-location; run-key/per-Object AEAD; content-addressed strong-LIST discovery; provider-absent crash cuts; bounded recovery envelope plus immutable Root/Seal/successor pointer lineage; provider proof/session lifecycle; per-binding frontier; fixed-slice Pulsar virtual-ledger Object path | Planned | `v2M3Check` |
