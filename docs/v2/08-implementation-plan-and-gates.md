...
| M4 | allocation-free Binding-scoped logical `BindingReadViewSnapshot`; deterministic typed protocol/profile source plan and one-shot pre-observability fallback; bounded generation-tagged hazard slots with stable conservative scanning and ABA-safe terminal drain; fused selector/terminal/proof-window/fold/capability control; exact per-source interval verification and protection-generation release CAS; four-child future evidence hierarchy; no physical deletion | **DESIGN HARD-FROZEN / implementation not started. Grills 32-35 and M4-A/B/C/D are accepted; `V2-OPEN-READ-08/09` and physical choices remain implementation/evidence gates, every M4 scenario remains `PLANNED`, and no receipt or Final exists.** | `v2M4DesignCheck`; future `v2M4Check` |
