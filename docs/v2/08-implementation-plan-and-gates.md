...
| M3 | one-cell NWG1 Object WAL groups; binding-context epoch authority and commit-set co-location; run-key/per-Object AEAD; prefix-first derived directory/frame capacity and routine-read proof separation; content-addressed strong-LIST discovery; async checkpoint pages with hard uncovered-tail bounds and mandatory sealed inventory; provider-absent crash cuts; bounded recovery envelope plus one immutable Root/Seal/successor pointer lineage; provider proof/session lifecycle; per-binding frontier; fixed-slice Pulsar virtual-ledger Object path with mass-takeover allocator evidence | Planned | `v2M3Check` |
