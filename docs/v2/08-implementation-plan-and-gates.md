...
M0, M1 and M2 gates are registered, and the current M1/M2 Final evidence is complete. The intermediate focused gates
remain explicitly non-promotable; M3-M8 task names remain future contracts until implemented.
...
[M3-I0 NWG1 input closure](detailed_design/m3/m3-i0-nwg1-implementation-input-closure.md) close only the
documentation/input frontier for NWG1 wire, crypto, negative verification, Object-WAL kernel traces, and capacity
evidence. M3 implementation begins with a current-source M2 regression input gate, then lands production projection /
codec/goldens, mutation runner, state-trace harness, Provider/KMS/control-plane integration, `NWKCP1`, publication,
allocator evidence, and Final aggregation in reviewable slices. No empty task, documentation-only receipt, synthetic
Root fixture, or focused local PASS may promote an M3 scenario.
