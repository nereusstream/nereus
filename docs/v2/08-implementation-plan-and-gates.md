...
| M4 | allocation-free Binding-scoped logical `BindingReadViewSnapshot`; deterministic typed protocol/profile source plan and one-shot pre-observability fallback; bounded generation-tagged hazard slots with stable conservative scanning and ABA-safe terminal drain; proof-window/control-plane integration; M4-owned evidence without M5 physical deletion | **Design in progress. Grills 32-34 read-view, source/fallback, and hazard/reclamation matrices are accepted and synchronized; Grill 35, hard freeze, implementation, and evidence remain open.** | `v2M4Check` |
...
[M4-C hazard/reclamation races](detailed_design/m4/m4-c-hazard-slot-reclamation.md), start the design boundary without
claiming implementation, scenario promotion, receipt, or Final. Intermediate focused gates remain explicitly
non-promotable; M5-M8 task names remain future contracts until implemented.
