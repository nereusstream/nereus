...
| M1 | typed protocol-native incarnations and deterministic IDs; closed aggregate logical schema v1; Kafka API-key-32000 TopicImage record with publication-boundary incremental/full validation; Pulsar selector CAS state machine, cached ACTIVE fence, and retired tombstone; `k=40`/64-KiB/256-assignment registry bootstrap contract; typed policy-scope resolver; capability-split metadata SPI; remove superseded V1 API | Planned | `v2M1Check` |
| M2 | Owner Epoch lane, typed frontier contract, BookKeeper foundation, Pulsar deterministic NPD1-data/NPO1-root pair plus ManagedLedger-owned dual-source handle/read pins/final-delete revalidation and persisted BK_DELETE state/retention policy, Kafka ledger-layout scale spike | Planned | `v2M2Check` |
| M3 | one-cell NWG1 Object WAL groups; binding-context epoch authority and commit-set co-location; run-key/per-Object AEAD; content-addressed strong-LIST discovery; async checkpoint pages with hard uncovered-tail bounds and mandatory sealed inventory; provider-absent crash cuts; bounded recovery envelope plus immutable Root/Seal/successor pointer lineage; provider proof/session lifecycle; per-binding frontier; fixed-slice Pulsar virtual-ledger Object path | Planned | `v2M3Check` |
