...
M2 is `InProgress` only after the K0-M production module graph and non-zero `v2M2KafkaK0ModuleCheck` landed together.
No M2 task may be registered as an empty or zero-test success. The still-unimplemented first aggregate,
`v2M2KafkaInputsCheck`, remains absent and is explicitly non-promotable. The later `v2M2KafkaFinalCheck` is only the Kafka
