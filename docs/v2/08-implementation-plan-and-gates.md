...
| M4 | allocation-free Binding-scoped logical `BindingReadViewSnapshot`; deterministic protocol/profile source-range validation and route/fallback plan; bounded generation-tagged hazard slots with ABA-safe terminal drain; proof-window/control-plane integration; M4-owned evidence without M5 protection-release execution or physical deletion | **Design in progress. Grill 32 read-view authority is accepted and synchronized; Grills 33-35, hard freeze, implementation, and evidence remain open.** | `v2M4Check` |
...
[M4-A read-view authority](detailed_design/m4/m4-a-read-view-authority.md) start the design boundary without claiming
implementation, scenario promotion, receipt, or Final. Intermediate focused gates remain explicitly non-promotable;
M5-M8 task names remain future contracts until implemented.
