...
| T-PROTOCOL-01 | Accepted | Kafka and Pulsar keep protocol-native hot paths and exact native frame bytes | protects Kafka semantics, opaque Pulsar payloads, and ManagedLedger parity | deliberate duplication at protocol boundaries and two checksum layers | ADR 0026, Kafka compatibility matrix, Pulsar native parity, and `V2-OBJ-004` |
| T-POSITION-01 | Accepted | Kafka Offset and Pulsar Position remain separate binding/incarnation-scoped Position Domains; Pulsar Object WAL uses explicit chain authority plus numerically compatible reserved-slice IDs | preserves native semantics and stock MessageId comparison without Object identity becoming position truth | deployment registry, Pulsar native-generator fork, and protocol-specific chain recovery | ADRs 0022/0027, `V2-POSITION-001..003`; reservation fencing, explicit-chain recovery, and reject `ledgerBase + entryId` mapping |
...
| T-OBJECT-01 | Accepted | Object WAL uses bounded group commit, SHA-256/v1 extent identity, CRC32C/v1 exact protocol-native frames, and typed provider proof/full-GET fallback | viable Object request cost with fail-closed stored-byte and opaque protocol-payload integrity | linger latency, SHA-256 plus frame CRC, rare full GET, cross-binding coupling, and recovery scanning | ADRs 0018/0021/0025/0026; per-binding frontier, provider proof, shard budgets, `V2-OBJ-001..004` |
| T-BK-01 | Accepted | BookKeeper ACK never waits for Object; native Pulsar metadata owns sealed-ledger offload and each attempt uses one deterministic data/root Object pair | preserves the performance-first hot path and whole-ledger native lifecycle with bounded cleanup | cold copy may lag one rollover; one data Object limits independent extent parallelism; two-provider GC proof | ADRs 0017/0020/0024 plus `V2-BK-001..004`; rollover bounds, pair verification, lag admission, and derived manifest |
...
| T-META-01 | Accepted | Kafka uses KRaft and Pulsar uses MetadataStore/Oxia; one immutable composite record physically owns binding plus initial epoch | native authority and no half-created/cross-key topic state | two backend integrations and whole-record schema/equality evolution | ADRs 0019/0023, `V2-META-001..002`; atomic Kafka replay, Oxia exact reread, and zero normal-append metadata I/O |
