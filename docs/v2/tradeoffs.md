...
| T-POSITION-01 | Accepted | Kafka Offset and Pulsar Position remain separate binding/incarnation-scoped Position Domains; Pulsar Object WAL uses a cell-owned virtual-ledger authority | preserves native semantics without BookKeeper or Object identity becoming position truth | shared algorithms plus virtual-ledger allocation/chain recovery are protocol-specific | ADR 0022, `V2-POSITION-001..002`; reject cross-domain compare, numeric ledger sorting, and `ledgerBase + entryId` mapping |
...
| T-OBJECT-01 | Accepted | Object WAL uses bounded group commit, capability-tiered uncertain-PUT proof, and distinct extent/payload checksum domains | viable Object request cost with fail-closed stored-byte and decoded-payload integrity | linger latency, two checksum costs, rare full-GET verification, cross-binding coupling, and recovery scanning | ADRs 0018/0021; per-binding typed durable frontier, shard/verification budgets, `V2-OBJ-001..003` |
| T-BK-01 | Accepted | BookKeeper ACK never waits for Object; Pulsar ManagedLedger metadata is sole authority and 0.2 offloads sealed non-current ledgers only | preserves the performance-first hot path and native immutable-ledger lifecycle | cold copy may lag one rollover; protocol-specific offload and two-provider GC proof | ADRs 0017/0020 plus `V2-BK-001..003`; lag admission and derived Pulsar manifest |
...
| T-META-01 | Accepted | Kafka uses KRaft; Pulsar uses MetadataStore/Oxia; binding plus initial epoch have one atomic visible aggregate | native authority and no half-created topic admission | two backend implementations; non-transactional path needs durable intent/recovery | ADR 0019, `V2-META-001..002`; capability-split SPI and no normal-append metadata I/O |
