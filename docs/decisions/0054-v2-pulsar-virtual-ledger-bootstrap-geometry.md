...
numeric space limits one reservation domain. All constants are bootstrap compatibility contracts, not dynamic topic,
tenant, Cell, or host settings.

Exhausting 256 lifetime assignments does not permit a fresh logical domain name to reuse that domain's numeric
interval. Additional assignment capacity requires either:

1. a new reservation domain whose immutable bootstrap evidence assigns a ledger-ID interval disjoint from every domain
   sharing the same Pulsar/native compatibility namespace and proves native allocation excludes it; or
