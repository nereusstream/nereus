...
public final class BookKeeperNamespaceCommand {
    ...
    public static AdminExitCode ensure(AdminConfiguration config, CommandLineArguments args) {
        ...
        try (SharedOxiaClientRuntime runtime =
            ...
            System.err.println("namespace ensure failed: "
    ...
    public static AdminExitCode verify(AdminConfiguration config, CommandLineArguments args) {
        ...
        try (SharedOxiaClientRuntime runtime =
            ...
            BookKeeperLedgerIdNamespaceReservation reservation = admin.verifyNamespace(timeout)
                    .get(timeout.toMillis(), TimeUnit.MILLISECONDS);

            return evaluateVerify(config, reservation, args);
        } catch (Exception e) {
            System.err.println("namespace verify failed: "
                    + AdminFailureClassifier.safeMessage(e));
            return AdminFailureClassifier.classify(
                    e, AdminExitCode.CONDITION_FAILED);
        }
    }

    static AdminExitCode evaluateEnsure(
            BookKeeperLedgerIdNamespaceReservation reservation,
            CommandLineArguments args) {
        String evidence = AdminEvidenceWriter.namespaceReservationEvidence(
                "bookkeeper namespace ensure", reservation);
    ...
    static AdminExitCode evaluateVerify(
        ...
        if (!reservation.operatorEvidenceSha256().equals(
                config.operatorEvidenceSha256())) {
            System.err.println("namespace operator evidence does not match configuration");
            return AdminExitCode.CONDITION_FAILED;
        }

        String evidence = AdminEvidenceWriter.namespaceReservationEvidence(
                "bookkeeper namespace verify", reservation);
        writeOutput(args.outputFile(), evidence);
