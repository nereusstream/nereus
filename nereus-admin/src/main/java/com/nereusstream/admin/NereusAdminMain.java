    ...
    static AdminExitCode run(String[] rawArgs) {
        ...
        try {
            return switch (args.command()) {
                ...
                default -> throw new IllegalStateException("parser admitted an unsupported command: " + args.command());
            };
        } catch (RuntimeException e) {
            System.err.println("unexpected error: " + AdminFailureClassifier.safeMessage(e));
            return AdminExitCode.INTERNAL_ERROR;
        }
    }

    private static AdminExitCode dispatchBookKeeper(AdminConfiguration config, CommandLineArguments args) {
        return switch (args.subcommand()) {
            ...
            default -> throw new IllegalStateException("parser admitted an unsupported BookKeeper command");
        };
    }

    private static AdminExitCode dispatchBookKeeperNamespace(AdminConfiguration config, CommandLineArguments args) {
        return switch (args.action().orElseThrow()) {
            ...
            default -> throw new IllegalStateException("parser admitted an unsupported namespace action");
        };
    }

    private static AdminExitCode dispatchObjectStore(AdminConfiguration config, CommandLineArguments args) {
        return switch (args.subcommand()) {
            ...
            default -> throw new IllegalStateException("parser admitted an unsupported object-store command");
        };
    }

    private static AdminExitCode dispatchObjectStorePersistence(AdminConfiguration config, CommandLineArguments args) {
        return switch (args.action().orElseThrow()) {
            ...
            default ->
                throw new IllegalStateException("parser admitted an unsupported object-store persistence action");
        };
    }
}
