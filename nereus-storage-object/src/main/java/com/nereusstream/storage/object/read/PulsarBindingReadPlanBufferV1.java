...
/** Caller-owned bounded typed Pulsar plan storage. */
public final class PulsarBindingReadPlanBufferV1 {
    ...
    public PulsarBindingReadPlanBufferV1(int capacity) {
        if (capacity <= 0 || capacity > 256) {
            throw new IllegalArgumentException("Pulsar read plan capacity must be in [1,256]");
        }
    ...
    public long startEntryIdInclusive(int index) {
        checkIndex(index);
        return starts[index];
    }

    public long endEntryIdExclusive(int index) {
        checkIndex(index);
        return ends[index];
    }
