...
/** Canonical NAEV4 evaluation derived only from one complete, validator-reproved NACP4 checkpoint. */
public final class AllocatorCampaignEvaluationSealV4 {
    public static final String SCHEMA = "NEREUS_V2_M3_ALLOCATOR_EVALUATION_V4";
    private static final byte[] MAGIC = {'N', 'A', 'E', 'V', '4', 0, 0, 0};
    private static final int WIRE_VERSION = 4;
    private static final int FIXED_LENGTH = 8 + 2 + 1 + 1 + 4 + 4 + 40 + 32 * 9;

    private AllocatorCampaignEvaluationSealV4() {}
    ...
    public static CanonicalBytes seal(CanonicalBytes checkpointBytes) {
        ...
        if (checkpoint.status() != AllocatorCampaignCheckpointV3.Status.COMPLETED) {
            throw invalid("allocator V4 interrupted or infrastructure-failed campaign cannot produce an evaluation");
        }
        ...
        return encode(new SealedEvaluation(
                ...
                AllocatorCampaignCheckpointV4.digest(checkpointBytes),
    ...
    public static CanonicalBytes encode(SealedEvaluation evaluation) {
        ...
        try {
            ...
            if (bytes.size() != FIXED_LENGTH) {
                throw new IllegalStateException("allocator V4 fixed evaluation encoder length differs");
            }
            return CanonicalBytes.copyOf(bytes.toByteArray());
        } catch (IOException failure) {
            throw new IllegalStateException("allocator V4 evaluation memory encoding failed", failure);
        }
    }
    ...
    public static SealedEvaluation decode(CanonicalBytes encoded) {
        ...
        if (bytes.length != FIXED_LENGTH) {
            throw invalid("allocator V4 evaluation length differs from fixed NAEV4");
        }
        try {
            DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes));
            if (!Arrays.equals(input.readNBytes(MAGIC.length), MAGIC) || input.readUnsignedShort() != WIRE_VERSION) {
                throw invalid("allocator V4 evaluation magic or version differs");
            }
            ...
            if (commit.length != 40) {
                throw new EOFException("allocator V4 evaluation commit is truncated");
            }
            ...
            if (input.available() != 0
                ...
                throw invalid("allocator V4 evaluation is not canonical NAEV4");
            }
            return evaluation;
        } catch (EOFException failure) {
            throw invalid("allocator V4 evaluation is truncated", failure);
        } catch (IOException failure) {
            throw invalid("allocator V4 evaluation cannot be decoded", failure);
        }
    }

    static Sha256Digest attachmentRoot(AllocatorCampaignCheckpointV4 checkpoint) {
        try {
            ByteArrayOutputStream bytes = new ByteArrayOutputStream();
            DataOutputStream output = new DataOutputStream(bytes);
            output.write("NEREUS-V2-M3-ALLOCATOR-ATTACHMENT-ROOT-V4".getBytes(StandardCharsets.US_ASCII));
            ...
            throw new IllegalStateException("allocator V4 attachment root encoding failed", failure);
        }
    }
    ...
    private static Sha256Digest readDigest(DataInputStream input) throws IOException {
        ...
        if (bytes.length != Sha256Digest.LENGTH) {
            throw new EOFException("allocator V4 evaluation digest is truncated");
        }
    ...
    private static <T> T enumValue(T[] values, int ordinal, String label) {
        if (ordinal < 0 || ordinal >= values.length) {
            throw invalid("allocator V4 " + label + " ordinal differs");
        }
    ...
    public record SealedEvaluation(
        ...
        public SealedEvaluation {
            ...
            if (!executionProfileDigest.equals(AllocatorNativeExecutionProfileV4.executionProfileDigest())
                ...
                throw invalid("allocator V4 sealed evaluation accounting, profile, or selection differs");
            }
        }
