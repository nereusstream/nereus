...
/** Separates valid V2 evaluation from exact-source promotion and keeps NONE/BOTH as non-failing decisions. */
public final class AllocatorCampaignPromotionGateV2 {
    public static final String DIAGNOSTIC_SCHEMA = "NEREUS_V2_M3_ALLOCATOR_DIAGNOSTIC_V2";
    private static final byte[] DIAGNOSTIC_MAGIC = {'N', 'A', 'D', 'V', '2', 0, 0, 0};
    private static final int DIAGNOSTIC_VERSION = 2;
    private static final int DIAGNOSTIC_LENGTH = 8 + 2 + 1 + 1 + 40 + 32 * 5;

    private AllocatorCampaignPromotionGateV2() {}
    ...
    public static Decision evaluate(
        ...
        AllocatorCampaignEvaluationSealV2.SealedEvaluation evaluation =
                AllocatorCampaignEvaluationSealV2.decode(evaluationBytes);
        ...
        CanonicalBytes expectedEvaluation = AllocatorCampaignEvaluationSealV2.seal(checkpointBytes);
        if (!evaluationBytes.equals(expectedEvaluation)
                || !evaluation.checkpointDigest().equals(AllocatorCampaignCheckpointV2.digest(checkpointBytes))
                || !evaluation.campaignId().equals(checkpoint.campaignId())
                || !evaluation
                        .attachmentRootDigest()
                        .equals(AllocatorCampaignEvaluationSealV2.attachmentRoot(checkpoint))) {
            return Decision.rejected(DecisionStatus.CHECKPOINT_LINK_INVALID);
        }
    ...
    public static CanonicalBytes encodeDiagnostic(DiagnosticAttestation diagnostic) {
        ...
        try {
            ...
            if (bytes.size() != DIAGNOSTIC_LENGTH) {
                throw new IllegalStateException("allocator V2 diagnostic encoder length differs");
            }
            return CanonicalBytes.copyOf(bytes.toByteArray());
        } catch (IOException failure) {
            throw new IllegalStateException("allocator V2 diagnostic memory encoding failed", failure);
        }
    }
    ...
    public static DiagnosticAttestation decodeDiagnostic(CanonicalBytes encoded) {
        ...
        if (bytes.length != DIAGNOSTIC_LENGTH) {
            throw new IllegalArgumentException("allocator V2 diagnostic length differs from fixed NADV2");
        }
        try {
            ...
            if (!Arrays.equals(input.readNBytes(DIAGNOSTIC_MAGIC.length), DIAGNOSTIC_MAGIC)
                ...
                throw new IllegalArgumentException("allocator V2 diagnostic magic or version differs");
            }
            ...
            if ((scenarioMask & ~allowedMask) != 0 || input.readUnsignedByte() != 0) {
                throw new IllegalArgumentException("allocator V2 diagnostic mask or reserved byte differs");
            }
            ...
            if (commit.length != 40) {
                throw new EOFException("allocator V2 diagnostic commit is truncated");
            }
            ...
            if (input.available() != 0
                ...
                throw new IllegalArgumentException("allocator V2 diagnostic is not canonical NADV2");
            }
            return diagnostic;
        } catch (EOFException failure) {
            throw new IllegalArgumentException("allocator V2 diagnostic is truncated", failure);
        } catch (IOException failure) {
            throw new IllegalArgumentException("allocator V2 diagnostic cannot be decoded", failure);
        }
    }
    ...
    private static Sha256Digest readDigest(DataInputStream input) throws IOException {
        ...
        if (bytes.length != Sha256Digest.LENGTH) {
            throw new EOFException("allocator V2 diagnostic digest is truncated");
        }
    ...
    public record JUnitSummary(long tests, long failures, long errors, long skips) {
        public JUnitSummary {
            if (tests < 0 || failures < 0 || errors < 0 || skips < 0) {
                throw new IllegalArgumentException("allocator V2 JUnit summary cannot be negative");
            }
        }
    }
    ...
    public record Decision(DecisionStatus status, Optional<Candidate> selectedCandidate) {
        public Decision {
            ...
            if ((status == DecisionStatus.PROMOTABLE) != selectedCandidate.isPresent()) {
                throw new IllegalArgumentException("allocator V2 promotion decision selection differs");
            }
        }
