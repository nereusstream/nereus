...
/** Strict canonical NARS3 created only from a successful V3 promotion check. */
public final class AllocatorCampaignSelectionV3 {
    public static final String SCHEMA = "NEREUS_V2_M3_ALLOCATOR_SELECTION_V3";
    private static final byte[] MAGIC = {'N', 'A', 'R', 'S', '3', 0, 0, 0};
    private static final int WIRE_VERSION = 3;
    private static final int FIXED_LENGTH = 8 + 2 + 1 + 1 + 40 + 32 * 11;

    private AllocatorCampaignSelectionV3() {}
    ...
    public static CanonicalBytes seal(
        ...
        if (decision.status() != DecisionStatus.PROMOTABLE) {
            throw invalid("allocator V3 selection cannot seal a non-promotable decision");
        }
        ...
        Selection selection = new Selection(
                ...
                Sha256Digest.hash(AllocatorCampaignPromotionGateV3.encodeDiagnostic(diagnostic)),
                verifiedDiagnosticReceiptDigest,
                junitDigest(junit));
    ...
    public static CanonicalBytes encode(Selection selection) {
        ...
        try {
            ...
            if (bytes.size() != FIXED_LENGTH) {
                throw new IllegalStateException("allocator V3 fixed selection encoder length differs");
            }
            return CanonicalBytes.copyOf(bytes.toByteArray());
        } catch (IOException failure) {
            throw new IllegalStateException("allocator V3 selection memory encoding failed", failure);
        }
    }
    ...
    public static Selection decode(CanonicalBytes encoded) {
        ...
        if (bytes.length != FIXED_LENGTH) {
            throw invalid("allocator V3 selection length differs from fixed NARS3");
        }
        try {
            DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes));
            if (!Arrays.equals(input.readNBytes(MAGIC.length), MAGIC) || input.readUnsignedShort() != WIRE_VERSION) {
                throw invalid("allocator V3 selection magic or version differs");
            }
            Candidate selected = enumValue(Candidate.values(), input.readUnsignedByte());
            if (input.readUnsignedByte() != 0) {
                throw invalid("allocator V3 selection reserved byte is nonzero");
            }
            byte[] commit = input.readNBytes(40);
            if (commit.length != 40) {
                throw new EOFException("allocator V3 selection commit is truncated");
            }
            ...
            throw invalid("allocator V3 selection is truncated", failure);
        } catch (IOException failure) {
            throw invalid("allocator V3 selection cannot be decoded", failure);
        }
    }
    ...
    private static Sha256Digest junitDigest(JUnitSummary junit) {
        try {
            ...
            DataOutputStream output = new DataOutputStream(bytes);
            output.write("NEREUS-V2-M3-ALLOCATOR-JUNIT-SUMMARY-V3".getBytes(StandardCharsets.US_ASCII));
            ...
            throw new IllegalStateException("allocator V3 JUnit summary encoding failed", failure);
        }
    }
    ...
    private static Sha256Digest readDigest(DataInputStream input) throws IOException {
        ...
        if (bytes.length != Sha256Digest.LENGTH) {
            throw new EOFException("allocator V3 selection digest is truncated");
        }
    ...
    private static Candidate enumValue(Candidate[] values, int ordinal) {
        if (ordinal < 0 || ordinal >= values.length) {
            throw invalid("allocator V3 selected candidate ordinal differs");
        }
    ...
    public record Selection(
        ...
        public Selection {
            ...
            if (selectedCandidate.nativePath()) {
                throw invalid("allocator V3 selection cannot select the native baseline");
            }
            for (Sha256Digest digest : List.of(
                ...
                if (Objects.requireNonNull(digest, "digest").isZero()) {
                    throw invalid("allocator V3 selection digest is zero");
                }
            }
        }
    }
