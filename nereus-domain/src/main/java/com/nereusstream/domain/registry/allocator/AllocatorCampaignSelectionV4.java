...
/** Strict canonical NARS4 created only from a successful V4 promotion decision. */
public final class AllocatorCampaignSelectionV4 {
    public static final String SCHEMA = "NEREUS_V2_M3_ALLOCATOR_SELECTION_V4";
    private static final byte[] MAGIC = {'N', 'A', 'R', 'S', '4', 0, 0, 0};
    private static final int WIRE_VERSION = 4;
    private static final int FIXED_LENGTH = 8 + 2 + 1 + 1 + 40 + 32 * 13;

    private AllocatorCampaignSelectionV4() {}
    ...
    public static CanonicalBytes seal(
        ...
        if (decision.status() != DecisionStatus.PROMOTABLE) {
            throw invalid("allocator V4 selection cannot seal a non-promotable decision");
        }
        ...
        return encode(new Selection(
                ...
                Sha256Digest.hash(AllocatorCampaignPromotionGateV4.encodeDiagnostic(diagnostic)),
                verifiedDiagnosticReceiptDigest,
                junitDigest(junit)));
    }
    ...
    public static CanonicalBytes encode(Selection selection) {
        ...
        try {
            ...
            if (bytes.size() != FIXED_LENGTH) {
                throw new IllegalStateException("allocator V4 fixed selection encoder length differs");
            }
            return CanonicalBytes.copyOf(bytes.toByteArray());
        } catch (IOException failure) {
            throw new IllegalStateException("allocator V4 selection memory encoding failed", failure);
        }
    }
    ...
    public static Selection decode(CanonicalBytes encoded) {
        ...
        if (bytes.length != FIXED_LENGTH) {
            throw invalid("allocator V4 selection length differs from fixed NARS4");
        }
        try {
            DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes));
            if (!Arrays.equals(input.readNBytes(MAGIC.length), MAGIC) || input.readUnsignedShort() != WIRE_VERSION) {
                throw invalid("allocator V4 selection magic or version differs");
            }
            Candidate selected = enumValue(Candidate.values(), input.readUnsignedByte());
            if (input.readUnsignedByte() != 0) {
                throw invalid("allocator V4 selection reserved byte is nonzero");
            }
            byte[] commit = input.readNBytes(40);
            if (commit.length != 40) {
                throw new EOFException("allocator V4 selection commit is truncated");
            }
            ...
            if (input.available() != 0
                ...
                throw invalid("allocator V4 selection is not canonical NARS4");
            }
            return selection;
        } catch (EOFException failure) {
            throw invalid("allocator V4 selection is truncated", failure);
        } catch (IOException failure) {
            throw invalid("allocator V4 selection cannot be decoded", failure);
        }
    }
    ...
    private static Sha256Digest junitDigest(JUnitSummary junit) {
        try {
            ...
            DataOutputStream output = new DataOutputStream(bytes);
            output.write("NEREUS-V2-M3-ALLOCATOR-JUNIT-SUMMARY-V4".getBytes(StandardCharsets.US_ASCII));
            ...
            throw new IllegalStateException("allocator V4 JUnit summary encoding failed", failure);
        }
    }
    ...
    private static Sha256Digest readDigest(DataInputStream input) throws IOException {
        ...
        if (bytes.length != Sha256Digest.LENGTH) {
            throw new EOFException("allocator V4 selection digest is truncated");
        }
    ...
    private static Candidate enumValue(Candidate[] values, int ordinal) {
        if (ordinal < 0 || ordinal >= values.length) {
            throw invalid("allocator V4 selected candidate ordinal differs");
        }
    ...
    public record Selection(
        ...
        public Selection {
            ...
            if (!executionProfileDigest.equals(AllocatorNativeExecutionProfileV4.executionProfileDigest())
                ...
                throw invalid("allocator V4 selection profile, plan, or candidate differs");
            }
            for (Sha256Digest digest : List.of(
                ...
                if (Objects.requireNonNull(digest, "digest").isZero()) {
                    throw invalid("allocator V4 selection digest is zero");
                }
            }
        }
    }
