...
/** Canonical NAEV3 evaluation derived only from one complete, validator-reproved NACP3 checkpoint. */
public final class AllocatorCampaignEvaluationSealV3 {
    public static final String SCHEMA = "NEREUS_V2_M3_ALLOCATOR_EVALUATION_V3";
    private static final byte[] MAGIC = {'N', 'A', 'E', 'V', '3', 0, 0, 0};
    private static final int WIRE_VERSION = 3;
    private static final int FIXED_LENGTH = 8 + 2 + 1 + 1 + 4 + 4 + 40 + 32 * 7;

    private AllocatorCampaignEvaluationSealV3() {}
    ...
    public static CanonicalBytes seal(CanonicalBytes checkpointBytes) {
        ...
        if (checkpoint.status() != AllocatorCampaignCheckpointV3.Status.COMPLETED) {
            throw invalid("allocator V3 interrupted or infrastructure-failed campaign cannot produce an evaluation");
        }
        ...
        SealedEvaluation sealed = new SealedEvaluation(
                ...
                AllocatorCampaignCheckpointV3.digest(checkpointBytes),
    ...
    public static CanonicalBytes encode(SealedEvaluation evaluation) {
        ...
        try {
            ...
            if (bytes.size() != FIXED_LENGTH) {
                throw new IllegalStateException("allocator V3 fixed evaluation encoder length differs");
            }
            return CanonicalBytes.copyOf(bytes.toByteArray());
        } catch (IOException failure) {
            throw new IllegalStateException("allocator V3 evaluation memory encoding failed", failure);
        }
    }
    ...
    public static SealedEvaluation decode(CanonicalBytes encoded) {
        ...
        if (bytes.length != FIXED_LENGTH) {
            throw invalid("allocator V3 evaluation length differs from fixed NAEV3");
        }
        try {
            DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes));
            if (!Arrays.equals(input.readNBytes(MAGIC.length), MAGIC) || input.readUnsignedShort() != WIRE_VERSION) {
                throw invalid("allocator V3 evaluation magic or version differs");
            }
            ...
            if (commitBytes.length != 40) {
                throw new EOFException("allocator V3 evaluation commit is truncated");
            }
            ...
            if (input.available() != 0
                ...
                throw invalid("allocator V3 evaluation is not canonical NAEV3");
            }
            return evaluation;
        } catch (EOFException failure) {
            throw invalid("allocator V3 evaluation is truncated", failure);
        } catch (IOException failure) {
            throw invalid("allocator V3 evaluation cannot be decoded", failure);
        }
    }

    static Sha256Digest attachmentRoot(AllocatorCampaignCheckpointV3 checkpoint) {
        try {
            ByteArrayOutputStream bytes = new ByteArrayOutputStream();
            DataOutputStream output = new DataOutputStream(bytes);
            output.write("NEREUS-V2-M3-ALLOCATOR-ATTACHMENT-ROOT-V3".getBytes(StandardCharsets.US_ASCII));
            ...
            throw new IllegalStateException("allocator V3 attachment root encoding failed", failure);
        }
    }
    ...
    private static Sha256Digest readDigest(DataInputStream input) throws IOException {
        ...
        if (bytes.length != Sha256Digest.LENGTH) {
            throw new EOFException("allocator V3 evaluation digest is truncated");
        }
    ...
    public record SealedEvaluation(
        ...
        public SealedEvaluation {
            ...
            if (campaignId.isZero()
                    ...
                    || dispositionCells < 0
                    || executedPerformanceCells > AllocatorCampaignV3.LOGICAL_PERFORMANCE_CELLS
                    || dispositionCells > AllocatorCampaignV3.LOGICAL_PERFORMANCE_CELLS
                ...
                throw invalid("allocator V3 sealed evaluation accounting or selection differs");
            }
        }
