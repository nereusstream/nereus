...
/**
 * Strict NACP4 authority. The nested NACP3 is an implementation-private logical-algebra payload; V4 lineage,
 * campaign identity, plan/profile identity, and the 42-second interval budget are independently canonicalized here.
 */
public final class AllocatorCampaignCheckpointV4 {
    public static final String SCHEMA = "NEREUS_V2_M3_ALLOCATOR_CHECKPOINT_V4";
    public static final int MAX_ENCODED_BYTES = AllocatorCampaignCheckpointV3.MAX_ENCODED_BYTES + 512;
    private static final byte[] MAGIC = {'N', 'A', 'C', 'P', '4', 0, 0, 0};
    ...
    private static final long V4_INTERVAL_BUDGET = 13_776;
    private static final long V4_INTERVAL_CHARGE = 42;
    ...
    private AllocatorCampaignCheckpointV4(
        ...
        if (!executionProfileDigest.equals(AllocatorNativeExecutionProfileV4.executionProfileDigest())
            ...
            throw invalid("allocator V4 checkpoint profile or plan identity differs");
        }
        if (!campaignId.equals(campaignId(logicalCheckpoint.source(), logicalCheckpoint.campaignId()))) {
            throw invalid("allocator V4 campaign identity differs from its source/profile tuple");
        }
    ...
    public static AllocatorCampaignCheckpointV4 initial(CanonicalBytes logicalCheckpointBytes) {
        ...
        if (logical.checkpointSequence() != 0) {
            throw invalid("allocator V4 initial checkpoint sequence differs");
        }
        return create(logicalCheckpointBytes, ZERO_DIGEST);
    }

    public static AllocatorCampaignCheckpointV4 resume(
            CanonicalBytes predecessorBytes, CanonicalBytes logicalCheckpointBytes) {
        AllocatorCampaignCheckpointV4 predecessor = decode(predecessorBytes);
        AllocatorCampaignCheckpointV3 logical = AllocatorCampaignCheckpointV3.decode(logicalCheckpointBytes);
        if (logical.checkpointSequence() != Math.addExact(predecessor.checkpointSequence(), 1)
            ...
            throw invalid("allocator V4 resume changed its logical source, sequence, or nested lineage");
        }
        return create(logicalCheckpointBytes, digest(predecessorBytes));
    }

    private static AllocatorCampaignCheckpointV4 create(
            CanonicalBytes logicalCheckpointBytes, Sha256Digest predecessorDigest) {
        AllocatorCampaignCheckpointV3 logical = AllocatorCampaignCheckpointV3.decode(logicalCheckpointBytes);
        return new AllocatorCampaignCheckpointV4(
    ...
    public static CanonicalBytes encode(AllocatorCampaignCheckpointV4 checkpoint) {
        ...
        try {
            ...
            if (bytes.size() > MAX_ENCODED_BYTES) {
                throw invalid("allocator V4 checkpoint exceeds its parser cap");
            }
            return CanonicalBytes.copyOf(bytes.toByteArray());
        } catch (IOException failure) {
            throw new IllegalStateException("allocator V4 checkpoint memory encoding failed", failure);
        }
    }

    public static AllocatorCampaignCheckpointV4 decode(CanonicalBytes encoded) {
        Objects.requireNonNull(encoded, "encoded");
        byte[] bytes = encoded.toByteArray();
        if (bytes.length == 0 || bytes.length > MAX_ENCODED_BYTES) {
            throw invalid("allocator V4 checkpoint length is outside its parser cap");
        }
        try {
            DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes));
            if (!Arrays.equals(input.readNBytes(MAGIC.length), MAGIC) || input.readUnsignedShort() != WIRE_VERSION) {
                throw invalid("allocator V4 checkpoint magic or version differs");
            }
            int statusOrdinal = input.readUnsignedByte();
            if (statusOrdinal >= Status.values().length || input.readUnsignedByte() != 0) {
                throw invalid("allocator V4 checkpoint status or reserved byte differs");
            }
            ...
            if (logicalLength <= 0 || logicalLength > AllocatorCampaignCheckpointV3.MAX_ENCODED_BYTES) {
                throw invalid("allocator V4 nested logical checkpoint length differs");
            }
            byte[] logical = input.readNBytes(logicalLength);
            if (logical.length != logicalLength || input.available() != 0) {
                throw invalid("allocator V4 checkpoint is truncated or has trailing bytes");
            }
            AllocatorCampaignCheckpointV4 checkpoint = new AllocatorCampaignCheckpointV4(
            ...
            if (checkpoint.status().ordinal() != statusOrdinal
                ...
                throw invalid("allocator V4 checkpoint is not canonical NACP4");
            }
            return checkpoint;
        } catch (EOFException failure) {
            throw invalid("allocator V4 checkpoint is truncated", failure);
        } catch (IOException failure) {
            throw invalid("allocator V4 checkpoint cannot be decoded", failure);
        }
    }
    ...
    private static RemainingBudgets deriveBudgets(AllocatorCampaignCheckpointV3 logical) {
        ...
        if (consumedV3 < 0 || consumedV3 % V3_INTERVAL_CHARGE != 0) {
            throw invalid("allocator V4 nested interval budget does not map to complete physical actions");
        }
        long intervalActions = consumedV3 / V3_INTERVAL_CHARGE;
        long remainingV4 =
                Math.subtractExact(V4_INTERVAL_BUDGET, Math.multiplyExact(intervalActions, V4_INTERVAL_CHARGE));
    ...
    private static Sha256Digest campaignId(SourceBinding source, Sha256Digest logicalCampaignId) {
        try {
            ByteArrayOutputStream bytes = new ByteArrayOutputStream();
            DataOutputStream output = new DataOutputStream(bytes);
            output.write("NEREUS-V2-M3-ALLOCATOR-CAMPAIGN-ID-V4".getBytes(StandardCharsets.US_ASCII));
            ...
            writeDigest(output, AllocatorNativeExecutionProfileV4.executionProfileDigest());
            writeDigest(output, AllocatorCampaignPlanProfileV4.zeroDecisionPlanDigest());
            writeDigest(output, logicalCampaignId);
            output.flush();
            return Sha256Digest.hash(CanonicalBytes.copyOf(bytes.toByteArray()));
        } catch (IOException failure) {
            throw new IllegalStateException("allocator V4 campaign identity encoding failed", failure);
        }
    }
    ...
    private static Sha256Digest readDigest(DataInputStream input) throws IOException {
        ...
        if (bytes.length != Sha256Digest.LENGTH) {
            throw new EOFException("allocator V4 checkpoint digest is truncated");
        }
    ...
    public record RemainingBudgets(
        ...
        public RemainingBudgets {
            if (setupSeconds < 0
                    ...
                    || faultSeconds > 7_200
                    || scaleSeconds > 5_400
                    || intervalSeconds > V4_INTERVAL_BUDGET
                    || cleanupSeconds > 1_640
                    || checkpointAndSealSeconds > 600) {
                throw invalid("allocator V4 remaining phase budget is outside its frozen bound");
            }
        }
