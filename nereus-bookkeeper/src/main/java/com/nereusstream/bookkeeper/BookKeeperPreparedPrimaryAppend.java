...
public final class BookKeeperPreparedPrimaryAppend implements PreparedPrimaryAppend, AutoCloseable {
    ...
    public BookKeeperPreparedPrimaryAppend(PrimaryAppendRequest request) {
        ...
        try {
            for (ByteBuf entry : entries) {
                total = Math.addExact(total, entry.readableBytes());
            }
    ...
    public PrimaryAppendRequest request() {
        return request;
    }
    ...
    public BookKeeperEntryMapping entryMapping() {
        return entryMapping;
    }
    ...
    public int recordCount() {
        return request.batch().recordCount();
    }
    ...
    public long logicalBytes() {
        return request.batch().entries().stream()
                .mapToLong(entry -> entry.payload().length)
    ...
    private void requireOpen() {
        if (closed.get()) {
            throw new IllegalStateException("prepared BookKeeper append is closed");
        }
    }
}
