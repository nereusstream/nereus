...
public final class BookKeeperReaderLeaseManager {
    public final class Lease implements AutoCloseable {
        ...
        public CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> revalidate() {
            ...
            return CompletableFuture.allOf(leaseFuture, rootFuture).thenApply(ignored -> {
                BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord> current = leaseFuture
                        .join()
                ...
                BookKeeperVersionedValue<BookKeeperLedgerRootRecord> root = rootFuture
                        .join()
    ...
    public CompletableFuture<Lease> claim(BookKeeperVersionedValue<BookKeeperLedgerRootRecord> root, Duration timeout) {
    ...
    private CompletableFuture<Lease> claimLocal(
            BookKeeperVersionedValue<BookKeeperLedgerRootRecord> root, BookKeeperOperationDeadline deadline) {
        SharedLease shared;
        ...
        synchronized (this) {
            ...
            if (releaseInProgress == null) {
                if (shared == null) {
                    ...
                    shared.serial = recoverSerial(shared).thenCompose(value -> renewIfRequired(value, deadline));
                }
                shared.references++;
        ...
        return Objects.requireNonNull(claim, "BookKeeper local reader claim")
                ...
                .whenComplete((ignored, failure) -> {
                    if (failure != null) {
                        releaseFailedClaim(exactShared);
                    }
                });
    }
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord>> claimSlot(
        ...
        if (attempt >= configuration.maxReaderLeasesPerLedger()) {
            return CompletableFuture.failedFuture(new NereusException(
                    ErrorCode.BACKPRESSURE_REJECTED, true, "BookKeeper ledger reader lease slots are exhausted"));
        }
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord>> create(
            BookKeeperVersionedValue<BookKeeperLedgerRootRecord> root, int slot) {
        ...
        BookKeeperLedgerReaderLeaseRecord desired = new BookKeeperLedgerReaderLeaseRecord(
                ...
                0);
        return metadata.createReaderLease(cluster, configuration.providerScopeSha256(), desired);
    }

    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord>> renewIfRequired(
            BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord> current, BookKeeperOperationDeadline deadline) {
        if (!current.value().processRunId().equals(processRunId)) {
        ...
        return deadline.bound(metadata.compareAndSetReaderLease(
                cluster, configuration.providerScopeSha256(), renewed, current.metadataVersion()));
    }
    ...
    private CompletableFuture<Void> deleteOwned(
            BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord> expected, int attempts) {
        return metadata.deleteReaderLease(
                ...
                .thenCompose(current -> {
                    ...
                    return CompletableFuture.failedFuture(
                            invariant("BookKeeper reader lease release did not converge"));
                });
    }
    ...
    private synchronized BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord> remember(
            SharedLease shared, BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord> value) {
    ...
    private synchronized BookKeeperVersionedValue<BookKeeperLedgerReaderLeaseRecord> currentOrNull(SharedLease shared) {
    ...
    private boolean sameOwner(BookKeeperLedgerReaderLeaseRecord expected, BookKeeperLedgerReaderLeaseRecord current) {
    ...
    private static boolean sameRoot(BookKeeperLedgerRootRecord root, BookKeeperLedgerReaderLeaseRecord lease) {
    ...
    private static void requirePinnedReadableRoot(
            BookKeeperLedgerRootRecord root, BookKeeperLedgerReaderLeaseRecord lease) {
