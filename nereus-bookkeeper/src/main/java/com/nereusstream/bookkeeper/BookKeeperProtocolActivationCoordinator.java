...
public final class BookKeeperProtocolActivationCoordinator {
    ...
    public BookKeeperProtocolActivationCoordinator(BookKeeperProtocolActivationStore store, Clock clock) {
    ...
    public CompletableFuture<BookKeeperProtocolActivation> prepare(
        ...
        BookKeeperProtocolActivationValue desired = BookKeeperProtocolActivationValue.prepared(
                exact, reservation, brokerReadinessEpoch, brokerReadinessSha256);
        return store.read(exact, reservation, deadline.remaining()).thenCompose(existing -> {
            ...
            if (current.value().equals(desired)
                    || current.value().lifecycle() == BookKeeperProtocolActivationLifecycle.ACTIVE) {
                return CompletableFuture.completedFuture(current);
            }
            return CompletableFuture.failedFuture(
                    condition("BookKeeper activation is already prepared with different readiness"));
        });
    }
    ...
    public CompletableFuture<BookKeeperProtocolActivation> activate(
        ...
        return store.read(exact, reservation, deadline.remaining()).thenCompose(existing -> {
            BookKeeperProtocolActivation current =
                    existing.orElseThrow(() -> condition("BookKeeper activation must be prepared first"));
            requireExact(current.value(), exact, reservation);
            if (current.metadataVersion() != requested.expectedMetadataVersion()) {
                return CompletableFuture.failedFuture(condition("BookKeeper activation metadata version mismatch"));
            }
            long activatedAt =
                    Math.max(Math.max(clock.millis(), 1), current.value().activatedAtMillis());
            BookKeeperProtocolActivationValue replacement = new BookKeeperProtocolActivationValue(
                    1,
                    ...
                    activatedAt);
            BookKeeperProtocolActivationValue.requireValidReplacement(current.value(), replacement);
            return store.compareAndSet(replacement, requested.expectedMetadataVersion(), deadline.remaining());
        });
    }

    private static BookKeeperLedgerIdNamespaceReservation requireNamespace(
            BookKeeperLedgerIdNamespaceReservation namespace, BookKeeperWalConfiguration configuration) {
        BookKeeperLedgerIdNamespaceReservation exact = Objects.requireNonNull(namespace, "namespace");
        if (exact.lifecycle() != BookKeeperLedgerIdNamespaceReservation.Lifecycle.ACTIVE
                || !exact.reservationId().equals(configuration.ledgerIdNamespaceReservationId())
                || !exact.clusterAlias().equals(configuration.clusterAlias())
                || !exact.bookKeeperProviderScopeSha256().equals(configuration.providerScopeSha256())
