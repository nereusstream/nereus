...
public final class BookKeeperPrimaryWalReader implements PrimaryWalReader {
    public static final ReadTargetReaderKey KEY = new ReadTargetReaderKey(
            com.nereusstream.api.target.ReadTargetType.BOOKKEEPER_ENTRY_RANGE, 1, Optional.empty(), Optional.empty());
    ...
    public long reservationBytes(ResolvedRange range) {
        ...
        return target.entryMapping() == BookKeeperEntryMapping.RANGED_NEREUS_ENTRY_V1
                ? BookKeeperRangedEntryCodecV1.encodedRangeBytes(exact.logicalBytes(), exact.entryCount())
                : exact.logicalBytes();
    }
    ...
    public CompletableFuture<PhysicalReadResult> readPhysicalWithStats(
        ...
        long reservedBytes =
                exactRanges.stream().mapToLong(this::reservationBytes).reduce(0, Math::addExact);
        ReadPermit permit = acquirePermit(reservedBytes);
        BookKeeperOperationDeadline deadline =
                new BookKeeperOperationDeadline(min(exactOptions.timeout(), configuration.operationTimeout()));
    ...
    private CompletableFuture<Void> readNext(
            List<ResolvedRange> ranges, int index, ReadAccumulator accumulator, BookKeeperOperationDeadline deadline) {
        ...
        return readAndVerify(accumulator.stream, range, target, deadline).thenCompose(verified -> {
    ...
    private CompletableFuture<VerifiedRange> readAndVerify(
        ...
        return deadline.bound(metadata.getRoot(cluster, configuration.providerScopeSha256(), target.ledgerId()))
                .thenApply(optional -> optional.orElseThrow(() -> new NereusException(
                        ErrorCode.PRIMARY_WAL_TARGET_NOT_FOUND, false, "BookKeeper ledger root is absent")))
                .thenCompose(root -> {
                    requireRoot(root.value(), stream, target);
                    return leases.claim(root, deadline.remaining())
                            .thenCompose(
                                    lease -> withReaderLease(lease, () -> readPinned(root, range, target, deadline)));
                });
    }
    ...
    private CompletableFuture<VerifiedRange> readPinned(
        ...
        return handles.borrow(key, () -> open(root.value(), target, deadline)).thenCompose(handleLease -> {
            CompletableFuture<VerifiedRange> read = readEntries(handleLease.handle(), range, target, deadline);
    ...
    private CompletableFuture<ReadHandle> open(
        ...
        byte[] password = Objects.requireNonNull(
                        passwordProvider.resolve(configuration.passwordRef()), "resolved BookKeeper password")
                .clone();
        CompletableFuture<ReadHandle> opened;
        try {
            opened = client.open(target.ledgerId(), configuration.digestType(), password, false, deadline);
    ...
    private CompletableFuture<VerifiedRange> readEntries(
        ...
        return deadline.bound(client.readUnconfirmed(handle, target.firstEntryId(), lastEntryId, deadline))
                .thenApply(entries -> copyAndVerify(entries, range, target));
    }

    private VerifiedRange copyAndVerify(
            LedgerEntries providerEntries, ResolvedRange range, BookKeeperEntryRangeReadTarget target) {
        ...
        if (!BookKeeperRangeChecksums.computeBytes(target.firstEntryId(), physicalEntries)
            ...
            throw new NereusException(
                    ErrorCode.PRIMARY_WAL_CHECKSUM_MISMATCH, false, "BookKeeper NBKR1 range checksum mismatch");
        }
        long physicalBytes =
                physicalEntries.stream().mapToLong(value -> value.length).reduce(0, Math::addExact);
        List<VerifiedEntry> entries = physicalEntries.stream()
                .map(value -> decodeEntry(target.entryMapping(), value))
                .toList();
        long logicalBytes =
                entries.stream().mapToLong(value -> value.payload().length).reduce(0, Math::addExact);
        long records = entries.stream().mapToLong(VerifiedEntry::recordCount).reduce(0, Math::addExact);
    ...
    private static VerifiedEntry decodeEntry(BookKeeperEntryMapping mapping, byte[] physicalEntry) {
        return switch (mapping) {
            case ONE_NEREUS_ENTRY_PER_BOOKKEEPER_ENTRY -> new VerifiedEntry(1, physicalEntry);
            case RANGED_NEREUS_ENTRY_V1 -> {
                BookKeeperRangedEntryCodecV1.DecodedEntry decoded = BookKeeperRangedEntryCodecV1.decode(physicalEntry);
    ...
    private CompletableFuture<VerifiedRange> withReaderLease(
        ...
        return read.handle((value, failure) -> lease.release().handle((ignored, releaseFailure) -> {
                    if (failure != null) {
    ...
    private static void requireRoot(
            BookKeeperLedgerRootRecord root, StreamId stream, BookKeeperEntryRangeReadTarget target) {
    ...
    private synchronized ReadPermit acquirePermit(long bytes) {
        ...
        if (bytes > configuration.maxReadBytesInFlight()
            ...
            throw new NereusException(
                    ErrorCode.BACKPRESSURE_REJECTED, true, "BookKeeper primary reader capacity is exhausted");
        }
        inFlightReads++;
        inFlightBytes += bytes;
    ...
    private static final class ReadAccumulator {
        ...
        private void add(ResolvedRange range, BookKeeperEntryRangeReadTarget target, VerifiedRange verified) {
            ...
            for (VerifiedEntry entry : verified.entries) {
                ...
                if (request.boundaryMode() == ReadBoundaryMode.EXACT_START && offset < request.startOffset()) {
                    throw new NereusException(
                ...
                boolean firstEntryOverflow =
                        request.firstEntryPolicy() == FirstEntryPolicy.ALLOW_FIRST_ENTRY_OVERFLOW && batches.isEmpty();
    ...
    private void ensureOpen() {
        if (closed.get()) {
            throw new NereusException(ErrorCode.STORAGE_CLOSED, false, "BookKeeper primary reader is closed");
        }
    }
