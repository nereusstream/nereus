        ...
        if (cause instanceof BKException bookKeeper) {
            ...
            if (code == BKException.Code.NoSuchLedgerExistsException
                ...
                return new NereusException(
                        ErrorCode.PRIMARY_WAL_TARGET_NOT_FOUND, false, "BookKeeper target is absent", cause);
            }
            if (code == BKException.Code.DigestMatchException) {
                return new NereusException(
                        ErrorCode.PRIMARY_WAL_CHECKSUM_MISMATCH, false, "BookKeeper digest validation failed", cause);
            }
            if (code == BKException.Code.UnauthorizedAccessException || code == BKException.Code.SecurityException) {
                return new NereusException(
                        operation == Operation.READ
                                ? ErrorCode.PRIMARY_WAL_READ_FAILED
                                : ErrorCode.PRIMARY_WAL_WRITE_FAILED,
                        false,
                        "BookKeeper authentication failed",
                        cause);
            }
            if (code == BKException.Code.LedgerFencedException || code == BKException.Code.LedgerClosedException) {
                return new NereusException(
                        ErrorCode.FENCED_APPEND, false, "BookKeeper ledger is fenced or closed", cause);
            }
            ...
            return new NereusException(
                    errorCode(operation),
                    retriable,
                    "BookKeeper " + operation.name().toLowerCase() + " failed: " + BKException.getMessage(code),
                    cause);
        }
        return new NereusException(
                errorCode(operation), true, "BookKeeper " + operation.name().toLowerCase() + " failed", cause);
    }
