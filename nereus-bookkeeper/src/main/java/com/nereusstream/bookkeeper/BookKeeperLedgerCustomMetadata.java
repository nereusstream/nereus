...
public final class BookKeeperLedgerCustomMetadata {
    ...
    private BookKeeperLedgerCustomMetadata(Map<String, byte[]> supplied) {
        ...
        supplied.entrySet().stream()
                .sorted(Map.Entry.comparingByKey(unsignedUtf8Comparator()))
                .forEach(entry -> copied.put(
                        text(entry.getKey(), "metadata key"),
    ...
    public static BookKeeperLedgerCustomMetadata create(
        ...
        if (segmentSequence < 0) {
            throw new IllegalArgumentException("segmentSequence must be non-negative");
        }
    ...
    public static BookKeeperLedgerCustomMetadata fromRoot(
            String cluster, BookKeeperWalConfiguration configuration, BookKeeperLedgerRootRecord root) {
        Objects.requireNonNull(configuration, "configuration");
    ...
    public Checksum requireExactImmutableLedgerMetadata(
        ...
        frame(
                ...
                ByteBuffer.allocate(Integer.BYTES)
                        .putInt(actual.getEnsembleSize())
                        .array());
        frame(
                digest,
                ByteBuffer.allocate(Integer.BYTES)
                        .putInt(actual.getWriteQuorumSize())
                        .array());
        frame(
                digest,
                ByteBuffer.allocate(Integer.BYTES)
                        .putInt(actual.getAckQuorumSize())
                        .array());
