...
public final class BookKeeperLedgerRecovery {
    ...
    public CompletableFuture<BookKeeperLedgerRecoveryResult> recoverWriter(
        ...
        return namespaceVerifier
                ...
                .thenCompose(namespace -> writerMetadata
                        .getWriter(cluster, session.streamId())
                        .thenCompose(optional -> {
                            BookKeeperVersionedValue<BookKeeperWriterStateRecord> current =
                            ...
                            return writerState
                                    .adoptForRecovery(current, session, reason)
                                    .thenCompose(recovering -> ledgerMetadata
                                            .getRoot(
                                                    cluster,
                                            ...
                                            .thenCompose(rootOptional -> {
                                                ...
                                                return seal(root, namespace, deadline, reason)
                                                        .thenCompose(sealedRoot -> reconcileActiveReservation(
                                                                        recovering, sealedRoot, reason)
                                                                .thenCompose(ignored -> writerState.finishRecovery(
                                                                        recovering, session, reason))
                                                                .thenApply(idle -> new BookKeeperLedgerRecoveryResult(
                                                                        idle, sealedRoot)));
                                            }));
                        }));
    }
    ...
    private CompletableFuture<Void> reconcileActiveReservation(
        ...
        return writerMetadata
                .getReservation(
                        cluster,
                        new com.nereusstream.api.StreamId(writer.value().streamId()),
                        reservationId)
                .thenCompose(optional -> {
    ...
    private CompletableFuture<Void> reconcileMandatoryProtections(
            BookKeeperLedgerRootRecord root, BookKeeperAppendReservationRecord reservation) {
    ...
    private CompletableFuture<Void> terminalizeOrProtectReservation(
        ...
        if (lifecycle == AppendReservationLifecycle.RESERVED || lifecycle == AppendReservationLifecycle.WRITING) {
            ...
            BookKeeperAppendReservationRecord abandoned = new BookKeeperAppendReservationRecord(
                    ...
                    0);
            return writerMetadata
                    .compareAndSetReservation(cluster, abandoned, reservation.metadataVersion())
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerProtectionRecord>> activateRecoveryProtection(
        ...
        return ledgerMetadata
                .getProtection(
                        cluster, configuration.providerScopeSha256(), owner.ledgerId(), owner.ledgerRangeSlot(), 2)
                .thenCompose(optional -> {
    ...
    public CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> sealUnowned(
            BookKeeperVersionedValue<BookKeeperLedgerRootRecord> root, Duration timeout, String reason) {
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> seal(
        ...
        } else if (lifecycle == BookKeeperLedgerLifecycle.ACTIVE || lifecycle == BookKeeperLedgerLifecycle.ALLOCATING) {
            sealing = ledgerMetadata.compareAndSetRoot(cluster, sealing(root.value(), reason), root.metadataVersion());
        } else {
            return CompletableFuture.failedFuture(new NereusException(
                    ErrorCode.METADATA_CONDITION_FAILED,
                    false,
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> recoveryOpen(
        ...
        BookKeeperLedgerCustomMetadata expected = BookKeeperLedgerCustomMetadata.create(
                ...
                configuration,
                namespace,
                new com.nereusstream.api.StreamId(root.streamId()),
                root.segmentSequence(),
                root.allocationId());
        ...
        return open.whenComplete((ignored, failure) -> Arrays.fill(password, (byte) 0))
                .thenCompose(handle -> {
                    try {
                        ...
                        return ledgerMetadata
                                .compareAndSetRoot(cluster, sealed(root, metadata, reason), sealing.metadataVersion())
    ...
    private BookKeeperLedgerRootRecord sealing(BookKeeperLedgerRootRecord before, String reason) {
        return replace(
                before,
                BookKeeperLedgerLifecycle.SEALING,
                before.lifecycleEpoch() + 1,
                clock.millis(),
                0,
                -1,
    ...
    private static void requireWriterRoot(BookKeeperWriterStateRecord writer, BookKeeperLedgerRootRecord root) {
