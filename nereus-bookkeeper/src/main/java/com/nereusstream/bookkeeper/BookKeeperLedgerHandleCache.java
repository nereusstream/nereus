...
public final class BookKeeperLedgerHandleCache implements AutoCloseable {
    ...
    public final class Lease implements AutoCloseable {
        ...
        private Lease(Entry entry, ReadHandle handle) {
            this.entry = entry;
            this.handle = handle;
        }
    ...
    public BookKeeperLedgerHandleCache(
        ...
        if (idleTime.isZero() || idleTime.isNegative()) {
            throw new IllegalArgumentException("idleTime must be positive");
        }
        ...
        synchronized (this) {
            if (closed) {
                return CompletableFuture.failedFuture(
                        new NereusException(ErrorCode.STORAGE_CLOSED, false, "BookKeeper handle cache is closed"));
            }
            ...
            if (selected == null) {
                ...
                try {
                    opening = Objects.requireNonNull(opener.get(), "opener future");
                ...
                opening.whenComplete((handle, failure) -> {
                    if (failure != null) {
                        removeFailed(installed);
                    } else {
                        closeIfCacheClosed(installed, handle);
                    }
                });
            }
            selected.references++;
    ...
    private synchronized List<ReadHandle> evictIdle(long now) {
        ...
        for (Entry candidate : candidates) {
            if (entries.remove(candidate.key, candidate)) {
                ReadHandle completed = candidate.opening.isCompletedExceptionally() || candidate.opening.isCancelled()
                        ? null
                        : candidate.opening.getNow(null);
                if (completed != null) {
    ...
    private List<ReadHandle> evictReleasedForCapacity() {
        ...
        while (entries.size() >= maxHandles
                || Math.multiplyExact((long) (entries.size() + 1), estimatedBytesPerHandle) > maxEstimatedBytes) {
    ...
    private synchronized void release(Entry entry) {
        if (entry.references <= 0) {
            throw new IllegalStateException("BookKeeper handle lease released twice");
        }
        entry.references--;
        entry.lastAccessNanos = System.nanoTime();
    }

    private synchronized void removeFailed(Entry entry) {
        entries.remove(entry.key, entry);
    }

    private synchronized void closeIfCacheClosed(Entry entry, ReadHandle handle) {
        if (closed && entries.remove(entry.key, entry)) {
            handle.closeAsync();
        }
    }
