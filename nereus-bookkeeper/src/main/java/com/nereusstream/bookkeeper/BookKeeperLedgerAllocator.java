...
public final class BookKeeperLedgerAllocator {
    ...
    public CompletableFuture<AllocatedBookKeeperLedger> allocate(BookKeeperLedgerAllocationRequest request) {
        ...
        return namespaceVerifier
                ...
                .thenCompose(reservation -> writerState
                        .requireIdle(exact.session())
    ...
    private CompletableFuture<AllocatedBookKeeperLedger> allocateCandidate(
        ...
        return claimSlot(request.streamId(), allocationId, candidate, ledgerIdentity, deadline)
                .thenCompose(slot -> createIntent(request, allocationId, candidate, segmentSequence, slot, idleWriter)
                        .thenCompose(intent -> writerState
                                .claimAllocation(idleWriter, request.session(), allocationId, candidate)
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperAllocationSlotRecord>> claimSlot(
        ...
        int hash = Integer.parseUnsignedInt(
                BookKeeperIdentityDigests.sha256(allocationId).substring(0, 8), 16);
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperAllocationSlotRecord>> claimSlot(
        ...
        BookKeeperAllocationSlotRecord desired = new BookKeeperAllocationSlotRecord(
                ...
                AllocationSlotLifecycle.CLAIMED,
                now,
                now,
                0);
    ...
    private CompletableFuture<BookKeeperVersionedValue<LedgerAllocationIntentRecord>> createIntent(
        ...
        LedgerAllocationIntentRecord intent = new LedgerAllocationIntentRecord(
                ...
                LedgerAllocationLifecycle.PREPARED,
                false,
                "",
                now,
                now,
    ...
    private CompletableFuture<AllocatedBookKeeperLedger> reserveRootAndCreate(
            ...
            BookKeeperOperationDeadline deadline,
            int attempt) {
        BookKeeperLedgerRootRecord desiredRoot =
                allocatingRoot(request, namespace, customMetadata, slot, intent, claimedWriter);
        return ledgerMetadata
                .createRoot(cluster, desiredRoot)
                .handle((root, failure) -> {
                    if (failure == null) {
                    ...
                    if (cause instanceof BookKeeperMetadataConditionFailedException) {
                        return abortPreTransmission(
                                ...
                                .thenCompose(
                                        idle -> allocateCandidate(request, namespace, idle, deadline, attempt + 1));
                    }
                ...
                .thenCompose(rootReserved -> {
                    ...
                    try {
                        password = Objects.requireNonNull(
                                        passwordProvider.resolve(configuration.passwordRef()), "resolved password")
                                .clone();
                    } catch (Throwable failure) {
                        return abortOwnedPreTransmission(request, slot, rootReserved, claimedWriter, root, failure);
                    }
    ...
    private CompletableFuture<AllocatedBookKeeperLedger> invokeCreate(
        ...
        try {
            create = client.createAdvanced(
                    root.value().ledgerId(), configuration, password, customMetadata.values(), deadline);
        } catch (Throwable failure) {
    ...
    private CompletableFuture<AllocatedBookKeeperLedger> activate(
            ...
            BookKeeperVersionedValue<BookKeeperWriterStateRecord> claimedWriter,
            BookKeeperVersionedValue<BookKeeperLedgerRootRecord> root) {
        return casRootActive(root).thenCompose(activeRoot -> casIntent(
                        intent, LedgerAllocationLifecycle.PHYSICAL_CREATED, false, metadataSha256, "")
                .thenCompose(
                        physical -> casIntent(physical, LedgerAllocationLifecycle.ACTIVATED, false, metadataSha256, ""))
                .thenCompose(activated -> writerState
                        .activate(claimedWriter, request.session(), activated.value(), activeRoot.value())
                        .thenCompose(activeWriter -> writerMetadata
                                .deleteAllocationSlot(cluster, slot.value().slot(), slot.metadataVersion())
    ...
    private CompletableFuture<AllocatedBookKeeperLedger> recordUncertain(
            ...
            BookKeeperOperationDeadline deadline) {
        return casSlot(slot, AllocationSlotLifecycle.CREATE_UNCERTAIN).thenCompose(uncertainSlot -> casIntent(
                        intent, LedgerAllocationLifecycle.CREATE_UNCERTAIN, true, "", "provider create outcome unknown")
                .thenCompose(uncertainIntent -> casRootUncertain(root).thenCompose(uncertainRoot -> probeUncertain(
                        ...
                        .thenCompose(ignored -> CompletableFuture.failedFuture(uncertainFailure(createFailure))))));
    }
        ...
        return probe.handle((metadata, failure) -> {
                    ...
                    try {
                        String metadataSha256 = expected.requireExactImmutableLedgerMetadata(
                                        root.value().ledgerId(), configuration, metadata)
                                .value();
                        return casIntent(
                                        intent,
                        ...
                        return casRootQuarantined(root, reason).thenCompose(quarantined -> casIntent(
    ...
    private CompletableFuture<UncertainProbe> sealRecoveredCreate(
        ...
        return sealed.handle((value, failure) -> new UncertainProbe(failure == null ? value : root, physical));
    }
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperWriterStateRecord>> abortPreTransmission(
        ...
        return casIntent(intent, LedgerAllocationLifecycle.ABORTED, false, "", reason)
                ...
                .thenCompose(idle -> writerMetadata
                        .deleteAllocationSlot(cluster, slot.value().slot(), slot.metadataVersion())
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperAllocationSlotRecord>> casSlot(
            BookKeeperVersionedValue<BookKeeperAllocationSlotRecord> current, AllocationSlotLifecycle lifecycle) {
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> casRootActive(
        ...
        return ledgerMetadata.compareAndSetRoot(
                cluster,
                rootReplacement(
                        before,
                        BookKeeperLedgerLifecycle.ACTIVE,
                        before.lifecycleEpoch() + 1,
                        false,
                        clock.millis(),
                        ""),
    ...
    private CompletableFuture<BookKeeperVersionedValue<BookKeeperLedgerRootRecord>> casRootUncertain(
        ...
        return ledgerMetadata.compareAndSetRoot(
                cluster,
                rootReplacement(before, BookKeeperLedgerLifecycle.ALLOCATING, before.lifecycleEpoch() + 1, true, 0, ""),
                current.metadataVersion());
    }
    ...
    private BookKeeperLedgerRootRecord allocatingRoot(
        ...
        return new BookKeeperLedgerRootRecord(
                1,
                slot.value().ledgerIdentitySha256(),
                configuration.clusterAlias(),
                configuration.providerScopeSha256(),
                intent.value().candidateLedgerId(),
                request.streamId().value(),
                intent.value().segmentSequence(),
                intent.value().allocationId(),
                slot.value().slot(),
                configurationBindingSha256,
                ...
                customMetadata.sha256().value(),
                BookKeeperLedgerLifecycle.ALLOCATING,
                1,
                now,
                0,
                0,
    ...
    private static NereusException uncertainFailure(Throwable cause) {
        return new NereusException(
                ...
                true,
                "BookKeeper ledger create outcome is unknown; durable recovery is required",
                unwrap(cause));
    }
