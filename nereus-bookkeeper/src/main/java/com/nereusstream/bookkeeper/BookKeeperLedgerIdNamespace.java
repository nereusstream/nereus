...
public final class BookKeeperLedgerIdNamespace {
    ...
    public BookKeeperLedgerIdNamespace(int prefixBits, long prefixValue) {
        ...
        if (prefixBits < 8 || prefixBits > 24) {
            throw new IllegalArgumentException("prefixBits must be in [8,24]");
        }
    ...
    public long candidate() {
        return candidate(new SecureRandom());
    }
    ...
    public long candidate(RandomGenerator random) {
        ...
        if (!contains(candidate)) {
            throw new IllegalStateException("generated ledger id failed namespace round-trip");
        }
        return candidate;
    }
}
