...
final class GcGlobalScopeTestSupport {
    ...
    static GcGlobalReferenceScope complete(StreamId... supplied) {
        List<StreamId> streams = java.util.Arrays.stream(supplied)
                ...
                .toList();
        GcGlobalReferenceScopeSnapshot snapshot = snapshot(streams, 1, sha('b'));
        return () -> CompletableFuture.completedFuture(snapshot);
    }

    static GcGlobalReferenceScopeSnapshot snapshot(List<StreamId> streams, long version, Checksum digest) {
        return new GcGlobalReferenceScopeSnapshot(
                true,
                streams.size(),
                1,
                streams,
                List.of(new GcAuthorityToken("/global/reference-scope", version, digest)));
    }
