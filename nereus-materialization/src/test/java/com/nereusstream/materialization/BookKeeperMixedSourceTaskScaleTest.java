...
final class BookKeeperMixedSourceTaskScaleTest {
    ...
    void executesBothTaskLimits() {
        ...
        GenerationMetadataStore generations = MaterializationPlannerTestSupport.generationStore(
                sources(), List.of(), durable, StorageProfile.BOOKKEEPER_WAL_ASYNC_OBJECT);
        ...
        DefaultMaterializationPlanner planner = new DefaultMaterializationPlanner(
                CLUSTER,
                MaterializationPlannerTestSupport.l0Store(MaterializationPlannerTestSupport.snapshot(
                        0, RECORD_COUNT, StorageProfile.BOOKKEEPER_WAL_ASYNC_OBJECT)),
                generations,
                17);

        MaterializationTask task = planner.plan(STREAM, new OffsetRange(0, RECORD_COUNT), policy, 1)
                .join()
                .get(0);
        ...
        assertThat(task.sources().stream()
                        .filter(source -> source.readTarget() instanceof BookKeeperEntryRangeReadTarget)
                        .count())
    ...
    private static List<VersionedGenerationCandidate> sources() {
        ...
        for (int index = 0; index < SOURCE_RANGE_COUNT; index++) {
            ...
            if ((index & 1) == 0) {
                ...
                result.add(MaterializationPlannerTestSupport.zero(
                        String.format("/index/mixed-scale-%03d", index), start, end, start, RECORDS_PER_SOURCE, end));
            }
        }
