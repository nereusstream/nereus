...
class MaterializationPlannerScaleTest {
    ...
    void plansAndDurablyRoundTripsOneTaskAtBothSourceAndRecordLimits() {
        ...
        GenerationMetadataStore generations =
                MaterializationPlannerTestSupport.generationStore(sources, List.of(), durable);
        ...
        DefaultMaterializationPlanner planner = new DefaultMaterializationPlanner(
                CLUSTER,
                MaterializationPlannerTestSupport.l0Store(MaterializationPlannerTestSupport.snapshot(0, RECORD_COUNT)),
                generations,
                17);

        List<MaterializationTask> planned = planner.plan(STREAM, new OffsetRange(0, RECORD_COUNT), policy, 1)
                .join();
        ...
        assertThat(planned).singleElement().satisfies(task -> {
            ...
            assertThat(task.sources()).extracting(SourceGeneration::range).containsExactlyElementsOf(expectedRanges());

            MaterializationTaskRecord record = MaterializationRecordMapper.plannedTask(task, CLOCK.millis());
            assertThat(F4MetadataCodecs.encodeEnvelope(record, MaterializationTaskRecord.class).length)
                    .isPositive()
    ...
    private static List<VersionedGenerationCandidate> sources() {
        ...
        for (int index = 0; index < SOURCE_RANGE_COUNT; index++) {
            ...
            result.add(MaterializationPlannerTestSupport.zero(
                    String.format("/index/scale-%03d", index), start, end, start, RECORDS_PER_SOURCE, end));
        }
