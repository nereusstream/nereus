...
final class GenerationPublicationTestSupport {
    ...
    static final ProjectionRef PROJECTION = new ProjectionRef(ProjectionType.VIRTUAL_LEDGER, "projection-publication");
    ...
    private static Context context(boolean topicCompacted, boolean emptyKafkaOutput, boolean directStream) {
        ...
        ObjectSliceReadTarget sourceTarget =
                target("source-object", "objects/publication/source-object", "source-slice", "11111111");
        ...
        AllocatedGeneration sourceAllocation = generations
                .allocateGeneration(CLUSTER, STREAM, ReadView.COMMITTED, sourcePublication)
                .join();
        Checksum sourcePolicy = sha("b");
        Optional<ProjectionRef> authorityProjection = directStream ? Optional.empty() : Optional.of(PROJECTION);
        ...
        VersionedGenerationIndex sourceCommitted = generations
                .compareAndSetIndex(
                        CLUSTER,
                        generation(
                                sourceTarget,
        ...
        SourceGeneration source = new SourceGeneration(
                ...
                new Checksum(
                        ChecksumType.SHA256,
                        ReadTargetCodecRegistry.phase15().encode(sourceTarget).identityChecksumValue()),
        ...
        MaterializationTask task = MaterializationTask.create(STREAM, new OffsetRange(0, 2), List.of(source), policy);
        ...
        ObjectSliceReadTarget outputTarget = target(
                ...
                emptyKafkaOutput ? "KAFKA_RECORD_BATCH_V1" : PayloadFormat.PULSAR_ENTRY_BATCH.name());
        MaterializationOutput output = new MaterializationOutput(
                ...
                new Checksum(
                        ChecksumType.SHA256,
                        ReadTargetCodecRegistry.phase15().encode(outputTarget).identityChecksumValue()),
                        ...
                        new MaterializationStreamRegistrationRecord(
                                ...
                                directStream
                                        ? DirectMaterializationStreamAuthority.identitySha256(
                                                        STREAM, StorageProfile.OBJECT_WAL_SYNC_OBJECT)
                                                .value()
    ...
    static GenerationMetadataStore loseFirstCommittedIndexResponse(GenerationMetadataStore delegate) {
    ...
    static GenerationMetadataStore hideSources(GenerationMetadataStore delegate) {
        return proxy(
                delegate,
                (method, args, result) ->
                        method.equals("getCandidate") ? CompletableFuture.completedFuture(Optional.empty()) : result);
    }

    static GenerationMetadataStore loseFirstGenerationAttachmentResponse(GenerationMetadataStore delegate) {
        AtomicBoolean lose = new AtomicBoolean(true);
        return proxy(delegate, (method, args, result) -> {
    ...
    private static ObjectSliceReadTarget target(String objectId, String objectKey, String sliceId, String checksum) {
        return target(objectId, objectKey, sliceId, checksum, MaterializationPolicy.COMMITTED_FORMAT);
    }

    private static ObjectSliceReadTarget target(
            String objectId, String objectKey, String sliceId, String checksum, String physicalFormat) {
        return target(objectId, objectKey, sliceId, checksum, physicalFormat, PayloadFormat.PULSAR_ENTRY_BATCH.name());
    }
    ...
    private static PhysicalObjectRootRecord activeRoot(PhysicalObjectIdentity identity, long createdAt) {
    ...
    static final class Context implements AutoCloseable {
        ...
        DefaultGenerationCommitter committer(GenerationMetadataStore store, GenerationProtocolActivationGuard guard) {
            ...
            return new DefaultGenerationCommitter(
                    ...
                    () -> {
                        ...
                        return new PublicationId(
                                String.valueOf((char) ('p' + value)).repeat(26));
                    },
                    authorityMode,
