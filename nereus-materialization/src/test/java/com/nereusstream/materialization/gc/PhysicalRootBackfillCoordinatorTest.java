...
class PhysicalRootBackfillCoordinatorTest {
    ...
    void emptyStableRegistryPublishesBothProofsAndRecoversLostCasResponse() {
        try (GenerationMetadataStore generations = GenerationMetadataStoreTestFactory.inMemory(CLOCK);
                GenerationProtocolActivationStore durableActivation = activationStore();
                CursorMetadataStore cursors = new FakeCursorMetadataStore()) {
            prepareRegistrationBackfill(durableActivation);
            GenerationProtocolActivationStore lossy = loseFirstSuccessfulCasResponse(durableActivation);
            FakePhysicalObjectMetadataStore physical = new FakePhysicalObjectMetadataStore();
            try (DefaultObjectProtectionManager protections = protectionManager(physical);
                    SourceRetirementMetadataStore sources = unsupported(SourceRetirementMetadataStore.class);
                    OxiaMetadataStore l0 = unsupported(OxiaMetadataStore.class);
                    ObjectStore objects = unsupported(ObjectStore.class)) {
                var coordinator = new DefaultPhysicalRootBackfillCoordinator(
                        CLUSTER,
                        l0,
                        generations,
                        sources,
                        ...
                        subject -> CompletableFuture.failedFuture(
                                new AssertionError("empty registry cannot read a projection")),
                        8,
                        Duration.ofSeconds(1),
                        CLOCK);

                PhysicalRootBackfillReport report = coordinator
                        .run(new PhysicalRootBackfillRequest(RUN_ID, READINESS_EPOCH, 4, Duration.ofSeconds(5)))
                        .join();
                ...
                assertThat(activation.value().physicalRootBackfill().complete()).isTrue();
                assertThat(activation.value().cursorSnapshotBackfill().complete())
                        .isTrue();
                assertThat(activation.value().physicalRootBackfill().runId()).isEqualTo(RUN_ID);
    ...
    void reachableCommitAndVisibleIndexCreateExactRootAndTwoOwnerProtections() {
        ...
        try (GenerationMetadataStore durableGenerations = GenerationMetadataStoreTestFactory.inMemory(CLOCK);
                GenerationProtocolActivationStore activations = activationStore();
                CursorMetadataStore cursors = new FakeCursorMetadataStore()) {
            prepareRegistrationBackfill(activations);
            VersionedMaterializationStreamRegistration registration = durableGenerations
                    .createOrVerifyStreamRegistration(CLUSTER, fixture.registration())
                    .join();
            GenerationMetadataStore generations = generationStore(durableGenerations, fixture.zeroIndex());
            FakePhysicalObjectMetadataStore physical = new FakePhysicalObjectMetadataStore();
            try (DefaultObjectProtectionManager protections = protectionManager(physical);
                    SourceRetirementMetadataStore sources = sourceStore(fixture.commitSource());
                ...
                var coordinator = new DefaultPhysicalRootBackfillCoordinator(
                        ...
                        protections,
                        objects,
                        projectionReader(fixture.projectionIdentity()),
                        2,
                        Duration.ofSeconds(1),
                        CLOCK);

                PhysicalRootBackfillReport report = coordinator
                        .run(new PhysicalRootBackfillRequest(RUN_ID, READINESS_EPOCH, 2, Duration.ofSeconds(5)))
                        .join();
                ...
                ObjectKeyHash objectHash = ObjectKeyHash.from(fixture.objectKey());
                var root = physical.getRoot(CLUSTER, objectHash).join().orElseThrow();
                assertThat(root.value().objectKey())
                        .isEqualTo(fixture.objectKey().value());
                assertThat(physical.scanProtections(CLUSTER, objectHash, Optional.empty(), 100)
                                .join()
                                .values())
                        .hasSize(2)
                        .extracting(value -> value.value().protectionTypeId())
                        .containsExactlyInAnyOrder(
                                com.nereusstream.metadata.oxia.records.ObjectProtectionType.REACHABLE_APPEND.wireId(),
                                com.nereusstream.metadata.oxia.records.ObjectProtectionType.VISIBLE_GENERATION
                                        .wireId());
            }
        }
    }
    ...
    void activeCursorSnapshotCreatesExactEtagRootAndPermanentOwnerProtection() {
        ...
        try (GenerationMetadataStore generations = GenerationMetadataStoreTestFactory.inMemory(CLOCK);
                ...
                CursorMetadataStore cursors =
                        boundedCursorPages(new FakeCursorMetadataStore(), 2, observedCursorPageSize)) {
            prepareRegistrationBackfill(activations);
            generations
                    .createOrVerifyStreamRegistration(CLUSTER, fixture.registration())
                    .join();
            cursors.createRetention(CLUSTER, fixture.retention()).join();
            cursors.createCursor(CLUSTER, fixture.cursor()).join();
            FakePhysicalObjectMetadataStore physical = new FakePhysicalObjectMetadataStore();
            try (DefaultObjectProtectionManager protections = protectionManager(physical);
                    SourceRetirementMetadataStore sources = unsupported(SourceRetirementMetadataStore.class);
                ...
                ObjectKeyHash hash = ObjectKeyHash.from(fixture.objectKey());
                var root = physical.getRoot(CLUSTER, hash).join().orElseThrow();
                assertThat(root.value().etag()).isEqualTo("cursor-etag");
                assertThat(physical.scanProtections(CLUSTER, hash, Optional.empty(), 100)
                                .join()
                                .values())
                        .singleElement()
                        .satisfies(value -> {
                            assertThat(value.value().protectionTypeId())
                                    .isEqualTo(com.nereusstream.metadata.oxia.records.ObjectProtectionType
                                            .CURSOR_SNAPSHOT_ROOT
                                            .wireId());
                            assertThat(value.value().ownerKey()).contains("/cursors/v1/");
                        });
            }
        }
    }
        ...
        try (GenerationMetadataStore generations = GenerationMetadataStoreTestFactory.inMemory(CLOCK);
            ...
            try (DefaultObjectProtectionManager protections = protectionManager(physical);
                ...
                var coordinator = new DefaultPhysicalRootBackfillCoordinator(
                        ...
                        protections,
                        objects,
                        subject -> CompletableFuture.failedFuture(new AssertionError()),
                        8,
                        Duration.ofSeconds(1),
                        CLOCK);

                PhysicalRootBackfillReport report = coordinator
                        .run(new PhysicalRootBackfillRequest(RUN_ID, 0, 1, Duration.ofSeconds(5)))
                        .join();
    ...
    void deletionActiveRolloverScansWithoutPublishingPartialProofs() {
        try (GenerationMetadataStore generations = GenerationMetadataStoreTestFactory.inMemory(CLOCK);
                GenerationProtocolActivationStore activations = activationStore();
                CursorMetadataStore cursors = new FakeCursorMetadataStore()) {
            VersionedGenerationProtocolActivation expected = prepareDeletionAuthority(activations);
            FakePhysicalObjectMetadataStore physical = new FakePhysicalObjectMetadataStore();
            try (DefaultObjectProtectionManager protections = protectionManager(physical);
                    SourceRetirementMetadataStore sources = unsupported(SourceRetirementMetadataStore.class);
                    OxiaMetadataStore l0 = unsupported(OxiaMetadataStore.class);
                ...
                PhysicalRootBackfillReport report = coordinator
                        .runRollover(
                                new PhysicalRootBackfillRequest(RUN_ID, READINESS_EPOCH - 1, 4, Duration.ofSeconds(5)),
                                expected)
                        .join();

                assertThat(report.failureCount()).isZero();
                assertThat(report.brokerReadinessEpoch()).isEqualTo(READINESS_EPOCH - 1);
                assertThat(activations.get(CLUSTER).join()).contains(expected);
                assertThat(expected.value().physicalRootBackfill().brokerReadinessEpoch())
                        .isEqualTo(READINESS_EPOCH);
                assertThat(expected.value().cursorSnapshotBackfill().brokerReadinessEpoch())
                        .isEqualTo(READINESS_EPOCH);
            }
        }
    }
    ...
    void streamAuthorityIgnoresObservationChurnButDetectsDurableChanges() {
        StreamMetadataSnapshot first = streamFixture().streamSnapshot();
        StreamMetadataSnapshot refreshed = observedSnapshot(first, 99, "synthetic", 9_999);

        assertThat(DefaultPhysicalRootBackfillCoordinator.sameStreamAuthority(first, refreshed))
                .isTrue();

        StreamMetadataSnapshot advancedCommit = new StreamMetadataSnapshot(
                refreshed.metadata(),
                new CommittedEndOffsetRecord(refreshed.metadata().streamId(), 2, 16, 2, refreshed.metadataVersion()),
                refreshed.trim());
        StreamMetadataSnapshot advancedTrim = new StreamMetadataSnapshot(
                refreshed.metadata(),
                refreshed.committedEnd(),
                new TrimRecord(refreshed.metadata().streamId(), 1, "retention", 10_000, refreshed.metadataVersion()));

        assertThat(DefaultPhysicalRootBackfillCoordinator.sameStreamAuthority(first, advancedCommit))
                .isFalse();
        assertThat(DefaultPhysicalRootBackfillCoordinator.sameStreamAuthority(first, advancedTrim))
                .isFalse();
    }

    @Test
    void appendRecoveryAuthorityIgnoresHeadVersionButDetectsNewCommit() {
        AppendRecoveryHead first = streamFixture().tail().observedHead();
        ...
        assertThat(DefaultPhysicalRootBackfillCoordinator.sameAppendRecoveryAuthority(first, refreshed))
                .isTrue();
        assertThat(DefaultPhysicalRootBackfillCoordinator.sameAppendRecoveryAuthority(first, advanced))
                .isFalse();
    }
    ...
    private static GenerationProtocolActivationStore activationStore() {
        return GenerationProtocolActivationStoreTestFactory.inMemory(
                CLOCK, F4MetadataTestValues.ATTEMPT, F4MetadataTestValues.referenceDomains());
    }

    private static StreamMetadataSnapshot observedSnapshot(
            StreamMetadataSnapshot source, long metadataVersion, String trimReason, long trimUpdatedAtMillis) {
    ...
    private static void prepareRegistrationBackfill(GenerationProtocolActivationStore store) {
        ...
        GenerationProtocolActivationRecord replacement = new GenerationProtocolActivationRecord(
                ...
                new GenerationBackfillProofRecord(
                        F4MetadataTestValues.ATTEMPT, READINESS_EPOCH, F4MetadataTestValues.HASH_A, true, 3_100),
    ...
    private static VersionedGenerationProtocolActivation prepareDeletionAuthority(
        ...
        GenerationProtocolActivationRecord replacement = new GenerationProtocolActivationRecord(
                ...
                3_200,
                0);
        return store.compareAndSet(CLUSTER, replacement, current.metadataVersion())
                .join();
    }

    private static GenerationBackfillProofRecord completeProof(String coverage, long completedAtMillis) {
        return new GenerationBackfillProofRecord(RUN_ID, READINESS_EPOCH, coverage, true, completedAtMillis);
    }

    private static DefaultObjectProtectionManager protectionManager(FakePhysicalObjectMetadataStore physical) {
        return new DefaultObjectProtectionManager(
                CLUSTER, physical, Duration.ofMinutes(5), Duration.ofSeconds(1), Duration.ofMinutes(10), CLOCK);
    }
    ...
    private static GenerationProtocolActivationStore loseFirstSuccessfulCasResponse(
        ...
        return new GenerationProtocolActivationStore() {
            ...
            public CompletableFuture<VersionedGenerationProtocolActivation> compareAndSet(
                ...
                return delegate.compareAndSet(cluster, replacement, expectedVersion)
                        .thenCompose(updated -> first.compareAndSet(true, false)
                                ? CompletableFuture.failedFuture(new RuntimeException("lost activation CAS response"))
    ...
    private static GenerationMetadataStore generationStore(
            GenerationMetadataStore delegate, VersionedGenerationZeroIndex zero) {
        return proxy(GenerationMetadataStore.class, (method, arguments) -> switch (method.getName()) {
            case "scanIndex" ->
                CompletableFuture.completedFuture(new GenerationScanPage(List.of(zero), Optional.empty()));
            case "getCandidateByKey" ->
                CompletableFuture.completedFuture(Optional.<VersionedGenerationCandidate>of(zero));
    ...
    private static OxiaMetadataStore l0Store(StreamFixture fixture) {
        return proxy(OxiaMetadataStore.class, (method, arguments) -> switch (method.getName()) {
            case "getStreamSnapshot" -> CompletableFuture.completedFuture(fixture.streamSnapshot());
            case "readAppendRecoveryTail" -> CompletableFuture.completedFuture(fixture.tail());
            case "getObjectManifest" -> CompletableFuture.completedFuture(Optional.of(fixture.manifest()));
            case "close" -> null;
            default -> throw new UnsupportedOperationException(method.getName());
        });
    }

    private static OxiaMetadataStore cursorL0Store(CursorFixture fixture) {
        return proxy(OxiaMetadataStore.class, (method, arguments) -> switch (method.getName()) {
            case "getStreamSnapshot" -> CompletableFuture.completedFuture(fixture.streamSnapshot());
            case "readAppendRecoveryTail" -> CompletableFuture.completedFuture(fixture.tail());
            case "close" -> null;
    ...
    private static SourceRetirementMetadataStore sourceStore(VersionedGenerationZeroCommit commit) {
        return proxy(SourceRetirementMetadataStore.class, (method, arguments) -> switch (method.getName()) {
            case "getCommitNodeByKey" -> CompletableFuture.completedFuture(Optional.of(commit));
            case "close" -> null;
            default -> throw new UnsupportedOperationException(method.getName());
        });
    }

    private static GenerationProjectionAuthorityReader projectionReader(ManagedLedgerProjectionIdentity identity) {
        return subject -> CompletableFuture.completedFuture(new GenerationProjectionAuthoritySnapshot(
                subject,
                true,
                Optional.of(identity),
                List.of(
                        new GcAuthorityToken("/projection/binding", 11, sha('1')),
                        new GcAuthorityToken("/projection/topic", 12, sha('2')))));
    }

    private static ObjectStore headOnlyStore(HeadObjectResult expected) {
        return new ObjectStore() {
            ...
            public CompletableFuture<HeadObjectResult> headObject(ObjectKey key, HeadObjectOptions options) {
                if (!key.equals(expected.key())) {
                    return CompletableFuture.failedFuture(new IllegalArgumentException("unexpected object key"));
                }
    ...
    private static StreamFixture streamFixture() {
        String managedLedger = "tenant/ns/persistent/backfill";
        StreamId streamId = ManagedLedgerProjectionNames.streamId(managedLedger, 1);
        ManagedLedgerProjectionIdentity projectionIdentity = new ManagedLedgerProjectionIdentity(
                7, 1, streamId.value(), ManagedLedgerProjectionNames.MIN_VIRTUAL_LEDGER_ID + 1);
        ProjectionRef projection = new ProjectionRef(ProjectionType.VIRTUAL_LEDGER, "backfill-projection");
        ObjectId objectId = new ObjectId("object-wal-backfill");
        ObjectKey objectKey = new ObjectKey("objects/backfill/wal-1");
        Checksum sliceChecksum = new Checksum(ChecksumType.CRC32C, "33333333");
        Checksum entryChecksum = new Checksum(ChecksumType.CRC32C, "44444444");
        Checksum storageChecksum = new Checksum(ChecksumType.CRC32C, "55555555");
        ...
        StreamCommitTargetRecord commit = new StreamCommitTargetRecord(
                streamId.value(),
                commitId,
                "",
                0,
                ...
                150,
                0);
        byte[] commitBytes = MetadataRecordCodecFactory.encodeEnvelope(commit, StreamCommitTargetRecord.class);
        Checksum commitSha = sha256(commitBytes);
        String commitKey = new OxiaKeyspace(CLUSTER).streamCommitKey(streamId, commitId);
        AppendRecoveryCommit recoveryCommit = new AppendRecoveryCommit(
                commitKey,
                ...
                commitSha);
        AppendRecoveryHead recoveryHead = new AppendRecoveryHead(streamId, commitId, 1, 8, 1, 5);
        AppendRecoveryTailPage tail = new AppendRecoveryTailPage(
                AppendRecoveryAnchor.genesis(streamId), recoveryHead, List.of(recoveryCommit), true, Optional.empty());
        StreamMetadataSnapshot snapshot = new StreamMetadataSnapshot(
                new StreamMetadataRecord(
                        streamId.value(),
                        ManagedLedgerProjectionNames.streamName(managedLedger, 1)
                                .value(),
                        "stream-name-hash",
                        ...
                        1,
                        5),
                new CommittedEndOffsetRecord(streamId.value(), 1, 8, 1, 5),
                new TrimRecord(streamId.value(), 0, "", 100, 5));
        ...
        VersionedGenerationZeroIndex zeroIndex = new VersionedGenerationZeroIndex(
                new OxiaKeyspace(CLUSTER).offsetIndexKey(streamId, 1, 0),
                GenerationZeroIndexEncoding.GENERIC_OFFSET_INDEX_TARGET_RECORD,
                indexValue,
                10,
                indexSha);
        StreamSliceManifestRecord slice = new StreamSliceManifestRecord(
                0,
                streamId.value(),
                "slice-1",
                1,
                ...
                8,
                List.of(),
                EntryIndexReferenceRecord.fromApi(entryIndex),
                sliceChecksum.type().name(),
                sliceChecksum.value(),
                PayloadFormat.PULSAR_ENTRY_BATCH.name(),
                "VISIBLE");
        ObjectManifestRecord manifest = new ObjectManifestRecord(
                objectId.value(),
                objectKey.value(),
                ...
                10_000,
                4);
        VersionedGenerationZeroCommit source = new VersionedGenerationZeroCommit(
                commitKey,
                streamId,
                commitId,
                AppendRecoveryCommitEncoding.GENERIC_STREAM_COMMIT_TARGET_V1,
                ...
                9,
                commitSha);
        HeadObjectResult head = new HeadObjectResult(objectKey, 8, storageChecksum, Optional.of("etag-1"), Map.of());
    ...
    private static CursorFixture cursorFixture() {
        String managedLedger = "tenant/ns/persistent/backfill-cursor";
        StreamId streamId = ManagedLedgerProjectionNames.streamId(managedLedger, 1);
        ManagedLedgerProjectionIdentity projectionIdentity = new ManagedLedgerProjectionIdentity(
                7, 1, streamId.value(), ManagedLedgerProjectionNames.MIN_VIRTUAL_LEDGER_ID + 3);
        ProjectionRef projection = new ProjectionRef(ProjectionType.VIRTUAL_LEDGER, "backfill-cursor-projection");
        MaterializationStreamRegistrationRecord registration = new MaterializationStreamRegistrationRecord(
                1,
                streamId.value(),
        ...
        StreamMetadataSnapshot snapshot = new StreamMetadataSnapshot(
                new StreamMetadataRecord(
                        ...
                        1,
                        5),
                new CommittedEndOffsetRecord(streamId.value(), 0, 0, 0, 5),
                new TrimRecord(streamId.value(), 0, "", 100, 5));
        AppendRecoveryHead recoveryHead = new AppendRecoveryHead(streamId, "", 0, 0, 0, 5);
        AppendRecoveryTailPage tail = new AppendRecoveryTailPage(
                AppendRecoveryAnchor.genesis(streamId), recoveryHead, List.of(), true, Optional.empty());
        String owner = "0".repeat(32);
        String attempt = "1".repeat(32);
        String snapshotId = "2".repeat(32);
        String cursorName = "subscription-a";
        long generation = 1;
        ObjectKey objectKey = new ObjectKey(KeyComponentCodec.encodeComponent(CLUSTER)
                + "/cursor-snapshots/v1/"
        ...
        CursorSnapshotReferenceRecord reference = new CursorSnapshotReferenceRecord(
                ...
                1,
                150);
        CursorRetentionRecord retention = new CursorRetentionRecord(
                0,
                projectionIdentity,
                owner,
                ...
                Optional.empty(),
                160);
        CursorStateRecord cursor = new CursorStateRecord(
                0,
                projectionIdentity,
                owner,
                cursorName,
                ...
                160,
                OptionalLong.empty());
        HeadObjectResult head = new HeadObjectResult(
                objectKey,
                8,
                new Checksum(ChecksumType.CRC32C, "66666666"),
                ...
                Map.of(
                        ...
                        "nereus-snapshot-id",
                        snapshotId));
        return new CursorFixture(projectionIdentity, registration, snapshot, tail, retention, cursor, objectKey, head);
    }
    ...
    private static String encodeProjectionRef(ProjectionRef projection) {
    ...
    private static void addProjectionPart(StringBuilder builder, String value) {
        builder.append(value.getBytes(StandardCharsets.UTF_8).length)
                .append(':')
                .append(value);
    }
    ...
    private static CursorMetadataStore boundedCursorPages(
            CursorMetadataStore delegate, int maximumPageSize, AtomicInteger observedPageSize) {
        return proxy(CursorMetadataStore.class, (method, arguments) -> {
            if (method.getName().equals("scanCursors")) {
                int pageSize = (int) arguments[3];
                observedPageSize.set(pageSize);
                if (pageSize > maximumPageSize) {
                    throw new IllegalArgumentException("cursor page exceeds the test metadata-store limit");
                }
            }
        ...
        return (T)
                Proxy.newProxyInstance(type.getClassLoader(), new Class<?>[] {type}, (instance, method, arguments) -> {
                    if (method.getName().equals("toString")) {
                        return "physical-root-backfill-test-" + type.getSimpleName();
                    }
