...
class MaterializationPlannerTest {
    ...
    void plansFinalSingleBookKeeperGenerationZeroThroughTheOrdinaryPlanner() {
        ...
        List<VersionedGenerationCandidate> sources =
                List.of(MaterializationPlannerTestSupport.zero("/index/bk-final", 0, 1, 0, 100, 1, target));
        GenerationMetadataStore store = MaterializationPlannerTestSupport.generationStore(
                sources, List.of(), null, StorageProfile.BOOKKEEPER_WAL_ASYNC_OBJECT);
        DefaultMaterializationPlanner planner = new DefaultMaterializationPlanner(
                ...
                MaterializationPlannerTestSupport.l0Store(
                        MaterializationPlannerTestSupport.snapshot(0, 1, StorageProfile.BOOKKEEPER_WAL_ASYNC_OBJECT)),
                store,
                1);

        List<MaterializationTask> tasks = planner.plan(
                        STREAM, new OffsetRange(0, 1), MaterializationPlannerTestSupport.policy(), 1)
                .join();
    ...
    void prefersKafkaNcp2ReplacementBeforeAReadableBookKeeperTail() {
        ...
        BookKeeperEntryRangeReadTarget retiredHead = new BookKeeperEntryRangeReadTarget(
                1,
                "primary",
                91,
                0,
                ...
                BookKeeperEntryMapping.ONE_NEREUS_ENTRY_PER_BOOKKEEPER_ENTRY,
                new Checksum(ChecksumType.SHA256, "7".repeat(64)));
        BookKeeperEntryRangeReadTarget readableTail = new BookKeeperEntryRangeReadTarget(
                1,
                "primary",
                92,
                0,
        ...
        List<VersionedGenerationCandidate> sources = List.of(
                MaterializationPlannerTestSupport.kafkaBookKeeperZero(
                        "/index/bk-retired-head", 0, 1, 0, 89, 1, retiredHead),
                MaterializationPlannerTestSupport.kafkaHigher(
                        "/index/ncp2-head", 0, 1, 1, 0, 89, 1, policy.digestSha256()),
                MaterializationPlannerTestSupport.kafkaBookKeeperZero(
                        "/index/bk-readable-tail", 1, 2, 89, 89, 2, readableTail));
        DefaultMaterializationPlanner planner = MaterializationPlannerTestSupport.directPlanner(
                sources, List.of(), 0, 2, StorageProfile.BOOKKEEPER_WAL_ASYNC_OBJECT);

        MaterializationTask task =
                planner.plan(STREAM, new OffsetRange(0, 2), policy, 1).join().get(0);

        assertThat(task.coverage()).isEqualTo(new OffsetRange(0, 2));
        assertThat(task.sources()).extracting(SourceGeneration::generation).containsExactly(1L, 0L);
        assertThat(task.sources().get(0).readTarget())
                .isInstanceOf(com.nereusstream.api.target.ObjectSliceReadTarget.class);
        assertThat(task.sources().get(1).readTarget()).isEqualTo(readableTail);
    }
    ...
    void plansWholeGapFreeGenerationZeroEdgesAndDoesNotClipAStraddlingTrim() {
        ...
        DefaultMaterializationPlanner planner = MaterializationPlannerTestSupport.planner(sources, List.of(), 1, 4);

        List<MaterializationTask> tasks = planner.plan(
                        STREAM, new OffsetRange(1, 4), MaterializationPlannerTestSupport.policy(), 10)
                .join();
        ...
        assertThat(task.sources())
                .extracting(source -> source.range())
    ...
    void stopsConservativelyAtTheFirstGapInsteadOfClippingOrSkippingIt() {
        ...
        DefaultMaterializationPlanner planner = MaterializationPlannerTestSupport.planner(sources, List.of(), 0, 6);

        List<MaterializationTask> tasks = planner.plan(
                        STREAM, new OffsetRange(0, 6), MaterializationPlannerTestSupport.policy(), 10)
                .join();
    ...
    void bootstrapsTopicCompactionFromCommittedSourcesWithoutCrossingTargetViewIdentity() {
        ...
        GenerationMetadataStore durable =
                com.nereusstream.metadata.oxia.GenerationMetadataStoreTestFactory.inMemory(Clock.systemUTC());
        GenerationMetadataStore store = MaterializationPlannerTestSupport.generationStore(sources, List.of(), durable);
        DefaultMaterializationPlanner planner = new DefaultMaterializationPlanner(
                MaterializationPlannerTestSupport.CLUSTER,
        ...
        MaterializationTask task =
                planner.plan(STREAM, new OffsetRange(0, 4), policy, 1).join().get(0);
        ...
        MaterializationTaskStore taskStore =
                new MaterializationTaskStore(MaterializationPlannerTestSupport.CLUSTER, store, Clock.systemUTC());
        var created = taskStore.create(task).join();
        assertThat(taskStore.requireTask(created)).isEqualTo(task);
        assertThat(task.policy().topicCompaction()).contains(new TopicCompactionSpec("latest", 1, "test-key-v1"));
    }
    ...
    void reachesTopicFixedPointFromTheExactPublishedTargetTaskIdentity() {
        ...
        MaterializationPolicy policy = MaterializationPolicyFactory.topicCompacted(
                new TopicCompactionSpec("latest", 1, "test-key-v1"), 2, 16, 1_000, 1_000_000, 128, "ZSTD");
        MaterializationTask planned = MaterializationPlannerTestSupport.planner(sources, List.of(), 0, 4)
