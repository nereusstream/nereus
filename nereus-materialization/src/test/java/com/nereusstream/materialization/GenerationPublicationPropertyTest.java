        ...
        for (int publisherCount : List.of(2, 3, 5, 8, 13)) {
            try (GenerationPublicationTestSupport.Context context = GenerationPublicationTestSupport.context()) {
                ...
                CompletableFuture.allOf(futures.toArray(CompletableFuture[]::new))
                        .join();
                List<GenerationCommitResult> results =
                        futures.stream().map(CompletableFuture::join).toList();

                assertThat(results)
                        .extracting(GenerationCommitResult::generation)
                        .containsOnly(results.get(0).generation());
                assertThat(results)
                        .extracting(GenerationCommitResult::publicationId)
                        .containsOnly(results.get(0).publicationId());
                assertThat(results)
                        .extracting(GenerationCommitResult::indexRecordSha256)
