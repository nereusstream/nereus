...
public final class GcMetadataRetirementRegistry {
    ...
    public GcMetadataRetirementRegistry(List<GcMetadataRetirementHandler> handlers) {
        ...
        for (GcMetadataRetirementHandler handler : List.copyOf(Objects.requireNonNull(handlers, "handlers"))) {
            Objects.requireNonNull(handler, "handler");
            String type = canonicalType(handler.removalType());
            if (exact.put(type, handler) != null) {
                throw new IllegalArgumentException("metadata-retirement removal types must be unique: " + type);
            }
        }
    ...
    public void requireSupports(List<GcPlannedMetadataRemoval> removals) {
        for (GcPlannedMetadataRemoval removal : List.copyOf(Objects.requireNonNull(removals, "removals"))) {
            if (!handlers.containsKey(removal.removalType())) {
                throw invariant("no metadata-retirement handler is registered for " + removal.removalType());
            }
        }
    }
    ...
    public CompletableFuture<GcMetadataRetirementOutcome> retire(
        ...
        if (!context.journal().plannedMetadataRemovals().contains(exactRemoval)) {
            return CompletableFuture.failedFuture(
                    invariant("metadata retirement is not present in the authenticated journal"));
        }
        GcMetadataRetirementHandler handler = handlers.get(exactRemoval.removalType());
        if (handler == null) {
            return CompletableFuture.failedFuture(
                    invariant("no metadata-retirement handler is registered for " + exactRemoval.removalType()));
        }
        return deadline.bound(
                        () -> handler.retire(context, exactRemoval, deadline),
                        "retire journaled metadata type " + exactRemoval.removalType())
                .thenApply(outcome -> Objects.requireNonNull(outcome, "metadata-retirement handler outcome"));
    }
