...
public record PhysicalRootBackfillReport(
    ...
    public PhysicalRootBackfillReport {
        new PhysicalRootBackfillRequest(runId, brokerReadinessEpoch, 1, java.time.Duration.ofMillis(1));
        ...
        requireNonNegative(rootsCreatedOrVerified, "rootsCreatedOrVerified");
        requireNonNegative(protectionsCreatedOrVerified, "protectionsCreatedOrVerified");
        requireNonNegative(failureCount, "failureCount");
        dataCoverageSha256 = requireSha256(dataCoverageSha256, "dataCoverageSha256");
        cursorCoverageSha256 = requireSha256(cursorCoverageSha256, "cursorCoverageSha256");
        ...
        if (boundedFailures.stream().anyMatch(Objects::isNull)
                || boundedFailures.size() != Math.min(MAX_FAILURE_DETAILS, failureCount)) {
