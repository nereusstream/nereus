...
public final class OwnerlessObjectGcExecutor {
    ...
    public OwnerlessObjectGcExecutor(
        ...
        this.abandonedAppendIntents = Objects.requireNonNull(abandonedAppendIntents, "abandonedAppendIntents");
     ...
     */
    public CompletableFuture<ExecutionResult> executeActive(VersionedPhysicalObjectRoot activeRoot) {
        ...
        if (active.value().lifecycle() != PhysicalObjectLifecycle.ACTIVE) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("ownerless discovery requires an exact ACTIVE root"));
        }
        ...
        return abandonedAppendIntents.inspectActive(active).thenCompose(inspection -> {
            ...
            GcCandidate candidate = candidate(active, now, inspection.notBeforeMillis());
            return garbageCollector
                    .mark(candidate, inspection.protectionRemovals(), inspection.metadataRemovals())
                    .thenCompose(mark -> {
                        if (mark.status() != PhysicalGcMarkStatus.MARKED) {
                            return CompletableFuture.completedFuture(ExecutionResult.markOnly(object, mark));
                        }
     ...
     */
    public CompletableFuture<ExecutionResult> recoverMarked(VersionedPhysicalObjectRoot markedRoot) {
        ...
        if (marked.value().lifecycle() != PhysicalObjectLifecycle.MARKED) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("ownerless recovery requires an exact MARKED root"));
        }
        long now = nonNegativeNow();
        if (now < marked.value().createdAtMillis()) {
            return CompletableFuture.completedFuture(ExecutionResult.skipped(object(marked)));
        }
        GcCandidate candidate = candidate(marked, now, marked.value().deleteNotBeforeMillis());
        return abandonedAppendIntents.inspectMarked(marked).thenCompose(inspection -> {
            if (!inspection.eligible()) {
    ...
    private CompletableFuture<ExecutionResult> advance(GcPlan plan, Optional<PhysicalGcMarkResult> mark) {
        return garbageCollector
                .advanceToDeleteIntent(plan, ignored -> abandonedAppendIntents
    ...
    private GcCandidate candidate(VersionedPhysicalObjectRoot root, long now, long notBeforeMillis) {
        ...
        if (root.value().lifecycle() == PhysicalObjectLifecycle.ACTIVE) {
            return GcCandidate.fromActiveRoot(
                    config, candidateIds.next(), root, query, object.identitySha256(), now, notBeforeMillis);
        }
        return GcCandidate.fromMarkedRoot(config, candidateIds.next(), root, query, object.identitySha256(), now);
    }
    ...
    public record ExecutionResult(
        ...
        public ExecutionResult {
            ...
            if (deletion.isPresent()
                ...
                throw new IllegalArgumentException("ownerless deletion requires durable delete intent");
            }
        }

        private static ExecutionResult skipped(ObjectKeyHash object) {
            return new ExecutionResult(object, Optional.empty(), Optional.empty(), Optional.empty());
        }

        private static ExecutionResult markOnly(ObjectKeyHash object, PhysicalGcMarkResult mark) {
            return new ExecutionResult(object, Optional.of(mark), Optional.empty(), Optional.empty());
        }
        ...
        private static ExecutionResult advanced(
            ...
            return new ExecutionResult(object, mark, Optional.of(advance), Optional.empty());
        }
        ...
        private static ExecutionResult deleted(
            ...
            return new ExecutionResult(object, mark, Optional.of(advance), Optional.of(deletion));
        }
    }
}
