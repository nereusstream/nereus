...
public record GcDomainSnapshotProof(
        String domainId, int protocolVersion, Checksum queryIdentitySha256, Checksum snapshotSha256) {
    ...
    public static GcDomainSnapshotProof from(GcReferenceSnapshot snapshot) {
        ...
        if (!snapshot.complete() || snapshot.veto()) {
            throw new IllegalArgumentException("a retirement proof requires a complete non-vetoing snapshot");
        }
