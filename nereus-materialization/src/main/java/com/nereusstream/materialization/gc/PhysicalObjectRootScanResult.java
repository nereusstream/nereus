...
public record PhysicalObjectRootScanResult(
        long activeRoots, long markedRoots, long deletingRoots, long deletedRoots, long quarantinedRoots) {
    public PhysicalObjectRootScanResult {
        if (activeRoots < 0 || markedRoots < 0 || deletingRoots < 0 || deletedRoots < 0 || quarantinedRoots < 0) {
    ...
    public long totalRoots() {
        return Math.addExact(
                ...
                Math.addExact(deletingRoots, Math.addExact(deletedRoots, quarantinedRoots)));
    }
}
