 ...
 */
public final class GenerationZeroMarkerRetirementHandler implements GcMetadataRetirementHandler {
    ...
    public GenerationZeroMarkerRetirementHandler(String cluster, SourceRetirementMetadataStore sourceRetirement) {
        this(loader(cluster, sourceRetirement), delete(cluster, sourceRetirement));
    }

    GenerationZeroMarkerRetirementHandler(
            GenerationZeroMarkerLoader markerLoader, GenerationZeroMarkerDelete markerDelete) {
    ...
    public CompletableFuture<GcMetadataRetirementOutcome> retire(
        ...
        if (!REMOVAL_TYPE.equals(exact.removalType())) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("generation-zero marker handler received another removal type"));
        }
        return load(exact.key(), deadline).thenCompose(optional -> {
            ...
            CompletableFuture<Void> delete = deadline.bound(
                    () -> markerDelete.delete(exact.key(), exact.metadataVersion(), exact.durableValueSha256()),
            ...
            return delete.handle((ignored, failure) -> {
                        ...
                        return load(exact.key(), deadline).thenCompose(reloaded -> {
                            ...
                            return CompletableFuture.failedFuture(
                                    invariant("generation-zero committed marker changed after uncertain delete"));
                        });
                    })
    ...
    private CompletableFuture<Optional<VersionedGenerationZeroMarker>> load(
            String exactKey, MaterializationDeadline deadline) {
        return deadline.bound(() -> markerLoader.load(exactKey), "reload exact generation-zero committed marker")
                .thenApply(value -> Objects.requireNonNull(value, "generation-zero marker lookup result"));
    }

    private static void requireExact(VersionedGenerationZeroMarker marker, GcPlannedMetadataRemoval removal) {
        if (!matches(marker, removal)) {
            throw invariant("generation-zero marker no longer matches the sealed journal");
        }
    }

    private static boolean matches(VersionedGenerationZeroMarker marker, GcPlannedMetadataRemoval removal) {
    ...
    private static GenerationZeroMarkerLoader loader(String cluster, SourceRetirementMetadataStore sourceRetirement) {
    ...
    private static GenerationZeroMarkerDelete delete(String cluster, SourceRetirementMetadataStore sourceRetirement) {
        String exactCluster = requireText(cluster, "cluster");
        SourceRetirementMetadataStore exact = Objects.requireNonNull(sourceRetirement, "sourceRetirement");
        return (key, expectedVersion, expectedDigest) ->
                exact.deleteCommittedMarkerByKey(exactCluster, key, expectedVersion, expectedDigest);
    }
