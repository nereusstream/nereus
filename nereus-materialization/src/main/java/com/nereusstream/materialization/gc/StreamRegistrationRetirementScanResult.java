...
public record StreamRegistrationRetirementScanResult(
        int shardsScanned, long registrationsScanned, Map<StreamRegistrationRetirementStatus, Long> statusCounts) {
    public StreamRegistrationRetirementScanResult {
        ...
        for (StreamRegistrationRetirementStatus status : StreamRegistrationRetirementStatus.values()) {
            ...
            if (count == null || count < 0) {
                throw new IllegalArgumentException("statusCounts must contain every status with a non-negative count");
            }
            exact.put(status, count);
            total = Math.addExact(total, count);
        }
        if (statusCounts.size() != exact.size() || total != registrationsScanned) {
            throw new IllegalArgumentException("registration-retirement status counts do not match the completed pass");
        }
