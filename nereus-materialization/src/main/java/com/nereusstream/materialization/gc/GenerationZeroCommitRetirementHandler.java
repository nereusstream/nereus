 ...
 */
public final class GenerationZeroCommitRetirementHandler implements GcMetadataRetirementHandler {
    ...
    public GenerationZeroCommitRetirementHandler(String cluster, SourceRetirementMetadataStore sourceRetirement) {
        this(loader(cluster, sourceRetirement), delete(cluster, sourceRetirement));
    }

    GenerationZeroCommitRetirementHandler(
            GenerationZeroCommitLoader commitLoader, GenerationZeroCommitDelete commitDelete) {
    ...
    public CompletableFuture<GcMetadataRetirementOutcome> retire(
        ...
        if (!REMOVAL_TYPE.equals(exact.removalType())) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("generation-zero commit handler received another removal type"));
        }
        return load(exact.key(), deadline).thenCompose(optional -> {
            ...
            CompletableFuture<Void> delete = deadline.bound(
                    () -> commitDelete.delete(exact.key(), exact.metadataVersion(), exact.durableValueSha256()),
            ...
            return delete.handle((ignored, failure) -> {
                        ...
                        return load(exact.key(), deadline).thenCompose(reloaded -> {
                            ...
                            return CompletableFuture.failedFuture(
                                    invariant("generation-zero commit node changed after uncertain delete"));
                        });
                    })
    ...
    private CompletableFuture<Optional<VersionedGenerationZeroCommit>> load(
        ...
        return deadline.bound(() -> commitLoader.load(exactKey), "reload exact generation-zero commit node")
                .thenApply(value -> Objects.requireNonNull(value, "generation-zero commit lookup result"));
    }

    private static void requireExact(VersionedGenerationZeroCommit commit, GcPlannedMetadataRemoval removal) {
        if (!matches(commit, removal)) {
            throw invariant("generation-zero commit no longer matches the sealed journal");
        }
    }

    private static boolean matches(VersionedGenerationZeroCommit commit, GcPlannedMetadataRemoval removal) {
    ...
    private static GenerationZeroCommitLoader loader(String cluster, SourceRetirementMetadataStore sourceRetirement) {
    ...
    private static GenerationZeroCommitDelete delete(String cluster, SourceRetirementMetadataStore sourceRetirement) {
        String exactCluster = requireText(cluster, "cluster");
        SourceRetirementMetadataStore exact = Objects.requireNonNull(sourceRetirement, "sourceRetirement");
        return (key, expectedVersion, expectedDigest) ->
                exact.deleteCommitNodeByKey(exactCluster, key, expectedVersion, expectedDigest);
    }
