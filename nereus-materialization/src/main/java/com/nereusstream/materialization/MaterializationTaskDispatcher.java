...
public interface MaterializationTaskDispatcher {
    CompletableFuture<Void> dispatch(VersionedMaterializationTask durableTask, MaterializationTask task);

    /**
     * Rejects new tasks, cancels queued work and drains/cancels active work by the deadline.
     */
    default CompletableFuture<Void> closeAsync(Duration timeout, ScheduledExecutorService scheduler) {
