 ...
 * verification.
 */
public final class RangedMaterializationFormatVerifier implements MaterializationFormatVerifier {
    private final RangedCompactedObjectVerifier verifier;

    public RangedMaterializationFormatVerifier(RangedCompactedObjectVerifier verifier) {
    ...
    public CompletableFuture<Void> verify(MaterializationTask task, MaterializationOutput output, Duration timeout) {
        try {
            ...
            if (exactTask.view() != ReadView.COMMITTED
                    || exactTask.taskKind() != TaskKind.LOSSLESS_REWRITE
                    || !exactTask.policy().targetPhysicalFormat().equals(MaterializationPolicy.KAFKA_COMMITTED_FORMAT)
            ...
            RangedCompactedObjectVerificationRequest request = new RangedCompactedObjectVerificationRequest(
                    ...
                    timeout);
            return verifier.verify(request).thenAccept(metadata -> requireAgreement(exactTask, exactOutput, metadata));
    ...
    private static void requireAgreement(
            MaterializationTask task, MaterializationOutput output, RangedCompactedObjectMetadata metadata) {
        if (!task.taskId().equals(output.taskId())
                ...
                || !metadata.rangeModel().equals(CompactedObjectFormatV2.RANGE_MODEL)
                ...
                || !output.physicalFormat().equals(CompactedObjectFormatV2.COMMITTED_PHYSICAL_FORMAT)
            ...
            throw new CompactedObjectFormatException("NCP2 metadata does not match task/output publication facts");
        }
    }
}
