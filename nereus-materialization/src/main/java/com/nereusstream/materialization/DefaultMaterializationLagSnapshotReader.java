 ...
 */
public final class DefaultMaterializationLagSnapshotReader implements MaterializationLagSnapshotReader {
    ...
    private static final Comparator<VersionedGenerationIndex> COVER_ORDER = Comparator.comparingLong(
            ...
            .thenComparing(Comparator.comparingLong(
                    ...
                    .reversed())
            .thenComparing(VersionedGenerationIndex::key, DefaultMaterializationLagSnapshotReader::compareUtf8);
    ...
    public DefaultMaterializationLagSnapshotReader(
        ...
        if (maxLiveCommits <= 0) {
            throw new IllegalArgumentException("maxLiveCommits must be positive");
        }
        this.pageSize = pageSize;
        this.maxLiveCommits = maxLiveCommits;
        this.walker = new AnchorAwareCommitWalker(this.cluster, l0, generations);
        this.scheduler = Objects.requireNonNull(scheduler, "scheduler");
        this.clock = Objects.requireNonNull(clock, "clock");
    }

    @Override
    public CompletableFuture<MaterializationLagSnapshot> measure(StreamId streamId, Duration timeout) {
        final Operation operation;
        try {
            operation = new Operation(Objects.requireNonNull(streamId, "streamId"), timeout);
    ...
    private final class Operation implements AutoCloseable {
        ...
        private Operation(StreamId streamId, Duration timeout) {
            this.streamId = streamId;
            this.deadline = new MaterializationDeadline(timeout, scheduler);
        }
        ...
        private CompletableFuture<MaterializationLagSnapshot> attempt(int attempt, String lastInstability) {
            if (attempt >= MAX_STABILITY_ATTEMPTS) {
                return CompletableFuture.failedFuture(
                        condition("materialization lag authority changed throughout measurement: " + lastInstability));
            }
            return loadAuthority().thenCompose(authority -> scanCoverage(
                            authority.bounds(), authority.registration().value().projectionRef())
                    .thenCompose(coverage -> {
                        requireCheckpointNotAhead(authority.checkpoint(), coverage);
                        return deadline.bound(
                                        () -> walker.walk(streamId, maxLiveCommits, pageSize),
                                        "walk stable append tail for materialization lag")
                                .thenCompose(walk -> measureStable(authority, coverage, walk))
                                .thenCompose(measured -> finalRevalidate(authority, coverage, measured.walk())
                                        .thenCompose(instability -> instability.isEmpty()
                                                ? CompletableFuture.completedFuture(measured.snapshot())
                                                : attempt(attempt + 1, instability.orElseThrow())));
                    }));
        }

        private CompletableFuture<Authority> loadAuthority() {
            return deadline.bound(
                            () -> l0.getStreamSnapshot(cluster, streamId),
                    ...
                    .thenCompose(snapshot -> deadline.bound(
                            ...
                            .thenCompose(registration -> deadline.bound(
                                    ...
                                    .thenApply(checkpoint -> authority(
                                            snapshot,
        ...
        private Authority authority(
            ...
            if (!snapshot.metadata().streamId().equals(streamId.value())
                ...
                throw invariant("materialization lag authority belongs to another stream", null);
            }
            ...
            try {
                ...
                throw invariant("materialization lag authority has an unknown state or profile", failure);
            }
            if ((state != StreamState.ACTIVE && state != StreamState.SEALED)
                    || profile != registeredProfile
                    || !profile.objectMaterializationEnabled()) {
                throw condition("stream no longer admits materialization lag measurement");
            }
            long trim = snapshot.trim().trimOffset();
            long head = snapshot.committedEnd().committedEndOffset();
            if (trim > head) {
                throw invariant("stream trim is ahead of committed end", null);
            }
            checkpoint.ifPresent(value -> requireCheckpointIdentity(value.value()));
            return new Authority(snapshot, registration, checkpoint, new Bounds(trim, head));
        }

        private void requireCheckpointIdentity(MaterializationCheckpointRecord value) {
            if (!value.streamId().equals(streamId.value())
                ...
                throw invariant("materialization lag checkpoint identity mismatch", null);
            }
        }

        private CompletableFuture<Coverage> scanCoverage(Bounds bounds, String projectionRef) {
            if (bounds.trimOffset() >= bounds.headOffset()) {
                return CompletableFuture.completedFuture(Coverage.at(bounds.trimOffset()));
            }
            return scanCandidates(bounds, Optional.empty(), new ArrayList<>())
                    .thenApply(candidates -> coverage(bounds, projectionRef, candidates));
        }

        private CompletableFuture<List<VersionedGenerationCandidate>> scanCandidates(
                Bounds bounds, Optional<F4ScanToken> continuation, ArrayList<VersionedGenerationCandidate> values) {
            ...
            return deadline.bound(
                    ...
                    .thenCompose(page -> appendCandidatePage(bounds, values, page));
        }

        private CompletableFuture<List<VersionedGenerationCandidate>> appendCandidatePage(
                Bounds bounds, ArrayList<VersionedGenerationCandidate> values, GenerationScanPage page) {
            values.addAll(page.values());
            if (values.size() > MAX_CANDIDATES) {
                return CompletableFuture.failedFuture(new NereusException(
                        ErrorCode.METADATA_LIMIT_EXCEEDED, false, "materialization lag candidate count exceeds 4096"));
            }
            if (page.continuation().isPresent()) {
                return scanCandidates(bounds, page.continuation(), values);
            }
        ...
        private Coverage coverage(Bounds bounds, String projectionRef, List<VersionedGenerationCandidate> candidates) {
            ...
            ArrayList<VersionedGenerationIndex> selected = new ArrayList<>();
            int steps = 0;
            while (cursor < bounds.headOffset()) {
                final long current = cursor;
                VersionedGenerationIndex next = healthy.stream()
                ...
                if (++steps > healthy.size()) {
                    throw invariant("materialization lag coverage did not converge", null);
                }
                selected.add(next);
        ...
        private boolean satisfiesPolicy(VersionedGenerationIndex candidate, String projectionRef, Bounds bounds) {
            ...
            if (!value.streamId().equals(streamId.value())
                ...
                throw invariant("generation scan returned an out-of-scope lag candidate", null);
            }
            ...
            try {
                if (!(targetCodecs.decode(value.readTarget()) instanceof ObjectSliceReadTarget target)
                    ...
                    throw invariant("current-policy lag target is incompatible", null);
                }
            } catch (NereusException failure) {
                throw failure;
            } catch (RuntimeException failure) {
                throw invariant("current-policy lag target cannot be decoded", failure);
            }
            return true;
        }

        private void requireCheckpointNotAhead(
                Optional<VersionedMaterializationCheckpoint> checkpoint, Coverage coverage) {
            checkpoint.ifPresent(value -> {
                if (value.value().contiguousCoveredOffset() > coverage.coveredOffset()) {
                    throw invariant("materialization checkpoint is ahead of verified committed coverage", null);
                }
            });
        }

        private CompletableFuture<Measured> measureStable(
                Authority authority, Coverage coverage, AnchorAwareCommitWalk walk) {
            requireWalkMatchesHead(authority.snapshot(), walk);
            long head = authority.bounds().headOffset();
            long covered = coverage.coveredOffset();
            if (covered == head) {
                return CompletableFuture.completedFuture(new Measured(
                        new MaterializationLagSnapshot(
                                ...
                                0,
                                0,
                                authority.snapshot().metadataVersion(),
                                Math.max(0, clock.millis())),
                        walk));
            }
            long anchor = walk.anchor().offsetEnd();
            if (anchor > head) {
                return CompletableFuture.failedFuture(invariant("recovery anchor is ahead of committed end", null));
            }
            SourceLag live = liveLag(walk, Math.max(covered, anchor));
            CompletableFuture<SourceLag> source;
            if (covered >= anchor) {
                source = CompletableFuture.completedFuture(live);
            } else {
                source = scanAndVerifyStats(covered, anchor, live.cumulativeSizeAtStart())
                        .thenApply(stats -> combine(stats, live));
            }
            return source.thenApply(lag -> {
                long lagRecords = Math.subtractExact(head, covered);
                long lagBytes = Math.subtractExact(
                        authority.snapshot().committedEnd().cumulativeSize(), lag.cumulativeSizeAtStart());
                if (lagRecords <= 0 || lagBytes < 0) {
                    throw invariant("non-empty materialization lag has invalid record or byte accounting", null);
                }
                long now = Math.max(0, clock.millis());
                long age = now <= lag.oldestPublishTimeMillis()
                        ? 0
                        : Math.subtractExact(now, lag.oldestPublishTimeMillis());
                return new Measured(
                        new MaterializationLagSnapshot(
                                streamId,
                                covered,
        ...
        private SourceLag liveLag(AnchorAwareCommitWalk walk, long covered) {
            AppendRecoveryCommit oldest = null;
            for (AppendRecoveryCommit commit : walk.commitsNewestFirst()) {
                if (commit.canonicalCommit().offsetEnd() > covered) {
                    oldest = commit;
                }
            }
            if (oldest == null) {
                if (covered == walk.observedHead().offsetEnd()) {
                    return SourceLag.empty(walk.observedHead().cumulativeSize());
                }
                throw condition("stable live tail does not cover materialization lag start");
            }
            var value = oldest.canonicalCommit();
            if (value.offsetStart() != covered) {
                throw condition("materialization lag start is not a complete live commit boundary");
            }
            return new SourceLag(
                    Math.subtractExact(value.cumulativeSize(), value.logicalBytes()), value.preparedAtMillis(), false);
        }

        private CompletableFuture<SourceLag> scanAndVerifyStats(
                long covered, long anchor, long cumulativeSizeAtAnchor) {
            return scanStats(covered, anchor, Optional.empty(), new ArrayList<>())
                    .thenCompose(stats -> verifyStats(stats, 0)
                            .thenApply(ignored -> statsLag(covered, anchor, cumulativeSizeAtAnchor, stats)));
        }

        private CompletableFuture<List<VersionedRangeRetentionStats>> scanStats(
                long covered,
                long anchor,
                Optional<F4ScanToken> continuation,
                ArrayList<VersionedRangeRetentionStats> values) {
            long minimum = Math.addExact(covered, 1);
            return deadline.bound(
                            () -> generations.scanRangeRetentionStats(
                                    cluster, streamId, minimum, anchor, continuation, pageSize),
                            "scan retention stats for checkpointed materialization lag")
                    .thenCompose(page -> appendStatsPage(covered, anchor, values, page));
        }

        private CompletableFuture<List<VersionedRangeRetentionStats>> appendStatsPage(
                long covered,
                long anchor,
                ArrayList<VersionedRangeRetentionStats> values,
            ...
            if (values.size() > MAX_RETENTION_STATS) {
                return CompletableFuture.failedFuture(new NereusException(
                        ErrorCode.METADATA_LIMIT_EXCEEDED, false, "materialization lag retention stats exceed 4096"));
            }
            if (page.continuation().isPresent()) {
                return scanStats(covered, anchor, page.continuation(), values);
            }
            return CompletableFuture.completedFuture(values.stream()
        ...
        private CompletableFuture<Void> verifyStats(List<VersionedRangeRetentionStats> stats, int index) {
            if (index >= stats.size()) {
                return CompletableFuture.completedFuture(null);
            }
            VersionedRangeRetentionStats expected = stats.get(index);
            RangeRetentionStatsRecord value = expected.value();
            return deadline.bound(
                            () -> generations.getCandidateByKey(
                                    cluster, streamId, ReadView.COMMITTED, value.sourceIndexKey()),
                            "revalidate retention stats source index")
                    .thenAccept(optional -> requireStatsSource(
                            expected,
                            optional.orElseThrow(() -> condition("materialization lag stats source index is absent"))))
                    .thenCompose(ignored -> verifyStats(stats, index + 1));
        }

        private SourceLag statsLag(
                long covered, long anchor, long cumulativeSizeAtAnchor, List<VersionedRangeRetentionStats> stats) {
            if (stats.isEmpty()) {
                throw condition("checkpointed materialization lag has no retention stats");
            }
            long offset = covered;
            long cumulative = -1;
            ...
            for (VersionedRangeRetentionStats wrapper : stats) {
                ...
                if (!value.streamId().equals(streamId.value())
                        ...
                        || (cumulative >= 0 && value.cumulativeSizeAtStart() != cumulative)) {
                    throw condition("retention stats do not form exact materialization lag coverage");
                }
                offset = value.offsetEnd();
                cumulative = value.cumulativeSizeAtEnd();
                oldest = Math.min(oldest, value.minPublishTimeMillis());
            }
            if (offset != anchor || cumulative != cumulativeSizeAtAnchor) {
                throw condition("retention stats do not reach the recovery anchor");
            }
            RangeRetentionStatsRecord first = stats.get(0).value();
            return new SourceLag(first.cumulativeSizeAtStart(), oldest, false);
        }

        private void requireStatsSource(VersionedRangeRetentionStats stats, VersionedGenerationCandidate candidate) {
            RangeRetentionStatsRecord expected = stats.value();
            if (!candidate.key().equals(expected.sourceIndexKey())
                    || candidate.metadataVersion() != expected.sourceIndexMetadataVersion()
                    || !candidate.durableValueSha256().value().equals(expected.sourceIndexIdentitySha256())) {
                throw condition("retention stats source identity changed");
            }
            if (candidate instanceof VersionedGenerationZeroIndex zero) {
                var value = zero.value();
                long cumulativeStart = Math.subtractExact(value.cumulativeSize(), value.logicalBytes());
                if (value.tombstoned()
                    ...
                    throw condition("generation-zero retention stats source changed");
                }
                return;
            }
            ...
            if (value.lifecycle() != GenerationLifecycle.COMMITTED
                ...
                throw condition("higher-generation retention stats source changed");
            }
        }

        private SourceLag combine(SourceLag checkpointed, SourceLag live) {
            if (live.empty()) {
                return checkpointed;
            }
            return new SourceLag(
                    checkpointed.cumulativeSizeAtStart(),
                    Math.min(checkpointed.oldestPublishTimeMillis(), live.oldestPublishTimeMillis()),
                    false);
        }
        ...
        private CompletableFuture<Optional<String>> finalRevalidate(
            ...
            return deadline.bound(
                    ...
                    .thenCompose(snapshot -> deadline.bound(
                            ...
                            .thenCompose(registration -> deadline.bound(
                                            () -> generations.getRecoveryRoot(cluster, streamId),
                                            "revalidate recovery root after materialization lag")
                                    .thenCompose(root -> {
                                        if (!sameStreamAuthority(snapshot, authority.snapshot())) {
                                            return CompletableFuture.completedFuture(
                                                    Optional.of("stream snapshot changed"));
                                        }
                                        if (!registration.equals(Optional.of(authority.registration()))) {
                                            return CompletableFuture.completedFuture(
                                                    Optional.of("stream registration changed"));
                                        }
                                        if (!root.equals(walk.recoveryRoot())) {
                                            return CompletableFuture.completedFuture(
                                                    Optional.of("recovery root changed"));
                                        }
                                        return scanCoverage(
                                                        authority.bounds(),
                                                        authority
                                                                ...
                                                                .value()
                                                                .projectionRef())
                                                .thenApply(current -> coverage.equals(current)
                                                        ? Optional.<String>empty()
                                                        : Optional.of("committed generation coverage changed"));
                                    })));
        }

        private void requireWalkMatchesHead(StreamMetadataSnapshot snapshot, AnchorAwareCommitWalk walk) {
            var head = walk.observedHead();
            if (!head.streamId().equals(streamId)
                    || head.offsetEnd() != snapshot.committedEnd().committedEndOffset()
                    || head.cumulativeSize() != snapshot.committedEnd().cumulativeSize()
                    || head.commitVersion() != snapshot.committedEnd().commitVersion()
                    || head.metadataVersion() != snapshot.metadataVersion()
                    || !walk.anchorReached()) {
                throw condition("append recovery tail changed or exceeded the materialization lag bound");
            }
        }
    ...
    private static MaterializationPolicy requirePolicy(MaterializationPolicy policy) {
        ...
        if (policy.view() != ReadView.COMMITTED
            ...
            throw new IllegalArgumentException("lag reader requires a lossless COMMITTED NCP1/NCP2 policy");
        }
        return policy;
    }
    ...
    private static boolean sameStreamAuthority(StreamMetadataSnapshot current, StreamMetadataSnapshot expected) {
    ...
    private record Coverage(long coveredOffset, List<VersionedGenerationIndex> selected) {
        private Coverage {
            if (coveredOffset < 0) {
                throw new IllegalArgumentException("coveredOffset must be non-negative");
            }
    ...
    private record SourceLag(long cumulativeSizeAtStart, long oldestPublishTimeMillis, boolean empty) {
        private SourceLag {
            if (cumulativeSizeAtStart < 0 || oldestPublishTimeMillis < 0) {
                throw new IllegalArgumentException("source lag facts must be non-negative");
            }
        }

        private static SourceLag empty(long cumulativeSize) {
            return new SourceLag(cumulativeSize, 0, true);
        }
    }

    private record Measured(MaterializationLagSnapshot snapshot, AnchorAwareCommitWalk walk) {}
}
