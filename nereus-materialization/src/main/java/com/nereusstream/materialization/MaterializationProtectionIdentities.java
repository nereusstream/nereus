...
final class MaterializationProtectionIdentities {
    ...
    static ObjectProtectionOwner taskOwner(VersionedMaterializationTask task) {
        return new ObjectProtectionOwner(task.key(), task.metadataVersion(), task.durableValueSha256());
    }

    static ObjectProtectionOwner indexOwner(VersionedGenerationIndex index) {
        return new ObjectProtectionOwner(index.key(), index.metadataVersion(), index.durableValueSha256());
    }

    static String sourceReferenceId(String cluster, MaterializationTask task, SourceGeneration source) {
        String canonical = cluster + '\0' + task.streamId().value() + '\0' + task.taskId() + '\0' + source.indexKey();
        return "ms1-" + DeterministicIds.stableHashComponent(canonical);
    }

    static String outputReferenceId(String cluster, MaterializationTask task, MaterializationOutput output) {
    ...
    static String visibleReferenceId(String cluster, MaterializationTask task, long generation, String publicationId) {
        String canonical = cluster
                + '\0'
                + task.streamId().value()
                + '\0'
