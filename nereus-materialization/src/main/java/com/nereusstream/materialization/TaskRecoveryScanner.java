...
public final class TaskRecoveryScanner {
    ...
    public TaskRecoveryScanner(MaterializationTaskStore tasks, MaterializationTaskRecovery recovery, int pageSize) {
    ...
    public CompletableFuture<TaskRecoveryScanResult> scan(
            StreamId streamId, MaterializationTaskMutationGuard mutationGuard) {
        try {
            return scanPage(
    ...
    private CompletableFuture<TaskRecoveryScanResult> scanPage(
        ...
        return tasks.scan(streamId, continuation, pageSize)
                .thenCompose(page -> recoverPage(page, mutationGuard, 0, scanned, actions)
                        .thenCompose(nextScanned -> {
                            if (page.continuation().isPresent()) {
                                return scanPage(streamId, mutationGuard, page.continuation(), nextScanned, actions);
                            }
                            return CompletableFuture.completedFuture(
                                    new TaskRecoveryScanResult(nextScanned, Map.copyOf(actions)));
                        }));
    }
    ...
    private CompletableFuture<Integer> recoverPage(
        ...
        return recovery.recover(task, mutationGuard).thenCompose(action -> {
            ...
            return recoverPage(page, mutationGuard, index + 1, Math.addExact(scanned, 1), actions);
        });
    }
}
