...
public record RecoveryCheckpointBuildResult(
        RecoveryCheckpointBuildStatus status, Optional<RecoveryCheckpointPlan> plan) {
    ...
    public static RecoveryCheckpointBuildResult ready(RecoveryCheckpointPlan plan) {
        return new RecoveryCheckpointBuildResult(RecoveryCheckpointBuildStatus.READY, Optional.of(plan));
    }
