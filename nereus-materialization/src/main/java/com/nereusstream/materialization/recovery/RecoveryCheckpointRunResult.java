...
public record RecoveryCheckpointRunResult(
        RecoveryCheckpointBuildStatus status, Optional<PublishedRecoveryCheckpoint> publication) {
    public RecoveryCheckpointRunResult {
        ...
        if ((status == RecoveryCheckpointBuildStatus.READY) != publication.isPresent()) {
            throw new IllegalArgumentException("only a published READY checkpoint may carry publication facts");
        }
    }

    public static RecoveryCheckpointRunResult skipped(RecoveryCheckpointBuildStatus status) {
    ...
    public static RecoveryCheckpointRunResult published(PublishedRecoveryCheckpoint publication) {
        return new RecoveryCheckpointRunResult(RecoveryCheckpointBuildStatus.READY, Optional.of(publication));
    }
}
