...
public record NereusRuntimeConfiguration(
    ...
    public NereusRuntimeConfiguration {
        ...
        bookKeeper.ifPresent(configuration -> validateBookKeeper(configuration, streamStorage, managedLedger));
        ...
        if (managedLedger.metadataTimeout().compareTo(oxia.requestTimeout()) != 0
                ...
                || cursorStorage.cursorMetadataOperationTimeout().compareTo(managedLedger.metadataTimeout()) != 0) {
        ...
        if (materialization.maxConcurrentWorkers() > streamStorage.maxConcurrentObjectReads()) {
            throw new IllegalArgumentException("materialization workers exceed core object-read concurrency");
        }
        long materializationReadBytes;
        try {
            materializationReadBytes =
                    Math.multiplyExact(materialization.sourceReadPageBytes(), materialization.maxConcurrentWorkers());
        } catch (ArithmeticException failure) {
            throw new IllegalArgumentException("materialization read-buffer requirement overflows", failure);
        }
        if (materializationReadBytes > streamStorage.maxReadBufferBytes()) {
            throw new IllegalArgumentException("materialization worker reads exceed the core read-buffer budget");
        }
        if (objectStore.requestTimeout().compareTo(materialization.operationTimeout()) > 0) {
            throw new IllegalArgumentException("object-store timeout must fit the materialization operation deadline");
        }
        if (materialization.closeTimeout().compareTo(managedLedger.closeTimeout()) > 0) {
            throw new IllegalArgumentException("materialization close timeout must fit managed-ledger close timeout");
        }
        if (retention.operationTimeout().compareTo(retention.closeTimeout()) > 0) {
            throw new IllegalArgumentException("retention operation timeout must fit retention close timeout");
        }
        if (retention.closeTimeout().compareTo(managedLedger.closeTimeout()) > 0) {
            throw new IllegalArgumentException("retention close timeout must fit managed-ledger close timeout");
        }
        if (retention.maxQueuedPlans() > managedLedger.maxPendingCallbacks()) {
            throw new IllegalArgumentException("retention queue exceeds managed-ledger callback capacity");
        }
        ...
        if (!physicalGc.maximumClockSkew().equals(materialization.maximumClockSkew())) {
            throw new IllegalArgumentException("physical GC and materialization maximumClockSkew must match");
        }
        if (objectStore.requestTimeout().compareTo(physicalGc.operationTimeout()) > 0) {
            throw new IllegalArgumentException("object-store timeout must fit the physical-GC operation deadline");
        }
        if (physicalGc.operationTimeout().compareTo(physicalGc.closeTimeout()) > 0) {
            throw new IllegalArgumentException("physical-GC operation timeout must fit its close timeout");
        }
        if (physicalGc.closeTimeout().compareTo(managedLedger.closeTimeout()) > 0) {
            throw new IllegalArgumentException("physical-GC close timeout must fit managed-ledger close timeout");
        }
        if (cursorStorage.cursorRecordsPerStreamMax()
            ...
            throw new IllegalArgumentException("cursor inventory exceeds the physical-GC complete-scan bound");
        }
    }

    private static MaterializationConfig defaultMaterialization(StreamStorageConfig streamStorage) {
        Objects.requireNonNull(streamStorage, "streamStorage");
        ...
        Path staging = Path.of(temporary, "nereus-materialization", streamStorage.processRunId())
                .toAbsolutePath()
                .normalize();
    ...
    private static void validateBookKeeper(
        ...
        if (!wal.clusterAlias().equals(streamStorage.cluster())) {
            throw new IllegalArgumentException("BookKeeper clusterAlias must equal the Nereus stream cluster");
        }
        ...
        if (wal.allocationTimeout().compareTo(streamStorage.appendTimeout()) > 0
            ...
            throw new IllegalArgumentException("BookKeeper allocation/seal/delete timeout exceeds its outer deadline");
        }
        if (wal.maxWritesInFlight() > streamStorage.maxInFlightAppends()) {
            throw new IllegalArgumentException("BookKeeper write concurrency exceeds core append concurrency");
        }
        if (wal.maxReadsInFlight() > streamStorage.maxConcurrentObjectReads()
                || wal.maxReadBytesInFlight() > streamStorage.maxReadBufferBytes()) {
            throw new IllegalArgumentException("BookKeeper read limits exceed core read resources");
        }
