...
public final class DefaultPhase4PhysicalDeletionActivationCoordinator
    ...
    DefaultPhase4PhysicalDeletionActivationCoordinator(
        ...
        this.expectedCapabilitySha256 =
                requireSha256(capabilityProbe.expectedCapabilitySha256(), "expectedCapabilitySha256");
    ...
    public CompletableFuture<ManagedLedgerPhysicalDeletionActivationResult> activate(
        ...
        try {
            ...
            if (!activationEnabled) {
                throw notReady("physical deletion activation requires enabled non-dry-run configuration");
            }
        ...
        return deadline.call(readinessProvider::requireGenerationCapabilityReadiness)
                .thenCompose(readiness -> deadline.call(() -> activations.get(cluster))
                        .thenCompose(optional -> start(exact, readiness, requireActivation(optional), deadline)));
    }
    ...
    public CompletableFuture<VersionedGenerationProtocolActivation> rollover(
        ...
        try {
            ...
            if (!activationEnabled) {
                throw notReady("deletion-active readiness rollover requires enabled non-dry-run configuration");
            }
            ...
            new ManagedLedgerPhysicalDeletionActivationRequest(
                    exactRegistration.runId(), maxConcurrentStreams, timeout);
        ...
        return deadline.call(readinessProvider::requireGenerationCapabilityReadiness)
                .thenCompose(readiness -> {
                    requireExactReadiness(exactRegistration.readiness(), readiness);
                    return deadline.call(() -> activations.get(cluster));
                })
                .thenCompose(optional -> {
                    VersionedGenerationProtocolActivation observed = requireActivation(optional);
                    if (!observed.equals(exactCurrent)) {
                        if (sameRolloverAuthority(observed.value(), exactRegistration)) {
                            return CompletableFuture.completedFuture(observed);
                        }
                        return CompletableFuture.failedFuture(
                                notReady("generation activation changed before readiness rollover backfill"));
                    }
                    PhysicalRootBackfillRequest request = new PhysicalRootBackfillRequest(
                            exactRegistration.runId(),
                            exactRegistration.readiness().brokerReadinessEpoch(),
                            maxConcurrentStreams,
                            deadline.remaining());
                    return deadline.call(() -> backfill.runRollover(request, exactCurrent))
                            .thenCompose(report ->
                                    afterRolloverBackfill(exactRegistration, exactCurrent, request, report, deadline));
                })
                .thenCompose(installed -> finalRolloverRevalidate(exactRegistration, installed, deadline));
    }

    private CompletableFuture<VersionedGenerationProtocolActivation> afterRolloverBackfill(
            GenerationRegistrationBackfillCompletion registration,
            VersionedGenerationProtocolActivation current,
        ...
        try {
            probeRequest = new ObjectStoreDeleteCapabilityRequest(registration.runId(), deadline.remaining());
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
        return deadline.call(() -> capabilityProbe.probe(probeRequest)).thenCompose(proof -> {
            requireCapabilityProof(proof);
            requireCurrentReadiness(registration.readiness());
            RolloverProofs proofs = rolloverProofs(registration, coverage, proof);
            return installRollover(registration, proofs, current, 0, deadline);
        });
    }

    private CompletableFuture<VersionedGenerationProtocolActivation> installRollover(
            GenerationRegistrationBackfillCompletion registration,
        ...
        if (attempt >= MAX_CAS_ATTEMPTS) {
            return CompletableFuture.failedFuture(notReady("readiness rollover CAS retry budget exhausted"));
        }
        final GenerationProtocolActivationRecord replacement;
        try {
            requireCurrentReadiness(registration.readiness());
            requireRolloverSource(registration, current);
            replacement = rolloverReplacement(current.value(), registration, proofs);
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
        return deadline.call(() -> activations.compareAndSet(cluster, replacement, current.metadataVersion()))
                .handle((updated, failure) -> {
                    ...
                    return deadline.call(() -> activations.get(cluster)).thenCompose(optional -> {
                        VersionedGenerationProtocolActivation reloaded = requireActivation(optional);
                        if (sameRolloverAuthority(reloaded.value(), registration)) {
                            return CompletableFuture.completedFuture(reloaded);
                        }
                        if (cause instanceof F4MetadataConditionFailedException
                                && reloaded.value().brokerCapabilityReadinessEpoch()
                                        < registration.readiness().brokerReadinessEpoch()) {
                            return installRollover(registration, proofs, reloaded, attempt + 1, deadline);
                        }
    ...
    private CompletableFuture<VersionedGenerationProtocolActivation> finalRolloverRevalidate(
            GenerationRegistrationBackfillCompletion registration,
            VersionedGenerationProtocolActivation installed,
            Deadline deadline) {
        try {
            requireCurrentReadiness(registration.readiness());
            if (!sameRolloverAuthority(installed.value(), registration)) {
                throw notReady("installed deletion authority does not match the new readiness epoch");
            }
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
        return deadline.call(() -> activations.get(cluster)).thenApply(optional -> {
            VersionedGenerationProtocolActivation current = requireActivation(optional);
            if (!sameRolloverAuthority(current.value(), registration)) {
                throw notReady("deletion authority changed after readiness rollover");
            }
            requireCurrentReadiness(registration.readiness());
            return current;
        });
    ...
    private CompletableFuture<VersionedGenerationProtocolActivation> resolveConcurrentRollover(
            GenerationRegistrationBackfillCompletion registration, Throwable original, Deadline deadline) {
        return deadline.call(() -> activations.get(cluster)).thenCompose(optional -> {
            VersionedGenerationProtocolActivation current = requireActivation(optional);
            if (sameRolloverAuthority(current.value(), registration)) {
    ...
    private CompletableFuture<ManagedLedgerPhysicalDeletionActivationResult> start(
        ...
        try {
            ...
            if (deletionEnabled(current.value())) {
                ...
                return finalRevalidate(request.runId(), basis, coverage, Status.ALREADY_ACTIVE, deadline);
            }
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
        ...
        return deadline.call(() -> backfill.run(backfillRequest))
                .thenCompose(report -> afterBackfill(request, basis, report, deadline));
    }
    ...
    private CompletableFuture<ManagedLedgerPhysicalDeletionActivationResult> afterBackfill(
        ...
        return deadline.call(() -> activations.get(cluster)).thenCompose(optional -> {
            ...
            return probeAndActivate(request, basis, coverage, current, deadline);
        });
    }
    ...
    private CompletableFuture<ManagedLedgerPhysicalDeletionActivationResult> probeAndActivate(
        ...
        try {
            requireCurrentReadiness(basis.readiness());
            probeRequest = new ObjectStoreDeleteCapabilityRequest(request.runId(), deadline.remaining());
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
        return deadline.call(() -> capabilityProbe.probe(probeRequest))
                .thenCompose(proof -> {
                    requireCapabilityProof(proof);
                    requireCurrentReadiness(basis.readiness());
                    return install(basis, coverage, proof, current, 0, deadline);
                })
                .thenCompose(
                        installed -> finalRevalidate(request.runId(), basis, coverage, installed.status(), deadline));
    }
    ...
    private CompletableFuture<InstallResult> install(
            ...
            Deadline deadline) {
        if (attempt >= MAX_CAS_ATTEMPTS) {
            return CompletableFuture.failedFuture(notReady("physical deletion activation CAS retry budget exhausted"));
        }
        try {
            requireCurrentReadiness(basis.readiness());
            requireCoverageAuthority(basis, coverage, current.value());
            if (deletionEnabled(current.value())) {
                requireDeletionAuthority(basis, coverage, current.value());
                return CompletableFuture.completedFuture(new InstallResult(current, Status.ALREADY_ACTIVE));
            }
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
        ...
        GenerationProtocolActivationRecord replacement =
                deletionReplacement(current.value(), proof.completedAtMillis());
        return deadline.call(() -> activations.compareAndSet(cluster, replacement, current.metadataVersion()))
                .handle((updated, failure) -> {
                    if (failure == null) {
                        try {
                            requireDeletionAuthority(basis, coverage, updated.value());
                            return CompletableFuture.completedFuture(new InstallResult(updated, Status.ACTIVATED));
                        } catch (Throwable invalid) {
                            return CompletableFuture.<InstallResult>failedFuture(invalid);
                        }
                    }
                    Throwable cause = unwrap(failure);
                    return deadline.call(() -> activations.get(cluster)).thenCompose(optional -> {
                        VersionedGenerationProtocolActivation reloaded = requireActivation(optional);
                        requireCoverageAuthority(basis, coverage, reloaded.value());
                        if (deletionEnabled(reloaded.value())) {
                            requireDeletionAuthority(basis, coverage, reloaded.value());
                            Status status = cause instanceof F4MetadataConditionFailedException
                                    ? Status.ALREADY_ACTIVE
                                    : Status.ACTIVATED;
                            return CompletableFuture.completedFuture(new InstallResult(reloaded, status));
                        }
                        if (cause instanceof F4MetadataConditionFailedException) {
                            return install(basis, coverage, proof, reloaded, attempt + 1, deadline);
                        }
                        return CompletableFuture.failedFuture(cause);
                    });
                })
    ...
    private CompletableFuture<ManagedLedgerPhysicalDeletionActivationResult> finalRevalidate(
            String runId, ActivationBasis basis, Coverage coverage, Status status, Deadline deadline) {
        try {
            requireCurrentReadiness(basis.readiness());
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
    ...
    private ActivationBasis captureBasis(
            GenerationCapabilityReadiness readiness, GenerationProtocolActivationRecord activation) {
        requireBaseAuthority(readiness, activation);
        return new ActivationBasis(readiness, activation.streamRegistrationBackfill());
    }

    private void requireRolloverSource(
            GenerationRegistrationBackfillCompletion registration, VersionedGenerationProtocolActivation current) {
        ...
        if (value.lifecycle() != GenerationProtocolActivationLifecycle.ACTIVE || !value.publicationEnabled()) {
            throw notReady("generation publication must be ACTIVE before readiness rollover");
        }
        ...
        if (!deletionEnabled(value)) {
            throw notReady("readiness rollover requires active physical deletion authority");
        }
        if (!value.objectStoreCapabilitySha256().equals(expectedCapabilitySha256)) {
            throw invariant("active physical deletion authority belongs to another object-store scope");
        }
        long nextEpoch = registration.readiness().brokerReadinessEpoch();
        if (nextEpoch == value.brokerCapabilityReadinessEpoch()) {
            throw notReady("deletion-active readiness rollover requires a changed opaque epoch token");
        }
        requireCompleteProofForEpoch(
                value.streamRegistrationBackfill(), value.brokerCapabilityReadinessEpoch(), "stream-registration");
        requireCompleteProofForEpoch(
                value.physicalRootBackfill(), value.brokerCapabilityReadinessEpoch(), "physical-root");
        requireCompleteProofForEpoch(
                value.cursorSnapshotBackfill(), value.brokerCapabilityReadinessEpoch(), "cursor-snapshot");
    }

    private Coverage requireSuccessfulRolloverReport(
            PhysicalRootBackfillRequest request, PhysicalRootBackfillReport report) {
        PhysicalRootBackfillReport exact = Objects.requireNonNull(report, "backfill report");
        if (!exact.runId().equals(request.runId())
                || exact.brokerReadinessEpoch() != request.expectedBrokerReadinessEpoch()) {
            throw invariant("physical-root rollover report does not match its request");
        }
        if (exact.failureCount() != 0 || !exact.boundedFailures().isEmpty()) {
            throw notReady(
                    "physical-root/cursor-root readiness rollover contains failures: " + exact.boundedFailures());
        }
        return new Coverage(
                exact.dataCoverageSha256().value(), exact.cursorCoverageSha256().value());
    }
    ...
    private RolloverProofs rolloverProofs(
        ...
        long readinessEpoch = registration.readiness().brokerReadinessEpoch();
        return new RolloverProofs(
                new GenerationBackfillProofRecord(
                        ...
                        completedAt),
                new GenerationBackfillProofRecord(
                        registration.runId(), readinessEpoch, coverage.physicalRootSha256(), true, completedAt),
                new GenerationBackfillProofRecord(
                        registration.runId(), readinessEpoch, coverage.cursorSnapshotSha256(), true, completedAt),
    ...
    private GenerationProtocolActivationRecord rolloverReplacement(
            ...
            RolloverProofs proofs) {
        long updatedAt =
                Math.max(Math.max(current.updatedAtMillis(), proofs.completedAtMillis()), Math.max(1, clock.millis()));
    ...
    private boolean sameRolloverAuthority(
            GenerationProtocolActivationRecord activation, GenerationRegistrationBackfillCompletion registration) {
        long readinessEpoch = registration.readiness().brokerReadinessEpoch();
        return activation.lifecycle() == GenerationProtocolActivationLifecycle.ACTIVE
                ...
                && activation.brokerCapabilityReadinessEpoch() == readinessEpoch
                && activation.requiredReferenceDomains().equals(requiredDomains)
                && activation.objectStoreCapabilitySha256().equals(expectedCapabilitySha256)
                && sameProof(
                        activation.streamRegistrationBackfill(),
                        readinessEpoch,
                        registration.coverageSha256().value())
                && completeProofForEpoch(activation.physicalRootBackfill(), readinessEpoch)
                && completeProofForEpoch(activation.cursorSnapshotBackfill(), readinessEpoch);
    }
    ...
    private void requireBaseAuthority(
        ...
        if (activation.lifecycle() != GenerationProtocolActivationLifecycle.ACTIVE
                || !activation.publicationEnabled()) {
            throw notReady("generation publication must be ACTIVE before physical deletion activation");
        }
        if (!activation.requiredReferenceDomains().equals(requiredDomains)) {
            throw invariant("durable generation reference-domain set differs from the local runtime");
        }
        if (activation.brokerCapabilityReadinessEpoch() != readiness.brokerReadinessEpoch()) {
            throw notReady("generation activation belongs to another broker readiness epoch");
        }
        GenerationBackfillProofRecord registration = activation.streamRegistrationBackfill();
        if (!registration.complete() || registration.brokerReadinessEpoch() != readiness.brokerReadinessEpoch()) {
            throw notReady("registration backfill is incomplete for the current readiness epoch");
        }
        requireCapabilityCompatible(activation);
    }

    private void requireCoverageAuthority(
            ActivationBasis basis, Coverage coverage, GenerationProtocolActivationRecord activation) {
        requireBaseAuthority(basis.readiness(), activation);
        if (!activation.streamRegistrationBackfill().equals(basis.registrationBackfill())) {
            throw notReady("registration backfill changed during physical deletion activation");
        }
    ...
    private void requireDeletionAuthority(
        ...
        if (!deletionEnabled(activation)
                || !activation.objectStoreCapabilitySha256().equals(expectedCapabilitySha256)) {
            throw notReady("physical deletion activation is not installed for the configured object-store scope");
        }
    }

    private void requireCapabilityCompatible(GenerationProtocolActivationRecord activation) {
        if (!activation.objectStoreCapabilitySha256().isEmpty()
                && !activation.objectStoreCapabilitySha256().equals(expectedCapabilitySha256)) {
            throw invariant("object-store capability differs within the same broker readiness epoch");
        }
    }

    private void requireCurrentReadiness(GenerationCapabilityReadiness expected) {
        ...
        if (current.isEmpty() || !current.orElseThrow().equals(expected)) {
            throw notReady("broker generation readiness changed during physical deletion activation");
        }
    }
    ...
    private static void requireExactReadiness(
        ...
        if (!Objects.requireNonNull(expected, "expected").equals(Objects.requireNonNull(actual, "actual"))) {
            throw notReady("broker generation readiness changed around deletion-authority rollover");
        }
    }
    ...
    private Coverage requireSuccessfulReport(
        ...
        PhysicalRootBackfillReport exact = Objects.requireNonNull(report, "backfill report");
        if (!exact.runId().equals(request.runId())
                || exact.brokerReadinessEpoch() != basis.readiness().brokerReadinessEpoch()) {
            throw invariant("physical-root backfill report does not match its activation request");
        }
        if (exact.failureCount() != 0 || !exact.boundedFailures().isEmpty()) {
            throw notReady("physical-root/cursor-root backfill contains failures: " + exact.boundedFailures());
        }
        return new Coverage(
                exact.dataCoverageSha256().value(), exact.cursorCoverageSha256().value());
    }

    private void requireCapabilityProof(ObjectStoreDeleteCapabilityProof proof) {
        ObjectStoreDeleteCapabilityProof exact = Objects.requireNonNull(proof, "capability proof");
        if (exact.protocolVersion() != ObjectStoreDeleteCapabilityProof.PROTOCOL_VERSION
                || !exact.capabilitySha256().equals(expectedCapabilitySha256)) {
            throw invariant("object-store capability proof does not match the configured V1 scope");
        }
    }

    private GenerationProtocolActivationRecord deletionReplacement(
            GenerationProtocolActivationRecord current, long capabilityCompletedAtMillis) {
        long updatedAt =
                Math.max(Math.max(current.updatedAtMillis(), capabilityCompletedAtMillis), Math.max(1, clock.millis()));
        return new GenerationProtocolActivationRecord(
                current.schemaVersion(),
                current.protocolVersion(),
                current.lifecycle(),
    ...
    private static Coverage coverage(GenerationProtocolActivationRecord activation) {
        return new Coverage(
    ...
    private static void requireCoverageProof(
            GenerationBackfillProofRecord proof, long readinessEpoch, String expectedCoverageSha256, String name) {
        if (!proof.complete()
                || proof.brokerReadinessEpoch() != readinessEpoch
                || !proof.coverageSha256().equals(expectedCoverageSha256)) {
            throw notReady(name + " backfill proof is absent or changed");
        }
    }

    private static void requireCompleteProofForEpoch(
            GenerationBackfillProofRecord proof, long readinessEpoch, String name) {
        if (!completeProofForEpoch(proof, readinessEpoch)) {
            throw notReady(name + " proof is incomplete for the active readiness epoch");
        }
    }

    private static boolean completeProofForEpoch(GenerationBackfillProofRecord proof, long readinessEpoch) {
        return proof.complete() && proof.brokerReadinessEpoch() == readinessEpoch;
    }

    private static boolean sameProof(GenerationBackfillProofRecord proof, long readinessEpoch, String coverageSha256) {
        return completeProofForEpoch(proof, readinessEpoch)
                && proof.coverageSha256().equals(coverageSha256);
    }

    private static boolean deletionEnabled(GenerationProtocolActivationRecord activation) {
        return activation.physicalDeleteEnabled() && activation.cursorSnapshotDeleteEnabled();
    }
    ...
    private static VersionedGenerationProtocolActivation requireActivation(
        ...
        return Objects.requireNonNull(optional, "activation result")
                .orElseThrow(() -> notReady("generation activation record is absent"));
    }
    ...
    private record ActivationBasis(
            GenerationCapabilityReadiness readiness, GenerationBackfillProofRecord registrationBackfill) {
        private ActivationBasis {
            Objects.requireNonNull(readiness, "readiness");
            Objects.requireNonNull(registrationBackfill, "registrationBackfill");
        }
    }

    private record Coverage(String physicalRootSha256, String cursorSnapshotSha256) {
        private Coverage {
            physicalRootSha256 = requireSha256(physicalRootSha256, "physicalRootSha256");
            cursorSnapshotSha256 = requireSha256(cursorSnapshotSha256, "cursorSnapshotSha256");
        }
    }

    private record InstallResult(VersionedGenerationProtocolActivation activation, Status status) {
    ...
    private record RolloverProofs(
        ...
        private RolloverProofs {
            ...
            if (capabilityCompletedAtMillis <= 0) {
                throw new IllegalArgumentException("capabilityCompletedAtMillis must be positive");
            }
        }
        ...
        private long completedAtMillis() {
            return Math.max(
                    ...
                    Math.max(
                            ...
                            Math.max(physicalRoot.completedAtMillis(), cursorSnapshot.completedAtMillis())));
        }
    }
    ...
    private static final class Deadline {
        ...
        private static Deadline start(Duration timeout, LongSupplier nanoTime) {
            ...
            try {
                ...
                throw new IllegalArgumentException("physical deletion activation deadline overflows", failure);
            }
        }
        ...
        private Duration remaining() {
            ...
            if (nanos <= 0) {
                throw new NereusException(ErrorCode.TIMEOUT, true, "physical deletion activation timed out");
            }
