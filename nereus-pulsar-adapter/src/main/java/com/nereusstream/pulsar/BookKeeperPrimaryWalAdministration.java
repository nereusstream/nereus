...
public final class BookKeeperPrimaryWalAdministration {
    ...
    public static BookKeeperPrimaryWalAdministration usingSharedRuntime(
        ...
        OxiaBookKeeperLedgerIdNamespaceReservationStore namespaces =
                new OxiaBookKeeperLedgerIdNamespaceReservationStore(
                        Objects.requireNonNull(oxia, "oxia"), Objects.requireNonNull(runtime, "runtime"));
        OxiaBookKeeperProtocolActivationStore activations = new OxiaBookKeeperProtocolActivationStore(oxia, runtime);
        return new BookKeeperPrimaryWalAdministration(
                configuration,
                new BookKeeperLedgerIdNamespaceReservationVerifier(namespaces, configuration.deploymentId()),
    ...
    BookKeeperPrimaryWalAdministration(
        ...
        this.deletionActivationCoordinator =
                Objects.requireNonNull(deletionActivationCoordinator, "deletionActivationCoordinator");
    }
    ...
    public CompletableFuture<BookKeeperLedgerIdNamespaceReservation> provisionNamespace(
        ...
        return namespaceCoordinator.provision(
                configuration.wal(), configuration.deploymentId(), operatorEvidenceSha256, timeout);
    }

    /** Reads and validates the exact active namespace without creating or updating metadata. */
    public CompletableFuture<BookKeeperLedgerIdNamespaceReservation> verifyNamespace(Duration timeout) {
        return namespaceVerifier.requireActive(configuration.wal(), Objects.requireNonNull(timeout, "timeout"));
    }

    public CompletableFuture<BookKeeperLedgerIdNamespaceReservation> revokeNamespace(
            String revocationEvidenceSha256, long expectedMetadataVersion, Duration timeout) {
    ...
    public CompletableFuture<BookKeeperProtocolActivation> prepareActivation(
            long brokerReadinessEpoch, String brokerReadinessSha256, Duration timeout) {
        ...
        return namespaceVerifier
                .requireActive(wal, deadline.remaining())
                .thenCompose(namespace -> activationCoordinator.prepare(
                        wal, namespace, brokerReadinessEpoch, brokerReadinessSha256, deadline.remaining()));
    }
    ...
    public CompletableFuture<BookKeeperProtocolActivation> activate(
        ...
        return namespaceVerifier
                .requireActive(wal, deadline.remaining())
                .thenCompose(namespace -> activationCoordinator.activate(wal, namespace, update, deadline.remaining()));
    }

    public CompletableFuture<Optional<BookKeeperProtocolActivation>> readActivation(Duration timeout) {
        BookKeeperOperationDeadline deadline = new BookKeeperOperationDeadline(timeout);
        BookKeeperWalConfiguration wal = configuration.wal();
        return namespaceVerifier
                .requireActive(wal, deadline.remaining())
                .thenCompose(namespace -> activationStore.read(wal, namespace, deadline.remaining()));
    }
