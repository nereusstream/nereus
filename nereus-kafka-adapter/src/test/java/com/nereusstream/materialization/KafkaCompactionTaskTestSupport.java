...
public final class KafkaCompactionTaskTestSupport {
    ...
    public static VersionedMaterializationTask planned(MaterializationTask task, long metadataVersion) {
        MaterializationTaskRecord value =
                MaterializationRecordMapper.plannedTask(task, 100).withMetadataVersion(metadataVersion);
        return versioned(value);
    }

    public static VersionedMaterializationTask cancelled(MaterializationTask task, long metadataVersion) {
    ...
    public static CompletableFuture<Void> publish(
            MaterializationTaskStore tasks, MaterializationTask task, long generation, PublicationId publicationId) {
        if (generation <= 0) {
            return CompletableFuture.failedFuture(new IllegalArgumentException("generation must be positive"));
        }
        return tasks.get(task.streamId(), task.taskId())
                .thenCompose(
                        optional -> advancePublication(tasks, task, optional.orElseThrow(), generation, publicationId));
    }
    ...
    private static CompletableFuture<Void> advancePublication(
        ...
        if (!tasks.requireTask(current).equals(task)) {
            return CompletableFuture.failedFuture(new IllegalArgumentException("publication task identity changed"));
        }
        MaterializationTaskRecord replacement;
        switch (current.value().lifecycle()) {
            case OUTPUT_READY ->
                replacement = MaterializationRecordMapper.publishing(
                        current.value(), publicationId, current.value().updatedAtMillis() + 1);
            case PUBLISHING -> {
                if (!current.value().publicationId().equals(publicationId.value())) {
                    return CompletableFuture.failedFuture(new IllegalArgumentException("publication identity changed"));
                }
                if (current.value().allocatedGeneration().isPresent()) {
                    ...
                    replacement = MaterializationRecordMapper.published(
                            current.value(), current.value().updatedAtMillis() + 1);
                } else {
                    replacement = MaterializationRecordMapper.attachGeneration(
            ...
            default -> {
                return CompletableFuture.failedFuture(new IllegalArgumentException("task is not publishable"));
            }
        }
        return tasks.compareAndSet(replacement, current.metadataVersion())
                .thenCompose(updated -> advancePublication(tasks, task, updated, generation, publicationId));
    }
