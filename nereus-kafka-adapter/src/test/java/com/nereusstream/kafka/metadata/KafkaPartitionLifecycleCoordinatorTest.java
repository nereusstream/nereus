...
class KafkaPartitionLifecycleCoordinatorTest {
    ...
    void topicNameReuseWithANewTopicIdCreatesAnotherStream() {
        ...
        KafkaPartitionBinding oldTopic =
                coordinator.ensureBinding(request(identity(2, 0, "orders"), 40)).join();
        KafkaPartitionBinding newTopic =
                coordinator.ensureBinding(request(identity(3, 0, "orders"), 41)).join();
