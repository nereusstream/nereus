 ...
 */
public final class TestStreamStorage implements StreamStorage, ConstrainedSemanticStreamReader {
    ...
    private BiFunction<StreamId, ReadRequest, CompletableFuture<SemanticReadResult>> semanticReader;
    ...
    private CompletableFuture<StreamMetadata> transition(StreamId streamId, StreamState target) {
        ...
        if (current == null) {
            return CompletableFuture.failedFuture(new IllegalArgumentException("unknown stream"));
        }
    ...
    @Override
    public synchronized CompletableFuture<SemanticReadResult> read(StreamId streamId, ReadRequest request) {
        return semanticReader == null ? unsupported() : semanticReader.apply(streamId, request);
    }
