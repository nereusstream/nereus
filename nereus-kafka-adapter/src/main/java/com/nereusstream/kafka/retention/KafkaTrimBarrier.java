...
public final class KafkaTrimBarrier {
    ...
    public CompletableFuture<Result> advanceDeleteRecords(Snapshot captured, long targetOffset) {
        ...
        String reason = "KAFKA_DELETE_RECORDS_V1:config="
        ...
        return advance(captured, new TrimIntent(TrimKind.DELETE_RECORDS, previous, targetOffset, reason));
    }
    ...
    private CompletableFuture<Result> advance(Snapshot captured, TrimIntent intent) {
        ...
        return checkpoints
                ...
                .thenCompose(checkpoint -> snapshots
                        .loadCurrent()
                        .thenApply(current -> {
    ...
    private CompletableFuture<Result> trimAndConfirm(
        ...
        return trim.handle((ignored, failure) -> failure == null ? null : unwrap(failure))
                .thenCompose(trimFailure -> confirmDurableHead(current, target, trimFailure)
                        .thenCompose(durable -> notifyDurable(current, checkpoint, intent, durable, false)));
    }
    ...
    private CompletableFuture<StableStreamHeadSnapshot> confirmDurableHead(
        ...
        return loaded.handle((head, loadFailure) -> {
            if (loadFailure != null) {
            ...
            if (head.trimOffset() < target) {
                ...
                throw new CompletionException(invariant("Kafka trim completed without durably advancing stream trim"));
            }
            return head;
        });
    }
    ...
    private CompletableFuture<Result> notifyDurable(
        ...
        try {
            CompletableFuture<Void> notified = Objects.requireNonNull(
                    durableTrimListener.onDurableTrim(current, durableHead.trimOffset(), checkpoint.reference()),
                    "Kafka durable-trim listener returned a null future");
            return notified.thenApply(ignored -> new Result(
    ...
    private void revalidate(Snapshot captured, Snapshot current, VerifiedCheckpoint checkpoint, TrimIntent intent) {
        long target = intent.targetOffset();
        ...
        boolean rooted = current.binding().value().checkpointReferences().contains(checkpoint.reference());
    ...
    private static VerifiedCheckpoint requireSufficientCheckpoint(
        ...
        if (reference.checkpointOffset() < target
                || reference.logStartOffsetAtCheckpoint()
                        > snapshot.sourceHead().trimOffset()
    ...
    private static void requireSameAuthority(StableStreamHeadSnapshot expected, StableStreamHeadSnapshot actual) {
    ...
    public record Snapshot(
        ...
        public Snapshot {
            ...
            if (!identity.durableId().equals(root.identity())
                    ...
                    || sourceHead.committedEndOffset()
                            != retention.virtualSegments().stableEndOffset()
            ...
            if (!authority.authorityType().equals("kafka-partition-leader-v1")
                ...
                throw new IllegalArgumentException("Kafka trim snapshot authority does not match the observed leader");
            }
        }
    }

    public record VerifiedCheckpoint(KafkaCheckpointReferenceRecord reference, byte[] verifiedObjectSha256) {
        public VerifiedCheckpoint {
            Objects.requireNonNull(reference, "reference");
            verifiedObjectSha256 = Objects.requireNonNull(verifiedObjectSha256, "verifiedObjectSha256")
                    .clone();
    ...
    private record TrimIntent(TrimKind kind, long previousLogStartOffset, long targetOffset, String trimReason) {
        private TrimIntent {
            Objects.requireNonNull(kind, "kind");
            Objects.requireNonNull(trimReason, "trimReason");
            if (previousLogStartOffset < 0 || targetOffset <= previousLogStartOffset || trimReason.isBlank()) {
