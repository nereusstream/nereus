 ...
 */
public final class KafkaRetentionDurableTrimListener
    implements KafkaTrimBarrier.DurableTrimListener {
  ...
  public KafkaRetentionDurableTrimListener(
      ...
      LocalLogStartUpdater localLogStartUpdater,
      Clock clock) {
    this.bindings = Objects.requireNonNull(bindings, "bindings");
    this.localLogStartUpdater =
        Objects.requireNonNull(localLogStartUpdater, "localLogStartUpdater");
    ...
    if (durableTrimOffset < checkpoint.logStartOffsetAtCheckpoint()
      ...
      return CompletableFuture.failedFuture(
          new IllegalArgumentException(
              "durable Kafka trim offset is outside the revalidated stream window"));
    }
  ...
  private CompletableFuture<VersionedKafkaPartitionBinding> publishObservedTrim(
    ...
    return bindings
        .get(revalidated.identity().durableId())
        .thenCompose(
            optional -> {
              VersionedKafkaPartitionBinding current =
                  optional.orElseThrow(
                      () -> invariant("Kafka binding disappeared while publishing durable trim"));
              ...
              if (durableTrimOffset > root.observedStableEndOffset()) {
                return CompletableFuture.failedFuture(
                    invariant("durable Kafka trim exceeds the binding stable end"));
              }
                  ...
                  .exceptionallyCompose(
                      failure ->
                          conditionFailure(failure)
                              ? publishObservedTrim(
                                  revalidated, durableTrimOffset, checkpoint, attempt + 1)
  ...
  private static void validateRoot(
    ...
    KafkaCheckpointReferenceRecord rooted =
        after.checkpointReferences().stream()
            .filter(reference -> reference.objectId().equals(checkpoint.objectId()))
            .findFirst()
            .orElseThrow(
                () -> invariant("verified Kafka checkpoint disappeared before trim publication"));
