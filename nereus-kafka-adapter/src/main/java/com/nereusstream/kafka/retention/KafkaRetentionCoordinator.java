...
public final class KafkaRetentionCoordinator {
  ...
  private CompletableFuture<RunResult> execute() {
    ...
    return loaded.thenCompose(
        snapshot -> {
          ...
          return barrier
              .advance(snapshot, plan)
              .thenApply(result -> new RunResult(plan, Optional.of(result)));
        });
  }
  ...
  public record RunResult(
      KafkaRetentionPlanner.Plan plan, Optional<KafkaTrimBarrier.Result> trimResult) {
    public RunResult {
