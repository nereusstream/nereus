...
public final class KafkaRetentionPlanner {
    ...
    public Plan plan(Snapshot snapshot) {
        ...
        if (segments.isEmpty() || (policy.cleanupPolicyFlags() & LogConfigHistoryEntry.CLEANUP_DELETE_FLAG) == 0) {
        ...
        int timePrefixCount =
                timePrefixCount(segments, closedCount, deletionUpperBound, snapshot.nowMillis(), policy.retentionMs());
        int sizePrefixCount =
                sizePrefixCount(segments, closedCount, deletionUpperBound, totalLogicalBytes, policy.retentionBytes());
        ...
        for (int index = 0; index < selectedCount; index++) {
            deletedLogicalBytes =
                    Math.addExact(deletedLogicalBytes, segments.get(index).logicalBytes());
        }
        long candidateLogStartOffset = segments.get(selectedCount - 1).endOffset();
        if (candidateLogStartOffset <= state.logStartOffset() || candidateLogStartOffset > deletionUpperBound) {
            throw new IllegalStateException("Kafka retention planner selected an invalid segment boundary");
        }
    ...
    private static int timePrefixCount(
            List<VirtualSegment> segments, int closedCount, long deletionUpperBound, long nowMillis, long retentionMs) {
        if (retentionMs < 0) {
            return 0;
        }
        int selected = 0;
    ...
    public record Snapshot(
        ...
        public Snapshot {
            ...
            if (!virtualSegments.configHistory().isEmpty()) {
                LogConfigHistoryEntry current = virtualSegments
                        .configHistory()
                        .get(virtualSegments.configHistory().size() - 1);
                if (!policy.equals(Policy.from(current))) {
                    throw new IllegalArgumentException("Kafka retention policy does not match current config history");
                }
            }
        }
    }
    ...
    public record Policy(
            long metadataOffset, Checksum configDigest, long retentionBytes, long retentionMs, int cleanupPolicyFlags) {
        public Policy {
