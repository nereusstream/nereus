...
public final class KafkaRetentionCheckpointGate implements KafkaTrimBarrier.CheckpointGate {
    ...
    private CompletableFuture<KafkaTrimBarrier.VerifiedCheckpoint> verifyExisting(
        ...
        try {
            verification = Objects.requireNonNull(
                    verifier.verify(snapshot, reference), "Kafka retention checkpoint verifier returned a null future");
        } catch (Throwable failure) {
            verification = CompletableFuture.failedFuture(failure);
        }
        return verification.thenApply(actual -> requireExact(reference, actual)).exceptionallyCompose(failure -> {
            ...
            return quarantine
                    .quarantine(
                            ...
                            exact)
                    .thenCompose(ignored -> tryExisting(snapshot, targetOffset, references, index + 1));
        });
    }
    ...
    private CompletableFuture<KafkaTrimBarrier.VerifiedCheckpoint> publishStableEnd(
        ...
        try {
            publication = Objects.requireNonNull(
                    publisher.publish(snapshot), "Kafka retention checkpoint publisher returned a null future");
        } catch (Throwable failure) {
            publication = CompletableFuture.failedFuture(failure);
        }
        return publication.thenApply(checkpoint -> {
    ...
    public interface CheckpointPublisher {
        CompletableFuture<KafkaTrimBarrier.VerifiedCheckpoint> publish(KafkaTrimBarrier.Snapshot snapshot);
    }
}
