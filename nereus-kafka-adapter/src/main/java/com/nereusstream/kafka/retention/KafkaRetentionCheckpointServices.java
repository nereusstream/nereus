...
public final class KafkaRetentionCheckpointServices
  ...
  @Override
  public CompletableFuture<KafkaTrimBarrier.VerifiedCheckpoint> publish(
            ...
            request ->
                publication
                    ...
                    .thenCompose(
                        object ->
                            bindings
                                .get(snapshot.identity().durableId())
                                .thenApply(
                                    current ->
                                        verifiedPublished(
                                            snapshot,
                                            ...
                                            current.orElseThrow(
                                                () ->
                                                    invariant(
                                                        "Kafka binding disappeared after checkpoint"
                                                            + " publication"))))));
  }
    ...
    try {
      loaded =
          Objects.requireNonNull(
              captures.capture(snapshot),
              "Kafka retention checkpoint capture provider returned a null future");
  ...
  private static Capture validateCapture(KafkaTrimBarrier.Snapshot snapshot, Capture capture) {
    ...
    if (capture.leaderEpoch() != snapshot.binding().value().observedLeaderEpoch()
      ...
      throw invariant(
          "Kafka retention checkpoint capture does not match the frozen partition view");
    }
    return capture;
  }
  ...
  private static KafkaTrimBarrier.VerifiedCheckpoint verifiedPublished(
    ...
    KafkaCheckpointReferenceRecord reference =
        current.value().checkpointReferences().stream()
            .filter(candidate -> candidate.objectId().equals(object.objectId().value()))
            .findFirst()
            .orElseThrow(
                () ->
                    invariant("published Kafka retention checkpoint is absent from binding root"));
    if (!current.value().identity().equals(snapshot.identity().durableId())
        ...
        || !Arrays.equals(
            reference.objectSha256(), HexFormat.of().parseHex(object.objectSha256().value()))
  ...
  private static Duration positive(Duration value) {
    ...
    if (value.isZero() || value.isNegative() || value.toMillis() <= 0) {
      throw new IllegalArgumentException(
          "Kafka retention checkpoint verification timeout must be positive");
    }
    return value;
  }
  ...
  public record Capture(
    ...
    public Capture {
      ...
      if (leaderEpoch < 0) {
        throw new IllegalArgumentException(
            "Kafka retention checkpoint leader epoch must be non-negative");
      }
    }
  }
}
