...
public final class KafkaCompactionPartitionPass {
    ...
    private CompletableFuture<RunResult> execute() {
        ...
        try {
            captured = Objects.requireNonNull(captures.capture(partition), "Kafka compaction capture future");
        } catch (Throwable failure) {
            captured = CompletableFuture.failedFuture(failure);
        }
        return captured.thenCompose(capture -> {
            requireCapture(capture);
            return findExisting(capture)
                    .thenCompose(existing ->
                            existing.isPresent() ? resume(capture, existing.orElseThrow()) : planFresh(capture));
        });
    }
    ...
    private CompletableFuture<Void> scanPlans(
            Optional<KafkaCompactionPlanScanToken> continuation, ArrayList<VersionedKafkaCompactionPlan> scanned) {
        ...
        try {
            page = Objects.requireNonNull(
                    plans.scanCompactionPlans(partition, continuation, limit), "Kafka compaction plan scan future");
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
        return page.thenCompose(value -> {
    ...
    private CompletableFuture<Optional<ExistingWork>> loadExisting(
            Capture capture, List<VersionedKafkaCompactionPlan> scanned, int index, Optional<ExistingWork> found) {
        ...
        if (!decoded.streamId().equals(capture.streamId())) {
            return taskFor(decoded).thenCompose(task -> {
                if (task.isPresent()) {
                    return CompletableFuture.failedFuture(
        ...
        return taskFor(decoded).thenCompose(optional -> {
            if (optional.isEmpty()) {
                return loadExisting(capture, scanned, index + 1, found);
            }
            ...
            return planCoordinator.recover(partition, task).thenCompose(recovered -> {
                if (!recovered.durablePlan().equals(durablePlan)
                        || !recovered.plan().equals(decoded)) {
                    return CompletableFuture.failedFuture(
                            invariant("Kafka compaction active-work plan changed during" + " recovery"));
                }
                ExistingWork work = new ExistingWork(decoded, durablePlan, durableTask, task);
                if (found.isPresent()) {
                    return CompletableFuture.failedFuture(
                            invariant("multiple task-rooted Kafka compaction plans " + "exist for one" + " partition"));
                }
    ...
    private CompletableFuture<Optional<VersionedMaterializationTask>> taskFor(KafkaCompactionPlan plan) {
        try {
            return Objects.requireNonNull(
                    tasks.get(plan.streamId(), plan.materializationTaskId()), "Kafka compaction task lookup future");
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
    }
    ...
    private CompletableFuture<RunResult> planFresh(Capture capture) {
        ...
        return sources.resolve(capture.streamId(), candidate, capture.outputPolicy())
                .thenCompose(resolved -> {
                    MaterializationTaskMutationGuard guard = sources.mutationGuard(resolved, capture.authorityGuard());
                    return planCoordinator
                            .converge(
                                    ...
                                    passOne,
                                    guard)
                            .thenCompose(converged -> tasks.get(
                                            converged.outputTask().streamId(),
                                            converged.outputTask().taskId())
                                    .thenCompose(optional -> {
                                        VersionedMaterializationTask durableTask = optional.orElseThrow(() -> invariant(
                                                "Kafka compaction task disappeared after plan" + " convergence"));
                                        if (!tasks.requireTask(durableTask).equals(converged.outputTask())) {
                                            return CompletableFuture.failedFuture(invariant("Kafka compaction "
                                                    + "converged task"
                                                    + " changed before"
                                                    + " claim"));
                                        }
                                        ExistingWork work = new ExistingWork(
                                                converged.plan(),
        ...
        return switch (work.durableTask().value().lifecycle()) {
            ...
            case RETRY_WAIT ->
                clock.millis() < work.durableTask().value().retryNotBeforeMillis()
                        ? CompletableFuture.completedFuture(RunResult.taskOnly(
                                RunOutcome.RETRY_DEFERRED, work.task().taskId()))
    ...
    private CompletableFuture<RunResult> executePlanned(Capture capture, ExistingWork work) {
        return activation(work.plan(), work.task()).thenCompose(activation -> {
            ...
            return publications
                    ...
                    .thenCompose(claimed -> executeClaimed(
                            capture, work.withTask(claimed, tasks.requireTask(claimed)), activation, claimId));
        });
    }
    ...
    private CompletableFuture<RunResult> executeClaimed(
        ...
        try {
            CompletableFuture<KafkaCompactionParquetPublisher.PreparedObject> preparing = taskProtections
            ...
            operation = lease.guard(preparing).thenCompose(prepared -> lease.stop(true)
                    .<CompletableFuture<PublicationResult>>handle((latest, stopFailure) -> {
                        if (stopFailure != null) {
                            prepared.close();
        ...
        return operation
                ...
                .<CompletableFuture<RunResult>>handle((result, failure) -> {
                    if (failure == null) {
    ...
    private CompletableFuture<RunResult> stopLeaseAndSettle(
        ...
        return lease.stop(false)
                .handle((ignored, stopFailure) -> {
                    if (stopFailure != null) {
    ...
    private CompletableFuture<RunResult> recoverPublication(Capture capture, ExistingWork work) {
        return activation(work.plan(), work.task())
                .thenCompose(activation -> publications.recoverPublication(
                        partition,
                        work.durableTask(),
                        work.plan(),
                        activation.mode(),
    ...
    private CompletableFuture<RunResult> retirePublished(
        ...
        return reloadWork(work).thenCompose(terminal -> {
            ...
            return retireTerminal(capture, terminal, RunOutcome.PUBLISHED_AND_RETIRED, Optional.of(publication));
        });
    }

    private CompletableFuture<RunResult> retireTerminal(
            Capture capture, ExistingWork work, RunOutcome outcome, Optional<PublicationResult> publication) {
        return retirer.retire(partition, work.durableTask(), work.durablePlan(), capture.authorityGuard())
                .thenApply(retirement -> new RunResult(
                        outcome, Optional.of(work.task().taskId()), publication, Optional.of(retirement)));
    }

    private CompletableFuture<ExistingWork> reloadWork(ExistingWork expected) {
        return tasks.get(expected.task().streamId(), expected.task().taskId()).thenCompose(optional -> {
            VersionedMaterializationTask durableTask =
                    optional.orElseThrow(() -> invariant("Kafka compaction task disappeared before retirement"));
            MaterializationTask task = tasks.requireTask(durableTask);
            expected.plan().requireMaterializationTask(task);
            return plans.getCompactionPlan(partition, task.taskId()).thenApply(currentPlan -> {
                VersionedKafkaCompactionPlan durablePlan = currentPlan.orElseThrow(
                        () -> invariant("Kafka compaction KCP1 disappeared " + "before task" + " retirement"));
    ...
    private CompletableFuture<RunResult> recoverClaim(Capture capture, ExistingWork work) {
        ...
        long safeExpiry = saturatingAdd(
                claim.expiresAtMillis(), configuration.maximumClockSkew().toMillis());
        ...
        return capture.authorityGuard()
                .revalidate()
                .thenCompose(ignored -> tasks.failClaim(
                        work.durableTask(),
                        TaskLifecycle.RETRY_WAIT,
                        TaskFailureClass.CLOSED,
                        EXPIRED_CLAIM_MESSAGE,
                        retryAt))
                .<CompletableFuture<RunResult>>handle((updated, failure) -> {
                    if (failure == null) {
                        return CompletableFuture.completedFuture(RunResult.taskOnly(
                                RunOutcome.EXPIRED_CLAIM_REQUEUED, work.task().taskId()));
                    }
                    Throwable original = unwrap(failure);
                    return tasks.get(work.task().streamId(), work.task().taskId())
                            .thenCompose(optional -> {
                                VersionedMaterializationTask reloaded = optional.orElseThrow(() ->
                                        condition("Kafka compaction task disappeared " + "during claim recovery"));
                                if (sameExpiredRetry(reloaded, work.durableTask(), retryAt)) {
                                    return CompletableFuture.completedFuture(RunResult.taskOnly(
                                            RunOutcome.EXPIRED_CLAIM_REQUEUED,
                                            work.task().taskId()));
                                }
        ...
        return tasks.get(work.task().streamId(), work.task().taskId())
                .thenCompose(optional -> {
                    ...
                    FailureDecision decision =
                            failureDecision(exact, current.value().attempt());
                    ...
                    return capture.authorityGuard()
                            ...
                            .thenCompose(ignored -> tasks.failClaim(
                                    ...
                                    retryAt))
                            .<CompletableFuture<RunResult>>handle((updated, transitionFailure) -> {
                ...
                .exceptionallyCompose(transitionFailure -> {
    ...
    private CompletableFuture<Activation> activation(KafkaCompactionPlan plan, MaterializationTask task) {
        ...
        KafkaCompactionCoverageRecord durableCoverage = new KafkaCompactionCoverageRecord(
                1,
                ...
                1);
        KafkaCompactionCoverageActivationMode mode = coverage.policySha256().equals(task.policyDigestSha256())
    ...
    private void requireCapture(Capture capture) {
        ...
        if (!binding.value().identity().equals(partition)
                ...
                || snapshot.virtualSegments().logStartOffset()
                        != binding.value().observedLogStartOffset()
    ...
    private static void requireMandatoryCoverage(
        ...
        KafkaCompactionPlanner.MandatoryCoverage expected = snapshot.mandatoryCoverage()
                .orElseThrow(() -> condition("Kafka compaction planner snapshot omits mandatory binding coverage"));
    ...
    private boolean sameClaim(
            VersionedMaterializationTask current, MaterializationTask task, String claimId, String processRunId) {
    ...
    private static CompletableFuture<Void> limit(String message) {
        return CompletableFuture.failedFuture(new NereusException(ErrorCode.METADATA_LIMIT_EXCEEDED, true, message));
    }
    ...
    private static Checksum sha256(byte[] value) {
        return new Checksum(
                com.nereusstream.api.ChecksumType.SHA256, HexFormat.of().formatHex(value));
    }
    ...
    public record RunResult(
        ...
        public RunResult {
            ...
            materializationTaskId = Objects.requireNonNull(materializationTaskId, "materializationTaskId");
            ...
            if ((outcome == RunOutcome.NO_CANDIDATE) != materializationTaskId.isEmpty()
                    ...
                    || ((outcome == RunOutcome.PUBLISHED_AND_RETIRED || outcome == RunOutcome.TERMINAL_RETIRED)
        ...
        static RunResult noCandidate() {
            return new RunResult(RunOutcome.NO_CANDIDATE, Optional.empty(), Optional.empty(), Optional.empty());
        }
    ...
    private final class ClaimLease {
        ...
        private <T> CompletableFuture<T> guard(CompletableFuture<T> operation) {
            ...
            exact.whenComplete((value, operationFailure) -> {
            ...
            failure.whenComplete((ignored, heartbeatFailure) -> {
                if (heartbeatFailure != null && guarded.completeExceptionally(unwrap(heartbeatFailure))) {
                    exact.cancel(true);
                }
            });
            guarded.whenComplete((ignored, guardedFailure) -> {
        ...
        private CompletableFuture<VersionedMaterializationTask> stop(boolean finalRenewal) {
            ...
            CompletableFuture<Void> stoppedTail = prior.thenCompose(ignored -> {
                boolean renew;
                synchronized (lock) {
            ...
            return stoppedTail.thenCompose(ignored -> {
                synchronized (lock) {
        ...
        private void scheduleLocked() {
            ...
            try {
                scheduled = scheduler.schedule(
                        this::renewScheduled, configuration.claimRenewInterval().toMillis(), TimeUnit.MILLISECONDS);
            } catch (RejectedExecutionException rejected) {
                NereusException closed = new NereusException(
                        ErrorCode.STORAGE_CLOSED, false, "Kafka compaction claim heartbeat was rejected", rejected);
        ...
        private void renewScheduled() {
            ...
            renewal.whenComplete((ignored, renewFailure) -> {
        ...
        private CompletableFuture<Void> renewOnce() {
            ...
            synchronized (lock) {
                ...
                requestedExpiry = Math.max(
                        addTime(priorExpiry, 1),
                        addTime(clock.millis(), configuration.claimDuration().toMillis()));
            }
            return tasks.heartbeat(expected, requestedExpiry)
                    .<CompletableFuture<VersionedMaterializationTask>>handle((updated, heartbeatFailure) -> {
                        ...
                        return tasks.get(domain.streamId(), domain.taskId()).thenCompose(optional -> {
                            VersionedMaterializationTask reloaded = optional.orElseThrow(
                                    () -> condition("Kafka compaction task " + "disappeared during " + "heartbeat"));
        ...
        private boolean sameClaimOwner(VersionedMaterializationTask current, VersionedMaterializationTask expected) {
