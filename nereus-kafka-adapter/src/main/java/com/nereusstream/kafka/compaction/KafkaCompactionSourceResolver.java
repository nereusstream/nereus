...
public final class KafkaCompactionSourceResolver {
    ...
    public CompletableFuture<ResolvedSources> resolve(
            StreamId streamId, KafkaCompactionPlanner.Candidate candidate, MaterializationPolicy outputPolicy) {
        try {
            StreamId exactStream = Objects.requireNonNull(streamId, "streamId");
            KafkaCompactionPlanner.Candidate exactCandidate = Objects.requireNonNull(candidate, "candidate");
            ...
            if (!exactCandidate.shouldCompact()) {
                throw new IllegalArgumentException("Kafka compaction source resolution requires a non-empty candidate");
            }
            return committedSources
                    .resolve(exactStream, exactCandidate.outputCoverage())
                    .thenCompose(outputResolution -> {
                        ...
                        return committedSources
                                ...
                                .thenApply(tailResolution -> {
                                    requireSameAuthority(outputResolution, tailResolution);
                                    ArrayList<SourceGeneration> decisionSources = new ArrayList<>(
                                    ...
                                    decisionSources.addAll(
                                            tailResolution.sourceSet().sources());
                                    ExactSourceSet combined = ExactSourceSet.create(
                                            ReadView.COMMITTED, exactCandidate.decisionHorizon(), decisionSources);
                                    ...
                                    return resolved(exactCandidate, resolution, outputSources, exactPolicy);
                                });
                    });
    ...
    public MaterializationTaskMutationGuard mutationGuard(
        ...
        MaterializationTaskMutationGuard authority = Objects.requireNonNull(bindingAuthority, "bindingAuthority");
    ...
    private CompletableFuture<Void> revalidateSources(ResolvedSources expected) {
        try {
            return Objects.requireNonNull(
                    committedSources.revalidate(expected.resolution()), "COMMITTED source revalidation future");
        } catch (RuntimeException failure) {
            return CompletableFuture.failedFuture(failure);
        }
    }

    private static CompletableFuture<Void> revalidateAuthority(MaterializationTaskMutationGuard authority) {
        try {
            return Objects.requireNonNull(authority.revalidate(), "Kafka compaction binding authority future");
        } catch (RuntimeException failure) {
            return CompletableFuture.failedFuture(failure);
        }
    }
    ...
    private static void requireKafkaPolicy(MaterializationPolicy policy) {
        TopicCompactionSpec expected = new TopicCompactionSpec(
        ...
        if (policy.view() != ReadView.TOPIC_COMPACTED
                ...
                || !policy.targetPhysicalFormat().equals(MaterializationPolicy.KAFKA_TOPIC_COMPACTED_FORMAT)
                || policy.topicCompaction().filter(expected::equals).isEmpty()) {
            throw new IllegalArgumentException("materialization policy is not the current Kafka compaction policy");
        }
    }
    ...
    private static void requireKafkaOutputSources(KafkaCompactionPlanner.Candidate candidate, ExactSourceSet sources) {
        if (sources.view() != ReadView.COMMITTED
    ...
    private static void requireSameAuthority(CommittedSourceSetResolution output, CommittedSourceSetResolution tail) {
    ...
    public record ResolvedSources(
        ...
        public ResolvedSources {
            ...
            if (!candidate.shouldCompact()
                ...
                throw new IllegalArgumentException("resolved Kafka compaction sources/task are inconsistent");
            }
        }
