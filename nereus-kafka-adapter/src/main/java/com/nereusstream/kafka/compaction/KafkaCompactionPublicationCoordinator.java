...
public final class KafkaCompactionPublicationCoordinator {
  ...
  public CompletableFuture<PublicationResult> publish(
    ...
    try {
      ...
      CompletableFuture<PublicationResult> result =
          loadBinding(exactPartition)
              .thenApply(
                  binding -> {
              ...
              .thenCompose(
                  basis ->
                      revalidatePublicationAuthority(
                              exactPartition,
                              exactClaim,
                              task,
                              plan,
                          ...
                          .thenCompose(
                              head -> {
                                MaterializationOutput output =
                                    output(task, closeOwned.written(), closeOwned, head);
  ...
  private CompletableFuture<VersionedMaterializationTask> claim(
    ...
    return tasks
        .claim(expected, claimId, processRunId, expiresAtMillis)
        .handle(
            (claimed, failure) -> {
              if (failure == null) {
                  ...
                  .thenCompose(
                      optional -> {
                        ...
                        if (reloaded.value().lifecycle() == expected.value().lifecycle()) {
                          return claim(
                              reloaded, claimId, processRunId, expiresAtMillis, attempt + 1);
                        }
  ...
  private CompletableFuture<HeadObjectResult> head(RangedCompactedObjectWriteResult written) {
    ...
    try {
      future =
          Objects.requireNonNull(
              objectStore.headObject(written.objectKey(), new HeadObjectOptions(timeout)),
              "NTC2 HEAD future");
    } catch (Throwable failure) {
      return CompletableFuture.failedFuture(failure);
    }
  ...
  private CompletableFuture<Void> verifyPrepared(PreparedObject prepared) {
    return objectVerifier.verifyExact(
        RangedCompactedObjectVerificationRequest.from(
            prepared.request(), prepared.written(), timeout),
        prepared.request());
  }
              ...
              return tasks
                  ...
                  .thenCompose(
                      optional -> {
                        ...
                        Optional<MaterializationOutput> durableOutput =
                            tasks.requireOutput(reloaded);
                        ...
                        if (sameClaim(
                                ...
                                current.value().workerClaim().orElseThrow().processRunId())
            ...
            optional -> {
              VersionedMaterializationTask current =
                  optional.orElseThrow(
                      () ->
                          new KafkaMetadataConditionFailedException(
                              "Kafka compaction task disappeared"));
  ...
  private CompletableFuture<VersionedKafkaPartitionBinding> loadBinding(
      KafkaPartitionId partition) {
    try {
          ...
          .thenApply(
              optional ->
                  optional.orElseThrow(
                      () ->
                          new KafkaMetadataConditionFailedException(
                              "Kafka partition binding is absent")));
    } catch (Throwable failure) {
      return CompletableFuture.failedFuture(failure);
    }
  }
  ...
  private void validateClaimAndPrepared(
    ...
    if (claimed.value().lifecycle() != TaskLifecycle.CLAIMED
      ...
      throw new IllegalArgumentException(
          "claimed task, recovered plan and prepared NTC2 object do not agree");
    }
    ...
    if (prepared.request().cumulativeSizeAtEnd()
      ...
      throw new IllegalArgumentException(
          "prepared NTC2 cumulative end differs from the exact task sources");
    }
  }
      ...
      case EXTEND -> {
        ...
        if (coverage.coverageVersion() == 0
            ...
            || !exactPrevious
                .coverage()
                .equals(
                    new com.nereusstream.api.OffsetRange(
  ...
  private static KafkaCompactionGenerationSet desiredGenerationSet(
    ...
    return switch (mode) {
      ...
      case REPLACE -> {
        KafkaCompactionGenerationSet replacement =
            KafkaCompactionGenerationSet.replacement(committed);
        KafkaCompactionGenerationSet before = previous.orElseThrow();
        if (replacement.coverage().startOffset() != before.coverage().startOffset()
            || replacement.coverage().endOffset() < before.coverage().endOffset()) {
          throw new KafkaMetadataConditionFailedException(
  ...
  private static SourceFacts sourceFacts(MaterializationTask task) {
    ...
    for (SourceGeneration source : task.sources()) {
      if (!source.projectionRef().equals(projection)) {
        throw new IllegalArgumentException(
            "Kafka compaction sources use different projection identities");
      }
  ...
  private static void validatePut(
      PutObjectResult uploaded, RangedCompactedObjectWriteResult written) {
  ...
  private static boolean sameClaim(
      VersionedMaterializationTask task, String claimId, String processRunId) {
  ...
  public record PublicationResult(
    ...
    public PublicationResult {
      ...
      if (output.view() != ReadView.TOPIC_COMPACTED
          ...
          || !Arrays.equals(
              binding.value().compactionCoverage().generationSetSha256(),
              generationSet.digestBytes())) {
