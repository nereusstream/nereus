...
public record KafkaCompactionPlan(
  ...
  public KafkaCompactionPlan(
    ...
    this.materializationPolicySha256 =
        requireSha256(materializationPolicySha256, "materializationPolicySha256");
            ...
            coverage -> {
              if (coverage.endOffset() != candidate.outputCoverage().startOffset()) {
                throw new IllegalArgumentException(
                    "Kafka compaction plan does not resume at mandatory coverage end");
              }
            });
  ...
  public static KafkaCompactionPlan create(
    ...
    ExactSourceSet exactDecisionSources =
  ...
  public void requireMaterializationTask(MaterializationTask task) {
    ...
    if (!exact.taskId().equals(materializationTaskId)
        ...
        || !exact
            ...
            .orElseThrow()
            .strategyId()
            .equals(compatibility.strategyId())
        || exact.policy().topicCompaction().orElseThrow().strategyVersion()
            != compatibility.strategyVersion()
        || !exact
            .policy()
            .topicCompaction()
            .orElseThrow()
            .keyCodecId()
            .equals(compatibility.keyCodecId())) {
      throw new IllegalArgumentException(
          "materialization task does not match the durable Kafka compaction plan");
    }
  }
  ...
  private static ExactSourceSet outputSources(
    ...
    if (prefix.get(prefix.size() - 1).range().endOffset()
      ...
      throw new IllegalArgumentException(
          "Kafka compaction output sources are not the exact decision prefix");
    }
  ...
  public record Compatibility(
    ...
    void requireCurrent() {
      if (!equals(current())) {
        throw new IllegalArgumentException(
            "Kafka compaction plan uses an unsupported compatibility tuple");
      }
    }
  }
}
