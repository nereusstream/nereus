...
public final class KafkaCompactionPlanner {
    ...
    public Candidate select(Snapshot snapshot) {
        ...
        long firstDirtyOffset =
                snapshot.mandatoryCoverage().map(MandatoryCoverage::endOffset).orElse(state.logStartOffset());
    ...
    private static boolean withinMinimumLag(VirtualSegment segment, long nowMillis, long minCompactionLagMs) {
        if (minCompactionLagMs == 0) {
            return false;
        }
        long segmentTimestamp = segment.largestTimestamp() >= 0 ? segment.largestTimestamp() : segment.closedAtMillis();
    ...
    public record Snapshot(
        ...
        public Snapshot {
            ...
            if (!history.isEmpty() && !policy.equals(Policy.from(history.get(history.size() - 1)))) {
                throw new IllegalArgumentException("Kafka compaction policy does not match current config history");
            }
            mandatoryCoverage.ifPresent(coverage -> {
    ...
    public record Candidate(
        ...
        public Candidate {
            ...
            Objects.requireNonNull(policy, "policy");
            previousMandatoryCoverage = Objects.requireNonNull(previousMandatoryCoverage, "previousMandatoryCoverage");
        ...
        private static Candidate none(Snapshot snapshot, long firstDirtyOffset, OffsetRange decisionHorizon) {
            return new Candidate(
