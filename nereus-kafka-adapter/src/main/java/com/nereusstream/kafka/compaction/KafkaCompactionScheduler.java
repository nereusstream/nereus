...
public final class KafkaCompactionScheduler implements AutoCloseable {
    private static final System.Logger LOGGER = System.getLogger(KafkaCompactionScheduler.class.getName());
    ...
    private void launch(PendingPass target) {
        ...
        synchronized (monitor) {
            if (active != target) {
                source.cancel(true);
                target.completion.completeExceptionally(closed("Kafka compaction scheduler no longer owns the pass"));
                return;
            }
            activeSource = source;
            cancelAfterPublish = state != State.RUNNING;
        }
        source.whenComplete((ignored, failure) -> executeCallback(() -> finish(target, unwrapNullable(failure))));
    ...
    private void scheduleLocked(Duration delay, Trigger trigger) {
        scheduled = scheduler.schedule(() -> scheduledTrigger(trigger), delay.toNanos(), TimeUnit.NANOSECONDS);
    }
    ...
    public record TriggerBatch(Set<Trigger> reasons, Trigger primary) {
        public TriggerBatch {
            ...
            if (reasons.isEmpty() || !reasons.contains(primary)) {
                throw new IllegalArgumentException("Kafka compaction trigger batch must contain its primary reason");
            }
            reasons = Collections.unmodifiableSet(EnumSet.copyOf(reasons));
            Trigger expected = reasons.stream()
                    .max((left, right) -> {
    ...
    private static final class PendingPass {
        ...
        private TriggerBatch freeze() {
            frozen = true;
            Trigger primary = triggers.stream()
                    .max((left, right) -> {
                        int priority = Integer.compare(left.priority(), right.priority());
                        return priority != 0 ? priority : left.name().compareTo(right.name());
                    })
