...
public interface KafkaActivatedGenerationAuthority {
    CompletableFuture<GenerationReadConstraint> resolve(StreamId streamId, KafkaCompactionCoverageRecord coverage);
    ...
    default CompletableFuture<VersionedKafkaPartitionBinding> repairIfQuarantined(
        ...
        return CompletableFuture.completedFuture(Objects.requireNonNull(binding, "binding"));
    }

    static KafkaActivatedGenerationAuthority unavailable() {
        return (streamId, coverage) -> CompletableFuture.failedFuture(new NereusException(
                ErrorCode.UNSUPPORTED_READ_SEMANTICS, false, "activated Kafka generation discovery is not configured"));
    }
}
