...
public final class KafkaCompactionStrategyV1 {
    ...
    public Decision decide(DecodedCompactionRecord record, RecordContext context) {
        ...
        if (context.transactionStatus() == TransactionStatus.OPEN) {
            throw new IllegalArgumentException("open Kafka transaction crossed the compaction output coverage");
        }
    ...
    private static Decision controlDecision(RecordContext context) {
        return switch (context.markerStatus()) {
            ...
            case DELETE_ELIGIBLE -> horizonPassed(context) ? Decision.DROP_EXPIRED_CONTROL : Decision.RETAIN_CONTROL;
            case NOT_CONTROL -> throw new IllegalArgumentException("Kafka control record lacks marker decision facts");
        };
    }
    ...
    private static void validateDataTransactionFacts(DecodedCompactionRecord record, RecordContext context) {
        if (!record.transactional() && context.transactionStatus() != TransactionStatus.NON_TRANSACTIONAL) {
            throw new IllegalArgumentException("non-transactional Kafka record has transactional cleaner facts");
        }
        if (record.transactional()
                && (context.transactionStatus() == TransactionStatus.NON_TRANSACTIONAL
                        || context.transactionStatus() == TransactionStatus.DECIDED)) {
            throw new IllegalArgumentException("transactional Kafka record lacks transaction outcome facts");
        }
    }
    ...
    public record RecordContext(
        ...
        public RecordContext {
            ...
            if (nowMillis < 0 || (deleteHorizonMillis.isPresent() && deleteHorizonMillis.getAsLong() < 0)) {
