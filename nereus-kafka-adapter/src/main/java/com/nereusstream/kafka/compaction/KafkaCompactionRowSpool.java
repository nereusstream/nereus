...
final class KafkaCompactionRowSpool {
    ...
    static Writer writer(
            StagingFileManager stagingFiles, OffsetRange outputCoverage, int maxRows, long maxLogicalBytes) {
        return new Writer(
    ...
    static final class Writer implements AutoCloseable {
        ...
        private Writer(StagingFileManager stagingFiles, OffsetRange outputCoverage, int maxRows, long maxLogicalBytes) {
    ...
    static final class Spool implements Flow.Publisher<KafkaTopicCompactedObjectRow>, AutoCloseable {
        ...
        public void subscribe(Flow.Subscriber<? super KafkaTopicCompactedObjectRow> subscriber) {
            ...
            try {
                RowSubscription subscription = new RowSubscription(
                        subscriber,
                        new DataInputStream(new BufferedInputStream(file.openVerifiedInputStream(), BUFFER_BYTES)));
                active = subscription;
        ...
        private final class RowSubscription implements Flow.Subscription {
            ...
            private KafkaTopicCompactedObjectRow readNext() throws IOException {
                ...
                KafkaCompactionDispositionV2 disposition = KafkaCompactionDispositionV2.fromWireId(input.readInt());
                int keyLength = input.readInt();
                byte[] key = readBytes(keyLength, KafkaCompactionKeyEncodingV2.MAX_ENCODED_KEY_BYTES, "key");
                int payloadLength = input.readInt();
                byte[] payload = readBytes(payloadLength, CompactedObjectFormatV2.MAX_PAYLOAD_BYTES, "payload");
                ...
                OptionalLong eventTime = input.readBoolean() ? OptionalLong.of(input.readLong()) : OptionalLong.empty();
                ...
                if (emittedRows > rows || emittedRecords > outputRecords || emittedLogicalBytes > logicalBytes) {
