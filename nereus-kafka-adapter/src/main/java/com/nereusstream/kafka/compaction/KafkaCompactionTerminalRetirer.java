...
public final class KafkaCompactionTerminalRetirer {
    ...
    public CompletableFuture<RetirementResult> retire(
        ...
        try {
            ...
            VersionedMaterializationTask exactTaskRoot = Objects.requireNonNull(expectedTask, "expectedTask");
            ...
            return revalidate(exactGuard).thenCompose(ignored -> loadPlan(exactPartition, exactTask, exactPlan)
                    .thenCompose(currentPlan -> currentPlan.isEmpty()
    ...
    private CompletableFuture<RetirementResult> requireBothAbsent(MaterializationTask expectedTask) {
        return getTask(expectedTask).thenApply(task -> {
            if (task.isPresent()) {
                throw invariant("Kafka compaction task remains after its KCP1 attachment disappeared", null);
            }
    ...
    private CompletableFuture<RetirementResult> retireTask(
        ...
        return getTask(expectedTask).thenCompose(currentTask -> {
            if (currentTask.isEmpty()) {
                return retirePlan(partition, expectedTask, expectedPlan, authorityGuard, 0, false, 0, 0);
            }
            ...
            return sourceProtections.release(exactTask, authorityGuard).thenCompose(released -> deleteTask(
                            expectedTask, exactTask, 0)
                    .thenCompose(attempts -> retirePlan(
                            partition, expectedTask, expectedPlan, authorityGuard, released, true, attempts, 0)));
        });
    }
    ...
    private CompletableFuture<Integer> deleteTask(
        ...
        return deleteTaskRoot(expected)
                .handle((ignored, failure) -> {
                    ...
                    return getTask(expectedTask).thenCompose(reloaded -> {
                        if (reloaded.isEmpty()) {
                            return CompletableFuture.completedFuture(attempt + 1);
                        }
                        ...
                        if (!actual.equals(expected)) {
                            return CompletableFuture.failedFuture(invariant(
                                    "Kafka compaction terminal task changed after " + "uncertain delete", original));
                        }
    ...
    private CompletableFuture<RetirementResult> retirePlan(
        ...
        if (planDeleteAttempt >= MAX_DELETE_RECOVERY_ATTEMPTS) {
            return CompletableFuture.failedFuture(invariant("Kafka compaction plan delete recovery exhausted", null));
        }
        return revalidate(authorityGuard)
                ...
                .thenCompose(current -> {
                    if (current.isEmpty()) {
                        return CompletableFuture.completedFuture(new RetirementResult(
                    ...
                    return deletePlanRoot(exact)
                            .handle((ignored, failure) -> {
                                ...
                                Throwable original = unwrap(failure);
                                return plans.getCompactionPlan(partition, expectedTask.taskId())
    ...
    private CompletableFuture<Void> requireTaskAbsent(MaterializationTask expectedTask) {
        return getTask(expectedTask).thenAccept(current -> {
    ...
    private CompletableFuture<Optional<VersionedKafkaCompactionPlan>> loadPlan(
            KafkaPartitionId partition, MaterializationTask expectedTask, VersionedKafkaCompactionPlan expectedPlan) {
        return plans.getCompactionPlan(partition, expectedTask.taskId()).thenApply(current -> {
            current.ifPresent(value -> requireSamePlan(expectedPlan, value));
            return current;
        });
    }

    private CompletableFuture<Optional<VersionedMaterializationTask>> getTask(MaterializationTask expectedTask) {
        try {
            return Objects.requireNonNull(
                    tasks.get(expectedTask.streamId(), expectedTask.taskId()), "Kafka compaction task get future");
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
    }
    ...
    private CompletableFuture<Void> deletePlanRoot(VersionedKafkaCompactionPlan expected) {
        try {
            return Objects.requireNonNull(plans.deleteCompactionPlan(expected), "Kafka compaction plan delete future");
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
    }
    ...
    private static void requireSamePlan(VersionedKafkaCompactionPlan expected, VersionedKafkaCompactionPlan actual) {
        if (!actual.equals(expected)) {
    ...
    private static CompletableFuture<Void> revalidate(MaterializationTaskMutationGuard guard) {
        try {
            return Objects.requireNonNull(guard.revalidate(), "Kafka compaction terminal authority guard future");
        } catch (Throwable failure) {
            return CompletableFuture.failedFuture(failure);
        }
    }
