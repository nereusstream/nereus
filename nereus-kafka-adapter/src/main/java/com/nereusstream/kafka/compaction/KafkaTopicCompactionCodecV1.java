...
public final class KafkaTopicCompactionCodecV1 implements RangedTopicCompactionCodec {
    ...
    public static final Checksum MESSAGE_FORMAT_SHA256 =
            new Checksum(ChecksumType.SHA256, "2940e62ac155a477052b955c1b30a2e7e77862bb7383d240b792ca064f472104");
    ...
    public void decode(ReadBatch rangedBatch, DecodedRecordConsumer consumer) {
        ...
        if (batch.baseOffset() != rangedBatch.range().startOffset()
            ...
            throw new IllegalArgumentException("Kafka compaction batch bytes do not match the exact source range");
        }
    ...
    @Override
    public RewrittenCompactionRecord rewrite(DecodedCompactionRecord survivor, CompactionRewriteContext context) {
        ...
        if (context.targetMagic() != RecordBatch.MAGIC_VALUE_V2
            ...
            throw new IllegalArgumentException("Kafka compaction rewrite context does not match codec V1");
        }
        ...
        DecodedCompactionRecord redecoded =
                toDecoded(batch, record, survivor.sourceRecordIndex(), survivor.sourceBatchSha256(), exactSource);
        if (!sameFacts(survivor, redecoded)) {
            throw new IllegalArgumentException("Kafka compaction rewrite token conflicts with selected survivor facts");
        }
        requireDeleteHorizon(survivor, context);

        byte[] rewritten = rewriteOne(
                batch,
                record,
    ...
    private static DecodedCompactionRecord toDecoded(
            RecordBatch batch, Record record, int sourceRecordIndex, Checksum sourceSha, byte[] exactBatch) {
        record.ensureValid();
        ...
        if (control) {
            ...
            controlKind = switch (marker.controlType()) {
                case COMMIT -> ControlKind.COMMIT;
                case ABORT -> ControlKind.ABORT;
                default ->
                    throw new IllegalArgumentException("Kafka compaction supports only commit/abort control markers");
            };
        ...
        OptionalLong eventTime = record.timestamp() < 0 ? OptionalLong.empty() : OptionalLong.of(record.timestamp());
    ...
    private static byte[] rewriteOne(
            RecordBatch batch, Record record, long offset, OptionalLong deleteHorizonMillis, Compression compression) {
    ...
    private static void requireRoundTrip(
        ...
        if (decoded.records().size() != 1
                ...
                || producerSequence(batch, decoded.records().get(0)) != producerSequence(sourceBatch, source)
                || !sameRecord(source, decoded.records().get(0))) {
            throw new IllegalStateException("Kafka compaction one-record rewrite failed its decode round trip");
        }
    }
    ...
    private static BatchAndRecords decodeExactBatch(byte[] exactBatch) {
        ...
        if (batches.hasNext()
                ...
                || !batch.isValid()) {
            throw new IllegalArgumentException("Kafka compaction source must contain one exact valid magic-v2 batch");
        }
        batch.ensureValid();
    ...
    private static boolean sameFacts(DecodedCompactionRecord expected, DecodedCompactionRecord actual) {
    ...
    private static void requireDeleteHorizon(DecodedCompactionRecord survivor, CompactionRewriteContext context) {
        ...
        if (source.isPresent() && !source.equals(target)) {
            throw new IllegalArgumentException("Kafka compaction rewrite must preserve an existing delete horizon");
        }
        if (source.isEmpty() && survivor.keyKind() == KeyKind.KEYED && !survivor.tombstone() && target.isPresent()) {
        ...
        if (source.isEmpty() && survivor.keyKind() == KeyKind.UNKEYED && target.isPresent()) {
            throw new IllegalArgumentException("Kafka unkeyed survivor cannot introduce a delete horizon");
        }
    }
