...
public record KafkaCompactionGenerationSet(
        StreamId streamId, OffsetRange coverage, List<GenerationCommitResult> generations, Checksum digestSha256) {
    ...
    public KafkaCompactionGenerationSet {
        ...
        if (cursor != coverage.endOffset() || !digestSha256.equals(canonicalDigest(streamId, coverage, generations))) {
            throw new IllegalArgumentException("Kafka compaction generation set coverage/digest is inconsistent");
        }
    }
    ...
    public static KafkaCompactionGenerationSet of(List<GenerationCommitResult> generations) {
        List<GenerationCommitResult> exact = List.copyOf(Objects.requireNonNull(generations, "generations"));
        if (exact.isEmpty()) {
    ...
    public KafkaCompactionGenerationSet extend(GenerationCommitResult generation) {
        GenerationCommitResult exact = requireTopicCompacted(generation);
        if (!exact.streamId().equals(streamId) || exact.coverage().startOffset() != coverage.endOffset()) {
            throw new IllegalArgumentException(
        ...
        return create(
                streamId,
                new OffsetRange(coverage.startOffset(), exact.coverage().endOffset()),
                extended);
    }
