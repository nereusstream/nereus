...
public final class KafkaCompactionParquetPublisher {
    ...
    public CompletableFuture<PreparedObject> prepare(
        ...
        try {
            ...
            execution.whenComplete((streaming, executionFailure) -> {
                ...
                try {
                    writing = Objects.requireNonNull(writer.write(request, streaming.rows()), "NTC2 writer future");
                } catch (Throwable failure) {
                    streaming.close();
                    result.completeExceptionally(failure);
                    return;
                ...
                writing.whenComplete((written, writeFailure) -> {
                    streaming.close();
    ...
    public record PreparedObject(
            KafkaTopicCompactedObjectWriteRequest request, RangedCompactedObjectWriteResult written, Evidence evidence)
