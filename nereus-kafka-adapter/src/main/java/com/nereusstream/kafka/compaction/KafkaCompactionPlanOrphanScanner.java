...
public final class KafkaCompactionPlanOrphanScanner {
  ...
  public CompletableFuture<ScanResult> scan(
    ...
    try {
      ...
      Accumulator accumulator =
          new Accumulator(exactPartition, exactGuard, clock.millis(), exactContinuation);
  ...
  private CompletableFuture<Void> scanPage(Accumulator accumulator) {
    ...
    return scanPlans(accumulator.partition, accumulator.continuation, limit)
        .thenCompose(
            page -> {
              ...
              return processPlans(accumulator, page.plans(), 0)
                  .thenCompose(
                      ignored ->
                          accumulator.continuation.isPresent()
                                  && accumulator.scanned < maxPlansPerScan
  ...
  private CompletableFuture<Void> processPlan(
      Accumulator accumulator, VersionedKafkaCompactionPlan expected) {
    if (!expected.value().identity().equals(accumulator.partition)) {
      return CompletableFuture.failedFuture(
          invariant("Kafka compaction plan scan returned another partition"));
    }
    accumulator.scanned++;
    ...
    return taskExists(streamId, taskId)
        .thenCompose(
            exists -> {
              if (exists) {
              ...
              return revalidate(accumulator.authorityGuard)
                  .thenCompose(
                      ignored ->
                          getPlan(accumulator.partition, taskId)
                              .thenCompose(
                                  current -> {
                                    if (current.isEmpty()) {
                                        ...
                                        .thenCompose(
                                            appeared -> {
                                              ...
                                              return revalidate(accumulator.authorityGuard)
                                                  .thenCompose(
                                                      ignoredAgain ->
                                                          deleteAndReconcile(accumulator, exact));
                                            });
                                  }));
            });
  }

  private CompletableFuture<Void> deleteAndReconcile(
      Accumulator accumulator, VersionedKafkaCompactionPlan expected) {
    CompletableFuture<Void> deleted;
    try {
      deleted =
          Objects.requireNonNull(
              plans.deleteCompactionPlan(expected),
              "Kafka compaction plan delete returned a null future");
    } catch (Throwable failure) {
  ...
  private CompletableFuture<Optional<VersionedKafkaCompactionPlan>> getPlan(
    ...
    try {
      return Objects.requireNonNull(
          plans.getCompactionPlan(partition, taskId),
          "Kafka compaction plan get returned a null future");
    } catch (Throwable failure) {
      return CompletableFuture.failedFuture(failure);
    }
  }
  ...
  private static void requireExact(
      VersionedKafkaCompactionPlan expected, VersionedKafkaCompactionPlan current) {
