...
final class KafkaCompactionWinnerIndex implements AutoCloseable {
  ...
  private void selectWinner(BitSet winners, long absoluteOffset) {
    if (outputCoverage.contains(absoluteOffset)) {
      winners.set(
          Math.toIntExact(Math.subtractExact(absoluteOffset, outputCoverage.startOffset())));
    }
  }
  ...
  private void mergeRuns(List<Run> inputs, WinnerConsumer consumer) {
    ...
    PriorityQueue<RunReader> queue =
        new PriorityQueue<>(Comparator.comparing(reader -> reader.current.key()));
    try {
      for (Run run : inputs) {
  ...
  private final class RunWriter implements AutoCloseable {
    ...
    private RunWriter() {
      try {
        output =
            new DataOutputStream(new BufferedOutputStream(file.outputStream(), RUN_BUFFER_BYTES));
  ...
  private byte[] requireKey(byte[] encodedKey) {
    ...
    if (key.length <= 0
        || key.length > maxKeyBytes
        || key[0] != KafkaCompactionKeyEncodingV2.KEYED_TAG) {
  ...
  private static final class ByteKey implements Comparable<ByteKey> {
    ...
    public int compareTo(ByteKey other) {
      ...
      for (int index = 0; index < common; index++) {
        int comparison =
            Integer.compare(
                Byte.toUnsignedInt(bytes[index]), Byte.toUnsignedInt(other.bytes[index]));
        if (comparison != 0) {
          return comparison;
        }
      }
