...
public final class KafkaCompactionStreamingExecutor {
    ...
    public CompletableFuture<StreamingResult> execute(
        ...
        try {
            ...
            KafkaCompactionBatchSource.PassStreams exactStreams = Objects.requireNonNull(streams, "streams");
            ownedStreams = exactStreams;
            DecisionStage decision =
                    new DecisionStage(plan.decisionSources(), plan.passOneSnapshot(), exactStreams.decisionHorizon());
            ...
            active.set(deciding);
            deciding.whenComplete((prepared, decisionFailure) -> {
                ...
                producing.whenComplete((spooled, outputFailure) -> {
    ...
    private StreamingResult streamingResult(KafkaCompactionPlan plan, PreparedFacts prepared, SpooledOutput spooled) {
    ...
    private final class DecisionStage extends BatchStage<PreparedFacts> {
        ...
        private DecisionStage(
            ...
            if (!exactSources.coverage().equals(snapshot.decisionHorizon())) {
                throw new IllegalArgumentException("Kafka compaction decision stream does not match KCP1 horizon");
            }
    ...
    private final class OutputStage extends BatchStage<SpooledOutput> {
        ...
        private OutputStage(
            ...
            if (!exactSources.coverage().equals(facts.outputCoverage())) {
                throw new IllegalArgumentException("Kafka compaction output stream does not match pass-one coverage");
            }
            ...
            this.spool = KafkaCompactionRowSpool.writer(
                    stagingFiles, facts.outputCoverage(), limits.maxOutputBatches(), limits.maxOutputBytes());
        }

        @Override
        void process(ReadBatch batch) {
            sourceVerifier.accept(batch);
            codec.decode(batch, record -> {
    ...
    private abstract class BatchStage<T> implements Flow.Subscriber<ReadBatch> {
        ...
        public final void onNext(ReadBatch batch) {
            submit(() -> {
                ...
                if (!awaiting) {
                    failOnSerial(new IllegalStateException("Kafka compaction source emitted a batch without demand"));
                    return;
                }
                awaiting = false;
                try {
                    ...
                    if (sourceBatches > limits.maxSourceBatches()) {
                        throw new IllegalArgumentException("Kafka compaction source exceeded its batch limit");
                    }
        ...
        private void requestOne() {
            ...
            if (subscription == null || awaiting) {
                failOnSerial(new IllegalStateException("Kafka compaction source subscription state is invalid"));
                return;
            }
            awaiting = true;
            try {
    ...
    public static final class StreamingResult implements AutoCloseable {
        ...
        private StreamingResult(
            ...
            this.decisionSourceSetSha256 = Objects.requireNonNull(decisionSourceSetSha256, "decisionSourceSetSha256");
            this.outputSourceSetSha256 = Objects.requireNonNull(outputSourceSetSha256, "outputSourceSetSha256");
    ...
    private record SpooledOutput(KafkaCompactionRowSpool.Spool rows, long sourceBatches) implements AutoCloseable {
