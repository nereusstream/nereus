...
public final class KafkaActivatedGenerationSetResolver
  ...
  public CompletableFuture<VersionedKafkaPartitionBinding> repairIfQuarantined(
    ...
    StreamId exactStream = Objects.requireNonNull(streamId, "streamId");
    VersionedKafkaPartitionBinding exactBinding =
  ...
  private RepairableSelection selectRepairable(
    ...
    List<VersionedGenerationIndex> actual =
        historical.generations().stream().map(value -> byKey.get(value.indexKey())).toList();
  ...
  private Optional<GenerationCommitResult> admitRepairable(
    ...
    return Optional.of(
        commitResult(streamId, index, historicalVersion, historicalDigest));
  }
  ...
  private CompletableFuture<VersionedKafkaPartitionBinding> repairSelection(
              ...
              values -> {
                if (actual == null) {
                  return generations
                      ...
                      .thenApply(
                          candidate -> {
                            VersionedGenerationCandidate present =
                                candidate.orElseThrow(
                                    () ->
                                        invariant(
                                            "committed NTC2 path member disappeared during"
                                                + " repair"));
                            if (!(present instanceof VersionedGenerationIndex committed)
                                || committed.value().lifecycle()
                                    != GenerationLifecycle.COMMITTED
                                || committed.metadataVersion() != historical.indexMetadataVersion()
                                || !committed
                                    .durableValueSha256()
                                    .equals(historical.indexRecordSha256())) {
                              throw invariant(
                                  "committed NTC2 path member changed during repair");
                            }
                            values.add(committed);
                            return values;
                          });
                }
                return repairGeneration(actual, timeout)
                    .thenApply(
                        committed -> {
                          values.add(committed);
                          return values;
    ...
    return repaired.thenCompose(
        values -> {
          KafkaCompactionGenerationSet desired =
              KafkaCompactionGenerationSet.of(
                  values.stream()
                      .map(
                          value ->
                              commitResult(
                      ...
                      .toList());
          return activateRepairCoverage(
              partition,
              binding.value().compactionCoverage(),
              desired,
              0);
        });
  }
  ...
  private CompletableFuture<VersionedGenerationIndex> repairGeneration(
      VersionedGenerationIndex quarantined,
      Duration timeout) {
  ...
  private CompletableFuture<Void> verifyPhysicalRepair(
      ObjectSliceReadTarget target,
      Duration timeout) {
            ...
            optional -> {
              VersionedPhysicalObjectRoot root =
                  optional.orElseThrow(
                      () -> invariant("mandatory NTC2 repair physical root disappeared"));
              requireRepairRoot(target, root.value());
              Checksum expected =
                  new Checksum(
                      ChecksumType.CRC32C, root.value().storageChecksumValue());
              return context
                  .objectStore()
                      ...
                      head -> {
                        if (head.objectLength() != root.value().objectLength()
                            ...
                            || (!root.value().etag().isEmpty()
                                && !head.etag().orElse("").equals(root.value().etag()))) {
                          throw invariant(
                            ...
                            .thenAccept(
                                read -> {
                                  if (!root.value().contentSha256().isEmpty()
                                    ...
                                    throw invariant(
                                        "mandatory NTC2 repaired content SHA-256 differs from"
  ...
  private CompletableFuture<VersionedPhysicalObjectRoot> reactivateRoot(
      ObjectSliceReadTarget target,
      int attempt) {
  ...
  private CompletableFuture<VersionedGenerationIndex> reactivateGeneration(
    ...
    if (attempt >= MAX_REPAIR_CAS_ATTEMPTS) {
      return CompletableFuture.failedFuture(
          invariant("mandatory NTC2 generation repair exhausted CAS retries"));
    }
  ...
  private CompletableFuture<VersionedKafkaPartitionBinding> activateRepairCoverage(
    ...
    if (attempt >= MAX_REPAIR_CAS_ATTEMPTS) {
      return CompletableFuture.failedFuture(
          invariant("mandatory NTC2 binding repair exhausted CAS retries"));
    }
    RepairContext context = repair.orElseThrow();
    return context
        .partitions()
        ...
        .thenCompose(
            optional -> {
              VersionedKafkaPartitionBinding current =
                  optional.orElseThrow(
                      () -> invariant("mandatory NTC2 repair binding disappeared"));
              ...
              if (coverage.coverageVersion() == 1
                  ...
                  && java.util.Arrays.equals(
                      coverage.generationSetSha256(), desired.digestBytes())) {
                return CompletableFuture.completedFuture(current);
              }
  ...
  private KafkaCompactionGenerationSet select(
    ...
    if (search.matches.isEmpty()) {
      throw invariant(
          "Kafka binding generation-set digest has no exact committed NTC2 path"
              + " [coverage="
      ...
      descriptions.add(
          "{generation="
              ...
              + record
                  ...
                  .equals(record.readTarget().identityChecksumValue())
              + ", payload="
  ...
  private Optional<GenerationCommitResult> admit(
    ...
    return Optional.of(
        commitResult(streamId, index, index.metadataVersion(), index.durableValueSha256()));
  }
  ...
  private static void requireRepairRoot(
      ObjectSliceReadTarget target, PhysicalObjectRootRecord root) {
  ...
  private static boolean samePublicationIdentity(
      GenerationIndexRecord expected, GenerationIndexRecord actual) {
  ...
  private static Optional<PriorIndexIdentity> priorIndexIdentity(String reason) {
    ...
    try {
      long version =
          Long.parseLong(
              reason.substring(versionStart + versionMarker.length(), digestStart));
      Checksum digest =
          new Checksum(
              ChecksumType.SHA256,
              reason.substring(digestStart + digestMarker.length()));
      if (version < 0) {
        return Optional.empty();
      }
  ...
  private record RepairableSelection(
      KafkaCompactionGenerationSet historical,
      List<VersionedGenerationIndex> quarantined) {
  ...
  private record PriorIndexIdentity(
