...
public record KafkaCanonicalCheckpointState(
  ...
  public KafkaCanonicalCheckpointState {
    if (checkpointOffset < 0
      ...
      throw new IllegalArgumentException(
          "canonical Kafka checkpoint must capture the exact stable end");
    }
    producerTransactionState =
        Objects.requireNonNull(producerTransactionState, "producerTransactionState");
  ...
  private static void validateSegmentIndexes(
      List<VirtualSegment> segments, KafkaDerivedIndexState indexes) {
          ...
          .forEach(
              sample -> {
                if (sample.entryStartOffset() >= segment.endOffset()) {
                  throw new IllegalArgumentException(
                      "NKC1 logical-byte sample crosses its virtual segment");
                }
              });
    ...
    for (SegmentTimeIndex timeIndex : indexes.timeIndexes()) {
      ...
      timeIndex
          ...
          .forEach(
              entry -> {
                if (entry.offset() >= segment.endOffset()
                    || entry.timestamp() > segment.largestTimestamp()) {
                  throw new IllegalArgumentException(
                      "NKC1 time-index entry exceeds its virtual segment");
                }
              });
    }
  }
