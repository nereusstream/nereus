...
public record KafkaVirtualSegmentState(
    ...
    public void requireBounds(long expectedLogStartOffset, long expectedStableEndOffset) {
        if (logStartOffset != expectedLogStartOffset || stableEndOffset != expectedStableEndOffset) {
            throw new IllegalArgumentException("Kafka virtual-segment state does not match checkpoint bounds");
        }
    }

    private static void validateConfigHistory(List<LogConfigHistoryEntry> history, long stableEndOffset) {
        ...
        for (LogConfigHistoryEntry entry : history) {
            ...
            if (entry.metadataOffset() <= previousMetadataOffset
                ...
                throw new IllegalArgumentException("Kafka log-config history must be ordered and checkpoint-bounded");
            }
    ...
    private static void validateSegments(
        ...
        for (int index = 0; index < segments.size(); index++) {
            ...
            if (index > 0
                ...
                throw new IllegalArgumentException("Kafka virtual segment offset and byte ranges must be dense");
            }
    ...
    public record VirtualSegment(
        ...
        public VirtualSegment {
            ...
            if (state == SegmentState.ACTIVE && closedAtMillis != 0
                    || state == SegmentState.CLOSED && (closedAtMillis <= 0 || closedAtMillis < createdAtMillis)) {
            ...
            if (logicalBytes == 0) {
                if (largestTimestamp != -1 || maxTimestampOffset != -1) {
                    throw new IllegalArgumentException("empty Kafka virtual segment cannot have a max timestamp");
                }
            } else if (largestTimestamp < 0 || maxTimestampOffset < baseOffset || maxTimestampOffset >= endOffset) {
                throw new IllegalArgumentException("Kafka virtual-segment max timestamp is outside its range");
            }
    ...
    public record LogConfigHistoryEntry(
        ...
        public LogConfigHistoryEntry {
            ...
            if (configDigest.type() != ChecksumType.SHA256
                    || !configDigest.equals(computeConfigDigest(
                            segmentBytes,
                            segmentMs,
                            segmentJitterMillis,
                            segmentIndexBytes,
                            ...
                            cleanupPolicyFlags))) {
                throw new IllegalArgumentException("Kafka log-config digest does not match its canonical fields");
            }
        }
        ...
        private static Checksum computeConfigDigest(
            ...
            try {
                ...
                return new Checksum(ChecksumType.SHA256, HexFormat.of().formatHex(digest.digest(fields.array())));
