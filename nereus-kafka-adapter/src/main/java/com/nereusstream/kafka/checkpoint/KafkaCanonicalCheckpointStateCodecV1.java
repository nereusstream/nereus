...
public final class KafkaCanonicalCheckpointStateCodecV1 {
    ...
    public List<KafkaCheckpointSection> encodeSections(KafkaCanonicalCheckpointState state) {
        ...
        sections.addAll(producerCodec.encodeSections(exact.producerTransactionState(), exact.checkpointOffset()));
        sections.add(leaderEpochCodec.encodeSection(
                exact.leaderEpochState(), exact.logStartOffset(), exact.stableEndOffset()));
        sections.add(virtualSegmentCodec.encodeSection(
                exact.virtualSegmentState(), exact.logStartOffset(), exact.stableEndOffset()));
        sections.addAll(derivedIndexCodec.encodeSections(
    ...
    public KafkaCanonicalCheckpointState decodeSections(
            List<KafkaCheckpointSection> sections, long checkpointOffset, long logStartOffset, long stableEndOffset) {
