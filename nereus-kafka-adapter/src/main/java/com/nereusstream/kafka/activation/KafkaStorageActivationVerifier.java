...
public final class KafkaStorageActivationVerifier {
    ...
    public CompletionStage<VerifiedKafkaStorageActivation> verifyCurrent() {
        CompletionStage<KafkaStorageClusterSnapshot> supplied =
                Objects.requireNonNull(clusterSnapshots.currentSnapshot(), "KRaft snapshot future");
        return supplied.thenCompose(this::loadAuthorities);
    }

    private CompletionStage<VerifiedKafkaStorageActivation> loadAuthorities(KafkaStorageClusterSnapshot snapshot) {
        KafkaStorageClusterSnapshot exact = Objects.requireNonNull(snapshot, "KRaft snapshot");
        requireSnapshot(exact);
        CompletableFuture<Optional<VersionedKafkaStorageProtocolActivation>> activation = store.getActivation();
        ...
        return CompletableFuture.allOf(all)
                .thenApply(ignored -> verifyLoaded(
                        exact,
                        required(activation.join(), "ACTIVE activation is absent"),
                        required(readiness.join(), "readiness proof is absent"),
                        capabilities));
    }
    ...
    private VerifiedKafkaStorageActivation verifyLoaded(
        ...
        require(
                active.lifecycle() == KafkaStorageActivationLifecycle.ACTIVE,
                "Kafka storage activation is not ACTIVE",
                true);
        require(
                active.kafkaClusterId().equals(snapshot.kafkaClusterId()),
                "activation Kafka cluster does not match KRaft",
                false);
        require(
                active.kafkaFeatureLevel() == snapshot.kafkaFeatureLevel(),
                "activation feature level does not match KRaft",
                false);
        require(
                active.preparedAtMetadataOffset() <= snapshot.metadataOffset(),
                "KRaft image precedes the activation preparation offset",
                true);
        require(
                active.allowedStorageProfiles().equals(localSpecification.supportedStorageProfiles()),
                "local storage profiles do not match ACTIVE",
                false);
        require(
                active.defaultStorageProfile().equals(localSpecification.defaultStorageProfile()),
                "local default storage profile does not match ACTIVE",
                false);

        require(
                ready.kafkaClusterId().equals(snapshot.kafkaClusterId()),
                "readiness Kafka cluster does not match KRaft",
                false);
        require(ready.expiresAtMillis() > now, "readiness proof is expired", true);
        require(
                ready.kraftMetadataOffset() <= snapshot.metadataOffset(),
                "KRaft image precedes the readiness source offset",
                true);
        require(ready.brokers().equals(snapshot.brokers()), "readiness broker set does not match KRaft", true);
        require(ready.readinessEpoch() >= active.activationEpoch(), "readiness epoch predates ACTIVE", false);
        if (ready.readinessEpoch() == active.activationEpoch()) {
            require(
                    Arrays.equals(active.requiredBrokerSetSha256(), ready.brokerSetSha256()),
                    "activation-epoch readiness changed its broker set",
                    false);
        }
        require(
                Arrays.equals(active.requiredCapabilitySha256(), ready.capabilitySha256()),
                "ACTIVE and readiness capability digests differ",
                false);
        require(
                Arrays.equals(ready.providerScopeSha256(), localSpecification.providerScopeSha256()),
        ...
        for (int index = 0; index < capabilityReads.size(); index++) {
            ...
            VersionedKafkaBrokerCapability capability = required(
                    ...
                    "capability is absent for broker " + expectedIdentity.brokerId() + " epoch "
                            + expectedIdentity.brokerEpoch());
            ...
            require(
                    ...
                    false);
            require(value.identity().equals(expectedIdentity), "capability identity does not match KRaft", false);
            require(
                    value.expiresAtMillis() > now,
                    "capability is expired for broker " + expectedIdentity.brokerId(),
                    true);
            require(
                    Arrays.equals(value.providerScopeSha256(), ready.providerScopeSha256()),
                    "broker capability provider scope differs from readiness",
                    false);
            byte[] digest = KafkaStorageCapabilityDigests.compatibilitySha256(value);
            if (requiredCapability == null) {
                requiredCapability = digest;
            }
            require(Arrays.equals(requiredCapability, digest), "broker capability compatibility facts differ", false);
            capabilities.add(capability);
        }
        require(requiredCapability != null, "KRaft broker set is empty", false);
        require(
                Arrays.equals(requiredCapability, ready.capabilitySha256()),
                "capability aggregate does not match readiness",
                false);
        require(
                Arrays.equals(requiredCapability, active.requiredCapabilitySha256()),
                "capability aggregate does not match ACTIVE",
                false);
        VersionedKafkaBrokerCapability local =
                capabilities.get(snapshot.brokers().indexOf(localSpecification.identity()));
        require(
                localSpecification.matchesImmutableFacts(local.value()),
                "published local capability differs from this broker process",
                false);
    ...
    private void requireSnapshot(KafkaStorageClusterSnapshot snapshot) {
        require(
                snapshot.kafkaClusterId().equals(localSpecification.kafkaClusterId()),
                "KRaft Kafka cluster does not match local configuration",
                false);
        require(
                ...
                false);
        require(
                snapshot.brokers().contains(localSpecification.identity()),
