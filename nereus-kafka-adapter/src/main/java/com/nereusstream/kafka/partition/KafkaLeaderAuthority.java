 ...
 */
public record KafkaLeaderAuthority(KafkaPartitionIdentity identity, int leaderId, int leaderEpoch, long brokerEpoch) {
    ...
    public AuthorityRelation relationTo(KafkaLeaderAuthority current) {
        ...
        if (!identity.equals(current.identity)) {
            return AuthorityRelation.DIFFERENT_PARTITION;
        }
        if (equals(current)) {
            return AuthorityRelation.EXACT;
        }
        if (leaderEpoch > current.leaderEpoch) {
            return AuthorityRelation.DOMINATES;
        }
        if (leaderEpoch < current.leaderEpoch) {
            return AuthorityRelation.STALE;
        }
