...
public final class KafkaMaterializationStreamRegistration {
    ...
    public KafkaMaterializationStreamRegistration(
            String cluster, GenerationMetadataStore generations, Clock clock) {
        ...
        if (!exactProfile.objectMaterializationEnabled()) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException(
                            "Kafka compaction registration requires an object-materialization"
                                + " profile"));
        }
    ...
    private static void requireExact(
        ...
        VersionedMaterializationStreamRegistration exact =
        ...
        if (!value.streamId().equals(streamId.value())
            ...
            throw new IllegalStateException(
                    "Kafka materialization registration differs from direct-stream authority");
        }
    }
