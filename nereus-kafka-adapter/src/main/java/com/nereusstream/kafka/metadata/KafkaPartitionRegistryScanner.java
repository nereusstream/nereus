...
public final class KafkaPartitionRegistryScanner {
    ...
    private CompletableFuture<Void> scanShard(
            int shard, Optional<String> continuation, int pageSize, ArrayList<VersionedKafkaPartitionBinding> roots) {
        if (shard == KafkaPartitionKeyspace.REGISTRY_SHARDS) {
            return CompletableFuture.completedFuture(null);
        }
        return store.scanRegistry(shard, continuation, pageSize).thenCompose(page -> loadRoots(page.values(), 0, roots)
    ...
    private CompletableFuture<Void> loadRoots(
            List<VersionedKafkaPartitionRegistry> hints, int index, ArrayList<VersionedKafkaPartitionBinding> roots) {
        if (index == hints.size()) {
            return CompletableFuture.completedFuture(null);
        }
