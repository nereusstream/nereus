...
public final class KafkaPartitionLifecycleCoordinator implements KafkaPartitionBindingLifecycle {
    ...
    public KafkaPartitionLifecycleCoordinator(
            KafkaPartitionMetadataStore bindings, StreamStorage streams, KafkaPartitionKeyspace keys, Clock clock) {
    ...
    public KafkaPartitionLifecycleCoordinator(
        ...
        this(bindings, streams, keys, clock, Optional.of(Objects.requireNonNull(materializations, "materializations")));
    }
    ...
    private CompletableFuture<KafkaPartitionBinding> ensureBinding(KafkaBindingRequest request, int attempt) {
        if (attempt >= MAX_RECONCILE_RETRIES) {
            return NereusException.failedFuture(
                    ErrorCode.METADATA_CONDITION_FAILED, true, "Kafka binding reconciliation retry budget exhausted");
        }
        KafkaPartitionId id = request.identity().durableId();
        return bindings.get(id).thenCompose(optional -> optional.map(root -> reconcileExisting(request, root, attempt))
                .orElseGet(() -> createRoot(request, attempt)));
    }

    private CompletableFuture<KafkaPartitionBinding> createRoot(KafkaBindingRequest request, int attempt) {
        long now = clock.millis();
        KafkaPartitionId id = request.identity().durableId();
        String attemptId = KafkaPartitionMetadataTransitions.deterministicCreateAttemptId(id, request.metadataOffset());
    ...
    private CompletableFuture<KafkaPartitionBinding> createOrVerifyStream(
        ...
        if (!current.isEmpty()
            ...
            claimed = bindings.claimOperation(
                    root, operation(KafkaPartitionOperationType.CREATE, expectedAttempt, request, now), now);
        }
        return claimed.thenCompose(owner -> {
                    ...
                    return streams.createOrGetStream(
                            ...
                            .thenCompose(
                                    metadata -> ensureMaterialization(metadata.streamId(), request.storageProfile())
                                            .thenCompose(ignored -> activate(
                                                    request, owner, streamName, attributes, metadata, attempt)));
                })
                .exceptionallyCompose(failure -> retryCondition(request, attempt, failure));
    }
    ...
    private CompletableFuture<Void> ensureMaterialization(StreamId streamId, StorageProfile profile) {
        if (materializations.isEmpty() || !profile.objectMaterializationEnabled()) {
    ...
    private CompletableFuture<KafkaPartitionBinding> activate(
        ...
        if (!stream.streamName().equals(streamName)
            ...
            return corruptAndFail(
                    owner, "STREAM_BINDING_MISMATCH", "deterministic Kafka stream facts do not match binding");
        }
        KafkaPartitionBindingRecord active = KafkaPartitionMetadataTransitions.activate(
                owner.value(),
                streamName.value(),
                stream.streamId().value(),
                Math.max(owner.value().lastAppliedMetadataOffset(), request.metadataOffset()),
                clock.millis());
        ...
        return deleting.thenCompose(owner -> {
                    ...
                    return streams.getStreamMetadata(streamId)
                                ...
                                CompletableFuture<StreamMetadata> terminal =
                                        switch (metadata.state()) {
                                            case ACTIVE ->
                                                streams.seal(
                                                                streamId,
                                                                new com.nereusstream.api.SealOptions(
                                                                        streamTimeout,
                                                                        "Kafka" + " topic" + " ID deletion"))
                                                        .thenCompose(ignored -> streams.delete(
                                                                streamId,
                            ...
                            .thenCompose(deleted -> {
                                ...
                                KafkaPartitionBindingRecord terminal = KafkaPartitionMetadataTransitions.markDeleted(
                                        owner.value(),
                                        owner.value().pendingOperation().attemptId(),
                                        clock.millis());
                ...
                .exceptionallyCompose(failure -> {
                    ...
                    return bindings.get(request.identity().durableId())
                            .thenCompose(latest -> delete(request, latest.orElseThrow(), streamTimeout, attempt + 1));
                });
    }

    private CompletableFuture<Void> publishHint(KafkaPartitionIdentity identity, VersionedKafkaPartitionBinding root) {
    ...
    private CompletableFuture<KafkaPartitionBinding> corruptAndFail(
        ...
        return bindings.compareAndSet(root, corrupt).handle((ignored, failure) -> {
            throw new CompletionException(new NereusException(
                    ErrorCode.METADATA_INVARIANT_VIOLATION, false, message, failure == null ? null : unwrap(failure)));
        });
    }
    ...
    private CompletableFuture<KafkaPartitionBinding> retryCondition(
        ...
        return conditionFailure(cause) ? ensureBinding(request, attempt + 1) : CompletableFuture.failedFuture(cause);
    }

    private static KafkaPartitionPendingOperationRecord operation(
            KafkaPartitionOperationType type, String attemptId, KafkaBindingRequest request, long now) {
        long ttl;
        try {
            ttl = request.operationTtl().toMillis();
            if (ttl <= 0) {
                throw new ArithmeticException("sub-millisecond operation TTL");
            }
