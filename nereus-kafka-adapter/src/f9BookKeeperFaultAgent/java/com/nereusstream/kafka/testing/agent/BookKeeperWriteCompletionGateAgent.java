 ...
 *
 * <p>The target release process still executes the stock BookKeeper client write. The gate only withholds the completed
 * future from the Nereus appender, creating a deterministic boundary where the entry is provider-applied while the durable
...
public final class BookKeeperWriteCompletionGateAgent {
    private static final String PROPERTY_PREFIX =
            "nereus.f9.bookkeeper.write.gate.";
    ...
    public static void premain(
        ...
        new AgentBuilder.Default()
                ...
                .type(named(
                        "com.nereusstream.bookkeeper.DefaultBookKeeperClientOperations"))
                .transform(
                        (builder,
                                type,
                                classLoader,
    ...
    private static Map<String, String> parseArguments(
        ...
        if (agentArguments == null || agentArguments.isBlank()) {
            throw new IllegalArgumentException(
                    "BookKeeper write gate agent arguments are required");
        }
        ...
        for (String token : agentArguments.split(",")) {
            ...
            if (separator <= 0 || separator == token.length() - 1) {
                throw new IllegalArgumentException(
                        "invalid BookKeeper write gate agent argument: "
                                + token);
            }
            ...
            if (parsed.putIfAbsent(key, value) != null) {
                throw new IllegalArgumentException(
                        "duplicate BookKeeper write gate agent argument: "
                                + key);
            }
        }
    ...
    private static void require(
        ...
        if (!configuration.containsKey(key)) {
            throw new IllegalArgumentException(
                    "missing BookKeeper write gate agent argument: " + key);
        }
    }
    ...
    public static void writeMarker(
        ...
        try {
            ...
            throw new IllegalStateException(
                    "cannot write BookKeeper gate marker " + marker,
                    failure);
        }
    }
    ...
    public static final class WriteCompletionAdvice {
        ...
        public static void gate(
            ...
            try {
                ...
                throw new IllegalStateException(
                        "cannot capture BookKeeper write gate",
                        failure);
            }
        ...
        public static final class ProviderCompletion
            ...
            public ProviderCompletion(
                    CompletableFuture<Long> delayed,
            ...
            public void accept(
                ...
                if (writtenEntryId == null
                        || writtenEntryId != requestedEntryId) {
                    delayed.completeExceptionally(
                            new IllegalStateException(
                                    "BookKeeper write returned another entry id"));
                    return;
                }
                try {
                    writeMarker(
                            path("applied"),
                            Long.toString(writtenEntryId));
        ...
        public static Path path(String name) {
            ...
            if (configured == null || configured.isBlank()) {
                throw new IllegalStateException(
                        "BookKeeper write gate property is absent: "
                                + name);
            }
