...
m5_project="nereus-v2-m5-retired-history-$$"
...
for task, class_name, expected in (
    ...
    ("v2M5BookKeeperDescriptorTest", "KafkaSealedBookKeeperDescriptorV2Test", 13),
...
path = root / "nereus-storage-object/build/test-results/v2M5RetiredHistoryTest/TEST-com.nereusstream.storage.object.retention.M5RetiredBatchHistoryV2Test.xml"
suite = ET.parse(path).getroot()
if {key: suite.attrib.get(key) for key in ("tests", "failures", "errors", "skipped")} != {
    "tests": "15", "failures": "0", "errors": "0", "skipped": "0"
}:
    raise SystemExit("retired history JUnit summary differs")
PY

printf 'PASS_V2_M5_RETIRED_HISTORY_REAL_BK_REGRESSION implementation-only image=%s id=%s\n' \
  "$m5_image" \
