...
readonly NEREUS_RELEASE_VERSION="0.1.0"
readonly NEREUS_RELEASE_TAG="v${NEREUS_RELEASE_VERSION}"
readonly NEREUS_DEVELOPMENT_VERSION="${NEREUS_RELEASE_VERSION}-f2-dev"
...
Source selection:
  ...
  --nereus-repo PATH          Final Nereus v0.1.0 commit/tag checkout.
...
admin_tag="${admin_image_repository}:${NEREUS_RELEASE_TAG}-n${nereus_source_sha:0:8}-${target_arch}"
...
admin_image_kind="nereus-admin-${NEREUS_RELEASE_TAG}"
admin_iid_file="${output_dir}/${admin_image_kind}-${target_arch}.iid"
...
run_nerdctl build \
  ...
  --build-arg "NEREUS_VERSION=${NEREUS_RELEASE_TAG}" \
  ${proxy_build_args[@]+"${proxy_build_args[@]}"} \
  --label "org.opencontainers.image.title=Nereus-Admin" \
  --label "org.opencontainers.image.version=${NEREUS_RELEASE_TAG}" \
...
test "${jar_count}" -eq 8
actual_sha="$(sha256sum "lib/com.nereusstream-nereus-pulsar-adapter-$2.jar" | awk "{print \$1}")"
test "${actual_sha}" = "$1"' sh "${adapter_sha256}" "${NEREUS_DEVELOPMENT_VERSION}"
run_nerdctl run --rm --net none --pull never \
  --platform "${target_platform}" \
  --entrypoint /bin/sh \
  "${admin_tag}" \
...
for image_kind in apache-pulsar-5.0.0-M1 nereus-pulsar-5.0.0-M1 "${admin_image_kind}"; do
  case "${image_kind}" in
    apache-pulsar-5.0.0-M1) image="${apache_tag}" ;;
    nereus-pulsar-5.0.0-M1) image="${nereus_tag}" ;;
    "${admin_image_kind}") image="${admin_tag}" ;;
  esac
...
manifest_file="${output_dir}/pulsar-5.0.0-M1-${target_arch}.env"
{
  ...
  printf 'NEREUS_VERSION=%s\n' "${NEREUS_RELEASE_VERSION}"
