...
m5_container="nereus-m5-permanent-done-oxia-$$"
m5_output="$m5_repo_root/build/m5-permanent-done-oxia/$m5_container"
mkdir -p "$m5_output"
m5_cleanup() {
...
docker run --detach \
  --name "$m5_container" \
  --label com.nereusstream.evidence=v2-m5-permanent-done-implementation \
...
paths.extend(root / name for name in ("build.gradle.kts", "nereus-storage-object/build.gradle.kts",
    "nereus-metadata-oxia/build.gradle.kts", "scripts/run-v2-m5-permanent-done-oxia-check.sh"))
...
"$m5_repo_root/gradlew" \
  ...
  "-Pv2M5DoneOxiaRestartCheckpoint=$m5_output/restart-checkpoint.txt" \
  v2M5PermanentDoneCheck \
  :nereus-metadata-oxia:v2M5PermanentDoneOxiaRestartWriteTest \
  --console=plain
...
"$m5_repo_root/gradlew" \
  ...
  "-Pv2M5RetentionOxiaServiceAddress=127.0.0.1:$m5_service_port" \
  "-Pv2M5DoneOxiaRestartCheckpoint=$m5_output/restart-checkpoint.txt" \
  :nereus-metadata-oxia:v2M5PermanentDoneOxiaRestartReadTest \
  --console=plain
...
for module, task, name, count in (
    ...
    ("nereus-metadata-oxia", "v2M5PermanentDoneOxiaRestartWriteTest", "com.nereusstream.metadata.oxia.v2.retention.M5PermanentDoneOxiaRestartTest", 1),
    ("nereus-metadata-oxia", "v2M5PermanentDoneOxiaRestartReadTest", "com.nereusstream.metadata.oxia.v2.retention.M5PermanentDoneOxiaRestartTest", 1),
):
...
summary = {
    "schema": "NEREUS_V2_M5_PERMANENT_DONE_RUN_V2",
    ...
    "continuousCompactedTerminals": 258, "maximumResidentDoneEntries": 8, "maximumResidentEncodedBytes": 8192,
    "compactDoneAndExistingIntentSurvivedServerRestart": True,
    "actualProviderOrBookKeeperDeletionVerified": False,
    "nativeUniqueNamespaceAdmissionComplete": False, "durableNamespaceQuotaIntegrated": False,
...
PY
printf 'PASS_V2_M5_PERMANENT_DONE_REAL_OXIA_NON_PROMOTABLE image=%s id=%s output=%s\n' \
