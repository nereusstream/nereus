...
m5_project="nereus-v2-m5-publication-tickets-$$"
m5_container="$m5_project-oxia"
m5_output="$m5_repo_root/build/m5-publication-tickets/$m5_project"
...
m5_oxia_owned_id="$(docker run --detach --name "$m5_container" --label com.nereusstream.evidence=v2-m5-publication-tickets \
...
"$m5_repo_root/gradlew" --no-daemon --no-configuration-cache --no-parallel --max-workers=2 --rerun-tasks \
  ...
  v2M5PublicationTicketsCheck :nereus-kafka-bookkeeper:v2M5PublicationTicketsRestartWriteTest --console=plain
...
"$m5_repo_root/gradlew" --no-daemon --no-configuration-cache --no-parallel --max-workers=2 \
  ...
  "-Pv2M5RetentionOxiaServiceAddress=127.0.0.1:$m5_port" \
  "-Pv2M5PublicationTicketsRestartCheckpoint=$m5_output/restart-checkpoint.txt" \
  :nereus-kafka-bookkeeper:v2M5PublicationTicketsRestartReadTest --console=plain
...
summary={'schema':'NEREUS_V2_M5_PUBLICATION_TICKETS_RUN_V2','suites':suites,
...
PY
printf 'PASS_V2_M5_PUBLICATION_TICKETS_NON_PROMOTABLE output=%s\n' "$m5_output"
