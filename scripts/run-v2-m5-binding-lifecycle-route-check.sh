...
m5_container="nereus-m5-binding-route-oxia-$$"
m5_output="$m5_repo_root/build/m5-binding-lifecycle-route-oxia/$m5_container"
mkdir -p "$m5_output"
m5_cleanup() {
...
"$m5_repo_root/gradlew" \
  ...
  v2M5BindingLifecycleRouteCheck \
...
for name, package in (
    ("OxiaBindingLifecycleMetadataStoreV2Test", "retention"),
    ("OxiaCanonicalControlMetadataStoreTest", "objectwal"),
):
    path = root / ("nereus-metadata-oxia/build/test-results/v2M5BindingLifecycleRouteTest/TEST-"
                   + f"com.nereusstream.metadata.oxia.v2.{package}.{name}.xml")
    data = path.read_bytes()
    suite = ET.fromstring(data)
    if {key: suite.attrib.get(key) for key in ("tests", "failures", "errors", "skipped")} != {
        "tests": "8", "failures": "0", "errors": "0", "skipped": "0"
    }:
        raise SystemExit(f"Binding route JUnit summary differs: {name}")
    (output / f"{name}.xml").write_bytes(data)
    results.append({"suite": name, "tests": 8, "xmlSha256": hashlib.sha256(data).hexdigest()})
summary = {
    "schema": "NEREUS_V2_M5_BINDING_LIFECYCLE_ROUTE_RUN_V1",
    "containerId": sys.argv[3], "startedBefore": sys.argv[4], "startedAfter": sys.argv[5],
    "sameServerContainerRestarted": True,
    "nativeControlRoute": "OxiaBindingLifecycleMetadataStoreV2",
    "versionToken": "M5O2_ROUTE_DIGEST_AND_EXACT_NATIVE_VERSION",
    "syntheticExternalProofFactDispatcher": True,
...
PY
printf 'PASS_V2_M5_BINDING_LIFECYCLE_ROUTE_REAL_OXIA_NON_PROMOTABLE image=%s id=%s output=%s\n' \
