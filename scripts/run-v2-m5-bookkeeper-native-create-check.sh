...
m5_project="nereus-v2-m5-bk-native-create-$$"
m5_container="$m5_project-oxia"
m5_output="$m5_repo_root/build/m5-bookkeeper-native-create/$m5_project"
...
m5_oxia_owned_id="$(docker run --detach --name "$m5_container" --label com.nereusstream.evidence=v2-m5-bk-native-create \
...
paths.extend([root/'nereus-storage-bookkeeper/build.gradle.kts',
              ...
              root/'nereus-metadata-oxia/build.gradle.kts',root/'scripts/run-v2-m5-bookkeeper-native-create-check.sh'])
...
"$m5_repo_root/gradlew" --no-daemon --no-configuration-cache --rerun-tasks \
  ...
  v2M5BookKeeperNativeCreateCheck :nereus-kafka-bookkeeper:v2M5BookKeeperOxiaRestartWriteTest \
  :nereus-kafka-bookkeeper:v2M5BookKeeperNativeCreateRestartWriteTest --console=plain
...
"$m5_repo_root/gradlew" --no-daemon --no-configuration-cache \
  ...
  :nereus-kafka-bookkeeper:v2M5BookKeeperNativeCreateRestartReadTest --console=plain
...
summary={'schema':'NEREUS_V2_M5_BK_NATIVE_CREATE_RUN_V2','suites':suites,
...
PY
printf 'PASS_V2_M5_BOOKKEEPER_NATIVE_CREATE_NON_PROMOTABLE output=%s\n' "$m5_output"
