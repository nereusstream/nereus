...
m5_project="nereus-v2-m5-kafka-run-roots-$$"
m5_container="$m5_project-oxia"
m5_output="$m5_repo_root/build/m5-kafka-run-roots/$m5_project"
...
m5_oxia_owned_id="$(docker run --detach --name "$m5_container" --label com.nereusstream.evidence=v2-m5-kafka-run-roots \
...
paths.extend([root/'nereus-storage-object/build.gradle.kts',root/'nereus-storage-bookkeeper/build.gradle.kts',
              ...
              root/'scripts/run-v2-m5-kafka-run-roots-check.sh',
...
"$m5_repo_root/gradlew" --no-daemon --no-configuration-cache --no-parallel --max-workers=2 --rerun-tasks \
  ...
  v2M5KafkaRunRootsCheck :nereus-kafka-bookkeeper:v2M5PublicationTicketsRestartWriteTest \
  :nereus-kafka-bookkeeper:v2M5KafkaRunRootsRestartWriteTest --console=plain
...
"$m5_repo_root/gradlew" --no-daemon --no-configuration-cache --no-parallel --max-workers=2 \
  ...
  :nereus-kafka-bookkeeper:v2M5KafkaRunRootsRestartReadTest --console=plain
...
summary={'schema':'NEREUS_V2_M5_KAFKA_RUN_ROOTS_RUN_V2','suites':suites,
...
PY
printf 'PASS_V2_M5_KAFKA_RUN_ROOTS_NON_PROMOTABLE output=%s\n' "$m5_output"
