#!/usr/bin/env python3
"""Pure frozen-plan projection for the M3 V2 bounded adaptive allocator campaign."""
...
ASCENDING_CONTEXT_RATES = (200, 250, 333, 500, 750, 1000)
FAULT_CUTS = (
...
def context_id(candidate: str, population: int, latency: int, rate: int) -> int:
    row = POPULATIONS.index(population) * 24 + LATENCIES.index(latency) * 6
    row += ASCENDING_CONTEXT_RATES.index(rate)
    if candidate == "NATIVE":
        return row
    return 48 + (CANDIDATES.index(candidate) - 1) * 48 + row
...
def plan() -> dict[str, object]:
    ...
    assert (intervals, faults, scales) == (288, 360, 32)
    return {
        "schema": SCHEMA,
